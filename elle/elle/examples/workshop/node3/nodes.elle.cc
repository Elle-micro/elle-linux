...
int DoSomethingToNode(int node) {
  ...
  for (i = 0, nb_count = 0; i < 3;
       i++)                  // loop through list of neighbouring nodes
  ...
  for (i = 0; i < 3; i++) // loop through list of neighbouring nodes
  {
    if (nbnodes[i] != NO_NB) // if node exists
    {
      ElleRelPosition(&node_pos, nbnodes[i], &rel_pos,
                      &sep); // get relative position of neighbour
      total_node = ElleNodeAttribute(node, CONC_A); // get conc value of node
      total_neighbour =
          ElleNodeAttribute(nbnodes[i], CONC_A); // get conc value of neighbour

      if (rel_pos.y > 0) {
        del = rel_pos.y * total_node /
              (sep * 2); // get parcel of concentration to move
        ElleSetNodeAttribute(node, total_node - del,
                             CONC_A); // remove parcel from node
        ElleSetNodeAttribute(nbnodes[i], total_neighbour + del,
                             CONC_A); // add parcel to neighbour
      } else {
        del = -rel_pos.y * total_neighbour /
              (sep * 2); // get parcel of concentration to move
        ElleSetNodeAttribute(node, total_node + del,
                             CONC_A); // add parcel to node
        ElleSetNodeAttribute(nbnodes[i], total_neighbour - del,
                             CONC_A); // remove parcel from neighbour
      }
    }
  }
}
