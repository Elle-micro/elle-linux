...
void SetUnodeAttributeFromNbs(int flynnid, int attr_id) {
  ...
  for (i = 0; i < count; i++) {
    ...
    ElleGetTriPtNeighbours(unodelist[i], nbnodes, bndflag,
                           0); // get the  list of neighbours for a unode
    num_nbs = nbnodes.size();

    for (total = 0, j = 0; j < num_nbs;
         j++) // loop through all neighbouring unodes
    {
      ElleGetUnodeAttribute(nbnodes[j], attr_id,
                            &val); // get a neighbouring unode attribute value
      total += val;
    }
    ElleGetUnodeAttribute(unodelist[i], attr_id,
                          &val); // get old unode attribute value
    total += val;

    val = total / (num_nbs + 1);
    ElleSetUnodeAttribute(unodelist[i], attr_id,
                          val); // set new unode attribute value
  }
}
