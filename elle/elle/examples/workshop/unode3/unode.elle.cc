...
void SetUnodeAttributeFromNbs(int flynnid, int attr_id) {
  ...
  for (loops = 0; loops < 1; loops++) {
    for (i = 0; i < count; i++) {
      ...
      ElleGetUnodeAttribute(unodelist[ii], attr_id,
                            &val); //  get the unodes attribute
      ...
      {
        ElleGetUnodeAttribute(nbnodes[j], attr_id,
                              &val); // get neighbouring unode attribute value
      ...
      if (histo[nodeval] ==
          num_nbs) // dont't bother if node is surrounded by equals
        continue;
      ...
      if (m < prob)
        ElleSetUnodeAttribute(unodelist[ii], attr_id,
                              (double)k); // set new unode attribute value
    }
  }
}
