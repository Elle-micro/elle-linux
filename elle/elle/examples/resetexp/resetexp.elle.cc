...
int InitExpand() {
  ...
  if (strlen(infile) > 0) {
    ...
    ElleAddDoubles();
    if (ElleUnodesActive() && !ElleUnodeAttributeActive(U_ATTRIB_A)) {
...
int ExpandGrain() {
  ...
  for (i = 0; i < EllemaxStages(); i++, Count++) {
    ...
    if (!(i % 10)) {
      ...
      totmass = mass + nmass;
      cout << i << "umass " << mass << "  nmass " << nmass << "  tot "
           << totmass << endl;
    }
    maxf = ElleMaxFlynns();
    ...
    for (n = 0; n < max; n++) {
      ...
      if (reset && ElleNodeIsActive(j)) {
        if (ElleNodeAttribute(j, N_ATTRIB_A) != 0.0) {
          ElleSetNodeAttribute(j, ElleNodeAttribute(j, CONC_A), N_ATTRIB_B);
          ElleSetNodeAttribute(j, 1, CONC_A);
        }
        ElleNeighbourNodes(j, nbs);
        for (k = 0; k < 3; k++)
          if (nbs[k] != NO_NB && ElleNodeAttribute(nbs[k], N_ATTRIB_A) != 0.0) {
            ElleSetNodeAttribute(nbs[k], ElleNodeAttribute(nbs[k], CONC_A),
                                 N_ATTRIB_B);
...
int MoveNodeOut(int node1, int expflynn, bool use_area) {
  ...
  if (cnt > 1) {
    ...
    i = 0;
    while (i < 3 && (nbnodes[i] == NO_NB || nbnodes[i] == nghbr[0]))
      i++;
    ...
    if (i == 3)
      OnError("MoveNodeOut - bnd nb not found", 0);
  } else {
    if ((nghbr[1] = ElleFindBndNb(node1, expflynn)) == NO_NB)
      OnError("MoveNodeOut", NONB_ERR);
    i = 0;
    while (i < 3 && (nbnodes[i] == NO_NB || nbnodes[i] == nghbr[1]))
      i++;
...
int CalcMove(int node, int *nbs, Coords *incr, bool use_area) {
  ...
  angle(xpts[1], ypts[1], xpts[2], ypts[2], xpts[0], ypts[0], &ang);
  ...
  if (ang < 0)
    a += PI;
  rotate_coords(xpts[2], ypts[2], xpts[1], ypts[1], &dir_x, &dir_y, a);
  if (dir_y == ypts[1] && dir_x == xpts[1])
    a = 0;
  else
...
int WriteCircleFile(char *nodefname, char *grainfname) {
  ...
  if ((num = fscanf(fin, "%19s\n", label)) != 1)
    return (READ_ERR);
  if (!id_match(FileKeys, FLYNNS, label))
    return (KEY_ERR);
  fprintf(fout, "%s\n", label);
  while (!feof(fin)) {
