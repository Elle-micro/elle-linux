...
void PlotUnodes(int col_range, int col_min, int attr, double min, double max);
void EllePlotPolygon(int index, float *xpts, float *ypts, int *id,
                     int num_nodes, int col, int outline, int bndattrib,
                     char *label);
...
void EllePlotRegions(int count) {
  ...
  case GRAINS:
    ...
    if ((draw_unodes != NONE && triangles) ||
        (draw_unodes != NONE && (!triangles || draw_unodes == U_LOCATION)))
    ...
    if (drawnodes != NONE)
      EllePlotNodes();
#if XY
    if (draw_unodes != NONE && (!triangles || draw_unodes == U_LOCATION))
  ...
  for (i = 0; i < max; i++) {
    if (ElleFlynnIsActive(i) && !isParent(i)) {
      ...
      if (ElleFlynnAttributeActive(COLOUR))
        ...
        switch (draw_attrib) {
        case GRAIN:
          ElleGetFlynnIntAttribute(i, &j, GRAIN);
          if (j >= 0)
            col = (j % col_range) + col_min;
          break;
        case EXPAND:
          ElleGetFlynnIntAttribute(i, &j, EXPAND);
          if (j == 0)
            col = col_min + (int)(col_range * 0.24);
          else
            col = col_min + (int)(col_range * 0.82);
          break;
        case MINERAL:
          ElleGetFlynnIntAttribute(i, &j, MINERAL);
          if (j == QUARTZ)
        ...
        case E3_ALPHA:
        case EULER_3:
          ElleGetFlynnEuler3(i, &val, &valb, &valc);
          if (arange > 0) {
      ...
      EllePlotPolygon(i, xpts, ypts, id, num_nodes, col, outline,
                      draw_bndattrib, 0);
      if (id)
        free(id);
      id = 0;
...
void EllePlotPolygon(int index, float *xptsorig, float *yptsorig, int *id,
                     int num_nodes, int col, int outline, int bndattrib,
                     char *label) {
  ...
  for (i = start; i < xflags[0] + 1; i++) {
    ...
    if (tmpbl.y < unitcell.cellBBox[TOPRIGHT].y &&
      ...
      if (label != 0) {
        if (polyCentroid(xpts, ypts, num_nodes, &xcentre, &ycentre, &area) !=
            2) {
          centre[0] = (float)xcentre;
          centre[1] = (float)ycentre;
          ElleCheckUnit(&centre[0], &centre[1], 1, xflagslabel, yflagslabel,
                        &lbl, &ltr);
          if (!xflagslabel[0] && !xflagslabel[1] && !yflagslabel[0] &&
              !yflagslabel[1])
            drawlabel_(&centre[0], &centre[1], &x_off, &y_off, &mode, label,
                       &len);
        }
      }
    }
  ...
  for (i = start; i < yflags[1] + 1; i++) {
    for (j = 0; j < num_nodes; j++)
      xpts[j] = xptsorig[j] + i * xoffset;
    for (j = 0; j < num_nodes; j++)
      ypts[j] = yptsorig[j] + i * incry;
...
void ElleShowStages(int stages) { SetStage(stages); }
void EllePlotBoundaryAttribute(int *id, int num, float *xpts, float *ypts) {
  int i, j, col;
  int col_range, col_max = E_COL_MAX, col_min = E_COL_MIN;
  int state = 3, oldcol;
  ...
  arange = amax - amin;
  if (arange == 0)
    FindBndAttributeRange(MISORIENTATION, &amin, &amax);
  col_range = col_max - col_min;
    ...
    if (ElleNodeIsActive(i)) {
      ...
      if (node_colour == NUM_NB || node_colour == TRPL_ONLY) {
        if (ElleNodeIsDouble(i))
          col = E_DFLT_DNODE_COL;
        else
          col = E_DFLT_TNODE_COL;
      } else {
...
void EllePlotUnodes() {
  ...
  if (ElleUnodesActive()) {
    ...
    if (attr != U_LOCATION && triangles == 1) {
      ...
      DrawUnodes(col_range, col_min, attr, min, max);
  }
}
...
void PlotUnodes(int col_range, int col_min, int attr, double min, double max) {
  ...
  for (i = 0; i < umax; i++) {
    if (range != 0) {
      switch (attr) {
      case CAXIS:
        err = ElleGetUnodeAttribute(i, &val, &val2, attr);
        break;
      case E3_ALPHA:
      case EULER_3:
        err = ElleGetUnodeAttribute(i, &val, attr);
        // val=fmod((val+90.0),30.0)/15;
      ...
      if (col < col_min)
        col = col_min;
      else if (col > col_min + col_range)
        col = col_min + col_range;
    }
    /*it->getPosition(&xy);*/
...
void PlotSegments(int col_range, int col_min, int attr, double min,
                  double max) {
...
void ElleUpdateSettings() {
  ...
  if (draw_attrib != NONE && draw_attrib != NUM_NB &&
      draw_attrib != TRPL_ONLY && !ElleNodeAttributeActive(draw_attrib))
    options->SetNodeAttribOption(NUM_NB, 0.0, 0.0);
  options->GetUnodeAttribOption(&draw_attrib);
  // This needs a rethink - display attribs may differ from file attribs
  if (draw_attrib == EULER_RGB)
    draw_attrib = E3_ALPHA;
...
void ElleShowStages(int stages) { (void)stages; }

void Init_Data(void) {}

void EllePlotRegions(int count) { (void)count; }

void ElleRescaleWin() {}

// the 2 dummy fns below are only necessary for diffusion
void GetUpdateFreq(int *val) { *val = 0; }
