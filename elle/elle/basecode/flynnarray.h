...
public:
  ...
  int flynnNodePositions(int flynnindex, std::vector<Coords> &rel_pos,
                         Coords *ref);
  int flynnNodeCount(int index) { return (_a[index].flynnNodeCount(index)); }
  ...
  bool hasAttribute(const int id) { return (_dfltAttributes.hasAttribute(id)); }
  int numDfltAttributes() { return (_dfltAttributes.numAttributes()); }
  int numFlynnAttributes(const int flynnindex);
  void initAttribute(const int id) { _dfltAttributes.initAttribute(id); }
  ...
  int setDfltAttribute(const int id, int val) {
    ...
    return 0;
  } // modernize: added explicit return 0 (fast-track analysis edit)
  ...
  int splitFlynn(const int type, int flynnindex, int *child1, int *child2,
                 float dx, float dy);
  int splitWrappingFlynn(int flynnindex, int node1, int node2, int *child1,
                         int *child2);
  int directSplitWrappingFlynn(int flynnindex, int node1, int node2,
  ...
  int writeUnodePolyFile(int, const int, const char *);
