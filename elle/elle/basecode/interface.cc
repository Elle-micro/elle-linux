...
int ElleMaxFlynns() { return (flynns.max()); }
...
int ElleNumberOfGrains() { return (flynns.numberOfGrains()); }

int ElleNumberOfSubgrains() { return (flynns.numberOfSubgrains()); }

int ElleMaxLevels() { return (flynns.maxLevels()); }

int ElleFlynnLevel(const int index) { return (flynns.getFlynnLevel(index)); }

int ElleFindSpareFlynn() { return (flynns.spareFlynn()); }
...
void ElleCleanFlynnArray() { flynns.clean(); }

int ElleUnodesActive() { return (UnodesActive()); }

void ElleCleanUnodeArray() { UnodesClean(); }
...
int getParent(int flynn1) { return (flynns.getParent(flynn1)); }
...
void EllePromoteFlynn(int flynnindex) { flynns.promoteFlynn(flynnindex); }
...
int ElleSplitFlynnandPromote(const int type, int index, int *c1, int *c2,
  ...
  if ((retval = ElleSplitFlynn(type, index, &subflynns[0], &subflynns[1], dx,
                               dy)) == 0) {
...
void ElleSplitFlynnOnArea(const int type, int index, double maxarea, float dx,
                          float dy) {
  int c1, c2;
  double area;
...
int ElleNucleateFlynnFromNode(int node, int *newid) {
  ...
  if (ElleNodeIsTriple(node)) {
    ...
    xyn.x =
        midpt.x + (midpt.x - xy0.x) * cos(ang) + (midpt.y - xy0.y) * sin(ang);
    xyn.y =
        midpt.y + (midpt.y - xy0.y) * cos(ang) - (midpt.x - xy0.x) * sin(ang);
    xynn.x =
        midpt.x + (midpt.x - xy1.x) * cos(ang) + (midpt.y - xy1.y) * sin(ang);
    xynn.y =
        midpt.y + (midpt.y - xy1.y) * cos(ang) - (midpt.x - xy1.x) * sin(ang);
    xpts[0] = xy1.x;
    ypts[0] = xy1.y;
    xpts[1] = xyn.x;
    ypts[1] = xyn.y;
...
int ElleNucleateFlynnFromNode(int node, int *newid) {
  ...
  if (ElleNodeIsTriple(node)) {
    ...
    midpt.x = xy0.x - (xy0.x - xy1.x) * 0.5;
    midpt.y = xy0.y - (xy0.y - xy1.y) * 0.5;
    xyn.x = (midpt.x - xy0.x) * cos(ang) + (midpt.y - xy0.y) * sin(ang);
    xyn.y = -(midpt.x - xy0.x) * sin(ang) + (midpt.y - xy0.y) * cos(ang);
    xyn.x += xy0.x;
    xyn.y += xy0.y;
    xynn.x = (midpt.x - xy0.x) * cos(-ang) + (midpt.y - xy0.y) * sin(-ang);
    xynn.y = -(midpt.x - xy0.x) * sin(-ang) + (midpt.y - xy0.y) * cos(-ang);
    xynn.x += xy0.x;
    xynn.y += xy0.y;
    xpts[0] = xy1.x;
    ypts[0] = xy1.y;
...
int ElleWriteParentFlynn(int index, FILE *fp) {
  ...
  if (index < ElleMaxFlynns()) {
    flynns.getChildList(index, l);
    if ((num = fprintf(fp, "%d", index)) < 0)
      err = WRITE_ERR;
    if (!err) {
      /*if ((num=fprintf(fp," %d",ElleGetFlynnFirstNode(index)))<0)*/
      /*err = WRITE_ERR;*/
      /*else*/
      if ((num = fprintf(fp, " %ld", l.size())) < 0)
        err = WRITE_ERR;
      for (it = l.begin(); it != l.end() && !err; it++)
...
void ElleRemoveFlynn(int flynnindex) { flynns.removeFlynn(flynnindex); }
...
int ElleFlynnNodePositions(int index, vector<Coords> &rel_pos, Coords *ref) {
  return (flynns.flynnNodePositions(index, rel_pos, ref));
}

double ElleFlynnArea(int index) { return (flynns.flynnArea(index)); }

int ElleFlynnNodeCount(int index) { return (flynns.flynnNodeCount(index)); }
...
int ElleSplitWrappingFlynn(int index, int node1, int node2, int *child1,
                           int *child2) {
  return (flynns.splitWrappingFlynn(index, node1, node2, child1, child2));
}

int ElleDirectSplitWrappingFlynn(int index, int node1, int node2, int *child1,
                                 int *child2, Coords *dir) {
  return (flynns.directSplitWrappingFlynn(index, node1, node2, child1, child2,
                                          dir));
}
...
unsigned char ElleFlynnHasAttribute(int index, int id) {
  ...
  if (id == EULER_3)
    ...
    found = flynns.hasFlynnAttribute(index, id);
  return (found ? 1 : 0);
}
...
void ElleSetFlynnCAxis(int index, Coords_3D *val) {
  int err = 0;
  /* modernize: intentional assignment in condition (behavior-neutral) */
...
void ElleSetFlynnIntAttribute(int index, int val, int id) {
  ...
  if (isFlynnIntegerAttribute(id))
    ...
    err = ATTRIBID_ERR;
  if (err)
    OnError("ElleSetFlynnIntAttribute", err);
}
...
void ElleGetFlynnIntAttribute(int index, int *val, int id) {
  ...
  if (isFlynnIntegerAttribute(id)) {
    ...
    err = ATTRIBID_ERR;
  if (err)
    OnError("ElleGetFlynnIntAttribute", err);
}
...
int ElleDefaultFlynnIntAttribute(int id) {
  ...
  if (isFlynnIntegerAttribute(id))
    ...
    err = ATTRIBID_ERR;
  if (err)
    OnError("ElleDefaultFlynnIntAttribute", err);
  return (val);
}
...
double ElleDefaultFlynnRealAttribute(int id) {
  ...
  if (isFlynnRealAttribute(id))
    ...
    err = ATTRIBID_ERR;
  if (err)
    OnError("ElleDefaultFlynnRealAttribute", err);
  return (fval);
}
...
void ElleSetFlynnRealAttribute(int index, double val, int id) {
  ...
  if (id == F_INCR_S)
    id = INCR_S;
  if ((err = flynns.setFlynnAttribute(
           index, id, val))) // modernize: intentional assignment in condition
                             // (behavior-neutral)
...
void ElleGetFlynnRealAttribute(int index, double *valptr, int id) {
  ...
  if (isFlynnAttribute(id)) {
    ...
    err = ATTRIBID_ERR;
  if (err)
    OnError("ElleGetFlynnRealAttribute", err);
}
  ...
  if (ElleFlynnAttributeActive(attr)) {
    ...
    for (i = 0; i < max; i++) {
      if (ElleFlynnIsActive(i) /*&& ElleFlynnHasAttribute(i,attr)*/) {
        ...
        case MINERAL:
        case GRAIN:
          ElleGetFlynnIntAttribute(i, &ival, attr);
          val = (double)ival;
          break;
        case VISCOSITY:
        case S_EXPONENT:
        ...
        case E_YY:
        case E_ZZ:
          ElleGetFlynnStrain(i, &val, attr);
          break;
        case E3_ALPHA:
        case EULER_3:
          ElleGetFlynnEuler3(i, &val, &b, &c);
...
int ElleWriteFlynnPolyFile(int flynnindex, char *fname) {
  if (ElleFlynnIsActive(flynnindex))
    return flynns.writePolyFile(flynnindex, fname);
  return 0;
}

int ElleWriteFlynnUnodePolyFile(int flynnindex, const int attrib, char *fname) {
  return flynns.writeUnodePolyFile(flynnindex, attrib, fname);
}
...
int ElleAddUnodeToFlynn(int id, Coords *xy, int start) {
  ...
  if (!ElleFlynnIsActive(start) || isParent(start)) {
    ...
    if (i != start)
      ...
      OnError("ElleAddUnodeToFlynn-no valid flynns", 0);
  }
  orig_xy = *xy;
  repos = 0;
  /*
  ...
  while (rgn == NO_NB && repos < 9) {
    ...
    if (rgn == NO_NB) {
      ...
      for (it = nbflynns.begin(); it != nbflynns.end() && rgn == NO_NB; it++) {
    ...
    if (rgn == NO_NB) {
      switch (repos) {
      case 1:
        orig_xy.x = xy->x + unitcell.xlength;
...
int ElleVoronoiFlynnUnodes(std::list<int> &flynnlist) {
  ...
  flynnlist.sort();
  for (std::list<int>::iterator it = flynnlist.begin(); it != flynnlist.end();
       it++) {
...
void ElleInitRandom(unsigned long int seed) { erandom.seed(seed); }

unsigned long int ElleRandom() { return (erandom.ran()); }
...
class which uses one of the gsl (GNU scientific Library) random number
generators. \verbatim [Multi-line code example] \endverbatim [Multi-line
detailed description]
*/
double ElleRandomD() { return (erandom.randouble()); }
///////////////////////////////////////////////////////
 ...
 * any flynns which currently have the default value (??)
    err = flynns.setFlynnAttribute(index,id,(double)val);
 */
...
void ElleCheckFlynnDefaults(int id) {
  ...
  case E_ZZ:
    ...
    break;
  default:
    OnError("ElleCheckFlynnDefaults", 0);
    break;
  }
  //
  ...
  max = ElleMaxFlynns();
  for (i = 0; i < max; i++) {
    if (ElleFlynnIsActive(i) && !hasParent(i) && !ElleFlynnHasAttribute(i, id))
      switch (id) {
      case EXPAND:
      case COLOUR:
      case SPLIT:
