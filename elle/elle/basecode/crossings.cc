...
int ElleCrossingsCheck(int node, Coords *pos_incr) {
  ...
  for (i = 0; i < 3; i++) {
    if (nbrgns[i] != NO_NB) {
      ...
      if (j = ElleFindIntersection(node, NO_NB, nbrgns[i], &currxy, &newxy,
                                   &intersect[count]))
        count += j;
      ElleNeighbourRegion(nbnodes[i], node, &rgn);
      if (j = ElleFindIntersection(node, nbnodes[i], rgn, &currxy, &newxy,
                                   &intersect[count]))
        count += j;
    }
  }
  if (count == 0) {
    for (i = 0; i < 3; i++) { /* for each segment */
      if (nbnodes[i] != NO_NB) {
        ...
        ElleNeighbourRegion(node, nbnodes[i], &rgn);
        if (j = ElleFindIntersection(node, nbnodes[i], rgn, &newxy, &nbxy,
                                     &intersect[count]))
          count += j;
        ElleNeighbourRegion(nbnodes[i], node, &rgn);
        if (j = ElleFindIntersection(node, nbnodes[i], rgn, &newxy, &nbxy,
    ...
    for (i = 0; i < 3; i++) { /* for each segment */
      if (tmpnbs[i] != NO_NB) {
        ...
        if (j = ElleFindIntersection(node, tmpnbs[i], chk_rgn, &newxy, &nbxy,
                                     &isectsegs[segcount]))
          segcount += j;
      }
    }
...
#ifdef DEBUG
        if (ElleNodeIsActive(newdj[0]))
          writeNodeLocalPolyFile(newdj[0]);
#endif
      }
    } else {
      if (ElleNodeIsActive(node))
...
void ElleMoveLink(Intersection *intersect, Angle_data *ang, Coords *newxy) {
  int node, nnew, i;
  ...
  if (ElleNodeIsTriple(nnew)) {
    ...
    if (end == ang->pivot)
      ElleUpdateFirstNodes(end, nnew, oldrgn);
  }
}
...
void ElleReplaceTripleJ(Intersection *intersect, int *dj, int num) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if ((nbindex = ElleFindNbIndex(nb, node)) == NO_NB)
      OnNodeError(node, "ReplaceTripleJ", NONB_ERR);
    ElleInsertDoubleJ(intersect[i].node1b, intersect[i].node2b, &dj[i],
                      1.0 / (num - i + 1));
...
int ElleFindIntersection(int node1, int nb, ERegion rgn, Coords *strt_pos,
  ...
  sprintf(msg, "%s %d %d Region=%d ", "ElleFindIntersection", node1, nb, rgn);
  /*
  ...
  for (j = 0; j < num_segs /*&& !found*/; j++) {
    ...
    if (k == 1) {
      ...
      found = 1;
      if (count < MAX_I_CNT - 1)
        count++;
      else
        OnError("FindIntersection", RANGE_ERR);
    }
    i = inb;
    inb = (i + 1) % num_nodes;
    x3 = x4;
    ...
    if (nbnodes[i] != NO_NB) {
      ...
      for (j = 0; j < 3; j++) {
        if (nbnbnodes[j] != NO_NB && nbnbnodes[j] != node) {
          ...
          if (angle(xy.x, xy.y, tmp2.x, tmp2.y, newxy->x, newxy->y, &currang))
            OnNodeError(node, "angle error", 0);
          if (angle(xy.x, xy.y, tmp2.x, tmp2.y, prev.x, prev.y, &prevang))
            OnNodeError(node, "angle error", 0);
          if ((fabs(currang - prevang)) < PI &&
                  ((currang < -eps && prevang > eps) ||
                   (currang > eps && prevang < -eps)) ||
              (fabs(currang) < angeps && fabs(currang) < fabs(prevang))) {
#ifdef DEBUG
  ...
  for (i = 0; i < 3 && *count == 0; i++) {
    ...
    for (j = 0; j < 3 && !found; j++) {
      ...
      if (k < 3) {
        ...
        if (angle(newxy->x, newxy->y, xy.x, xy.y, tmp2.x, tmp2.y, &currang))
          OnNodeError(node, "angle error", 0);
        if (angle(prev.x, prev.y, xy.x, xy.y, tmp2.x, tmp2.y, &prevang))
          OnNodeError(node, "angle error", 0);
...
int ElleRegionIsSimple(ERegion poly, Intersection *intersect) {
  ...
  if (num_nodes > 3) { // can't intersect unless more than 3 sides
    ...
    for (seg = 0; seg < num_nodes - 2 && !found; seg++) {
      ...
      for (next_seg = seg + 2, j = 0;
           next_seg < num_nodes && j < count && !found; next_seg++, j++) {
        next_index = (next_seg + 1) % num_nodes;
        k = 0;
        if (EllePtInRect(testrect, 4, &pos[next_seg]))
          k = lines_intersect(pos[seg].x, pos[seg].y, pos[seg + 1].x,
                              pos[seg + 1].y, pos[next_seg].x, pos[next_seg].y,
                              pos[next_index].x, pos[next_index].y, &xy.x,
                              &xy.y);
        /*if (k==2) printf("colinear\n");*/
        if (k == 1) {
#ifdef DEBUG
...
// only do the immediate nb regions
  ...
  while (its != nbflynnset.end()) {
    curr = *its;
    if (ElleFlynnIsActive(curr) && !ElleRegionIsSimple(curr, &isect)) {
      ...
      if (rgna == rgnb &&
          (ElleFindNbIndex(isect.node2b, isect.node1a) == NO_NB ||
           ElleNodeIsTriple(isect.node2b) || ElleNodeIsTriple(isect.node1a)) &&
          (ElleFindNbIndex(isect.node2a, isect.node1b) == NO_NB ||
           ElleNodeIsTriple(isect.node2a) || ElleNodeIsTriple(isect.node1b))) {
#if XY
        ...
        dir[0].x -= xy.x;
        dir[0].y -= xy.y;
        rotate_coords(dir[0].x, dir[0].y, 0, 0, &dir[1].x, &dir[1].y, PI_2);
        rotate_coords(dir[0].x, dir[0].y, 0, 0, &dir[2].x, &dir[2].y, -PI_2);
      ...
      if (ElleReplaceIntersection(&isect, &fnew) == 0) {
        ...
        if (small_flynn != NO_NB) {
          ...
          while (it != nbflynns.end() &&
                 !ElleFlynnAttributesAllowMerge(small_flynn, *it))
            it++;
          if (it == nbflynns.end())
            it = nbflynns.begin();
  ...
  while (its != nbflynnset.end()) {
    ...
    if (ElleFlynnIsActive(*its) && (ElleRegionArea(*its) < min_area)) {
      ...
      // try for one that allows merge
      it = nbflynns.begin();
      while (it != nbflynns.end() && !ElleFlynnAttributesAllowMerge(*its, *it))
        it++;
#if XY
      // Need to check if nb flynns are smaller?
      std::list<int>::iterator itmp = nbflynns.begin();
      while (itmp != nbflynns.end() && ElleRegionArea(*itmp) < area)
        itmp++;
...
int ElleReplaceIntersection(Intersection *intersect, int *rgn_new) {
  ...
  *rgn_new = NO_NB;
  ...
  if (bnd2a1b) {
    ...
    ResetRegionEntries(newb, &tmpa, intersect->node1b, intersect->chk_rgn,
                       rgnb);
    ElleClearNeighbour(intersect->node1b,
                       ElleFindNbIndex(newb, intersect->node1b));
    ElleClearNeighbour(newb, ElleFindNbIndex(intersect->node1b, newb));
    ...
    ResetRegionEntries(newa, &tmpa, intersect->node1a, intersect->chk_rgn,
                       rgna);
    ElleClearNeighbour(newa, ElleFindNbIndex(intersect->node1a, newa));
