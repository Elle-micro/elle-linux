...
public:
  Flynn()
      : state(INACTIVE), color(false), first_node(NO_NB), parent(NO_NB) {
  } // modernize: reorder init list to match declaration order
    // (behavior-neutral)
  Flynn(bool stateval, int first, int p)
      : state(stateval), color(false), first_node(first), parent(p) {
  } // modernize: reorder init list to match declaration order
    // (behavior-neutral)
  ...
  void getChildList(std::vector<int> &l) {
    it = child_list.begin();
  ...
  bool hasFlynnAttribute(const int id) { return (attributes.hasAttribute(id)); }
  void removeFlynnAttribute(const int id) { attributes.removeAttribute(id); }
  ...
  int setFlynnAttribute(const int id, int val) {
    attributes.setAttribute(id, val);
    return (0);
  }
  ...
  void getUnodeList(std::vector<int> &l) {
    it = unode_list.begin();
