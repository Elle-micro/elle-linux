...
int EllePtInRegion(ERegion poly, Coords *pt) {
  ...
  if (num_nodes > 2) {
    if ((coords = (double *)malloc(num_nodes * sizeof(double) * 2)) == 0)
      OnError("EllePtInRegion", MALLOC_ERR);
    /*
...
int EllePtInRegion(Coords *bndpts, int num, Coords *pt) {
  ...
  if ((coords = (double *)malloc(num * sizeof(double) * 2)) == 0)
    OnError("EllePtInRegion", MALLOC_ERR);
...
int EllePtInRect(Coords *bndpts, int num, Coords *pt) {
  int in = 0;
  if (num == 4 && pt->x >= bndpts[BASELEFT].x && pt->x <= bndpts[TOPRIGHT].x &&
...
double polyArea(double x[], double y[], int n) {
  int i, j; // modernized: removed obsolete register
  double ai, area = 0;
  if (n < 3)
    return 0;
