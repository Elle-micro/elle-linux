...
void Partition(double prev[2], Incr_Data *incr_enrich, Incr_Data *incr_sweep,
               int increments, double *total_gb_mass, double *nb_mass_f,
               vector<double> &delta_mass, double gb_area_f, double node_mass_i,
               double node_sweep_frac, int attr_id, set_int *enrichlist,
               set_int *sweeplist, set_int *unodelist, double roi);
void UpdateUnodes(set_int *unodelist, vector<double> &delta_mass, int attr_id);
double SumTriangleWeights(Coords *current, double *incr_areas, Coords *mid_pts,
                          int increments, double ROI);
double PointWeight(Coords *line_pos, Coords *current, double ROI);
double EstimateConcFromUnodeValues(Coords *pt_pos, vector<int> *unodelist);
void ReassignUnodes(double tri_pos[3][2], double roi, int enrich_id,
                    int sweep_id, int attr_id, int *enrich_indx,
                    int *sweep_indx, set_int **unodeset, set_int *flynnset);
double SumLineWeights(double tri_pos[3][2], Coords current, double ROI);
void set_up_mid_line(double tri_pos[3][2], double pivot[2], double *inc_width,
                     double *line_height_inc, double *length, double *height,
                     double del[2][2]);
void set_up_tri_values(double tri_pos[3][2], double pivot[2], double gb_area,
                       double gb_area_f, double conc_node, double conc_nb,
...
void UpdateGBMUnodeValues(int n, Coords new_incr, set_int **unodeset,
  ...
  for (i = 0; i < num_unodes; i++)
    delta_mass.push_back(0.0);
  for (j = 0; j < Increments; j++)
    if (!(Weights[j] = (double *)malloc(num_unodes * sizeof(double))))
      OnError("UpdateGBMUnodeValues", MALLOC_ERR);
  for (i = 0; i < 3; i++)
    gb_area_f[i] = gb_area_i[i] = gb_mass[i] = gb_mass_f[i] = nb_area[i] =
        nb_mass[i] = nb_mass_f[i] = nb_sweep_frac[i] = sweep_area[i] =
            gb_nb_mass[i] = 0.0;
...
ElleGetFlynnIntAttribute(rgnr, &e2, EXPAND); // prevents trip junctions getting
too much mass if (triarea>0) { sweep_id = rgnl; enrich_id = rgnr;
}
else {
sweep_id = rgnr;
enrich_id = rgnl;
      ...
      */
      ...
      gb_area_i[i] =
          pointSeparation(&pivot_node, &old_node) * gb_width / ElleUnitLength();
      ...
      if (e1 != e2) // only if segment sweeps lattice
      {
        ...
        ReassignUnodes(tri_pos, roi, enrich_id, sweep_id, attr_id, &enrich_indx,
                       &sweep_indx, unodeset, flynnset);
        ...
        for (j = 0; j < Increments; j++)
          InitIncrData(&incr_sweep[j]);
        for (j = 0; j < Increments; j++)
          InitIncrData(&incr_enrich[j]);
        for (k = 0; k < Increments; k++)
          for (j = 0; j < num_unodes; j++)
            Weights[k][j] = 0.0;

        set_up_tri_values(tri_pos_s, pivot, gb_area_i[i], gb_area_f[i], conc_n,
                          conc_nb, &nb_sweep_frac[i], sweep_area[i], incr_sweep,
                          Increments);
        set_up_tri_values(tri_pos_e, pivot, gb_area_i[i], gb_area_f[i], conc_n,
                          conc_nb, &nb_sweep_frac[i], sweep_area[i],
                          incr_enrich, Increments);

        Partition(tri_pos[0], incr_sweep, incr_enrich, Increments,
                  &gb_mass_f[i], &nb_mass_f[i], delta_mass, gb_area_f[i],
                  gb_mass[i], (1.0 - nb_sweep_frac[i]), attr_id,
                  unodeset[enrich_indx], unodeset[sweep_indx],
                  unodeset[COMBINED_LIST], roi);
      }
    }
  }
  for (i = 0; i < 3; i++) {
  ...
  delta_unode_mass = accumulate(delta_mass.begin(), delta_mass.end(), 0.0);
  delta_node_mass =
      total_gb_mass_f - total_gb_mass_i + total_nb_mass_f - total_nb_mass_i;
  ...
  if ((delta_unode_mass =
    ...
    for (i = 0; i < 3; i++) {
      if (nb[i] != NO_NB) {
        delta_nb = delta_unode_mass * (sweep_area[i] / total_sweep_area) *
                   nb_sweep_frac[i];
        if (delta_nb < -nb_mass_f[i])
          delta_nb = -nb_mass_f[i];
  ...
  if (total_gb_mass_f < 0.0)
    total_gb_mass_f = 0.0;
  for (i = 0; i < 3; i++)
    if (nb[i] != NO_NB)
      ElleSetNodeAttribute(
          nb[i], (nb_mass[i] + nb_mass_f[i]) / (nb_area[i] + gb_area_f[i] / 2),
          attr_id);
    ...
    if ((delta_unode_mass =
      ...
      for (set_int ::iterator it = unodelist->begin();
        ...
        if (u_mass > 0) {
          j = 0;
          while (j < max &&
                 (delta_mass[j] >= 0.0 || (delta_mass[j] + u_mass) < 0.0))
            j++;
          if (j < max) {
...
void ReassignUnodes(double tri_pos[3][2], double roi, int enrich_id,
  ...
  *enrich_indx = NO_NB, *sweep_indx = NO_NB;
   ...
   */
  for (it = unodeset[*sweep_indx]->begin(); it != unodeset[*sweep_indx]->end();
       it++) {
    ElleGetUnodePosition(*it, &xy);
    pnt[0] = xy.x;
    pnt[1] = xy.y;
    if (find(unodeset[*enrich_indx]->begin(), unodeset[*enrich_indx]->end(),
             *it) == unodeset[*enrich_indx]->end() &&
...
void Partition(double prev_pt[2], Incr_Data *incr_sweep, Incr_Data *incr_enrich,
  ...
  for (it = sweeplist->begin(); it != sweeplist->end(); it++)
    if (find(enrichlist->begin(), enrichlist->end(), *it) != enrichlist->end())
  ...
  for (k = 0, frac = (increments2 - 1) / increments2; k < increments;
    ...
    for (cnt = 0, it = unodelist->begin(); it != unodelist->end() && !err;
         it++, cnt++) {
      i = (*it);
      ElleGetUnodePolyInfo((*it), attr_id, &current, &conc);
      ...
      if (weight != 0.0 || weight2 != 0) {
        ...
        if (find(reassigned.begin(), reassigned.end(), i) != reassigned.end()) {
          weight1 /= 2;
          weight /= 2;
        }
        Weights[k][cnt] = weight1;
        tmp = conc * u_area * weight1;
        if (find(sweeplist->begin(), sweeplist->end(), i) != sweeplist->end()) {
    ...
    if (Total_Enrich_Weights[0] > 0)
      conc_e /= Total_Enrich_Weights[0];
    ...
    if (Total_Sweep_Weights[1] > 0)
      conc_s1 /= Total_Sweep_Weights[1];
    ...
    if (conc_s != conc_b || conc_e != conc_b) {
      ...
      mass_chge =
          conc_s_f * incr_sweep[k].area * Overlap_frac * swept_area_frac;
      mass_chge_s = -incr_sweep[k].swept_mass_i + mass_chge;
      mass_chge =
          conc_e_f * (1.0 - Overlap_frac) * incr_sweep[k].area +
...
mass_chge_e += mass_chge;
    ...
    for (cnt = 0, it = unodelist->begin(); it != unodelist->end() && !err;
      ...
      i = (*it);
      if (Weights[k][cnt] &&
          find(sweeplist->begin(), sweeplist->end(), i) != sweeplist->end())
        delta_mass[cnt] += (/*incr[k].swept_mass_i+*/ mass_chge_s) *
                           Weights[k][cnt] / Total_Sweep_Weights[0];
      if (Weights[k][cnt] && find(enrichlist->begin(), enrichlist->end(), i) !=
                                 enrichlist->end()) {
        /*if (find(reassigned.begin(),reassigned.end(),i)==*/
        /*reassigned.end())*/
  ...
  *total_gb_mass = node_mass_i + node_chge_e;
  if (*total_gb_mass > tot_gbmass_f)
    *total_gb_mass = tot_gbmass_f;
  ...
  for (k = 0, frac = (increments2 - 1) / increments2; k < increments;
    ...
    for (cnt = 0, it = unodelist->begin(); it != unodelist->end() && !err;
      ...
      if (weight != 0.0 || weight1 != 0.0) {
        Weights[k][cnt] = weight1;
        if (find(sweeplist->begin(), sweeplist->end(), i) != sweeplist->end()) {
          if (print && (k == 0 || k == 19 || k == 39))
            cout << "sweep " << i << ' ' << conc << endl;
          Total_Sweep_Weights[1] += weight1;
          conc_s1 += weight1 * conc;
        }
        if (find(enrichlist->begin(), enrichlist->end(), i) !=
    ...
    if (conc_s1 != conc_s_f) {
      ...
      conc_s_f = (conc_s1 * Total_Sweep_Weights[1] / tot_weight * incr[k].area +
                  incr[k].gb_mass_i) *
                 pc / (incr[k].area + gb_incr_area);
      if (conc_s_f < conc_s1)
        mass_chge_s += (conc_s_f - conc_s1) * incr[k].area *
                       Total_Sweep_Weights[1] / tot_weight;
      conc_e_f =
          (incr[k].gb_mass_i - mass_chge_s +
           conc_e1 * incr[k].area * Total_Enrich_Weights[1] / tot_weight) /
          (incr[k].area + gb_incr_area);
      mass_chge_b = (conc_e_f - conc_e1) * Total_Enrich_Weights[1] /
                    tot_weight * incr[k].area;
      if (mass_chge_b > (incr[k].gb_mass_i - mass_chge_s))
        conc_e_f = (incr[k].gb_mass_i - mass_chge_s) / incr[k].area *
                       tot_weight / Total_Enrich_Weights[1] +
                   conc_e1;
      ...
      if (conc_e1 != conc_e_f) {
        for (cnt = 0, it = unodelist->begin(); it != unodelist->end() && !err;
             it++, cnt++) {
          i = (*it);
          if (Weights[k][cnt] && find(enrichlist->begin(), enrichlist->end(),
                                      i) != enrichlist->end()) {
            ElleGetUnodePolyInfo((*it), attr_id, &current, &conc);
            /*tmp = (conc_e_f*Total_Enrich_Weights[1]/tot_weight-conc)* */
            tmp = (conc_e_f - conc) * Weights[k][cnt] /
                  Total_Enrich_Weights[1] * incr[k].area;
            delta_mass[cnt] += tmp;
            mass_chge_e += tmp;
          }
        }
...
void set_up_tri_values(double tri_pos[3][2], double pivot[2], double gb_area,
                       double gb_area_f, double conc_n, double conc_nb,
                       double *nb_sweep_frac, double sweep_area,
                       Incr_Data *incr, int increments) {
  int i, j, k;
...
void set_up_mid_line(double tri_pos[3][2], double pivot[2], double *inc_width,
  ...
  */
  ...
  mindist =
      sqrt(((tri_pos[0][0] - tri_pos[1][0]) * (tri_pos[0][0] - tri_pos[1][0])) +
           ((tri_pos[0][1] - tri_pos[1][1]) * (tri_pos[0][1] - tri_pos[1][1])));

  *line_height_inc = mindist / increments;
  *length = sqrt(((pivot[0] - mid[0]) * (pivot[0] - mid[0])) +
                 ((pivot[1] - mid[1]) * (pivot[1] - mid[1])));
...
void calc_mass() {
  ...
  for (i = 0; i < max; i++) {
    if (ElleFlynnIsActive(i)) {
      ...
      for (vector<int>::iterator it = unodelist.begin(); it != unodelist.end();
           it++) {
  ...
  // printf("lattice_mass,gb_mass,total_mass:%12.10le %12.10le
...
double point_to_line(Coords pt, double line_pt1[2], double line_pt2[2]) {
  ...
  a2 = ((pt.y - line_pt1[1]) * (line_pt2[0] - line_pt1[0])) -
       ((pt.x - line_pt1[0]) * (line_pt2[1] - line_pt1[1]));

  height =
      2 * sqrt((a2 * a2) /
               (((line_pt2[0] - line_pt1[0]) * (line_pt2[0] - line_pt1[0])) +
                ((line_pt2[1] - line_pt1[1]) * (line_pt2[1] - line_pt1[1]))));
...
#if XY
void set_up_weights(double tri_pos[3][2], double *incr_areas, Coords *mid_pts,
                    double *mass, int increments, int attr_id,
                    set_int *unodelist, double roi, int bnd);
void SweepUnodes(double tri_pos[3][2], double *incr_areas, Coords *mid_pts,
                 double *mass, int increments, int attr_id, set_int *allunodes,
                 double roi, int bnd);
...
void PartitionMass(int n, int attr_id, double pc, double *swept_area,
                   double *swept_mass, double *put_mass, double *gb_area_i,
                   double *gb_area_f, double *gb_mass);
...
void set_up_weights(double tri_pos[3][2], double *incr_areas, Coords *mid_pts,
                    ...
                    set_int *unodelist, double roi, int bnd);
// remove swept mass
SweepUnodes(tri_pos, tri_areas, mid_pts, &swept_mass[i], Increments, attr_id,
            unodeset[COMBINED_LIST], roi, i);
RemoveSweptMass(unodeset[COMBINED_LIST], attr_id);

PartitionMass(n, attr_id, partition_coeff, swept_area, swept_mass, put_mass,
              gb_area_f, gb_mass);
...
void SweepUnodes(double tri_pos[3][2], double *incr_areas, Coords *mid_pts,
                 double *mass, int increments, int attr_id, set_int *unodelist,
                 double roi, int bnd) {
  /*
    ...
    for (set_int ::iterator it = unodelist->begin();
      ...
      if (total > 0.0) {
        ...
        err = ElleSetUnodeAttribute(
            (*it), (unode_mass - swept_mass) / Unode_area, attr_id);
...
void EnrichUnodes(double *p_mass, set_int *unodelist, int attr_id) {
  ...
  if (unodelist != 0) {
    for (set_int ::iterator it = unodelist->begin();
      ...
      unode_mass = conc * Unode_area;
      err = ElleSetUnodeAttribute((*it), (unode_mass + put_mass) / Unode_area,
                                  attr_id);
      total -= put_mass;
    }
    if (fabs(total) > 1e-20)
...
double IncrementWeight(Coords *centre, double *incr_areas, Coords *mid_pts,
                       int increments, double ROI) {
  int k;
