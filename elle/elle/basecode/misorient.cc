    ...
    if (ElleNodeIsActive(j)) {
      ...
      for (i = 0; i < 3; i++) {
        if (nbnodes[i] != NO_NB && nbnodes[i] < j) {
          if ((val = ElleGetBoundaryAttribute(j, nbnodes[i])) > 0) {
            if (!set) {
              *min = *max = val;
              set = 1;
            } else {
...
double Misorient(double curra[3], double currb[3]) {
  ...
  /*
      if (val > 1.0+eps || val < -1.0-eps ) {
          sprintf(logbuf,"CalcMisorient - adjusting val for acos from %lf for
     bnd %d, %d\n",val,rgn1,rgn2); Log( 1,logbuf );
      }
  */
  if (val > 1.0)
    val = 1.0;
...
/*!
        ...
        \return		\a minang - smallest misorientation for symmetry
(degrees)

        \par		Description:
                        To calculate the smallest misorientation
(disorientation), the crystal symmetry operators (symm_ops) are applied in turn
...
\verbatim
                        ...
                        minang = Misorientation(ang1, ang2, symm, (const
int)symm_ops);
...

{
  ...
  orientmatZXZ(rmap1, (double)curra1, (double)currb1,
...
void CalcMisorient(int rgn1, int rgn2, float *orient) {
  ...
  currb2 *= DTOR;
  currc2 *= DTOR;
  orientmat(rmap1, (double)curra1, (double)currb1,
            (double)currc1); // gives rotation matrix
  orientmat(rmap2, (double)curra2, (double)currb2,
  ...
  if (val > 1.0 || val < -1.0)
    sprintf(logbuf,
            "CalcMisorient - adjusting val for acos from %lf for bnd %d, %d\n",
            val, rgn1, rgn2);
  Log(1, logbuf);
  if (val > 1.0)
    val = 1.0;
