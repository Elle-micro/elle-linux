...
void ElleInitTriAttributes() {
  if ((MeshData.tri =
           (struct triangulateio *)malloc(sizeof(struct triangulateio))) == 0)
...
void ElleClearTriAttributes() {
  if (MeshData.tri != 0)
    cleanio(MeshData.tri);
  if (MeshData.attribute != 0) {
    if (MeshData.attribute->elem != 0)
      free(MeshData.attribute->elem);
...
void ElleSetTriReferenceId(int ref) { MeshData.ref_id = ref; }
...
void ElleAllocTriAttributes(int index) {
  ...
  if ((MeshData.attribute[MeshData.num_attributes - 1].elem = (double *)malloc(
           sizeof(double) * MeshData.tri->numberoftriangles)) == 0)
    OnError("ElleAllocTriAttributes", MALLOC_ERR);
...
void ElleSetTriAttributeFromNodes(int index, int attr_id) {
  int i, j, numpts, *p, *node;
  ELLE_MAYBE_UNUSED int numptattrib;
  ...
  for (i = 0, p = MeshData.tri->trianglelist;
    ...
    for (j = 0, node = p; j < 3; j++, node++) {
      ...
      if (MeshData.tri->pointmarkerlist[(*node)] == 0) {
        switch (attr_id) {
        case CAXIS:
          ElleGetUnodeAttribute(id, &val, &val2, attr_id);
          break;
        default:
          ElleGetUnodeAttribute(id, &val, attr_id);
          break;
        }
        total += val;
        num++;
    ...
    if (num > 0)
      MeshData.attribute[index].elem[i] = total / num;
  }
}
...
int ElleMaxTriangles() { return (MeshData.tri->numberoftriangles); }
...
void ElleGetTriPtNeighbours(int id, std::vector<int> &ids,
  ...
  if (index != NO_NB) {
    ...
    for (i = 0, v_it = nbnodes.begin(); v_it != nbnodes.end(); v_it++, i++) {
      ...
      std::pair<int, double> p((*v_it),
                               polar_angle(nbxy.x - xy.x, nbxy.y - xy.y));
    ...
    for (it = ordered_list.begin(); it != ordered_list.end(); it++) {
      ...
      ids.push_back(j);
      if (MeshData.tri->pointmarkerlist[(*it).first] == 0)
        k = 0;
      else
        k = 1;
      on_bnd.push_back(k);
...
void ElleGetNodeTriPtNeighbours(int id, std::vector<int> &ids,
  ...
  if (index != NO_NB) {
    ...
    while (it != ordered_list.end() &&
           MeshData.tri->pointattributelist[(*it).first] != nb_id)
      it++;
    start = it;
...
#if XY
void ElleGetTriPtNeighbours(int id, int **nbs, int *num, unsigned char bnd) {
