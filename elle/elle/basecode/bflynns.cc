    ...
    if (str[0] == '#')
      ...
      if (Region_ver)
        if ((num = fscanf(fp, "%d %d", &s, &u)) != 2)
          return (READ_ERR);
      /* s and u are read for legacy format but otherwise unused */
      if (Region_ver) {
        ELLE_UNUSED(s);
        ELLE_UNUSED(u);
      }
...
int SaveZIPParentData(gzFile in) {
  ...
  if (!id_match(FileKeys, PARENTS, label))
    return (KEY_ERR);
  gzprintf(in, "%s\n", label);
  for (i = 0; i < max && !err; i++) {
    if (ElleFlynnIsActive(i) && isParent(i)) {
...
int ElleNewGrain(int first, int last, ERegion old, ERegion *rnew) {
  ...
  *rnew = ElleFindSpareFlynn();
  ...
  if (ElleNodeIsDouble(first) && ElleNodeIsDouble(last) &&
      ElleFindNbIndex(first, last) == NO_NB) {
...
void ElleNucleateGrainFromNode(int node, ERegion *new_rgn) {
  ...
  if (ElleNodeIsTriple(node)) {
    ...
    i = 1;
    if (nbnodes[0] != NO_NB && nbnodes[0] != nb[0])
      nb[1] = nbnodes[0];
    else if (nbnodes[1] != NO_NB)
      nb[1] = nbnodes[1];
    else
      nb[1] = nbnodes[2];
    ...
    xy1.x = (midpt.x - xy0.x) * cos(ang) + (midpt.y - xy0.y) * sin(ang);
    xy1.y = -(midpt.x - xy0.x) * sin(ang) + (midpt.y - xy0.y) * cos(ang);
    xy1.x += xy0.x;
    xy1.y += xy0.y;
