...
void ElleRotateTripleJ(int node1, Coords *xy_mp, int nb1, int nb2,
                       Coords *xynew);
void RecalculateTripleSwitchCONC(int node1, int node2, int node3, int node4,
                                 int node5, int node6);
void ElleTripleJMoveDir(int node, Coords *xy_mp, int nb1, int nb2, Coords *dir);
...
int SaveZIPNodeLocation(gzFile out) {
  /* * writes location data in a node file * location label then position data
   * on a separate line for each node with the format: %d %f %f * (node id
   * number, x, y) */
  char label[20];
  int i, max;
  Coords pos;
  ...
  max = ElleMaxNodes();
  if (!id_match(FileKeys, LOCATION, label))
    return (KEY_ERR);
  gzprintf(out, "%s\n", label);
  for (i = 0; i < max; i++) {
...
int LoadZIPLocationData(gzFile in, char str[]) {
  /* * routine to be called after the location keyword has * been read in an
   * Elle file Reads lines with the format %d %lf %lf (node id, x, y) */
  int err = 0, nn;
  ELLE_MAYBE_UNUSED int num;
  Coords current;
  ELLE_MAYBE_UNUSED double tmpx, tmpy;
...
int LoadZIPStressData(gzFile in, char str[], int *keys, int count) {
  /* * Read stress data in a node file. * Stress labels then stress data on a
   * separate line for each node with the format: %d %f %f %f %f %f * (node id
   * number, tau xx, tau yy, tau zz, tau xy, pressure or the order read into
   * keys) */
  int i;
  int nn, err = 0;
  ELLE_MAYBE_UNUSED int num;
...
int LoadZIPVelocityData(gzFile in, char str[]) {
  /* * routine to be called after the velocity keyword has * been read in an
   * Elle file Reads lines with the format %d %f %f(node id, velocity in *
...
int LoadZIPNodeMarginAttribData(gzFile in, char str[], int attr_id) {
  /* * routine to be called after a node margin attrib keyword has * been read
   * in an Elle file Reads lines with the format %d %lf %lf %lf(node id, val for
   * *        2 or 3 flynns) */
  char *ptr, *start;
...
int SaveZIPStressData(gzFile out, int *keys, int count) {
  /* * Writes stress data in an elle file. * Stress data on a separate line for
   * each node * with the format: %d %e %e %e %e %e (node id number, count
   * values in the order defined by keys) */
  char label[20];
  int i, j, max;
  int index[NUM_STRESS_VALS];
  ...
  for (i = 0; i < count; i++) {
    if (!id_match(StressKeys, keys[i], label))
      return (KEY_ERR);
    gzprintf(out, "%s ", label);
  }
  gzprintf(out, "\n");
...
int SaveZIPNodeMarginAttribData(gzFile out, int attr_id) {
  /* * writes margin real attribute data in a node file * line for each node
   * with the format: %d %lf %lf %lf (node id number, val for 3 flynns) */
  char label[20];
  int j, i, max, loc_id;
...
int SaveZIPStrainData(gzFile out, int *keys, int count) {
  /* * Writes strain data in an elle file. * Strain data on a separate line for
   * each node * with the format: %d %f %f (node id number, count values in the
   * order defined by keys) */
  char label[20];
  int i, j, max;
  int index[NUM_STRAIN_VALS];

  if (!id_match(FileKeys, STRAIN, label))
    return (KEY_ERR);
  gzprintf(out, "%s ", label);
  for (i = 0; i < count; i++) {
    if (!id_match(StrainKeys, keys[i], label))
      return (KEY_ERR);
    gzprintf(out, "%s ", label);
  }
  gzprintf(out, "\n");
...
int SaveZIPNodeRealAttribute(gzFile out, int id) {
  /* * writes real attribute data in a node file * line for each node with the
   * format: %d %f * (node id number, val) */
  char label[20];
  int i, max;
  double val, dflt;
  double eps = 1e-15;
...
int SaveZIPNodeConcA(gzFile out) {
  /* * writes ConcA data in a node file * line for each node with the format: %d
   * %f * (node id number, conc) */
  char label[20];
  int i, max;
  int indx;
  double val, dflt;
  ...
  if (!id_match(FileKeys, CONC_A, label))
    return (KEY_ERR);
  gzprintf(out, "%s\n", label);
...
int SaveZIPNodeVelocity(gzFile out) {
  /* * writes velocity data in a node file * velocity label then velocity data
   * on a separate line for each node with the format: %d %f %f * (node id
   * number, x direction velocity, y direction velocity) */
  char label[20];
  int i, max;
  int indxx, indxy;
...
int LoadZIPNodeRealAttribData(gzFile in, char str[], int id) {
  /* * routine to be called after real attribute keyword has * been read in an
   * Elle file Reads lines with the format %d %f (node id, conc) */
  int j, err = 0, num, nn;
  int max, indx;
  double val;
  ELLE_UNUSED(num);
  ...
  while (!gzeof(in) && !err) {
    ...
    if (str[0] == '#')
      ...
      /* need to set up an array of defaults, rather than hardwired in code.
       * Assumes the default for these is 0 */
    } else if (str[0] < '0' || str[0] > '9')
      return (0);
    else {
      if ((nn = atoi(str)) == MAX_INDX)
...
int LoadZIPStrainData(gzFile in, char str[], int *keys, int count) {
  /* * Read strain data in a node file. * Strain labels then strain data on a
   * separate line for each node with the format: %d %f %f * (node id number,
   * psr1, bulk psr1 or the order read into keys) */
  int i;
  int nn, num, err = 0;
  int index[NUM_STRAIN_VALS];
...
int ElleInsertDoubleJOnBnd(int node1, int bnd1, int node2, int bnd2, int *nn,
                           float frac) {
  /*
   * node1,node2 are the nodes defining the segment
...
int ElleDeleteDoubleJ(int id) {
  ...
  if (nb1 == nb2)
    ...
    if (ElleFindIntersection(id, NO_NB, rgn1, &xy, &tmpxy, &isect[0]))
      count++;
    if (ElleFindIntersection(id, nb1, rgn2, &xy, &tmpxy, &isect[0]))
      count++;
    if (ElleFindIntersection(id, NO_NB, rgn2, &xy, &tmpxy, &isect[0]))
      count++;
    if (ElleFindIntersection(id, nb2, rgn1, &xy, &tmpxy, &isect[0]))
      count++;
    /*
    ...
    if (fabs(ElleRegionArea(rgn1)) < (double)minarea)
      ...
      while (ElleCheckForTwoSidedGrain(nb1, &nb)) {
        ...
        else if (ElleNodeIsTriple(nb1))
          ElleRemoveHangingNode(nb, nb1);
        else
          ElleRemoveHangingNode(nb1, nb);
      }
    } else
      delete_err = 1;
  }
...
void ElleDeleteSingleJ(int node) {
  ...
  while (ElleNodeIsSingle(node)) {
    ...
    i = 0;
    while (nbnodes[i] == NO_NB && i < 3)
      i++;
    if (i == 3)
      OnNodeError(node, "ElleDeleteSingleJ", NONB_ERR);
    nb = nbnodes[i];
    if (ElleNodeCONCactive())
...
void ElleRemoveHangingNode(int dblnode, int trplnode) {
  ...
  for (j = 0; j < 3 && !found; j++) {
    if (nbnodes[j] != NO_NB) {
      full_id_rm = tmp[j];
      if (!ElleIdMatch(&full_id, &full_id_rm))
        found = 1;
    }
  }
  j = ElleFindNbIndex(trplnode, newfirst);
  if (newfirst == NO_NB || j == NO_NB)
    OnNodeError(trplnode, msg, NONB_ERR);
  if (ElleNodeCONCactive()) {
 ...
 */
void ElleFindCentre(int node, int node1, int node2, int node3, Coords *xy) {
  Coords xy1, xy2, xy3;
  Coords xy_mp;
...
void ElleTripleJMoveDir(int node, Coords *xy_mp, int nb1, int nb2,
  ...
  */
  index = 0;
  while (ang_order[index].second != node)
    index++;
  index1 = (index + 1) % 3;
  index2 = (index1 + 1) % 3;
  /*
  ...
  */
  ...
  if (ang_order[index1].second == nb2)
    rotate_coords_ac(xy_nb2.x, xy_nb2.y, xy.x, xy.y, &xnew, &ynew, ang);
  else
    rotate_coords_ac(xy_nb1.x, xy_nb1.y, xy.x, xy.y, &xnew, &ynew, ang);
  dir->x = xnew;
  dir->y = ynew;
}
...
void ElleRotateTripleJ(int node1, Coords *xy_mp, int nb1, int nb2,
  ...
  ElleNodePlotXY(nb1, &xy_nb1, xy_mp);
  ElleNodePlotXY(nb2, &xy_nb2, xy_mp);
  angle(xy_mp->x, xy_mp->y, xy_nb1.x, xy_nb1.y, xy1.x, xy1.y, &ang1);
  if (ang1 < 0)
    ang1 = 2.0 * PI + ang1;
  angle(xy_mp->x, xy_mp->y, xy_nb2.x, xy_nb2.y, xy1.x, xy1.y, &ang2);
  if (ang2 < 0)
    ang2 = 2.0 * PI + ang2;
  if ((ang = (ang2 - ang1)) < 0)
    ang = 2.0 * PI + ang;
  rotate_coords_ac(xy_nb1.x, xy_nb1.y, xy_mp->x, xy_mp->y, &xnew, &ynew,
                   ang * 0.5);
...
void ElleRotateSegment(int node1, int node2, double angle) {
  ...
  rotate_coords((double)xy1.x, (double)xy1.y, (double)xy_mp.x, (double)xy_mp.y,
                &xnew, &ynew, angle);
  xy1.x = xnew;
  xy1.y = ynew;
  ElleSetPosition(node1, &xy1);
  rotate_coords((double)xy2.x, (double)xy2.y, (double)xy_mp.x, (double)xy_mp.y,
                &xnew, &ynew, angle);
  xy2.x = xnew;
  xy2.y = ynew;
...
unsigned char AngleProblem(int node1, int node2, int node3, int node4,
  ...
  ElleNodePlotXY(node2, &xy2, &xy1);
  ElleNodePlotXY(node3, &xy3, &xy1);
  angle(xy1.x, xy1.y, xy3.x, xy3.y, xy2.x, xy2.y, &ang1);
  if (ang1 > PI) {
    ElleNodePlotXY(node5, &xy5, &xy1);
    angle(xy2.x, xy2.y, xy1.x, xy1.y, xy5.x, xy5.y, &ang2);
    if (ang2 > PI)
      problem = 1;
  } else {
    ElleNodePlotXY(node6, &xy6, &xy1);
    angle(xy2.x, xy2.y, xy6.x, xy6.y, xy1.x, xy1.y, &ang1);
    if (ang1 > PI) {
      ElleNodePlotXY(node4, &xy4, &xy1);
      angle(xy1.x, xy1.y, xy4.x, xy4.y, xy2.x, xy2.y, &ang2);
      if (ang2 > PI)
        problem = 1;
    }
  }
...
int ElleSwitchTripleNodes(int node1, int node2) {
  ...
  if (ElleNodeIsActive(node1))
    err = ElleNodeTopologyCheck(node1);
  return (err);
}
...
int ElleSwitchTripleNodes(int node1, int node2) {
  ...
  i = 0;
  while (i < 3 && (rgn[i] == full_id[1] || rgn[i] == full_id[3]))
    i++;
  if (i == 3)
    OnError(msg, ID_ERR);
  full_id[0] = rgn[i];
  ...
  if (ElleIdMatch(&tmp, &full_id[3])) {
    ...
    indx6in2 = indx5in2;
    for (i = 0; i < 3; i++)
      if (i != indx1in2 && i != indx6in2)
        indx5in2 = i;
    node5 = ElleIndexNb(node2, indx5in2);
    indx2in5 = ElleFindNbIndex(node2, node5);
  } else {
    for (i = 0; i < 3; i++)
      if (i != indx1in2 && i != indx5in2)
        indx6in2 = i;
  ...
  if (ElleIdMatch(&tmp, &full_id[1])) {
    ...
    indx3in1 = indx4in1;
    for (i = 0; i < 3; i++)
      if (i != indx2in1 && i != indx3in1)
        indx4in1 = i;
    node4 = ElleIndexNb(node1, indx4in1);
    indx1in4 = ElleFindNbIndex(node1, node4);
  } else {
    for (i = 0; i < 3; i++)
      if (i != indx2in1 && i != indx4in1)
        indx3in1 = i;
  ...
  if (ElleNodeCONCactive() && !ElleUnodeCONCactive())
    RecalculateTripleSwitchCONC(node1, node2, node3, node4, node5, node6);
  ...
  while (ElleNodeIsActive(node1) && ElleCheckForTwoSidedGrain(node1, &nb)) {
    checked = 1;
    if (ElleNodeIsTriple(nb))
      ElleRemoveTripleJLink(node1, nb);
    else
      ElleRemoveHangingNode(nb, node1);
  }
  while (ElleNodeIsActive(node2) && ElleCheckForTwoSidedGrain(node2, &nb)) {
    checked = 1;
    if (ElleNodeIsTriple(nb))
      ElleRemoveTripleJLink(node2, nb);
    else
      ElleRemoveHangingNode(nb, node2);
  }
  /*
  ...
  if (!checked) {
    ...
    if (ang.prev_angle > 0 && ang.prev_angle < PI_2) {
      ang.pivot = node2;
      ang.nb = node6;
      if (ElleFindIntersection(node4, node2, full_id[3], &xya, &xy,
                               &isect[0]) == 1)
    ...
    if (ang.prev_angle > 0 && ang.prev_angle < PI_2) {
      ang.pivot = node1;
      ang.nb = node3;
      if (ElleFindIntersection(node5, node1, full_id[1], &xya, &xy,
                               &isect[0]) == 1)
        ElleMoveLink(&isect[0], &ang, &xy);
    }
  }
...
int ElleSwitchTripleNodesForced(int node1, int node2) {
  ...
  if (EllePhaseBoundary(node1, node2) ||
      (EllePhaseBoundary(node1, node3) && EllePhaseBoundary(node2, node6)))
    return (0);
  if (indx1in2 == NO_NB || indx2in1 == NO_NB || indx5in2 == NO_NB ||
      indx2in5 == NO_NB || indx4in1 == NO_NB || indx1in4 == NO_NB)
    OnError(msg, NONB_ERR);
...
int ElleSwitchTripleNodes(int node1, int node2) {
  ...
  ElleNodePlotXY(node5, &xy5, &xy1);
  ElleNodePlotXY(node6, &xy6, &xy1);
  if (!lines_intersect(xy4.x, xy4.y, xy2.x, xy2.y, xy5.x, xy5.y, xy1.x, xy1.y,
                       &xyc.x, &xyc.y) &&
      !lines_intersect(xy4.x, xy4.y, xy2.x, xy2.y, xy3.x, xy3.y, xy1.x, xy1.y,
                       &xyc.x, &xyc.y) &&
      !lines_intersect(xy5.x, xy5.y, xy1.x, xy1.y, xy6.x, xy6.y, xy2.x, xy2.y,
                       &xyc.x, &xyc.y)) {
    ElleSetNeighbour(node1, indx4in1, node5, &full_id[1]);
    ElleSetNeighbour(node2, indx5in2, node4, &full_id[3]);
    ElleSetRegionEntry(node1, indx2in1, full_id[2]);
    ...
    if (ElleNodeIsActive(node1))
      writeNodeLocalPolyFile(node1);
#endif
  } else if (!lines_intersect(xy3.x, xy3.y, xy2.x, xy2.y, xy4.x, xy4.y, xy1.x,
                              xy1.y, &xyc.x, &xyc.y) &&
             !lines_intersect(xy3.x, xy3.y, xy2.x, xy2.y, xy6.x, xy6.y, xy1.x,
                              xy1.y, &xyc.x, &xyc.y) &&
             !lines_intersect(xy6.x, xy6.y, xy1.x, xy1.y, xy5.x, xy5.y, xy2.x,
                              xy2.y, &xyc.x, &xyc.y)) {
  ...
  if (ElleNodeIsDouble(node3))
    ElleCheckDoubleJ(node3);
  if (ElleNodeIsDouble(node4))
    ElleCheckDoubleJ(node4);
...
int ElleSwitchTripleNodesForced(int node1, int node2) {
  ...
  /* how did these TJs get so close? */
    ...
    fprintf(stderr, "Problem with 2-phase triple switch\n");
#ifdef DEBUG
  if (ElleNodeIsActive(node1))
    writeNodeLocalPolyFile(node1);
#endif
...
int ElleFindBndIndex(int node, ERegion full_id) {
  ...
  */
  ...
  for (i = 0; i < 3 && !found; i++) {
    if (tmp[i] == full_id || (tmp[i] != NO_NB && isChildOf(tmp[i], full_id))) {
      idx = i;
      found = 1;
    }
  }
...
int ElleFindBndNbIndex(int nb, int node, ERegion full_id) {
  ...
  */
  ...
  while (i < 3 && !found) {
    if (nbnodes[i] == nb) {
      if (rgn[i] != full_id && !isChildOf(rgn[i], full_id)) {
        idx = i;
        found = 1;
      }
    }
...
int ElleFindBndNode(int node_num, int prev_node, int id) {
  ...
  while (i < 3 && !found) {
    if (nbnodes[i] != NO_NB && nbnodes[i] != prev_node &&
      ...
      j = 0;
      while (nbnbnodes[j] != node_num && j < 3)
        j++;
      ElleNeighbourRegion(idx, nbnbnodes[j], &nbrgn);
      if (nbrgn != id && !isChildOf(nbrgn, id))
        found = 1;
      else
        i++;
    } else
...
int ElleFindBndNb(int node_num, ERegion id) {
  ...
  i = 0;
  while (i < 3 && !found) {
    if (rgn[i] == id || (rgn[i] != NO_NB && isChildOf(rgn[i], id))) {
      idx = nbnodes[i];
      ElleNeighbourNodes(idx, nbnbnodes);
      j = 0;
      while (nbnbnodes[j] != node_num)
        j++;
      ElleNeighbourRegion(idx, nbnbnodes[j], &nbrgn);
      if (nbrgn != id && !isChildOf(nbrgn, id))
        found = 1;
...
unsigned char ElleNodeMarginAttributeActive(int id) {
  ...
  switch (id) {
  case CONC_A:
    found = ElleNodeAttributeActive(N_MCONC_A_1);
    break;
  case N_MATTRIB_A:
  case ATTRIB_A:
    found = ElleNodeAttributeActive(N_MATTRIB_A_1);
    break;
  case N_MATTRIB_B:
  case ATTRIB_B:
    found = ElleNodeAttributeActive(N_MATTRIB_B_1);
    break;
  case N_MATTRIB_C:
  case ATTRIB_C:
    found = ElleNodeAttributeActive(N_MATTRIB_C_1);
    break;
  default:
    break;
  }
...
int ElleWriteNodeData(FILE *fp) {
  ...
  for (i = 0; i < maxa && !err; i++) {
    ...
    case N_MCONC_A:
    case N_MCONC_A_1:
      err = ElleWriteNodeMarginAttribData(fp, N_MCONC_A);
      break;
    case N_MATTRIB_A:
    case N_MATTRIB_A_1:
    ...
    case N_ATTRIB_E:
    case N_ATTRIB_F:
      err = ElleWriteNodeRealAttribute(fp, active[i]);
      break;
      // these have already been written, so step over
    case N_MCONC_A_2:
    case N_MCONC_A_3:
...
int ElleReadStressData(FILE *fp, char str[], int *keys, int count) {
  ...
  while (!feof(fp) && !err) {
    ...
    int _elle_res1 = fscanf(fp, "\n");
    (void)_elle_res1; // modernize: unused result (A26)
  }
  return (0);
}
...
int ElleReadStrainData(FILE *fp, char str[], int *keys, int count) {
  ...
  while (!feof(fp) && !err) {
    ...
    int _elle_res2 = fscanf(fp, "\n");
    (void)_elle_res2; // modernize: unused result (A26)
  }
  return (0);
}
...
double ElleSwitchLength() { return (ElleminNodeSep()); }
...
double ElleSmallSwitchLength() { return (ElleminNodeSep() * 0.1); }
...
int ElleNodeOnRgnBnd(int node, ERegion id) {
  ...
  ElleNeighbourNodes(node, nbnodes);
  if ((nb = ElleFindBndNb(node, id)) != NO_NB)
    found = 1;
  /*
      for (i=0,found=0;i<3&&!found;i++) {
...
void ElleRelPosition(Coords *orig, int nb, Coords *xyrel, double *sep) {
  ...
  tmp = xyrel->x * xyrel->x + xyrel->y * xyrel->y;
  *sep = sqrt(tmp);
}
...
void ElleNumberOfNodes(int *doubles, int *triples) {
  ...
  for (i = 0; i < max; i++)
    if (ElleNodeIsActive(i)) {
      if (ElleNodeIsDouble(i))
        numdble++;
      else if (ElleNodeIsTriple(i))
        numtrple++;
    }
  *doubles = numdble;
  *triples = numtrple;
...
void RecalculateTripleSwitchCONC(int node1, int node2, int node3, int node4,
  ...
  for (i = 0; i < 3; i++)
    if (nbs[i] != NO_NB)
      newlen += ElleNodeSeparation(node4, nbs[i]);
  ...
  for (i = 0; i < 3; i++)
    if (nbs[i] != NO_NB)
      newlen += ElleNodeSeparation(node5, nbs[i]);
...
void ResetRegionEntries(int start, int *prev, int end, int old, int newrgn) {
  int nb1, i, last, next;
