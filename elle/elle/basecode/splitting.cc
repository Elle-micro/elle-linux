...
#if XY
void CreateBoundary(vector<PointData> &nodes, int nodenbbnd, int nbnodebnd,
...
void CreateBoundary(vector<PointData> &nodes, int nodenbbnd, int nbnodebnd,
  ...
  ELLE_UNUSED(success); // modernize: declared but intentionally unused (A26)
  double min, sqx, sqy;
...
void RemoveBoundary(int *start, int *finish, int rgn1, int rgn2) {
  ...
  if (!(ElleNodeOnRgnBnd(next, rgn1) && ElleNodeOnRgnBnd(next, rgn2))) {
    tmp = *start;
    *start = *finish;
    *finish = tmp;
    if ((next = ElleFindBndNode(*start, *start, rgn1)) == NO_NB)
...
int RandomSplit(struct triangulateio *out, vector<PointData> &boundary,
  ...
  ELLE_UNUSED(ids);     // modernize: unused param (A26)
  ELLE_UNUSED(current); // modernize: declared but intentionally unused (A26)
  ELLE_UNUSED(prev);    // modernize: declared but intentionally unused (A26)
  ;
  vector<int> nbnodes;
  ...
  next = boundary.size();
  startindx = 0;
  while (startindx < num && elle_id[startindx] != start)
    startindx++;
  if (startindx < num)
    previndx = j = startindx;
  else
    OnError("RandomSplit - start node not found", 0);
  /*
  ...
  while (!failed && end == NO_NB) {
    ...
    numtries = 1;
    if (cnt < 3)
      failed = 1; /* only nb bnd pts, won't split flynn */
#if XY
    while (!failed &&
           ((out->pointmarkerlist[next] &&
            ...
            (out->pointmarkerlist[next] &&
             (boundary.size() == 1 && index_diff(startindx, next, num) == 1)) ||
    ...
    while (!failed
           /*&& ((out->pointmarkerlist[next] &&
            * boundary.size()==1)*/
           && ((out->pointmarkerlist[next] && elle_id[next] != NO_NB &&
                ElleFindNbIndex(elle_id[next], start) != NO_NB) ||
               (next < num && elle_id[next] != NO_NB &&
...
int DirectSplit(struct triangulateio *out, vector<PointData> &boundary,
                double f_area, int *elle_id, int num, int start, float dx,
                float dy) {
  int startindx, newid, end, j, previndx, next;
  ...
  if (startindx < num)
    ...
    OnError("DirectSplit - start node not found", 0);
  /*
   * Put the starting node in the boundary list
   */
  ElleNodePosition(start, &xy);
  ...
  */
  if ((fabs(angle1) > PI_3 && sign1 == 1) ||
      (fabs(angle1) < PI_3 && sign1 == 0))
    found1 = 1;
  if ((fabs(angle2) > PI_3 && sign2 == 1) ||
      (fabs(angle2) < PI_3 && sign2 == 0))
    found2 = 1;
  ...
  while (!failed && end == NO_NB) {
    ...
    if (!failed) {
      ...
      if (boundary.size() == 1) {
         ...
         */
        angle(out->pointlist[previndx * 2], out->pointlist[previndx * 2 + 1],
              out->pointlist[j * 2], out->pointlist[j * 2 + 1],
              out->pointlist[previndx * 2] + ddx,
              out->pointlist[previndx * 2 + 1] + ddy, &tmpangle);
        if (fabs(tmpangle) > PI_2)
          sign = 0;
        else
          sign = 1;
      }
 ...
 */
void findNeighbourPts(int node, int prev, struct triangulateio *out, int *cnt,
                      vector<int> &nbnodes, unsigned char incl_bnd) {
  unsigned char found;
  int i, j, nb;
  ...
  *cnt = 0;
  for (i = 0; i < out->numberofedges; i++) {
    ...
    if (out->edgelist[i * 2] == node && out->edgelist[i * 2 + 1] != prev &&
        (incl_bnd || out->pointmarkerlist[out->edgelist[i * 2 + 1]] == 0)) {
      nb = out->edgelist[i * 2 + 1];
    } else if (out->edgelist[i * 2 + 1] == node &&
               out->edgelist[i * 2] != prev &&
               (incl_bnd || out->pointmarkerlist[out->edgelist[i * 2]] == 0)) {
      nb = out->edgelist[i * 2];
    }
    if (nb != NO_NB) {
      ...
      */
       ...
       */
      angle(out->pointlist[node * 2], out->pointlist[node * 2 + 1],
            out->pointlist[nb * 2], out->pointlist[nb * 2 + 1],
            out->pointlist[node * 2] + ddnode->x,
            out->pointlist[node * 2 + 1] + ddnode->y, &tmpangle);
      if (fabs(tmpangle) > (PI_2 * 1.3) && sign == 1)
        found = 1;
      if (fabs(tmpangle) < (PI_2 / 1.3) && sign == 0)
        found = 1;
      /*
      ...
      if (!found) {
        ...
        if (itang == angles.end()) {
          angles.push_back(tmpangle);
          indices.push_back(nb);
        } else {
          angles.insert(itang, tmpangle);
          indices.insert(itnb, nb);
        }
        (*cnt)++;
  ...
  for (itnb = indices.begin(), index = 0; itnb != indices.end();
       itnb++, index++)
    if (index < nbnodes.size())
      nbnodes[index] = *itnb;
    else
      nbnodes.push_back(*itnb);
...
int WriteTriangulation(struct triangulateio *out, const char *name) {
  ...
  if (!outfile)
    OnError("WriteTriangulation", OPEN_ERR);
  outfile << out->numberofpoints << " 2 " << out->numberofpointattributes
          << " 1" << endl;
