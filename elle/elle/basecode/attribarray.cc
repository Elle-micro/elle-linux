...
void AttributeArray::initAttribute(const int idval) {
  ...
  case U_FINITE_STRAIN:
    for (i = 0; i < NUM_FINITE_STRAIN_VALS; i++, index++) {
    ...
    break;
  case U_STRAIN:
    for (i = 0; i < NUM_FLYNN_STRAIN_VALS; i++, index++) {
...
void AttributeArray::removeAttribute(const int idval) {
  ...
  switch (idval) {
  case U_STRAIN:
    for (i = 0; i < NUM_FLYNN_STRAIN_VALS; i++) {
    ...
    break;
  case U_FINITE_STRAIN:
    for (i = 0; i < NUM_FINITE_STRAIN_VALS; i++) {
    ...
    break;
  case EULER_3:
    while (it_aa != aa.end() && it_aa->getType() != E3_ALPHA)
      it_aa++;
    if (it_aa != aa.end())
      it_aa->remove();
  ...
  default:
    it_aa = aa.begin();
    while (it_aa != aa.end() && it_aa->getType() != idval)
      it_aa++;
    if (it_aa != aa.end()) {
      aa.erase(it_aa);
      if (idval == CAXIS) {
...
void AttributeArray::setAttribute(const int idval, const int avalue) {
  it_aa = aa.begin();
  while (it_aa != aa.end() && (*it_aa).getType() != idval)
    it_aa++;
  ...
  if (it_aa != aa.end())
    ...
    it_aa = aa.begin();
    while (it_aa != aa.end() && (*it_aa).getType() != NO_NB)
      it_aa++;
...
int AttributeArray::getAttribute(const int idval, int *valptr) {
  int err = 0;
  it_aa = aa.begin();
  while (it_aa != aa.end() && (*it_aa).getType() != idval)
    it_aa++;
  if (it_aa != aa.end())
    *valptr = (int)((*it_aa).getValue());
  else
    err = ATTRIBID_ERR;
  return (err);
}

void AttributeArray::setAttribute(const int idval, const double avalue) {
  it_aa = aa.begin();
  while (it_aa != aa.end() && (*it_aa).getType() != idval)
    it_aa++;
  // reset existing attribute
...
int AttributeArray::getAttribute(const int idval, double *valptr) {
  int err = 0;
  it_aa = aa.begin();
  while (it_aa != aa.end() && (*it_aa).getType() != idval)
    it_aa++;
  if (it_aa != aa.end())
    *valptr = (*it_aa).getValue();
  else
    err = ATTRIBID_ERR;
  return (err);
}

void AttributeArray::setAttribute(const int idval, const Coords_3D *avalue) {
  it_aa = aa.begin();
  while (it_aa != aa.end() && (*it_aa).getType() != idval)
    it_aa++;
  if (it_aa != aa.end()) {
    (*it_aa).setValue(avalue->x);
    it_aa++;
    (*it_aa).setValue(avalue->y);
    it_aa++;
...
int AttributeArray::getAttribute(const int idval, Coords_3D *valptr) {
  int err = 0;
  ...
  while (it_aa != aa.end() && (*it_aa).getType() != idval)
    it_aa++;
  if (it_aa != aa.end()) {
    valptr->x = (*it_aa).getValue();
    it_aa++;
    if ((*it_aa).getType() == CAXIS_Y) {
      valptr->y = (*it_aa).getValue();
      it_aa++;
    } else
      err = ATTRIBID_ERR;
    if (!err && ((*it_aa).getType() == CAXIS_Z)) {
      valptr->z = (*it_aa).getValue();
      it_aa++;
    } else
      err = ATTRIBID_ERR;
  } else
...
bool AttributeArray::hasAttribute(const int idval) {
  ...
  if (it_aa == aa.end() && idval == EULER_3) {
    tmpid = E3_ALPHA;
    it_aa = aa.begin();
    while (it_aa != aa.end() && it_aa->getType() != tmpid)
      it_aa++;
