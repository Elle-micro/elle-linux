...
int ElleReadGBEnergyLUT(FILE *fp, char str[]) {
  ...
  if ((fgets(str, 80, fp) != NULL) && !feof(fp)) {
    start = str;
    if ((ptr = strtok(start,
                      "\t\r\n"))) { // modernize: parenthesize assignment in
...
int ElleWriteGBEnergyLUT(FILE *fp) {
  ...
  if (!id_match(FileKeys, GBE_LUT, label))
    return (KEY_ERR);
  fprintf(fp, "%s\n", label);
