...
void sortCoordsOnAngle(vector<Coords> &pts, Coords *ref) {
  ...
  for (i = 0, it = pts.begin(); it != pts.end(); i++, it++) {
    ...
    while (ito != ordered_list.end() && trial.second > (*ito).second)
      ito++;
...
void findPolygon(REAL *pointlist, REAL *normlist, int *edgelist, int nseg,
                 int *seg_count, int *segs_left, list<int> &processed) {
  ...
  while (!finished && startindx < nseg) {
    ...
    while (startindx < nseg &&
           (seg_count[startindx] == 2 || edgelist[startindx * 2] == -1 ||
            edgelist[startindx * 2 + 1] == -1))
      startindx++;
    if (startindx < nseg) {
    ...
    while (!finished && startindx < nseg && valid) {
      ...
      nb.first = curr;
      nb.second =
          polar_angle(pointlist[curr * 2] - pointlist[next * 2],
      ...
      while (i < nseg) {
        if (seg_count[i] < 2 && i != index[1] &&
            (edgelist[i * 2] == next || edgelist[i * 2 + 1] == next)) {
          if (edgelist[i * 2] == next)
            trial.first = edgelist[i * 2 + 1];
          else
            trial.first = edgelist[i * 2];
          if (trial.first == -1)
            trial.second = polar_angle(normlist[i * 2], normlist[i * 2 + 1]);
          else
            trial.second = polar_angle(
                pointlist[trial.first * 2] - pointlist[next * 2],
                pointlist[trial.first * 2 + 1] - pointlist[next * 2 + 1]);
          ito = ordered_list.begin();
          it = seg_index.begin();
          while (ito != ordered_list.end() && trial.second > (*ito).second) {
            ito++;
      ...
      if (ordered_list.size() > 1) {
        ito = ordered_list.begin();
        it = seg_index.begin();
        while (ito != ordered_list.end() && ito->first != curr) {
          ito++;
          it++;
        }
