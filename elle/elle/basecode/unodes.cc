...
#include <string>
/*#include "timefn.h"*/
// A26 portability macros for unused annotations
...
int findUnodeNeighbours(struct triangulateio *out, struct triangulateio *vorout,
                        Coords *bnd, int num);
...
int findFlynnUnodeArea(int flynnid, struct triangulateio *out,
                       struct triangulateio *vorout, Coords *bnd,
                       int num_bnd_nodes, int *xflags, int *yflags,
...
int UnodesActive() { return (Unodes != 0); }

int ElleVoronoiActive() { return (V_data != 0 && V_data->vpoints.size() > 0); }
...
int ElleInitUnodes() { return (ElleInitUnodes(104)); }
...
void ElleSetDefaultUnodeAttribute(double val1, double val2, double val3,
  ...
  if (err)
    OnError("ElleSetDefaultUnodeAttribute", err);
}
...
void ElleRemoveUnodeAttribute(int u_id, const int id) {
  if (Unodes) {
    if (u_id < Unodes->size())
      (*Unodes)[u_id].removeAttribute(id);
    else
      OnError("Invalid unode index", 0);
  }
}
...
int ElleInitUnodes(int numPerRow, int pattern) {
  ELLE_MAYBE_UNUSED int row, col, maxr, maxc;
  int i, last = 0;
  CellData unitcell;
  Coords xy, start;
  ...
  if (Unodes == 0 || V_data == 0)
    return (MALLOC_ERR);
  bool haveflynns = (ElleNumberOfGrains() > 0);
  ElleCellBBox(&unitcell);
  double xoffset = unitcell.cellBBox[TOPLEFT].x - unitcell.cellBBox[BASELEFT].x;
  double yoffset = unitcell.cellBBox[TOPRIGHT].y - unitcell.cellBBox[TOPLEFT].y;
  ELLE_UNUSED(xoffset);
  ELLE_UNUSED(yoffset);
  ...
  if (pattern == SQ_GRID)
    ystep = unitcell.ylength / numPerRow;
  double xlen = unitcell.xlength - xstep * 0.1;
  double ylen = unitcell.ylength - ystep * 0.1;
  ELLE_MAYBE_UNUSED double tot_x = 0.0;
  ELLE_MAYBE_UNUSED double tot_y = 0.0;
  start.x = xy.x = 0.0;
  if (pattern == SQ_GRID)
    start.y = xy.y = 0.0;
  else
    start.y = xy.y = ystep / 2;
  i = row = 0;
  while (xy.y < ylen) {
    while (xy.x < xlen) {
      Unode tmp(i, &xy);
      ...
      if (haveflynns)
        last = ElleAddUnodeToFlynn(i, &xy, last);
      i++;
      xy.x += xstep;
    }
    row++;
    if (row % 2 && pattern == HEX_GRID)
      xy.x = start.x + xstep / 2;
    else
      xy.x = start.x;
    xy.y += ystep;
  }
...
int ElleInitRanUnodes(int upercell, int subset, int pattern) {
  ELLE_MAYBE_UNUSED int row, col, maxr, maxc;
  int i, last = 0;
  ELLE_MAYBE_UNUSED CellData unitcell;
  Coords xy, start;
  time_t timetmp;
    ...
    for (j = 0; j < subset + extra; j++) {
      if (pattern == SEMI_RAN_GRID) {
        for (m = 0; m < upercell; m++) {
          ...
          Unodes->push_back(tmp);
          if (haveflynns)
            last = ElleAddUnodeToFlynn(k, &xy, last);
          k++;
        }
      } else {
        xy.x = ElleRandomD();
...
int ElleRotateUnodes(double rad, Coords *origin) {
  ...
  if (Unodes) {
    for (vector<Unode>::iterator it = Unodes->begin(); it != Unodes->end();
         it++) {
      it->getPosition(&xy);
      rotate_coords(xy.x, xy.y, origin->x, origin->y, &newxy.x, &newxy.y, rad);
...
int UnodeAttributeRange(int attr, double *min, double *max) {
  ELLE_MAYBE_UNUSED unsigned char set = 0;
  double val = 0.0, b = 0.0;
  ELLE_MAYBE_UNUSED double c = 0.0;
  Coords_3D xyz;
...
int ElleReadUnodeData(FILE *fp, char str[], int *attr, int maxa) {
   ...
   */
  int err = 0, i, j, num;
  ELLE_MAYBE_UNUSED int found = 0, last = 0;
  double val = 0;
  Coords xy, xynb;
  ...
  if (UnodeAttributeActive(CONC_A))
...
int ElleReadUnodeRealAttribData(FILE *fp, char str[], int attr_id) {
   ...
   */
  int err = 0, i, num;
  ELLE_MAYBE_UNUSED int j = 0, found = 0, last = 0;
  int max;
  double val = 0;
  ELLE_MAYBE_UNUSED double val_3[3] = {0, 0, 0};
  ELLE_MAYBE_UNUSED Coords xy;
    ...
    if ((num = fscanf(fp, "%s", str)) != 1 && !feof(fp))
      ...
      for (i = 0; i < count; i++) {
        ...
        if (keys[0] == CAXIS) {
          ...
          PolarToCartesian(&xyz.x, &xyz.y, &xyz.z, val, val2);
          if (nn < max)
            (*Unodes)[nn].setAttribute(keys[i], &xyz);
          dump_comments(fp);
        } else if (nn < max)
          (*Unodes)[nn].setAttribute(keys[i], val);
      }
    }
    num = fscanf(fp, "\n");
  }
...
int ElleWriteUnodeData(const char *fname) {
  ...
  if (Unodes) {
    ...
    if ((err = ElleWriteUnodeLocation(outf)))
      return (err); // modernize: intentional assignment in condition
                    // (behavior-neutral)
    ...
    for (i = 0; i < maxa && !err; i++) {
      ...
      case CONC_A:
        file_id = U_CONC_A;
        if (!id_match(FileKeys, file_id, label))
          err = KEY_ERR;
        outf << label << endl;
...
int ElleWriteUnodeLocation(ofstream &outf) {
  ...
  if (Unodes) {
    if (!id_match(FileKeys, UNODES, label))
      err = KEY_ERR;
    else
      outf << label << endl;
    if (!err) {
  ...
  if (Unodes) {
    ...
    for (j = 0; j < count; j++)
      if (keys[0] == CAXIS) {
        ...
        CartesianToPolar(dfltval_3.x, dfltval_3.y, dfltval_3.z, &dflts[0],
                         &dflts[1]);
        dflts[0] *= RTOD;
        dflts[1] *= RTOD;
      } else
    ...
    for (it = Unodes->begin(); it != Unodes->end() && outf; it++) {
      ...
      for (j = 0; j < count; j++) {
        ...
        if (it->hasAttribute(keys[j]))
          if (keys[0] == CAXIS) {
            ...
            CartesianToPolar(val_3.x, val_3.y, val_3.z, &vals[0], &vals[1]);
            vals[0] *= RTOD;
            vals[1] *= RTOD;
          } else
      ...
      for (j = 0, write = 0; j < writecnt; j++)
        if (vals[j] != dflts[j] &&
            (keys[j] != CONC_A || vals[j] > dflts[j] + eps ||
             vals[j] < dflts[j] - eps))
          write = 1;
      if (write) {
        outf << it->id();
...
int ElleWriteUnodeData(const char *fname) {
  ...
  if (Unodes) {
    ...
    for (i = 0; i < maxa && !err; i++) {
      if (!id_match(FileKeys, attr_ids[0], label))
        err = KEY_ERR;
      outf << ' ' << label;
    }
    outf << endl;
...
int LoadZIPUnodeRealAttribData(gzFile in, char str[], int attr_id) {
  /* * read data for all unconnected nodes * line for each node with the format:
   * %d %f [[%f]%f].. (unode id, attr val,....) */
  int err = 0, i;
  ELLE_MAYBE_UNUSED int j = 0, num = 0, found = 0, last = 0;
  int max;
  double val = 0;
  ELLE_MAYBE_UNUSED double val_3[3] = {0, 0, 0};
  ELLE_MAYBE_UNUSED Coords xy;
...
int LoadZIPUnodeRealAttribData(gzFile in, char str[], int *keys, int count) {
  /* * Read unode attribute data in an elle file. * Labels then one or more data
   * on a separate line for each unode with the format: %d %f %f .... * (unode
   * id number then values in the order read into keys) */
  int i;
  int nn, err = 0;
  ELLE_MAYBE_UNUSED int num = 0;
  int max;
  double val;
...
int LoadZIPUnodeData(gzFile in, char str[], int *attr, int maxa) {
  /* * read data for all unconnected nodes * labels for any attributes * id,
   * position and attribute data on a separate line for each node with the
   * format: %d %f %f [[%f]%f].. * (unode id , x, y, attr val,....) */
  int err = 0, i, j;
  ELLE_MAYBE_UNUSED int num = 0, found = 0, last = 0;
  double val = 0;
  Coords xy, xynb;
...
max = ElleMaxUnodes(); for ( i = 0; i < count; i++ ) ElleInitUnodeAttribute(
keys[i] ); if ( UnodeAttributeActive( CONC_A ) ) ElleSetUnodeCONCactive( 1 );
while ( !gzeof( in ) && !err ) { str = ( ( char * ) ( gzReadSingleString( in )
).c_str() ); else if ( str[0] == '#' ) gzReadLineSTD( in ); else if ( !strcmp(
str, SET_ALL ) ) { for ( i = 0; i < count; i++ ) { val = atof( (
gzReadSingleString( in ) ).c_str() ); DfltUnodeAttrib.setAttribute( keys[i], val
); } } else if ( str[0] < '0' || str[0] > '9' ) return ( 0 ); else { if ( ( nn =
atoi( str ) ) == MAX_INDX ) return ( MAXINDX_ERR ); for ( i = 0; i < count; i++
...
int SaveZIPUnodeAttributeData(gzFile out, int *keys, int count) {
   ...
   * id and attribute data on a separate line for each node *  with the format:
   * %d %f [[%f]%f].. * (unode id , attr val,....) */

  int err = 0, j = 0;
  ELLE_MAYBE_UNUSED int i = 0;
  ELLE_MAYBE_UNUSED int file_id = NO_VAL, cnt = 0;
  int write = 0;
  ELLE_MAYBE_UNUSED double val, val_3[3];
  ELLE_MAYBE_UNUSED double dfltval, dfltval_3[3];
  double eps = 1e-15;
  vector<Unode>::iterator it;

  if (Unodes) {
    /* this should work on dflt attrib list which is a member of unodearray
     * class */
    double dflts[MAX_VALS];
    double vals[MAX_VALS];
    for (j = 0; j < count; j++)
...
int SaveZIPUnodeData(gzFile out) {
  /* * writes data for all unconnected nodes * label * id, position on a
   * separate line for each node with the format: %d %f %f * (unode id , x, y) *
   * writes attribute data for all unconnected nodes * attribute label(s) id and
   * attribute data on a separate line for each node *  with the format: %d %f
   * [[%f]%f].. * (unode id , attr val,....) */
  char label[20];
  int err = 0, i, j;
  ELLE_MAYBE_UNUSED int oldp;
  int file_id = NO_VAL, cnt = 0;
  ELLE_MAYBE_UNUSED double val, val_3[3];
  ELLE_MAYBE_UNUSED double dfltval, dfltval_3[3];
  vector<Unode>::iterator it;
  ...
  if (Unodes) {
    if ((err = SaveZIPUnodeLocation(out)))
      return (err);
    /* this should work on dflt attrib list which is a member of unodearray
     * class */
    int maxa = DfltUnodeAttrib.numAttributes();
    ...
    /* if (!outf)  err=WRITE_ERR; if (!err) { outf.precision(oldp);
     * outf.setf(ios::fixed,ios::floatfield); } */
    delete[] attr_ids;
  }
  return (err);
...
void TriangulateUnodes(int flynnid, struct triangulateio *out,
                       ELLE_MAYBE_UNUSED int attr) {
  ELLE_MAYBE_UNUSED char fname[81];
  int *ids, num, i;
  int num_bnd_nodes, xflags[2], yflags[2];
  ELLE_MAYBE_UNUSED double attr_val;
  ...
  triflags.edges = 0;
  triflags.voronoi = 0;
  if (tripolypts(out, &triflags, num_bnd_nodes, xvals, yvals, attrib, num, 1))
    OnError("error triangulating", 0);
  /*
   * if unodes close to boundary, triangle may reset the bnd marker
   */
  for (i = num_bnd_nodes; i < num; i++)
...
void ElleSetUnodePosition(int id, Coords *xy) {
  if (Unodes) {
    if (id < Unodes->size())
      (*Unodes)[id].setPosition(xy);
    else
      OnError("Invalid unode index", 0);
  }
}

void ElleGetUnodePolyInfo(int id, const int attrib, Coords *xy, double *val) {
  int err = 0;
  ...
  if ((*Unodes)[id].hasAttribute(attrib))
    ...
    err = DfltUnodeAttrib.getAttribute(attrib, val);
  if (err)
...
int ElleGetUnodeAttribute(int id, double *val, const int attr_id) {
  ...
  case CAXIS:
    ...
    CartesianToPolar(caxisval.x, caxisval.y, caxisval.z, val, &val2);
    *val *= RTOD;
    val2 *= RTOD;
    break;
  default:
...
int ElleGetUnodeAttribute(int id, double *val1, double *val2, double *val3,
  ...
  case EULER_3:
    if ((*Unodes)[id].hasAttribute(attr_id)) {
      ...
      if (!err)
        err = (*Unodes)[id].getAttribute(E3_BETA, val2);
      if (!err)
        err = (*Unodes)[id].getAttribute(E3_GAMMA, val3);
      ...
      if (!err)
        err = DfltUnodeAttrib.getAttribute(E3_BETA, val2);
      if (!err)
        err = DfltUnodeAttrib.getAttribute(E3_GAMMA, val3);
    }
    break;
  case CAXIS:
    if ((*Unodes)[id].hasAttribute(attr_id))
...
int ElleGetUnodeAttribute(int id, double *val1, double *val2,
  ...
  case CAXIS:
    ...
    CartesianToPolar(caxisval.x, caxisval.y, caxisval.z, val1, val2);
    *val1 *= RTOD;
    *val2 *= RTOD;
    break;
  default:
...
int ElleSetUnodeAttribute(int id, double val1, double val2, double val3,
  ...
  case EULER_3:
    ...
    if (!err)
      err = (*Unodes)[id].setAttribute(E3_BETA, val2);
    if (!err)
      err = (*Unodes)[id].setAttribute(E3_GAMMA, val3);
    break;
  case CAXIS:
    caxisval.x = val1;
    caxisval.y = val2;
...
int ElleSetUnodeAttribute(int id, double val1, double val2, const int attr_id) {
  int err = 0;
  Coords_3D caxisval;
...
double ElleUnodeROI() { return (ROI); }

int ElleUnodeCONCactive() { return (CONCactive); }

void ElleSetUnodeCONCactive(int val) { CONCactive = val; }

void ElleLocalLists(int n, Coords *incr, set_int **unodeset, set_int **flynnset,
                    double roi) {
  int i, j, max, err = 0;
  ...
  for (i = 0, it = flynnset[0]->begin(); it != flynnset[0]->end(); i++, it++) {
    ...
    for (vector<int>::iterator vit = unodelist.begin();
      ...
      for (j = 0; j < 3; j++) {
        if (nb[j] != NO_NB) {
          if (current.x > rect_big[j][0][0] && current.y > rect_big[j][0][1] &&
...
void CombineLocalLists(set_int **unodelists,
                       ELLE_MAYBE_UNUSED set_int **flynnlists) {
  ELLE_MAYBE_UNUSED int i = 0, j;
  set_int ::iterator it;
...
void ElleReassignUnodes(set_int *unodelist, set_int *flist) {
  ...
  for (it = unodelist->begin(); it != unodelist->end(); it++) {
    for (f_it = flist->begin(), found = 0; f_it != flist->end() && !found;
         f_it++) {
  ...
  */
  ...
  for (it = unodelist->begin(); it != unodelist->end(); it++) {
    ...
    while (adjust < 3 && !found) {
      ...
      if (!found) {
        cout << xy.x << " " << xy.y << endl;
        for (set_int::iterator tmpf_it = flist->begin();
             tmpf_it != flist->end(); tmpf_it++) {
          cout << *tmpf_it << " ";
        ...
        if (adjust > 1) {
          ...
          adjust++;
          Log(1, "ElleAddUnodeToFlynn: unode not in a flynn, adjusting unode "
                 "position");
        } else {
          xy.x += eps;
          adjust++;
...
void ElleUnodeAddNearNb(int id, int nb) {
  if (id >= 0 && id < ElleMaxUnodes() && nb >= 0 && nb < ElleMaxUnodes())
...
void ElleSetUnodeFlynn(int id, int flynnid) { (*Unodes)[id].setFlynn(flynnid); }

int ElleUnodeFlynn(int id) { return ((*Unodes)[id].flynn()); }
...
int ElleVoronoiUnodes() {
  ...
  */
  ...
  if (roi < (udata.xoffset * 0.2))
    roi = udata.xoffset * 0.2;
  ...
  */
  for (i = 0; i < vorout.numberofpoints; i++) {
    ...
    if (id != i) {
      for (k = 0; k < vorout.numberofedges; k++) {
        if (vorout.edgelist[k * 2] == i)
          vorout.edgelist[k * 2] = id;
        if (vorout.edgelist[k * 2 + 1] == i)
          vorout.edgelist[k * 2 + 1] = id;
      }
    }
  }
  /*
...
int VoronoiRegionUnodes(Coords *bnd, int numpts, struct triangulateio *out,
  ...
  ELLE_UNUSED(num_tripts);
  int xflags[2], yflags[2], xflags_brdr[2], yflags_brdr[2];
  double roi;
  ELLE_MAYBE_UNUSED double start = 0.0;
  struct flagvals triflags;
  ...
  ElleCellBBox(&unitcell);
  roi = ElleUnodeROI() * 2; // assumes constant
  if (roi < (unitcell.xoffset * 0.2))
    roi = unitcell.xoffset * 0.2;
  ...
  triflags.bndpts = 0;
  triflags.voronoi = 1;
  if (trivorpolypts(out, vorout, &triflags, 0, xvals, yvals, attrib,
                    num_brdrpts, 1))
    OnError("error triangulating", 0);
...
int findUnodeNeighbours(struct triangulateio *out, struct triangulateio *vorout,
  ...
  for (i = 0; i < out->numberoftriangles; i++) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      id[j] =
          (int)
              out->pointattributelist[tri_id[j] * out->numberofpointattributes];
      int incr = ((id[j] >= 0) ? -1 : 1);
      id[j] += incr;
    }
    for (j = 0; j < 3; j++) {
  ...
  for (j = 0; j < out->numberofedges; j++) {
    id[0] = (int)out->pointattributelist[out->edgelist[j * 2] *
                                         out->numberofpointattributes] -
            1;
    id[1] = (int)out->pointattributelist[out->edgelist[j * 2 + 1] *
                                         out->numberofpointattributes] -
            1;
...
int findUnodeArea(VoronoiData *vdata) {
  ...
  for (i = 0; i < maxu; i++) {
    ...
    for (it = pt_list.begin(); it != pt_list.end(); it++) {
      ...
      pair<double, int> tmp(polar_angle(vxy.x - xy.x, vxy.y - xy.y), *it);
      ito = ordered_list.begin();
      while (ito != ordered_list.end() && tmp.first > (*ito).first)
        ito++;
...
int VoronoiFlynnUnodes(int flynnid, vector<int> &sublist) {
  ...
  for (it = unodelist.begin(); it != unodelist.end(); it++, i++) {
    ElleGetUnodePosition((*it), &current);
    if (!ElleCoordRelativePosition(&current, bnd, num_bnd_nodes, xflags, yflags,
                                   &rel_pos)) {
      int k;
  ...
  triflags.bndpts = 0;
  triflags.voronoi = 1;
  if (trivorpolypts(&out, &vorout, &triflags, num_limit, xvals, yvals, attrib,
                    num, 1))
    OnError("error triangulating", 0);
#ifdef DEBUG
  WriteVoronoiAsPoly(&vorout);
  ...
  err = findFlynnUnodeArea(flynnid, &out, &vorout, bnd, num_bnd_nodes, xflags,
                           yflags, sublist);
...
int findFlynnUnodeArea(int flynnid, struct triangulateio *out,
  ...
  for (i = 0; i < out->numberofedges; i++) {
    ...
    if (u_id >= 0 &&
        (find(unodelist.begin(), unodelist.end(), u_id) != unodelist.end())) {
  ...
  for (i = 0; i < tmpnodes.size(); i++) {
    if (tmpnodes[i].size() > 0) {
      ...
      ElleCoordRelativePosition(&current, bnd, num_bnd_nodes, xflags, yflags,
                                &rel_pos);
      ...
      for (j = res[index].size() - 1; j >= 0; j--) {
        ...
        matched = false;
        nb_flynn = NO_NB;
        relpos.x = res[index][j].x;
        relpos.y = res[index][j].y;
        ...
        while (it != flynnnodes.end() && !matched) {
          if (PntOnLine(
                  (int)((prev.second).x / eps), (int)((prev.second).y / eps),
                  (int)((it->second).x / eps), (int)((it->second).y / eps),
                  (int)(relpos.x / eps), (int)(relpos.y / eps)) == 2) {
            int dj;
            ElleNodeUnitXY(&relpos);
            ElleInsertDoubleJAtPoint(prev.first, it->first, &dj, &relpos);
            tmppair.first = dj;
 ...
 */
int FindUnode(Coords *ptxy, Coords *origin, double dx, double dy,
              int numperrow) {
  int i, j, id = NO_VAL;
  ELLE_UNUSED(j);
...
void ElleFindLocalUnodes(set_int &flist, list<int> *ulist, Coords *bnd,
                         int numpts, Coords *centre) {
  int i, max;
  Coords xy;

  if (Unodes) {
    for (set_int::iterator f_it = flist.begin(); f_it != flist.end(); f_it++) {
      vector<int> l;
