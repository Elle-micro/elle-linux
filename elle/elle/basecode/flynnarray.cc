...
void FlynnArray::removeFlynn(int index) {
  ...
  if (index < (int)_a.size()) {
    ...
    OnError("removing invalid flynn", 0);
}
...
void FlynnArray::promoteFlynn(int flynnindex) {
  ...
  if (flynnindex < (int)_a.size()) {
    ...
    if (pindex != NO_NB) {
      ...
      if (gpindex != NO_NB)
        _a[gpindex].addChild(flynnindex);
      if (!_a[pindex].isParent())
        removeFlynn(pindex);
    }
  }
}
...
int FlynnArray::spareFlynn() {
  int index = 0;
  while (index < (int)_a.size() && _a[index].getState())
    index++;
...
void FlynnArray::setFlynnFirstnode(int index, int first) {
  if (index >= (int)_a.size())
    OnError("setFlynnFirstnode", RANGE_ERR);
...
int FlynnArray::makeLClist(int index, list<int> &LClist) {
  ...
  while (l_it != l.end()) {
    if (_a[*l_it].isParent())
      makeLClist(*l_it, LClist);
    else
      LClist.push_back(*l_it);
    l_it++;
  }
  return 0;
}
...
void FlynnArray::addFlynnChild(int index, int child) {
  if (index >= (int)_a.size())
    OnError("addFlynnChild", LIMIT_ERR);
...
void FlynnArray::updateFirstNodes(int old, int nn, const int flynnindex) {
  ...
  while (_a[p].hasParent()) {
    p = _a[p].getParent();
    if (_a[p].getFirstNode() == old)
      _a[p].setFirstNode(nn);
  }
}

void FlynnArray::newFlynn(int start, int end, int oldindex, int *newindex) {
  int i, j, n, prev;
  *newindex = spareFlynn();
   ...
   */
  if (ElleNodeIsDouble(start) && ElleNodeIsDouble(end)) {
...
void FlynnArray::neighbourRegions(const int flynnid, list<int> &nbflynns) {
  ...
  while (vi != ids.rend()) {
    ...
    tmp.push_back(rgn);
    pair<int, double> seg(rgn, ElleNodeSeparation(*vi, prev));
    segs.push_back(seg);
    prev = *vi;
    vi++;
  }
  ...
  while (its != tmp.end()) {
    ...
    its1 = tmpcnt.begin();
    its2 = nbflynns.begin();
    while (its1 != tmpcnt.end() && *its1 > k) {
      its1++;
      its2++;
    }
    nbflynns.insert(its2, i);
    tmpcnt.insert(its1, k);
    while (its != tmp.end() && (*its == i))
      its++;
  }
#endif
}
...
void FlynnArray::copyAttributes(const int src, const int dest) {
  ...
  for (i = 0; i < maxa; i++) {
    if (!_a[dest].hasFlynnAttribute(attr[i]))
      ...
      case COLOUR:
        ...
        break;
      case CAXIS:
        getFlynnAttribute(src, attr[i], &xyz);
        _a[dest].setFlynnAttribute(attr[i], &xyz);
        while (i < maxa && (attr[i] == CAXIS_Z || attr[i] == CAXIS_Y ||
                            attr[i] == CAXIS_X || attr[i] == CAXIS))
          i++;
        break;
      case VISCOSITY:
      case S_EXPONENT:
  ...
  delete[] attr;
  if (err)
    OnError("copyAttributes", err);
}

int FlynnArray::setFlynnAttribute(const int index, const int id, int val) {
  int err = 0;
  if (index < (int)_a.size())
...
int FlynnArray::setFlynnAttribute(const int index, const int id, double val) {
  int err = 0;
  if (index < (int)_a.size())
    err = _a[index].setFlynnAttribute(id, val);
  else
...
//
int FlynnArray::getFlynnAttribute(const int index, const int id, int *valptr) {
  bool found = 0;
...
int FlynnArray::mergeFlynns(int flynn1, int flynn2) {
  ...
  if (flynn1 == flynn2 || _a[flynn1].isParent() || _a[flynn2].isParent() ||
    ...
    for (i = 0; i < num_elle_nodes; i++) {
      node = ids[i];
      if (ElleNodeIsTriple(node) && ElleFindBndIndex(node, flynn2) != NO_NB) {
        if (start == NO_NB)
          start = node;
        else if (finish == NO_NB)
          finish = node;
        cnt++;
      }
    }
    ...
    if (cnt != 2) {
      ...
      if (cnt == 4)
        ...
        cout << "invalid TJ count " << cnt << endl;
    } else {
      // check attributes
...
bool FlynnArray::attributesAllowMerge(const int flynn1, const int flynn2) {
  ...
  for (i = 0; allowed && i < maxa; i++) {
    ...
    if (x.isIntAttribute()) {
      ...
      if (err)
        OnError("attributesAllowMerge", err);
      allowed = (ival1 == ival2);
    }
  }
  if (active)
...
int FlynnArray::splitFlynn(const int type, int flynnindex, int *child1,
                           int *child2, float dx, float dy) {
  ...
  if (!ElleNodeIsDouble(ids[i])) {
    ...
    if (i != first_try) {
      ...
      ElleInsertDoubleJ(ids[first_try], ids[(first_try + 1) % num_elle_nodes],
                        &newid, 0.5);
      free(ids);
      old = num_elle_nodes;
      _a[flynnindex].flynnNodes(flynnindex, &ids, &num_elle_nodes);
  ...
  case DIRECT:
    ...
    while (
        DirectSplit(&out, boundary, area, elle_id, num, ids[start], dx, dy)) {
      walks++;
      if (walks > 9) {
        i = (start + 1) % num_elle_nodes;
        while (i != first_try && !ElleNodeIsDouble(ids[i]))
...
int FlynnArray::splitWrappingFlynn(int flynnindex, int node1, int node2,
  ...
  next = i = (startindex + 1) % num_elle_nodes;
  start = NO_NB;
...
int FlynnArray::directSplitWrappingFlynn(int flynnindex, int node1, int node2,
                                         int *child1, int *child2,
                                         Coords *dir) {
  unsigned char done = 0;
  ...
  if ((sep = ElleNodeSeparation(splitstart, splitend)) > EllemaxNodeSep()) {
    ...
    for (i = 0, num = 0; i < num_elle_nodes; i++) {
      ...
      if (i == 0)
        ...
        j = i - 1;
      if (ElleNodeIsTriple(ids[j]) && ElleNodeIsTriple(ids[i])) {
        xy.x = current.x - (current.x - prev.x) * 0.5;
        xy.y = current.y - (current.y - prev.y) * 0.5;
        xvals[num] = xy.x;
        yvals[num] = xy.y;
    ...
    while (!done && walks < 10) {
      ...
      if (DirectSplit(&out, boundary, ElleRegionArea(flynnindex), elle_id, num,
                      ids[start], (float)dir->x, (float)dir->y) == 0) {
        done = 1;
        /* does the new boundary end within the right segment
        i=0;
...
int FlynnArray::flynnNodePositions(int flynnindex, vector<Coords> &rel_pos,
                                   Coords *ref) {
...
int FlynnArray::writePolyFile(int flynnindex, const char *fname) {
  ...
  if (!outfile)
    OnError("writePolyFile", OPEN_ERR);
  _a[flynnindex].flynnNodes(flynnindex, &ids, &num);
...
int FlynnArray::writeUnodePolyFile(int flynnindex, const int attrib,
   ...
   */
  for (vector<int>::iterator it = unode_ids.begin(); it != unode_ids.end();
       it++, i++) {
    if (attrib != NO_VAL && UnodeAttributeActive(attrib)) {
      ElleGetUnodePolyInfo(*it, attrib, &xy, &val);
      outfile << i << ' ' << xy.x << ' ' << xy.y << ' ' << val << ' ' << "0"
              << endl;
    } else {
      ElleGetUnodePosition(*it, &xy);
      outfile << i << ' ' << xy.x << ' ' << xy.y << ' ' << *it << ' ' << "0"
              << endl;
    }
  }
  /*
