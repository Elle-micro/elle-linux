...
int ElleCheckTripleJ(int node) {
  ...
  if (!ElleNodeIsActive(node) || !ElleNodeIsTriple(node))
    return (err);
  for (j = 0; j < 3; j++)
    idTnb[j] = NO_NB;
  ElleNeighbourNodes(node, nbnodes);
...
void ElleNodeSameMineral(int node, int *same, int *samemin) {
  ...
  *samemin = NO_NB;
  if (ElleFlynnAttributeActive(MINERAL)) {
    ...
    for (i = 0; i < 3; i++)
      if (mintype[i] != NO_NB && mintype[i] != themin)
        *same = 0;
    /*
            for(i=0;i<3;i++)
                if(mintype[i]!=NO_NB)
...
void ElleCheckUnit(float *xpts, float *ypts, int num, int *xflags, int *yflags,
  ...
  xoffset =
      (float)(unitcell.cellBBox[TOPLEFT].x - unitcell.cellBBox[BASELEFT].x);
  ...
  while (bl->y < (miny - unitcell.ylength * yflags[0]))
    yflags[0]++;
  while (tr->y > (maxy + unitcell.ylength * yflags[1]))
    yflags[1]++;
