...
public:
  Unode()
      : _id(0), _flynn(NO_VAL), _area(0), _attributes(0), _vpoints(),
        _near_nbs() {}
  Unode(int idval, Coords *xy)
      : _id(idval), _pos(*xy), _area(0), _attributes(0), _vpoints(),
        _near_nbs() {}
  Unode(int idval, int flynn, Coords *xy)
      : _id(idval), _flynn(flynn), _pos(*xy), _area(0), _attributes(0),
        _vpoints(), _near_nbs() {}
  ...
  void setPosition(const Coords *xy) {
    _pos.x = xy->x;
    _pos.y = xy->y;
  }
  ...
  void removeAttribute(const int id) { _attributes.removeAttribute(id); }
  ...
  void addVoronoiPt(const int index) {
    ...
    if ((it = find(_vpoints.begin(), _vpoints.end(), index)) == _vpoints.end())
      _vpoints.push_back(index);
  }
  // catch postNb not in list?
  void insertVoronoiPt(const int postNb, const int index) {
    std::list<int>::iterator it;
    if ((it = find(_vpoints.begin(), _vpoints.end(), postNb)) != _vpoints.end())
      _vpoints.insert(it, index);
  }
  void voronoiPts(std::list<int> &l) {
    for (std::list<int>::iterator it = _vpoints.begin(); it != _vpoints.end();
         it++)
      l.push_back(*it);
  }
  ...
  void addNearNb(const int nb) { _near_nbs.insert(nb); }
  void deleteNearNb(const int nb) { _near_nbs.erase(nb); }
  int numberOfVoronoiPts() { return (_vpoints.size()); }
  int numberOfNearNbs() { return (_near_nbs.size()); }
