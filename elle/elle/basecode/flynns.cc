...
void Flynn::addChild(int child) {
  if (child_list.size() > 0) {
    it = child_list.begin();
    while ((*it) < child && it != child_list.end())
      it++;
...
void Flynn::removeChild(int child) {
  ...
  it = child_list.begin();
  while (it != child_list.end() && *it != child)
    it++;
...
int Flynn::findFirstNode(std::list<int> &LClist) {
  ...
  while (it != LClist.end() && first == NO_NB) {
    std::vector<int> nodes;
    flynnNodes(*it, nodes);
    for (v_it = nodes.begin(); v_it != nodes.end() && first == NO_NB; v_it++) {
      ElleRegions(*v_it, rgns);
      i = 0;
      while (i < 3 && first == NO_NB) {
        if (rgns[i] != NO_NB && (l_it = find(LClist.begin(), LClist.end(),
                                             rgns[i])) == LClist.end())
          first = *v_it;
        i++;
      }
