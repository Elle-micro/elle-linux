...
#include "unodes.h"
#include "update.h"
#include <elle/common/portability.h> // modernize: portability macros (A26)
#include <fstream>
#include <iostream>
#include <math.h>
#include <stdio.h>
...
int InsertAndUpdateBnode(int bnode1, int bnode2, int dj, int *nb_bnode);
...
int Voronoi2Flynns(bool voronoi_by_flynn) {
  ...
  double area = 0;
  Coords xy, vxy;
  ELLE_UNUSED(j);       // modernize: declared but intentionally unused (A26)
  ELLE_UNUSED(max);     // modernize: declared but intentionally unused (A26)
  ELLE_UNUSED(last);    // modernize: declared but intentionally unused (A26)
  ELLE_UNUSED(matched); // modernize: declared but intentionally unused (A26)
  ELLE_UNUSED(area);    // modernize: declared but intentionally unused (A26)
  ELLE_UNUSED(xy);      // modernize: declared but intentionally unused (A26)
  ELLE_UNUSED(vxy);     // modernize: declared but intentionally unused (A26)
  ...
  if (ElleVoronoiActive() && !err) {
    ...
    for (i = 0; i < ElleMaxUnodes(); i++) {
      ...
      if ((count = pt_list.size()) > 2) { // how many segs is minimum?
        ...
        while (seg < count) {
          ...
          if (curr_node >= ElleMaxNodes() || !ElleNodeIsActive(curr_node)) {
            ...
            if ((k = ElleFindNbIndex(prev, curr_node)) == NO_NB) {
              ...
              err = InsertAndUpdateBnode(curr_node, prev, dj, &nb_bnode);
#if XY
#endif
          ...
          if (ElleNodeIsTriple(prev)) {
            if ((k = ElleFindNbIndex(curr_node, prev)) == NO_NB) {
              dj = ElleFindSpareNode();
              err = InsertAndUpdateBnode(prev, curr_node, dj, &nb_bnode);
          ...
          if ((err = ElleSetNeighbour(prev, // modernize: intentional assignment
                                            // in condition (behavior-neutral)
        ...
        if ((err = ElleSetNeighbour(last, ElleFindNbIndex(first, last), first,
                                    &i)))
          return (err);
...
int InsertAndUpdateBnode(int bnode1, int bnode2, int dj, int *nb_bnode) {
  ...
  while (index1 < 3 && (fabs(nodecoords[index1].x - tmp.x) > eps ||
                        fabs(nodecoords[index1].y - tmp.y) > eps))
    index1++;
  if (index1 == 3)
    cout << "HELP\n";
  j = (j + 2) % 4;
  ...
  while (index2 < 3 && (fabs(nodecoords[index2].x - tmp.x) > eps ||
                        fabs(nodecoords[index2].y - tmp.y) > eps))
    index2++;
  if (index2 == 3)
    cout << "HELP\n";
  ...
  if ((k = ElleFindNbIndex(bnode1, nb[index2])) != NO_NB) {
    ...
    ElleSetNeighbour(dj, NO_NB, bnode1, 0);
...
void initDfltFlynnAttributesFromUnodes() {
  ...
  int f_attr = NO_VAL;
  ELLE_MAYBE_UNUSED double val = 0, dflt = 0;
  list<int> pt_list;
  list<int>::iterator it;
...
void copyUnodeAttributesToFlynn(int unodeid, int flynnid) {
  ...
  int j, *attr_id, maxa = 0;
  int u_attr = NO_VAL;
  double val = 0;
  ELLE_MAYBE_UNUSED double dflt = 0;
