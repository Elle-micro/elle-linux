...
dimension
The following line consists of a '#' and a double. Each element is separated by
comma, space or tab. example input
#,dimension
#,0.01

**********************************************************/
int ReadPNMFile(char *fname, int *width, int *height, unsigned long *max,
                unsigned char **img, std::vector<int> &rgbcols,
                std::vector<Coords_3D> &orient, double *dim);
...
int ElleReadImage(char *fname, int ***image, int *rows, int *cols, int decimate,
  ...
  if ((err = ReadPNMFile(fname, &rowsP, &colsP, &maxvalP, &img, rgbcols, orient,
                         dim)) != 0) {
    sprintf(logbuf, "\nError reading %s\n", fname);
    Log(2, logbuf);
    return (err);
  ...
  *image = imatrix(0L, (long)rowsP / decimate, 0L, (long)colsP / decimate);
  ...
  for (ptr = img, i = 0; i < rowsP; i++) {
    for (j = 0; j < colsP; j++) {
      ...
      colr =
          *ptr + ((*(ptr + 1)) * maxvalP) + ((*(ptr + 2)) * maxvalP * maxvalP);
      ptr += 3;
...
int ReadPNMFile(char *fname, int *width, int *height, unsigned long *max,
  ...
  *height = *width = *max = 0;
  ...
  case '2':
  case '5':
    Log(0, "Greyscale PNM not implemented");
    err = READ_ERR;
    break;
  default:
    err = READ_ERR;
  ...
  while (!done) {
    ...
    if (c != '#') {
      ...
      if (strlen(str) < 2 || !isdigit(str[1])) {
        ...
        if (!done) {
          for (i = 0; i < 3; i++)
            rgbcols.push_back(intval[i]);
          // convert geo convention to math
          GeoToMath(&ori.x, &ori.y);
          // convert caxis to euler
          PolarToCartesian(&tmpori[0], &tmpori[1], &tmpori[2], (ori.x) * DTOR,
                           (ori.y) * DTOR);
    ...
    if (c != '#') {
      ...
      if (c == ',' || c == ' ' || c == '\t') {
        ...
        if (!isdigit(nextc)) {
          ...
          if ((num = fscanf(fp, "%c%f", &sep, &fval)) != 2)
            err = READ_ERR;
          else
            *dim = fval;
        }
