...
Unode *Segment::GetStartNode() { return Segment::StartNode; }

void Segment::SetStartNode(Unode *unode) { Segment::StartNode = unode; }

void Segment::SetStartNode(int idval) {
  Segment::StartNode = ElleGetParticleUnode(idval);
}

Unode *Segment::GetEndNode() { return Segment::EndNode; }

void Segment::SetEndNode(Unode *unode) { Segment::EndNode = unode; }

void Segment::SetEndNode(int idval) {
  Segment::EndNode = ElleGetParticleUnode(idval);
}

double Segment::GetLength() { return Segment::length; }
...
void Segment::SetLength(double length) { Segment::length = length; }
  ...
  while (!feof(fp) && !err) {
    ...
    if (str[0] == '#')
      ...
      if ((num = fscanf(fp, "%lf", &val)) != 1)
        return (READ_ERR);
      if (i < max)
        Segments[i].setAttribute(attr_id, val);
      num = fscanf(fp, "\n");
    }
  }
  return (err);
...
int ElleReadSegmentData(FILE *fp, char str[]) {
  ...
  int err = 0, i, num, id_start_unode, id_end_unode;
  ELLE_MAYBE_UNUSED int found = 0, last = 0, test = 0;
  ELLE_MAYBE_UNUSED double val = 0;
  ...
  while (!feof(fp) && !err) {
    ...
    if (str[0] == '#')
      ...
      cout << tmp.id() << " " << tmp.GetStartNode()->id() << " "
           << tmp.GetEndNode()->id() << endl;
      cout << tmp.GetLength() << " " << tmp.GetStartNodePos().x << " "
           << tmp.GetEndNodePos().x << endl
           << endl;
    }
    num = fscanf(fp, "\n");
  }
...
int ElleWriteSegmentConnections(ofstream &outf) {
  ...
  if (Segments.size() > 0) {
    if (!id_match(FileKeys, SEGMENTS, label))
      err = KEY_ERR;
    else
      outf << label << endl;
    if (!err) {
...
int ElleWriteSegmentData(const char *fname) {
  ...
  if (Segments.size() > 0) {
    ...
    if ((err = ElleWriteSegmentConnections(outf)))
      return (err);
    ...
    for (i = 0; i < maxa && !err; i++) {
      if (!id_match(FileKeys, attr_ids[i], label)) {
        cerr << "Ignoring unknown segment attribute " << attr_ids[i] << endl;
      } else {
        outf << label << endl;
    ...
    //              outf << endl;
    //              err = ElleWriteUnodeAttributeData(outf,&attr_ids[i],cnt);
    //              i += 2; // allow for 3 attrib in dflt list,for loop will
    //              incr i break;
    //            case DISLOCDEN:
    //              file_id=U_DISLOCDEN;
    //              // FALL THROUGH
