...
int ElleWriteRegionAttributeData(FILE *fp) {
  ...
  for (i = 0; i < maxa && !err; i++) {
    ...
    case GRAIN:
    case SPLIT:
      err = ElleWriteFlynnIntAttribData(fp, active[i]);
      break;
    case MINERAL:
      err = ElleWriteMineralData(fp);
      break;
    case CAXIS:
      err = ElleWriteCAxisData(fp);
      ...
      break;
    case E3_ALPHA:
      err = ElleWriteEuler3Data(fp);
      if (ElleEnergyLUTSize() > 0)
        err = ElleWriteGBEnergyLUT(fp);
      while (i < (maxa - 1) &&
             (active[i + 1] == E3_BETA || active[i + 1] == E3_GAMMA))
        i++;
      break;
    case F_INCR_S:
    case INCR_S:
    ...
    case E_YY:
    case E_ZZ:
      if ((keys = (int *)malloc(NUM_FLYNN_STRAIN_VALS * sizeof(int))) == 0)
        OnError("ElleWriteRegionAttributeData", MALLOC_ERR);
      for (j = 0; j < NUM_FLYNN_STRAIN_VALS; j++)
        keys[j] = NO_INDX;
      j = 0;
    ...
    case TAU_1:
    case PRESSURE:
      if ((keys = (int *)malloc(NUM_STRESS_VALS * sizeof(int))) == 0)
        OnError("ElleWriteRegionAttributeData", MALLOC_ERR);
      for (j = 0; j < NUM_STRESS_VALS; j++)
        keys[j] = NO_INDX;
      j = 0;
      while (i < maxa && id_match(StressKeys, active[i], label)) {
        keys[j] = active[i];
        j++;
        i++;
      }
    ...
    case AGE:
    case CYCLE:
      if ((keys = (int *)malloc(NUM_FLYNN_AGE_VALS * sizeof(int))) == 0)
        OnError("ElleWriteRegionAttributeData", MALLOC_ERR);
      for (j = 0; j < NUM_FLYNN_AGE_VALS; j++)
        keys[j] = NO_INDX;
      j = 0;
    ...
    case F_ATTRIB_J:
    case F_ATTRIB_K:
      err = ElleWriteFlynnRealAttribData(fp, active[i]);
      break;
    default:
      err = ATTRIBID_ERR;
      break;
...
int ElleReadFlynnIntAttribData(FILE *fp, char str[], int attr) {
  ...
  while (!feof(fp) && !err) {
    ...
    if (str[0] == '#')
      ...
      if ((full_id = atoi(str)) == MAX_INDX)
        return (MAXINDX_ERR);
      if (Region_ver)
        if ((num = fscanf(fp, "%d %d", &s, &u)) != 2)
          return (READ_ERR);
      if ((num = fscanf(fp, "%d", &val)) != 1)
        return (READ_ERR);
...
int ElleWriteMineralData(FILE *fp) {
  ...
  if (!id_match(FileKeys, MINERAL, label))
    return (KEY_ERR);
  fprintf(fp, "%s\n", label);
  dflt = ElleDefaultFlynnMineral();
  if (!id_match(MineralKeys, dflt, val))
    return (KEY_ERR);
  ...
  for (i = 0; i < max; i++) {
    if (ElleFlynnIsActive(i) && ElleFlynnHasAttribute(i, MINERAL)) {
      ...
      if (mineral_id != dflt) {
        if (!id_match(MineralKeys, mineral_id, val))
          return (KEY_ERR);
...
int ElleWriteFlynnIntAttribData(FILE *fp, int key) {
  ...
  if (!id_match(FileKeys, key, label))
    return (KEY_ERR);
  fprintf(fp, "%s\n", label);
...
int ElleWriteAgeData(FILE *fp, int *keys, int count) {
  ...
  if (!id_match(FileKeys, AGE, label))
    return (KEY_ERR);
  fprintf(fp, "%s\t", label);
  for (i = 0; i < count; i++) {
    if (!id_match(FlynnAgeKeys, keys[i], label))
      return (KEY_ERR);
    fprintf(fp, "%s ", label);
  }
  fprintf(fp, "\n");
...
int ElleWriteFlynnStrainData(FILE *fp, int *keys, int count) {
  ...
  if (!id_match(FileKeys, FLYNN_STRAIN, label))
    return (KEY_ERR);
  fprintf(fp, "%s\t", label);
  for (i = 0; i < count; i++) {
    if (!id_match(FlynnStrainKeys, keys[i], label))
      return (KEY_ERR);
    fprintf(fp, "%s ", label);
  }
  fprintf(fp, "\n");
...
int ElleWriteFlynnStressData(FILE *fp, int *keys, int count) {
  ...
  if (!id_match(FileKeys, FLYNN_STRESS, label))
    return (KEY_ERR);
  fprintf(fp, "%s\t", label);
  for (i = 0; i < count; i++) {
...
int ElleWriteCAxisData(FILE *fp) {
  ...
  if (!id_match(FileKeys, CAXIS, label))
    return (KEY_ERR);
  fprintf(fp, "%s\n", label);
...
int SaveZIPGrainData(gzFile out) {
  ...
  if (!id_match(FileKeys, GRAIN, label))
    return (KEY_ERR);
  gzprintf(out, "%s\n", label);
...
int SaveZIPColourData(gzFile out) {
  ...
  if (!id_match(FileKeys, COLOUR, label))
    return (KEY_ERR);
  gzprintf(out, "%s\n", label);
...
int SaveZIPDislocDenData(gzFile out) {
  ...
  if (!id_match(FileKeys, DISLOCDEN, label))
    return (KEY_ERR);
  gzprintf(out, "%s\n", label);
...
int SaveZIPEnergyData(gzFile out) {
  ...
  int max;
  float val, dflt;
  if (!id_match(FileKeys, ENERGY, label))
    return (KEY_ERR);
  gzprintf(out, "%s\n", label);
...
int SaveZIPExpandData(gzFile out) {
  ...
  if (!id_match(FileKeys, EXPAND, label))
    return (KEY_ERR);
  gzprintf(out, "%s\n", label);
...
int SaveZIPViscosityData(gzFile out) {
  ...
  if (!id_match(FileKeys, VISCOSITY, label))
    return (KEY_ERR);
  gzprintf(out, "%s\n", label);
