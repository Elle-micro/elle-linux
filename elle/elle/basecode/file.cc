...
int ElleReadData(char *filename) {
  ...
  while (!feof(fp) && !finished) {
    ...
    switch (key) {
    case E_OPTIONS:
      if ((err = ElleReadOptionData(
               fp, str))) // modernize: parenthesize assignment in condition
                          // (fast-track analysis edit)
        finished = 1;
      break;
    case REGIONS:
    case FLYNNS:
      if ((err = ElleReadFlynnData(
               fp, str))) // modernize: parenthesize assignment in condition
                          // (fast-track analysis edit)
        finished = 1;
      break;
    case PARENTS:
      if ((err = ElleReadParentData(
               fp, str))) // modernize: parenthesize assignment in condition
                          // (fast-track analysis edit)
        finished = 1;
      break;
    case UNODES:
      ...
      if ((err = ElleReadUnodeData(fp, str, keys,
                                   i))) // modernize: parenthesize assignment in
                                        // condition (fast-track analysis edit)
        finished = 1;
      /*ElleVoronoiUnodes();*/
      break;
    case SEGMENTS:
      if ((err = ElleReadSegmentData(
               fp, str))) // modernize: parenthesize assignment in condition
                          // (fast-track analysis edit)
        finished = 1;
      break;
    case LOCATION:
      if ((err = ElleReadLocationData(
               fp, str))) // modernize: parenthesize assignment in condition
                          // (fast-track analysis edit)
        finished = 1;
      break;
    case AGE:
      ...
      if (finished)
        err = READ_ERR;
      else if ((err = ElleReadAgeData(
                    fp, str, keys, i))) // modernize: parenthesize assignment in
                                        // condition (fast-track analysis edit)
        finished = 1;
      break;
    case ENERGY:
    ...
    case F_ATTRIB_E:
    case F_ATTRIB_F:
      if ((err = ElleReadFlynnRealAttribData(
               fp, str, key))) // modernize: parenthesize assignment in
                               // condition (fast-track analysis edit)
        finished = 1;
      break;
    case F_ATTRIB_I:
    ...
    case SPLIT:
    case GRAIN:
      if ((err = ElleReadFlynnIntAttribData(
               fp, str, key))) // modernize: parenthesize assignment in
                               // condition (fast-track analysis edit)
        finished = 1;
      break;
    ...
    case ATTRIB_E:
    case ATTRIB_F:
      if ((err = ElleReadNodeRealAttribData(
               fp, str, key))) // modernize: parenthesize assignment in
                               // condition (fast-track analysis edit)
        finished = 1;
      break;
    case N_MCONC_A:
      if ((err = ElleReadNodeMarginAttribData(
               fp, str, key))) // modernize: parenthesize assignment in
                               // condition (fast-track analysis edit)
        finished = 1;
      break;
    case STRESS: /*
      ...
      if (finished)
        err = READ_ERR;
      else if ((err = ElleReadStressData(
                    fp, str, keys, i))) // modernize: parenthesize assignment in
                                        // condition (fast-track analysis edit)
        finished = 1;
      break;
    case STRAIN: /*
      ...
      if (finished)
        err = READ_ERR;
      else if ((err = ElleReadStrainData(
                    fp, str, keys, i))) // modernize: parenthesize assignment in
                                        // condition (fast-track analysis edit)
        finished = 1;
      break;
    case CAXIS:
      if ((err = ElleReadCAxisData(
               fp, str))) // modernize: parenthesize assignment in condition
                          // (fast-track analysis edit)
        finished = 1;
      ...
      break;
    case EULER_3:
      if ((err = ElleReadEuler3Data(
               fp, str))) // modernize: parenthesize assignment in condition
                          // (fast-track analysis edit)
        finished = 1;
      break;
    case GBE_LUT:
      if ((err = ElleReadGBEnergyLUT(
               fp, str))) // modernize: parenthesize assignment in condition
                          // (fast-track analysis edit)
        finished = 1;
      break;
    case MINERAL:
      if ((err = ElleReadMineralData(
               fp, str))) // modernize: parenthesize assignment in condition
                          // (fast-track analysis edit)
        finished = 1;
    ...
    case FLYNN_STRAIN: /*
      ...
      if (finished)
        err = READ_ERR;
      else if ((err = ElleReadFlynnStrainData(
                    fp, str, keys, i))) // modernize: parenthesize assignment in
                                        // condition (fast-track analysis edit)
        finished = 1;
      break;
    case FLYNN_STRESS: /*
      ...
      if (finished)
        err = READ_ERR;
      else if ((err = ElleReadFlynnStressData(
                    fp, str, keys, i))) // modernize: parenthesize assignment in
                                        // condition (fast-track analysis edit)
        finished = 1;
      break;
    ...
    case U_VISCOSITY:
      if (key == U_CONC_A)
        key = CONC_A;
      if ((err = ElleReadUnodeRealAttribData(
               fp, str, &key, 1))) // modernize: parenthesize assignment in
                                   // condition (fast-track analysis edit)
        finished = 1;
      break;
    ...
    case U_EULER_3: /*
      ...
      keys[1] = E3_BETA;
      keys[2] = E3_GAMMA;
      if ((err = ElleReadUnodeRealAttribData(fp, str, keys, 3)))
        finished = 1;
      break;
    case U_CAXIS: /*
                   ...
                   */
      keys[0] = CAXIS;
      if ((err = ElleReadUnodeRealAttribData(fp, str, keys, 1)))
        finished = 1;
      break;
    case S_PERMEABILITY:
    case S_FLUID_FLUX:
      if ((err = ElleReadSegmentAttribData(
               fp, str, key))) // modernize: parenthesize assignment in
                               // condition (fast-track analysis edit)
        finished = 1;
      break;
...
int ElleWriteData(const char *filename) {
  ...
  fclose(fp);
  if (!err)
    err = ElleWriteUnodeData(filename);
  if (!err)
    err = ElleWriteSegmentData(filename);
  return (err);
}
...
int ElleReadDisplayOptions(char *filename) {
  ...
  if ((fp = fopen(filename, "r")) == NULL)
    return (OPEN_ERR);
  if ((num = fscanf(fp, "%s", input_str)) != 1)
    return (READ_ERR);
  else {
