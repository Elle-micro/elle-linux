...
char *ES_StatsFilename() { return (Stats.file); }

void ES_SetstatsInterval(int val) { Stats.frequency = val; }

int ES_statsInterval() { return (Stats.frequency); }

void ES_SetSaveNodes(int val) { Stats.nodes = val; }

int ES_SaveNodes() { return (Stats.nodes); }

void ES_SetSaveGrains(int val) { Stats.grains = val; }

int ES_SaveGrains() { return (Stats.grains); }

void ES_SetSaveavArea(int val) { Stats.av_area = val; }

int ES_SaveavArea() { return (Stats.av_area); }

void ES_SetSaveBLength(int val) { Stats.bnd_length = val; }

int ES_SaveBLength() { return (Stats.bnd_length); }

void ES_SetSavem2Sides(int val) { Stats.sides_m2 = val; }

int ES_Savem2Sides() { return (Stats.sides_m2); }

void ES_SetSavem2Areas(int val) { Stats.areas_m2 = val; }

int ES_Savem2Areas() { return (Stats.areas_m2); }
...
int ES_WriteStatistics(int count) {
  ...
  if (count < ES_statsInterval()) {
    ...
    fprintf(fp, "*\n"); /* for cricket graph */
    fprintf(fp, "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s", "time", "Grains", "DJs",
            "TJs", "BLength", "Area", "m2_sides", "m2_area");
    fprintf(fp, "\t%s\t%s\t%s\t%s\t%s", "minang", "maxang", "ratio", "blen",
            "acc");
    fprintf(fp, "\n");
  } else if ((fp = fopen(ES_StatsFilename(), "a")) == 0)
    OnError(ES_StatsFilename(), OPEN_ERR);
  ElleNumberOfNodes(&djs, &tjs);
  fprintf(fp, "%6d\t%8d\t%8d\t%8d\t%.2f\t%12.10f\t%.3f\t%.3f", count,
          ElleNumberOfGrains(), djs, tjs, ES_BoundaryLength(),
          ES_AverageGrainArea(), ES_SecondMomentGrainSides(),
          ES_SecondMomentGrainArea());
...
int PanozzoAnalysis(FoliationData *data) {
      ...
      for (j = 0; j < 3; j++) {
        if (nbnodes[j] != NO_NB && nbnodes[j] > i) {
          ...
          if (len > 0.0) {
            ...
            printf("PanozzoAnalysis-len 0"); // modernize: remove extra unused
                                             // format arg (behavior-neutral)
        }
      }
    }
  }
  ...
  if (maxL > 0.0)
    for (i = 0; i < 181; i++)
      rosedata[i] /= maxL;
  if (minL > 0.0)
    data->ratio = (float)(maxL / minL);
  data->maxAng = (float)maxA;
  data->minAng = (float)minA;
  data->gblength = totlen;
  if (maxA > minA)
    data->accuracy = (float)fabs(maxA - minA - 90.0);
  else
    data->accuracy = (float)fabs(minA - maxA - 90.0);
  return 0; // modernize: add explicit return for non-void function
            // (behavior-neutral)
}
...
int ES_NodeAttributeStats(int attrib_index, NodeAttributeStats *n_stats) {
  ...
  if (index == NO_NB)
    ...
    if (num_active > 0)
      n_stats->average = total / num_active;
    n_stats->min = val_min;
    n_stats->max = val_max;
  }
  return (err);
