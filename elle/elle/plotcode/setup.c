...
char **argv;
{
  ...
  if (ElleDisplay()) {
    if (Init_App(argc, argv, &Plot_info.log_file_ptr))
      exit(1);
    ...
    case STD_MAP:
      ...
      break;
    default:
      fprintf(stderr, "Invalid colour map/n");
      break;
    }
    if (err = Set_Colours(red, green, blue, irgbv, STEPS))
      return (1);
  }
...
int RescaleBox(float xmin, float xmax, float ymin, float ymax) {
  int chng_plot = 0;
  ...
  chng_plot = ((fabs(oldwdth - (Pwindo[XCMAX] - Pwindo[XCMIN])) > 0) ||
                       (fabs(oldhgt - (Pwindo[YCMAX] - Pwindo[YCMIN])) > 0)
                   ? 1
                   : 0);
...
void MeshParams(hgt, wdth, mesh) float hgt, wdth;
int *mesh;
{
  int prev_nny3;
...
void set_box_dimensions(pwin, x, y, width, height) float *pwin, x, y, width,
    height;
{
...
void init_box(pwin, rescale) unsigned char rescale;
float *pwin;
{
  int pen_state;
  ...
  scale_(&pwin[ULEFT], &pwin[URGHT], &pwin[XMIN], &pwin[XMAX], &pwin[UBASE],
         &pwin[UTOP], &pwin[YMIN], &pwin[YMAX]);
  pen_state = PEN_UP;
  plotu_(&pwin[XCMIN], &pwin[YCMIN], &pen_state);
}

void create_error_message(message, err_num) char *message;
int err_num;
{
  char buf[20];
  ...
  switch (err_num) {
  case MALLOC_ERR:
    strcat(message, " - Memory error");
    break;
  case READ_ERR:
    strcat(message, " - Error reading file");
    break;
  case OPEN_ERR:
    strcat(message, " - Error opening file");
    break;
  case EOF_ERR:
    strcat(message, " - End of file");
    if (Plot_info.inp_file != NULL)
      sprintf(buf, " %2d records", Plot_info.inp_file->rec_max);
    strcat(message, buf);
    break;
  case VHB_ERR:
    strcat(message, " - No VHB array");
    break;
  case SSQ_ERR:
    strcat(message, " - No SSQ array");
    break;
  case ELFX_ERR:
    strcat(message, " - No IELFIX array");
    break;
  case REF_ERR:
    strcat(message, " - Reference error");
    break;
  case LAG_ERR:
    strcat(message, " - No Lagrangian arrays");
    break;
  case STR_ERR:
    strcat(message, " - Error reading string");
    break;
  case NUM_ERR:
    strcat(message, " - Error reading number");
    break;
  case VAR_ERR:
    strcat(message, " - Unknown variable");
    break;
  case KEY_ERR:
    strcat(message, " - Unknown keyword");
    break;
  case INIT_ERR:
    strcat(message, " - Initial variable must be Options");
    break;
  case RANGE_ERR:
    strcat(message, " - Value out of range");
    break;
  case STGS_ERR:
    strcat(message, " - row, col or orientation mismatch");
    break;
  default:
    break;
  }
...
error_msg(err_num, str) int err_num;
char *str;
{
...
format_number_(str, num, format) char *str;
float *num;
int *format;
{
  char form[7];
