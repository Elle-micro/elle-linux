...
ToggleItem StatsOption_items_toggle[] = {
    {SO_NODES},  {SO_GRAINS},   {SO_AV_AREA},
    {SO_BNDLEN}, {SO_M2_SIDES}, {SO_M2_AREAS},
};

static valid_terms statsoption_terms[] = {
    "Update Interval", O_UPDFREQ,     "Node count",
    SO_NODES,          "Grain count", SO_GRAINS,
    "Boundary length", SO_BNDLEN,     "Average area",
    SO_AV_AREA,        "m2 Sides",    SO_M2_SIDES,
    "m2 Areas",        SO_M2_AREAS,   NULL};

void StatsOptsDialog(w, client_data, call_data) Widget w;
...
XtPointer call_data;
{
  ...
  if (!opts_dialog) {
    ...
    for (j = 0; j < XtNumber(StatsOption_items_int); j++) {
      id_match(statsoption_terms, StatsOption_items_int[j].id, wdgt_label);
      XtVaCreateManagedWidget(wdgt_label, xmLabelGadgetClass, rowcol1, NULL);

      GetStatsOptionValue(StatsOption_items_int[j].id, &val);
      sprintf(buf, "%4d", val);
      StatsOpts_wdgts[i] = XtVaCreateManagedWidget(
          "txt_wdgt", xmTextFieldWidgetClass, rowcol1, XmNuserData,
          (XtPointer)StatsOption_items_int[j].id, XmNvalue, buf, XmNcolumns, 4,
          NULL);
      i++;
    }
    ...
    toggle_box =
        XtVaCreateManagedWidget("toggle", xmRowColumnWidgetClass, pane, NULL);

    /*
     * create ToggleButton gadgets with Labels
     */
    for (j = 0; j < XtNumber(StatsOption_items_toggle); j++) {
      id_match(statsoption_terms, StatsOption_items_toggle[j].id, wdgt_label);
      StatsOpts_wdgts[i] = XtVaCreateManagedWidget(
          wdgt_label, xmToggleButtonGadgetClass, toggle_box, XmNuserData,
          (XtPointer)StatsOption_items_toggle[j].id, NULL);
      /*
...
XtPointer client_data, call_data;
{
  ...
  for (i = 0; i < items; i++) {
    XtVaGetValues(StatsOpts_wdgts[i], XmNuserData, &ptr, NULL);
  ...
  for (j = 0; j < items; j++, i++) {
    XtVaGetValues(StatsOpts_wdgts[i], XmNuserData, &ptr, XmNset, &t_val, NULL);
    val = (t_val ? 1 : 0);
    SetStatsOptionValue((int)ptr, val);
  }
...
void SetStatsOptionValue(int id, int val) {
  ...
  case SO_NODES:
    ...
    break;
  case SO_GRAINS:
    ES_SetSaveGrains(val);
    break;
  case SO_AV_AREA:
    ES_SetSaveavArea(val);
    break;
  case SO_BNDLEN:
    ES_SetSaveBLength(val);
    break;
  case SO_M2_SIDES:
    ES_SetSavem2Sides(val);
    break;
  case SO_M2_AREAS:
    ES_SetSavem2Areas(val);
    break;
  case O_UPDFREQ:
    ES_SetstatsInterval(val);
    break;
  default:
    break;
  }
