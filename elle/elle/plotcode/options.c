...
void OptsDialog();
void RunOptsDialog();
void BuildOptionBar(OptsBarItem *info, Widget parent, Widget *bar, int *fl_cnt,
                    MaxMinData *maxmin);
...
OptsBarItem Baroptions[] = {
    ...
    {"Node Attribute", node_terms, &(Buttons[1][0]), change_bar, O_NODEATTRIB,
     O_NODEATTRMIN, O_NODEATTRMAX},
...
OptionItem RunOption_items[] = {
    /*{ RO_DISPLAY, SYB_INT },*/
    {RO_STAGES, SYB_INT},        {RO_SVFREQ, SYB_INT},
    {RO_SVFILEROOT, SYB_STRING}, {RO_SWITCHDIST, SYB_DOUBLE},
    {RO_SPEEDUP, SYB_DOUBLE},
};

void RunOptsDialog(w, client_data, call_data) Widget w;
...
XtPointer call_data;
{
  ...
  if (!run_opts_dialog) {
    ...
    run_opts_dialog = XtVaCreatePopupShell(
        "Set_Runtime_Options", xmDialogShellWidgetClass, *parent, NULL);

    button_items_run[0].data = &run_user_data;
    button_items_run[1].data = &run_user_data;

    pane = XtVaCreateWidget("pane", xmPanedWindowWidgetClass, run_opts_dialog,
                            XmNsashWidth, 1, XmNsashHeight, 1, NULL);

    rowcol1 = XtVaCreateWidget(
        "rowcol1", xmRowColumnWidgetClass, pane, XmNpacking, XmPACK_COLUMN,
        XmNnumColumns, XtNumber(RunOption_items), XmNorientation, XmHORIZONTAL,
        XmNisAligned, True, XmNentryAlignment, XmALIGNMENT_END, NULL);
    ...
    for (i = 0, j = 0; j < XtNumber(RunOption_items); j++) {
      ...
      case SYB_INT:
        ElleGetRunOptionValue(RunOption_items[j].id, &val);
        sprintf(buf, "%4d", val);
        break;
      case SYB_FLOAT:
        ElleGetRunOptionValue(RunOption_items[j].id, (int *)(&fval));
        sprintf(buf, "%f", fval);
        break;
      case SYB_DOUBLE:
        ElleGetRunOptionValue(RunOption_items[j].id, (int *)(&dval));
        sprintf(buf, "%.3e", dval);
        break;
      case SYB_STRING:
        ElleGetRunOptionValue(RunOption_items[j].id, (int *)name);
        strncpy(buf, name, 29);
        break;
      default:
        break;
      }
      XtVaCreateManagedWidget(wdgt_label, xmLabelGadgetClass, rowcol1, NULL);

      RunOpts_wdgts[i] = XtVaCreateManagedWidget(
          "txt_wdgt", xmTextFieldWidgetClass, rowcol1, XmNuserData,
          (XtPointer)RunOption_items[j].id, XmNvalue, buf, XmNcolumns, 6, NULL);
      i++;
    }
    /*
     ...
     */
    CreateActionArea(pane, button_items_run, XtNumber(button_items_run),
                     wdth * 12);

    XtTranslateCoords(w, (Position)0, (Position)0, &x, &y);
    XtVaSetValues(run_opts_dialog, XmNx, x, XmNy, y, NULL);
    XtManageChild(pane);
...
void OptsDialog(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
{
  XmString bar, label;
  static Widget opts_dialog, toggle_box, bar_menus[NUMBARS], level, level_menu,
      level_button[MAX_LEVEL_OPTIONS];
  ...
  if (!opts_dialog) {
    for (i = 0; i < NUMBARS; i++)
      bar_menus[i] = NULL;
    user_data.num_ints = user_data.num_toggles = user_data.num_floats = 0;
    parent = (Widget *)client_data;
    opts_dialog = XtVaCreatePopupShell("Set_Display_Options",
                                       xmDialogShellWidgetClass, *parent, NULL);

    button_items_opts[0].data = &user_data;
    button_items_opts[1].data = &user_data;

    pane = XtVaCreateWidget("pane", xmPanedWindowWidgetClass, opts_dialog,
                            XmNsashWidth, 1, XmNsashHeight, 1, NULL);

    i = 0;
#if XY
#endif
    rowcol1 = XtVaCreateWidget("rowcol1", xmRowColumnWidgetClass, pane,
                               XmNpacking, XmPACK_COLUMN, XmNnumColumns, 2,
                               XmNorientation, XmVERTICAL, XmNisAligned, True,
    ...
    for (j = 0; j < XtNumber(Option_items_int); j++) {
      ...
      GetOptionValue(&Tmp, Option_items_int[j].id, (XtPointer)(&val));
      sprintf(buf, "%4d", val);
      Txt_int_wdgts[i] = XtVaCreateManagedWidget(
          "txt_wdgt", xmTextFieldWidgetClass, rowcol1, XmNuserData,
          (XtPointer)Option_items_int[j].id, XmNvalue, buf, XmNcolumns, 4,
          NULL);
      i++;
    }
    ...
    toggle_box = XtVaCreateManagedWidget("toggle", xmRowColumnWidgetClass, pane,
    ...
    for (j = 0; j < XtNumber(Option_items_toggle); j++) {
      ...
      Txt_int_wdgts[i] = XtVaCreateManagedWidget(
          wdgt_label, xmToggleButtonGadgetClass, toggle_box, XmNuserData,
          (XtPointer)Option_items_toggle[j].id, NULL);
      XtAddCallback(Txt_int_wdgts[i], XmNvalueChangedCallback, toggled,
                    (XtPointer)Option_items_toggle[j].id);
      GetOptionValue(&Tmp, Option_items_toggle[j].id, (XtPointer)(&val));
    ...
    XtVaCreateManagedWidget("sep1", xmSeparatorGadgetClass, toggle_box, NULL);

    for (j = 0; Baroptions[j].label != NULL; j++)
      BuildOptionBar(&(Baroptions[j]), toggle_box, &bar_menus[j], &fl_cnt,
                     &MaxMin[j]);
    ...
    n = 0;
    XtSetArg(args[n], XmNlabelString, bar);
    n++;
    XtSetArg(args[n], XmNsubMenuId, level_menu);
    n++;
    level = XmCreateOptionMenu(toggle_box, "level_menu", args, n);
    GetOptionValue(&Tmp, O_BNDLEVEL, (XtPointer)(&state));
    for (k = 1, n = 0; n < MAX_LEVEL_OPTIONS; n++, k++) {
      sprintf(buf, "%2d", k);
      level_button[n] =
          XtVaCreateManagedWidget(buf, xmPushButtonGadgetClass, level_menu,
                                  XmNuserData, O_BNDLEVEL, NULL);
    ...
    /* Create the action area */
    CreateActionArea(pane, button_items_opts, XtNumber(button_items_opts),
                     wdth);
    ...
    XtTranslateCoords(w, (Position)0, (Position)0, &x, &y);
    XtVaSetValues(opts_dialog, XmNx, x, XmNy, y, NULL);
    XtManageChild(pane);
  } else {
#if XY
#endif
...
#if XY
    if (id_match(boundary_terms, Tmp.boundary_attrib, buf)) {
#if XmVersion < 1002
      label = XmStringCreateSimple(buf);
#else
      label = XmStringCreateLocalized(buf);
#endif
      XtVaSetValues(XmOptionButtonGadget(bnd_bar_menu), XmNlabelString, label,
                    NULL);
      XmStringFree(label);
    }
#endif
   ...
   */
  for (j = 0; Baroptions[j].label != NULL; j++) {
    GetActiveAttributeList(Baroptions[j].userdata, &f_attr, &num_f_attr,
                           &start);
    GetOptionValue(&Tmp, Baroptions[j].userdata, (XtPointer)(&state));
      ...
      for (n = 0, found = 0; n < num_f_attr && !found; n++)
        if (Baroptions[j].items[k].id == f_attr[n] ||
            (Baroptions[j].items[k].id == MISORIENTATION &&
             f_attr[n] == E3_ALPHA))
          found = 1;
      if (Baroptions[j].items[k].id == TRPL_ONLY)
        found = 1;
      if (Baroptions[j].items[k].id == U_LOCATION && ElleUnodesActive())
        found = 1;
      if (found) {
        XtSetSensitive(list[k], True);
        if (Baroptions[j].items[k].id == state)
          tmp_w = list[k];
      } else {
...
void BuildOptionBar(OptsBarItem *info, Widget parent, Widget *bar, int *fl_cnt,
  ...
  n++;
  bar_menu = *bar = XmCreateOptionMenu(parent, "region_menu", args, n);
  for (k = 0, tmp_w = NULL; info->items[k].name; k++) {
    info->buttonlist[k] =
        XtVaCreateManagedWidget(info->items[k].name, xmPushButtonGadgetClass,
                                menu, XmNuserData, maxmin, NULL);
    if (info->items[k].id == state)
      tmp_w = info->buttonlist[k];
    XtAddCallback(info->buttonlist[k], XmNactivateCallback, info->callback,
  ...
  rowcol2 = XtVaCreateWidget("min_max", xmRowColumnWidgetClass, parent,
                             XmNpacking, XmPACK_COLUMN, XmNnumColumns, 4,
                             XmNorientation, XmVERTICAL, XmNisAligned, True,
                             XmNentryAlignment, XmALIGNMENT_END, NULL);
  sprintf(buf, "%s", "Min:");
  XtVaCreateManagedWidget(buf, xmLabelGadgetClass, rowcol2, NULL);

  maxmin->id = info->userdata;
  Txt_fl_wdgts[*fl_cnt] = maxmin->min = XtVaCreateManagedWidget(
      "rgn_min", xmTextFieldWidgetClass, rowcol2, XmNuserData,
      (XtPointer)info->userdata_min, XmNcolumns, 10, NULL);
  ...
  Txt_fl_wdgts[*fl_cnt] = maxmin->max = XtVaCreateManagedWidget(
      "rgn_max", xmTextFieldWidgetClass, rowcol2, XmNuserData,
      (XtPointer)info->userdata_max, XmNcolumns, 10, NULL);
  maxmin->max_id = info->userdata_max;
  (*fl_cnt)++;
  XtManageChild(rowcol2);

  XtVaCreateManagedWidget("sep", xmSeparatorGadgetClass, parent, NULL);
}

/*
 * dialog ok button callback function for plotting options
 */
static void OptsDlgDone(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
  ...
  for (i = 0; i < items; i++) {
    XtVaGetValues(Txt_int_wdgts[i], XmNuserData, &ptr, NULL);
  ...
  for (i = 0; i < items; i++) {
    XtVaGetValues(Txt_fl_wdgts[i], XmNuserData, &ptr, NULL);
 ...
 */
static void RunOptsDlgDone(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
  unsigned char reset = 0;
  char *str;
  ...
  for (i = 0; i < items; i++) {
    XtVaGetValues(RunOpts_wdgts[i], XmNuserData, &ptr, NULL);
    id = (int)ptr;
    str = XmTextFieldGetString(RunOpts_wdgts[i]);
    j = 0;
    while (j < XtNumber(RunOption_items) && RunOption_items[j].id != id)
      j++;
    ...
    case SYB_DOUBLE:
      ...
      break;
    case SYB_STRING:
      ElleSetRunOptionValue(id, (int *)str);
      break;
    default:
      break;
    }
 ...
 */
static void UserCancel(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
  OptionDataItem *user_data;
...
void toggled(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
  int val;
...
void change_bar(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
  char buf[10];
...
void change_level(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
  char buf[10];
  int attr = (int)client_data;
...
int GetActiveAttributeList(int id, int **list, int *num, int *start) {
  ...
  *start = 1;
  switch (id) {
  case O_FLYNNATTRIB:
    ElleFlynnDfltAttributeList(list, num);
    break;
  case O_NODEATTRIB:
    ElleNodeAttributeList(list, num);
    *start = 2;
    break;
  case O_UNODEATTRIB:
    ElleUnodeAttributeList(list, num);
    *start = 2;
    break;
  case O_BNDATTRIB:
    ElleFlynnDfltAttributeList(list, num);
...
static void CreateActionArea(parent, actions, num_actions, wdth) Widget parent;
...
Dimension wdth;
{
  ...
  action_area = XtVaCreateWidget("action_area", xmFormWidgetClass, parent,
                                 XmNfractionBase, num_actions * 2 + 1, XmNwidth,
                                 wdth, NULL);

  for (i = 0; i < num_actions; i++) {
    widget = XtVaCreateManagedWidget(
        actions[i].label, xmPushButtonWidgetClass, action_area,
        XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, i * 2 + 1,
        XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM,
        /*
                    XmNrightAttachment,         XmATTACH_POSITION,
                    XmNrightPosition,        i*2 + 2,
        */
        XmNshowAsDefault, i == 0, XmNdefaultButtonShadowThickness, 1, NULL);
    ...
    if (i == 0) {
      ...
      height += 2 * h;
      XtVaSetValues(action_area, XmNdefaultButton, widget, XmNpaneMaximum,
