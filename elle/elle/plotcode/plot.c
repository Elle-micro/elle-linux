...
static XtResource resources[] = {
     ...
     *    },
     */
    {XtNtoggleCallback, XtCToggleCallback, XtRCallback, sizeof(XtPointer),
     offset(plot.callback), XtRCallback, NULL},
    {XtNpixmapWidthInCells, XtCPixmapWidthInCells, XtRDimension,
     sizeof(Dimension), offset(plot.pixmap_width_in_cells), XtRImmediate,
     (XtPointer)DEFAULT_COLUMNS},
    {XtNpixmapHeightInCells, XtCPixmapHeightInCells, XtRDimension,
     sizeof(Dimension), offset(plot.pixmap_height_in_cells), XtRImmediate,
     (XtPointer)DEFAULT_ROWS},
    {XtNcellWidthInPixels, XtCCellWidthInPixels, XtRDimension,
     sizeof(Dimension), offset(plot.cell_width_in_pixels), XtRImmediate,
     (XtPointer)DEFAULT_CELL_WIDTH},
    {XtNcellHeightInPixels, XtCCellHeightInPixels, XtRDimension,
     sizeof(Dimension), offset(plot.cell_height_in_pixels), XtRImmediate,
    ...
    */
    {XtNcurX, XtCCurX, XtRInt, sizeof(int), offset(plot.cur_x), XtRImmediate,
     (XtPointer)0},
    {XtNcurY, XtCCurY, XtRInt, sizeof(int), offset(plot.cur_y), XtRImmediate,
     (XtPointer)0},
    {XtNplotList, XtCPlotList, XtRString, sizeof(String), offset(plot.plots),
     XtRImmediate, (XtPointer)0},
    {XtNlabelList, XtCLabelList, XtRString, sizeof(String), offset(plot.labels),
     XtRImmediate, (XtPointer)0},
    {XtNcellArray, XtCCellArray, XtRString, sizeof(String), offset(plot.cell),
     XtRImmediate, (XtPointer)0},
    {XtNshowEntireBitmap, XtCShowEntireBitmap, XtRBoolean, sizeof(Boolean),
     offset(plot.show_all), XtRImmediate, (XtPointer)TRUE},
};
...
static char defaultTranslations[] = "<Btn1Down>:    OptionalLabel()        \n\
    <Btn2Down>:    UndrawCell()            \n\
...
extern struct {
  float psca_x, psca_y, ixo, iyo, iox, ioy;
} a00000_;
...
static void GetBorderGC(w) Widget w;
{
  ...
  Border_gc = cw->plot.border_gc =
      XCreateGC(XtDisplay(cw), cw->plot.big_picture, mask, &values);
}

static void GetDrawGC(w) Widget w;
{
  PlotWidget cw = (PlotWidget)w;
  XGCValues values;
  ...
  Draw_gc = cw->plot.draw_gc =
      XCreateGC(XtDisplay(cw), cw->plot.big_picture, mask, &values);
}

static void GetUndrawGC(w) Widget w;
{
  PlotWidget cw = (PlotWidget)w;
  XGCValues values;
  XtGCMask mask = GCForeground | GCBackground;
...
/* ARGSUSED */
static void Initialize(treq, tnew, args, num_args) Widget treq, tnew;
ArgList args;
Cardinal *num_args;
{
  char msg[100];
...
void ConfigureCells(PlotWidget new) {
  ...
  if (new->core.width == 0) {
    if (new->plot.show_all == False)
      new->core.width = (new->plot.pixmap_width_in_pixels > pixel_wdth)
                            ? pixel_wdth
  ...
  if (new->core.height == 0) {
    if (new->plot.show_all == False)
      new->core.height = (new->plot.pixmap_height_in_pixels > pixel_hgt)
                             ? pixel_hgt
  ...
  Plot_info.pixels_per_cm =
      (float)HeightOfScreen(scr) / HeightMMOfScreen(scr) * 10;

  dumiro = (int)Settings.orient;
  dumlpl = 11;
  dumils = 2;
  ...
  for (i = 0, indx = 0; i < new->plot.pixmap_height_in_cells; i++) {
    for (j = 0; j < new->plot.pixmap_width_in_cells; j++, indx++) {
      ...
      new->plot.cell[indx].rect.y =
          i *new->plot.cell_height_in_pixels + Plot_info.title_offset;
      new->plot.cell[indx].rect_cm_x =
          new->plot.cell[indx].rect.x / a00000_.psca_x;
  ...
  set_box_dimensions(Pwindo, new->plot.cell[Plot_info.curr_cell].rect_cm_x,
                     new->plot.cell[Plot_info.curr_cell].rect_cm_y -
                         new->plot.cell_height_in_cm,
                     new->plot.cell_width_in_cm, new->plot.cell_height_in_cm);
  CreateBigPixmap(new);
}

void ChangeCellConfig(cw) PlotWidget cw;
{
...
static void Reinitialize(tnew) Widget tnew;
{
  char msg[100];
...
/* ARGSUSED */
static void Redisplay(w, event) Widget w;
XExposeEvent *event;
{
  PlotWidget cw = (PlotWidget)w;
  register int x, y;
  ...
  /*
      XCopyArea(XtDisplay(cw), cw->plot.big_picture,
                  ...
                  width, height, x, y);
  */
  XCopyArea(XtDisplay(cw), cw->plot.big_picture, XtWindow(cw), cw->plot.draw_gc,
...
Cardinal *num_args;
{
  ...
  */
  ...
  if (curcw->plot.cell_height_in_pixels != newcw->plot.cell_height_in_pixels) {
    ChangeCellSize(curcw, newcw->plot.cell_width_in_pixels,
                   newcw->plot.cell_height_in_pixels);
    do_redisplay = True;
  }

  if (curcw->plot.pixmap_width_in_cells != newcw->plot.pixmap_width_in_cells) {
    newcw->plot.pixmap_width_in_cells = curcw->plot.pixmap_width_in_cells;
    XtWarning(
        "BitmapEdit: pixmap_width_in_cells cannot be set by XtSetValues.\n");
  }

  if (curcw->plot.pixmap_height_in_cells !=
      newcw->plot.pixmap_height_in_cells) {
    newcw->plot.pixmap_height_in_cells = curcw->plot.pixmap_height_in_cells;
    XtWarning(
        "BitmapEdit: pixmap_height_in_cells cannot be set by XtSetValues.\n");
  }

  return do_redisplay;
}

void clear_X(w) Widget w;
{ Destroy(w); }

static void Destroy(w) Widget w;
{
  int i, num;
  PlotWidget cw = (PlotWidget)w;
  if (cw->plot.big_picture)
 ...
 */
void ChangeCell(prev, new) int prev, new;
{
  ...
  /*}*/
  /* this assumes only one cell */
  set_box_dimensions(Pwindo, cw->plot.cell[Plot_info.curr_cell].rect_cm_x,
                     cw->plot.cell[Plot_info.curr_cell].rect_cm_y -
                         cw->plot.cell_height_in_cm,
                     cw->plot.cell_width_in_cm, cw->plot.cell_height_in_cm);
  init_box(Pwindo, 0);
}

void set_box(pwin, plot) float *pwin;
PlotWidget plot;
{

  /* this assumes only one cell */
  set_box_dimensions(pwin, plot->plot.cell[Plot_info.curr_cell].rect_cm_x,
                     plot->plot.cell[Plot_info.curr_cell].rect_cm_y -
                         plot->plot.cell_height_in_cm,
                     plot->plot.cell_width_in_cm, plot->plot.cell_height_in_cm);
}

void DrawCellBorder(w, number) Widget w;
int number;
{
  PlotWidget cw = (PlotWidget)w;
  /* update Plot_info */
  Plot_info.curr_x = Plot_info.origin_x = (float)cw->plot.cell[number].rect.x;
  Plot_info.curr_y = Plot_info.origin_y = (float)cw->plot.cell[number].rect.y;

  DrawBorder(cw->plot.draw_gc, DRAW, cw, number);
}

void UndrawCellBorder(w, number) Widget w;
int number;
{
  PlotWidget cw = (PlotWidget)w;
...
void UpdateCell(w, number, mode) Widget w;
int number, mode;
{
...
void DeleteCell(number) int number;
{
...
void clearbg_() {
  ...
  XFillRectangle(XtDisplay(cw), cw->plot.big_picture, cw->plot.undraw_gc, 0,
                 Plot_info.title_offset, cw->plot.pixmap_width_in_pixels + 2,
...
/* BLANK cell*/
static void UndrawCell(w, event) Widget w;
XEvent *event;
{
  PlotWidget cw = (PlotWidget)w;
  XExposeEvent fake_event;
  ...
  if (((XButtonEvent *)event)->y > Plot_info.title_offset) {
    ...
    free(Plot_info.title);
    Plot_info.title = NULL;
    XFillRectangle(XtDisplay(cw), cw->plot.big_picture, cw->plot.undraw_gc, 1,
                   1, (unsigned int)cw->plot.pixmap_width_in_pixels - 1,
...
#if XY
static void ToggleCell(w, event) Widget w;
XEvent *event;
{
  PlotWidget cw = (PlotWidget)w;
...
static void DrawBorder(gc, mode, w, number) GC gc;
int mode;
Widget w;
int number;
{
  ...
  if (mode == UNDRAW) {
    ...
    XDrawRectangle(XtDisplay(cw), XtWindow(cw), cw->plot.border_gc,
                   cw->plot.cell[number].rect.x, cw->plot.cell[number].rect.y,
 ...
 */
drawlabel_(x, y, x_offset, y_offset, mode, label, len) float *x, *y;
...
char *label;
{
  ...
  */
  ...
  if (Plot_info.font != NULL) {
    ...
    XTextExtents(Plot_info.font, label, strlen(label), &direction, &ascent,
                 &descent, &overall);
    width = overall.width;
  }
  state = PEN_UP;
  ...
  xtmp = a00000_.iox;
  ytmp = a00000_.ioy;
  if (*y_offset == J_CENTRE)
    ytmp += (ascent + descent) / 2;
  else if (*y_offset == J_TOP)
    ytmp += ascent + 1;
  else if (*y_offset == J_BASE)
    ytmp -= descent + 1;
  if (*x_offset == J_RIGHT)
    xtmp -= width + 1;
  ...
  XDrawImageString(XtDisplay(cw), cw->plot.big_picture, cw->plot.draw_gc,
                   (int)xtmp, (int)ytmp, label, strlen(label));
 ...
 */
labelcolourbar_(min, max, scale, barlen, xlevel, ylevel, vertical) float *min,
    *max, *scale, *barlen, *xlevel, *ylevel;
int *vertical;
{
  char number[12];
  int current_pen;
  ...
  scale_max = *max * *scale;
  format =
      MAX(fabs((double)scale_min), fabs((double)scale_max)) > 10.0 ? 60 : 62;
...
drawautolabels_(label, labellen, value, format, pwindo,
                current_pen) char *label;
int *format, *labellen;
float *value, *pwindo;
int *current_pen;
{
  ...
  *current_pen = FOREGROUND;
  ...
  y_offset = J_TOP;
  mode = 1;
  drawlabel_(&pwindo[XCMIN], &pwindo[YCMIN], &x_offset, &y_offset, &mode, label,
             labellen);
  if (*format) {
    ...
    */
    ...
    mode = 1;
    len = strlen(number);
    drawlabel_(&pwindo[XCMAX], &pwindo[YCMIN], &x_offset, &y_offset, &mode,
               number, &len);
  }
  *current_pen = old_pen;
  setpencolor_(current_pen);
...
float *x, *y;
{
  ...
  cms_to_pixels(*x, *y, &(fake_event.x), &(fake_event.y), a00000_.psca_x,
                a00000_.psca_y, (int)a00000_.iyo);
  LocateOptionalLabel(plotArea, &fake_event);
  return (0);
}

void DrawTitle(title) char *title;
{
  ...
  XSetFont(XtDisplay(cw), cw->plot.draw_gc, Plot_info.font->fid);
  XTextExtents(Plot_info.font, title, strlen(title), &direction, &ascent,
               &descent, &overall);
  width = overall.width;
  XDrawImageString(XtDisplay(cw), cw->plot.big_picture, cw->plot.draw_gc,
                   Plot_info.title_offset, Plot_info.title_offset * 2 / 3,
                   title, strlen(title));
...
static void OptionalLabel(w, event) Widget w;
XButtonEvent *event;
{
  extern Widget LabelPrompt;

  if (!(LabelPrompt && XtIsManaged(LabelPrompt)))
    return;
}

void LocateOptionalLabel(w, event) Widget w;
XButtonEvent *event;
{
  XGCValues valuesold, valuesnew;
  XtGCMask mask = GCLineStyle | GCForeground;
  ...
  float xcm, ycm;
  PlotWidget cw = (PlotWidget)w;
  int newx = (cw->plot.cur_x + event->x) / cw->plot.cell_width_in_pixels;
  int newy = (cw->plot.cur_y + event->y) / cw->plot.cell_height_in_pixels;
  XExposeEvent fake_event;

  /* this fn should call other fns if allowing draw or undraw */
  int mode = DRAW;

  if (strlen(Plot_info.curr_label) == 0)
    return;
  width = cw->plot.pixmap_width_in_pixels;
  descent = ascent = Plot_info.title_offset;
  if (Plot_info.font != NULL) {
    XSetFont(XtDisplay(cw), cw->plot.draw_gc, Plot_info.font->fid);
    XTextExtents(Plot_info.font, Plot_info.curr_label,
                 strlen(Plot_info.curr_label), &direction, &ascent, &descent,
                 &overall);
    width = overall.width;
  }
  XDrawImageString(XtDisplay(cw), cw->plot.big_picture, cw->plot.draw_gc,
                   event->x, event->y, Plot_info.curr_label,
                   strlen(Plot_info.curr_label));
  /* store the label info */
  XGetFontProperty(Plot_info.font, XA_POINT_SIZE, &size);
  store_label(w, Plot_info.curr_label, Plot_info.fontname, size, event->x,
              event->y, width, ascent + descent, Settings.fg);
 ...
 */
pixels_to_cms(x, y, cm_x, cm_y, cnvrt_x, cnvrt_y, hgt) int x, y, hgt;
 ...
 */
cms_to_pixels(x, y, pix_x, pix_y, cnvrt_x, cnvrt_y, hgt) int hgt, *pix_x,
    *pix_y;
...
CreateBigPixmap(w) Widget w;
{
  PlotWidget cw = (PlotWidget)w;
  extern Drawable DrawArea;

  cw->plot.big_picture = XCreatePixmap(
      XtDisplay(cw), RootWindow(XtDisplay(cw), DefaultScreen(XtDisplay(cw))),
      cw->plot.pixmap_width_in_pixels + 2, cw->plot.pixmap_height_in_pixels + 2,
      XDefaultDepth(XtDisplay(cw), DefaultScreen(XtDisplay(cw))));
  DrawArea = (Drawable)(cw->plot.big_picture);
}

InitBigPixmap(w) Widget w;
{
  PlotWidget cw = (PlotWidget)w;

  XFillRectangle(XtDisplay(cw), cw->plot.big_picture, cw->plot.undraw_gc, 0, 0,
...
static void Resize(w) Widget w;
{
  ...
  if ((cw->core.width < cw->plot.pixmap_width_in_pixels) ||
    ...
    new_cell_height_in_pixels =
        cw->core.height / cw->plot.pixmap_height_in_cells;
    /* if size change mandates a new pixmap, make one */
    ChangeCellSize(cw, new_cell_width_in_pixels, new_cell_width_in_pixels);
  }
  Redisplay(w, NULL);
}

void ChangeCellSize(w, new_cell_width, new_cell_height) Widget w;
int new_cell_width;
int new_cell_height;
{
  PlotWidget cw = (PlotWidget)w;
  ...
  cw->plot.pixmap_width_in_pixels =
      cw->plot.pixmap_width_in_cells * cw->plot.cell_width_in_pixels;

  cw->plot.pixmap_height_in_pixels =
      cw->plot.pixmap_height_in_cells * cw->plot.cell_height_in_pixels;
  ...
  for (x = 0; x < cw->plot.pixmap_width_in_cells; x++) {
    for (y = 0; y < cw->plot.pixmap_height_in_cells; y++) {
      if ((int)cw->plot.cell[x + (y * cw->plot.pixmap_width_in_cells)].drawn ==
          DRAWN)
...
static void DoCell(w, x, y, gc) Widget w;
int x, y;
GC gc;
{
  PlotWidget cw = (PlotWidget)w;
...
void DrawPreviewLine(w, x1, y1, x2, y2) Widget w;
int x1, y1, x2, y2;
{
  extern PlotWidget plotArea;
  extern GC Draw_gc;

  XDrawLine(XtDisplay(w), XtWindow(plotArea), Draw_gc, x1, y1, x2, y2);
}
...
void DrawRectangle(w, xp1, yp1, xp2, yp2) Widget w;
int xp1, yp1, xp2, yp2;
{
  extern GC Draw_gc;
  PlotWidget cw = (PlotWidget)w;

  XDrawRectangle(XtDisplay(cw), cw->plot.big_picture, Draw_gc, xp1, yp1,
                 xp2 - xp1, yp2 - yp1);
}
...
XtWidgetGeometry *proposed, *answer;
{
  ...
  if (cw->plot.show_all == True)
    ...
    answer->width = (cw->plot.pixmap_width_in_pixels > width)
                        ? width
                        : cw->plot.pixmap_width_in_pixels;
  ...
  if (cw->plot.show_all == True)
    ...
    answer->height = (cw->plot.pixmap_height_in_pixels > height)
                         ? height
                         : cw->plot.pixmap_height_in_pixels;
  ...
  else if (answer->width == cw->core.width && answer->height == cw->core.height)
    return XtGeometryNo;
  else
      ...
      if (o_node = (*s_addr)->changed_opts) {
        if (s_addr != &(cw->plot.plots)) { /* append to previous */
          ...
          *o_addr = o_node;
          ...
          do { /* first plot - change initial settings */
            switch (o_node->opt.type) {
            case SYB_INT:
              val = (XtPointer) & (o_node->opt.int_val);
              break;
            case SYB_FLOAT:
              val = (XtPointer) & (o_node->opt.float_val);
              break;
            case SYB_BOOL:
              val = (XtPointer) & (o_node->opt.bool_val);
              break;
            }
            o_tmp = o_node;
            o_node = o_node->next;
