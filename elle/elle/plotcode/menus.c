...
String default_resources[] = {
    /* Plot resources*/
    "*cellWidthInPixels:      500", "*cellHeightInPixels:     200",
    /*
        "*pixmapWidthInCells:       2",
        "*pixmapHeightInCells:      2",
    */
    /* Appearance Resources*/
    "*main_Menu*Foreground:     black", "*main_Menu*Background:     grey75",
    "*plotArea*Foreground:   white", "*plotArea*Background:   black",
    "currvalsArea*Foreground:   black", "currvalsArea*Background:   grey75",
    "*Help*Foreground:     white", "*Help*Background:   grey75", NULL};
...
MenuItemData runopts[] = {
    {"Run", &xmPushButtonGadgetClass, NO_VALUE, 0, RunChosen, (XtPointer)GRAINS,
     NULL, NO_VALUE},
...
MenuItemData statsopts[] = {
    {"Set Options...", &xmPushButtonGadgetClass, NO_VALUE, 0, StatsOptsDialog,
     (XtPointer)&main_Menu, NULL, NO_VALUE},
    NULL,
};
...
MenuItemData fileopts[] = {
    ...
    {"Save...", &xmPushButtonGadgetClass, NO_VALUE, 0, NULL, (XtPointer)SAVE,
     filesaveopts, NO_VALUE},
    {"Sep", &xmSeparatorGadgetClass, NO_VALUE, 0, NULL, (XtPointer)SEP, NULL,
     NO_VALUE},
    {"Exit", &xmPushButtonGadgetClass, NO_VALUE, 0, Exit_Proc, (XtPointer)EXIT,
     NULL, NO_VALUE},
    /*
...
MenuItemData mainopts[] = {
    ...
    {"File", &xmCascadeButtonGadgetClass, NO_VALUE, 0, NULL, (XtPointer)0,
     fileopts, NO_VALUE},
    {"Run", &xmCascadeButtonGadgetClass, NO_VALUE, 0, NULL, (XtPointer)GRAINS,
     runopts, NO_VALUE},
    {"Display", &xmCascadeButtonGadgetClass, NO_VALUE, 0, NULL, (XtPointer)0,
     optionopts, NO_VALUE},
    {"Stats", &xmCascadeButtonGadgetClass, NO_VALUE, 0, NULL, (XtPointer)0,
     statsopts, NO_VALUE},
    NULL,
};

static void Syntax(argc, argv) int argc;
char *argv[];
{
  int i;
 ...
 * Working Messages
 */
WorkingMsg(display) unsigned char display;
{
  char message[80];
...
#endif
  if (!workingdialog) {
    XtSetArg(args[i], XmNmessageString, xmstr);
    i++;
    XtSetArg(args[i], XmNtitle, "STATUS");
    i++;
    ...
    XtUnmanageChild(
        (Widget)XmMessageBoxGetChild(workingdialog, XmDIALOG_OK_BUTTON));
    XtUnmanageChild(
        (Widget)XmMessageBoxGetChild(workingdialog, XmDIALOG_HELP_BUTTON));
    XtUnmanageChild(
        (Widget)XmMessageBoxGetChild(workingdialog, XmDIALOG_CANCEL_BUTTON));
  } else
...
void create_working_message(message) char *message;
{
  switch (Plot_info.plot_type) {
  case XYPLOT:
    strcpy(message, "Plotting ");
...
ErrorMsg(msg, w, err_num) char *msg;
Widget w;
int err_num;
{
  ...
  if (!errdialog) {
    ...
    i++;
    XtSetArg(args[i], XmNtitle, "ERROR MESSAGE");
    i++;
    errdialog = XmCreateErrorDialog(w, "err", args, i);
    XtUnmanageChild(
        (Widget)XmMessageBoxGetChild(errdialog, XmDIALOG_HELP_BUTTON));
    XtUnmanageChild(
        (Widget)XmMessageBoxGetChild(errdialog, XmDIALOG_CANCEL_BUTTON));
  } else
    XtVaSetValues(errdialog, XmNmessageString, xmstr, NULL);
  XmStringFree(xmstr);
...
#if XY
#endif
static void FileOpenSelectionOK(w, client_data, cbs) Widget w;
  ...
  if (filename[strlen(filename) - 1] != E_DIR_SEPARATOR) {
    ...
    case SOLN:
      ...
      if (!err) {
        ...
        ErrorMsg(filename, w, err);
      break;
    default:
      break;
    }
  } else
    ErrorMsg("The file selected is a directory", w, 0);
  XtFree(filename);
} /* FileOpenSelectionOK */
 ...
 */
static void FileSelectionCancel(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
{ XtUnmanageChild(w); } /* FileSelectionCancel */
 ...
 */
void OpenDialog(title, file_type) char *title;
int file_type;
{
  ...
  if (!fileSelection) {
    ...
    XtSetArg(args[i], XmNdialogTitle, tmp1);
    i++;
...
#endif
      XtSetArg(args[i], XmNdirMask, tmp2);
      i++;
    }
    XtSetArg(args[i], XmNuserData, file_type);
    i++;
    fileSelection =
        (Widget)XmCreateFileSelectionDialog(main_Menu,       /* parent widget */
    ...
    XtAddCallback(fileSelection, XmNcancelCallback, FileSelectionCancel, NULL);
 ...
 */
static void FileOpenChosen(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
{
  switch ((int)client_data) {
  case SOLN:
    OpenDialog("Solution files", SOLN);
    break;
  default:
    break;
  }
 ...
 */
static void FileSaveChosen(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
{
 ...
 * maps a file selection box
 */
void SaveDialog(title, file_type) char *title;
int file_type;
{
  ...
  if (!fileSave) {
    ...
    XtUnmanageChild(
        (Widget)XmFileSelectionBoxGetChild(fileSave, XmDIALOG_HELP_BUTTON));
    XtAddCallback(fileSave, XmNcancelCallback, FileSelectionCancel, NULL);
    XtAddCallback(fileSave, XmNokCallback, FileSaveSelectionOK, NULL);
  } else
    XtSetValues(fileSave, args, i);
  XmStringFree(tmp1);
  XmStringFree(tmp2);
...
static void FileSaveSelectionOK(w, client_data, cbs) Widget w;
XtPointer client_data;
XmFileSelectionBoxCallbackStruct *cbs; /*always third param*/
{
...
void warning_msg(err_num, str) int err_num;
char *str;
 ...
 */
void Exit_Proc() { Exit_App(0); }

void Exit_App(err) int err;
{
...
void ExitDialog() {
  ...
  i++;
  XtSetArg(args[i], XmNokLabelString, label1);
  i++;
  XtSetArg(args[i], XmNcancelLabelString, label2);
  i++;
  XtSetArg(args[i], XmNtitle, "WARNING");
  i++;
  warndialog = XmCreateWarningDialog(main_Menu, "warn", args, i);
  XtAddCallback(warndialog, XmNokCallback, Exit_Proc, NULL);
  XtUnmanageChild(
      (Widget)XmMessageBoxGetChild(warndialog, XmDIALOG_HELP_BUTTON));
  XmStringFree(msg);
  XmStringFree(label1);
  XmStringFree(label2);
...
void RunChosen(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
{
  ...
  if (XtIsRealized(main_Menu)) {
    XDefineCursor(XtDisplay(topLevel), XtWindow(XtParent(main_Menu)), Waiting);
    XFlush(XtDisplay(topLevel));
  }
  ...
  if (strlen(ElleFile()) == 0) { /*****hack ************/
    XDefineCursor(XtDisplay(topLevel), XtWindow(XtParent(main_Menu)), None);
    XFlush(XtDisplay(topLevel));
    ErrorMsg("No data file open", main_Menu, 0);
  } else {
    /* WorkingMsg(1); */
...
void RerunChosen(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
{
...
MenuItemData *data;
{
  ...
  i = 0;
  if (spacing > 0) {
    XtSetArg(args[i], XmNmarginWidth, spacing);
    i++;
    XtSetArg(args[i], XmNmarginHeight, spacing);
    i++;
    XtSetArg(args[i], XmNspacing, spacing);
    i++;
  }
  pulldown = XmCreatePulldownMenu(parent, "plldwn", args, i);

  str = XmStringCreateSimple(menu_title);
  cascade = XtVaCreateManagedWidget(menu_title, xmCascadeButtonGadgetClass,
  ...
  for (i = 0; data[i].label != NULL; i++) {
     ...
     */
    if (data[i].submenu) {
      if (data[i].submenu_data != NO_VALUE)
        owner = data[i].submenu_data;
      else
        owner = NO_VALUE;
      widget =
          BuildMenu(pulldown, data[i].label, data[i].spacing, data[i].submenu);
    } else
      widget = XtVaCreateManagedWidget(data[i].label, *data[i].class, pulldown,
                                       NULL);
    if (data[i].bg != NO_VALUE) {
      bg = Colours[data[i].bg].pixel;
      XmGetColors(XtScreen(pulldown), cmap, bg, &fg, &top_shdw, &bottom_shdw,
                  &select_col);
      XtVaSetValues(widget, XmNbackground, bg, XmNborderColor, fg,
                    XmNtopShadowColor, top_shdw, XmNbottomShadowColor,
                    bottom_shdw, XmNarmColor, select_col, NULL);
    }
    /*
     * note - activateCallback
     */
    if (data[i].callback)
      XtAddCallback(widget, XmNactivateCallback, data[i].callback,
                    data[i].callback_data);
    if (owner != NO_VALUE)
      XtVaSetValues(widget, XmNuserData, owner, NULL);
  }
  return cascade;
}
...
Widget top;
{
  ...
  /* create main window */
  mainWindow =
      XtVaCreateManagedWidget("mainWindow", xmMainWindowWidgetClass, top,
                              /*XmNresizable, True,*/
                              NULL);

  main_Menu = XmCreateMenuBar(mainWindow,  /* parent widget*/
                              "main_Menu", /* widget name */
                              NULL, 0);
  XtManageChild(main_Menu);

  for (i = 0; mainopts[i].label != NULL; i++) {
    button = BuildMenu(main_Menu, mainopts[i].label, mainopts[i].spacing,
                       mainopts[i].submenu);
  }
  /*
...
XtVaSetValues( main_Menu,XmNmenuHelpWidget,button,NULL );
  ...
  plotArea =
      XtVaCreateManagedWidget("plotArea", plotWidgetClass, mainWindow, NULL);

  currvalsArea =
      XtVaCreateManagedWidget("current values", xmLabelWidgetClass, mainWindow,
                              XmNalignment, XmALIGNMENT_BEGINNING, NULL);

  XmMainWindowSetAreas(mainWindow, /* widget */
                       main_Menu,  /* menu */
                       NULL,       /* command */
                       NULL,       /* h scroll */
                       NULL,       /* v scroll */
  ...
  XtVaSetValues(mainWindow, XmNmessageWindow, currvalsArea,
                /*XmNresizable, True,*/
                NULL);

  XtVaGetValues(main_Menu, XmNforeground, &fg, XmNbackground, &bg, NULL);

  XtVaSetValues(currvalsArea, XmNforeground, fg, XmNbackground, bg,
                /*XmNresizable, True,*/
                NULL);
...
void ResizePlotWin(int chng_plot) {
  ...
  XtVaGetValues(plotArea, XtNwidth, &wdth, XtNheight, &hgt, NULL);
...
void update_() { UpdateCell(plotArea, Plot_info.curr_cell, DRAW); }

Init_App(argc, argv, optsfp) int argc;
char **argv;
FILE **optsfp;
{
  int err = 0, i;
  ...
  if ((Dsply = XtOpenDisplay(app_context, NULL, NULL, "Xelle", table,
                             XtNumber(table), &argc, argv)) == 0)
...
Set_Colours(red, green, blue, irgbv, steps) int *red, *green, *blue,
    *irgbv, steps;
{
  extern Display *Dsply;

  if ((Plot_info.max_colours =
           CreateColormap(Dsply, XDefaultScreenOfDisplay(Dsply), red, green,
                          blue, irgbv, steps)) == 0)
    return (1);
#if XY
...
Run_App(logfp) FILE *logfp;
{
  ...
  topLevel = XtVaAppCreateShell(NULL, "Xelle", applicationShellWidgetClass,
                                Dsply, XtNcolormap, cmap,
                                /*XmNallowShellResize,True,*/
                                NULL);
  if (Create_widgets(topLevel))
    return (1);
