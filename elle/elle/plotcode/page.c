...
int *red, *green, *blue, *irgbv, steps;
{
  ...
  */
  ...
  createcolormap_(red, green, blue, irgbv, limits, &steps, &start, &ncolors);
  free(limits);
  return (0);
}
...
FILE **optsfp;
{
  ...
  Page.cell_height_in_pts =
      (int)((pt_hgt - y_offset) / (float)Page.height_in_cells);
  Page.cell_width_in_pts =
      (int)((pt_wdth - x_offset * 2) / (float)Page.width_in_cells);
  /*
  ...
  if (Page.cell == NULL)
    if ((Page.cell =
             (cell_data *)calloc(Page.width_in_cells * Page.height_in_cells,
                                 sizeof(cell_data))) == NULL)
      return (MALLOC_ERR);
  ...
  for (i = 0, indx = 0; i < Page.height_in_cells; i++) {
    for (j = 0; j < Page.width_in_cells; j++, indx++) {
      ...
      Page.cell[indx].rect.y =
          i * Page.cell_height_in_pts + Plot_info.title_offset;
  ...
  set_box_dimensions(Pwindo, 0.0, 0.0, Page.cell_width_in_cm,
                     Page.cell_height_in_cm);
  /*
...
void Syntax(argc, argv) int argc;
char **argv;
{
...
int SetStage(int stage) { return (0); }
