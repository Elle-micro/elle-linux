...
static int Rgb0[STEPS * 3] = {32767, 32767, 32767, 36863, 36863, 36863, 40959,
                              40959, 40959, 45055, 45055, 45055, 49151, 49151,
                              49151, 53247, 53247, 53247, 57343, 57343, 57343,
                              61439, 61439, 61439, 65535, 65535, 65535};
static int Rgb1[STEPS * 3] = {0,     0,     65535, 32767, 32767, 65535, 32767,
                              65535, 65535, 0,     65535, 0,     65535, 65535,
                              32767, 65535, 65535, 0,     65535, 32767, 32767,
                              65535, 0,     0,     65535, 32767, 65535};
#endif

struct {
  int lplot, irot, il34;
  float a, b, c, d, asp, thet;
} p00000_;
struct {
  int xorig, yorig;
} p00001_;
 ...
 ***************************************************************/
xpak_init(iro, lpl, ils, dsply) int *iro, *lpl, *ils;
Display *dsply;
{
  /*Dsply = dsply;*/
  p00000_.lplot = *lpl;
...
CreateColormap(dsply, scrn, red, green, blue, irgbv, steps) Display *dsply;
Screen *scrn;
...
int steps;
{
  ...
  for (i = 0; i < USEFL_SIZE; i++) {
    ...
    if ((result = XAllocColor(dsply, cmap, &Colours[i])) == 0) {
      fprintf(stderr, "Could not match or allocate color %d\n", i);
      fprintf(stderr, "If displaying on a PC screen, the display should be set "
                      "to 256 colours\n");
      return (0);
    }
  }
  ...
  for (k = 0; k < intervals; k++) {
    ...
    colb = irgbv[icolb];
    for (i = nc0; i <= nc1;
         i++, colr += r_inc, colg += g_inc, colb += b_inc, col_end++) {
  ...
  */
  ...
  if (result == 0) {
     ...
     */
    while (ncolors &&
           !XAllocColorCells(dsply, cmap, False, NULL, 0, pixels, ncolors))
      ncolors--;
    if (ncolors < PLOT_COLOURS) {
      ...
      if (ncolors < (2 * intervals + USEFL_SIZE)) {
        ...
        fprintf(stderr, "If displaying on a PC screen, the display should be "
                        "set to 256 colours\n");
        return (0);
      }
    }
...

{
  ...
  if (*ivel != 0) {
    if (*ivel > 0)
      ...
      linewidth = (int)((0 - *ivel) * (a00000_.psca_y / 10 + 0.5));

    linestyle = LineSolid;
    if (Draw_gc)
      XSetLineAttributes(Dsply, Draw_gc, linewidth, linestyle, CapButt,
                         JoinBevel);
  }
}
...
setlinewidth_(wdth) float *wdth;
{
  ...
  if (*wdth != 0) {
    if (*wdth > 0)
      ...
      linewidth = (int)((0.0 - *wdth) * (a00000_.psca_y / 10) + 0.5);
    if (linewidth < 1)
      linewidth = 1;
  } else
    linewidth = 1;
...
origin_(xmin, px1, ymin, py1) float *xmin, *ymin;
float *px1, *py1;
...
scale_(xmin, xmax, px1, px2, ymin, ymax, py1, py2) float *xmin, *xmax, *ymin,
    *ymax;
...
int *i;
{
  ...
  */
  ...
  if (ABS(*i) == 2) {
    XDrawLine(Dsply, DrawArea, Draw_gc, (int)a00000_.iox, (int)a00000_.ioy,
              (int)ixv, (int)iyv);
  }
...

{
  ...
  */
  ...
  XDrawImageString(mydisplay, mywindow, mygc, (int)ixv, (int)iyv, iword,
                   *nchar);
 ...
 */
drawrectangleu_(x1, y1, x2, y2) float *x1, *y1, *x2, *y2;
{
  int state = 3;
 ...
 */
fillpoly_(xf, yf, nf, colour, outline) float *xf, *yf;
  ...
  */
  ...
  if (*outline < 2) {
    ...
    for (i = 0; i < *nf; i++) {
      ...
      iyv = (int)((a00000_.iyo - yp * a00000_.psca_y) - p00001_.yorig + 0.5);
      points[i].x = ixv;
      points[i].y = iyv;
    }

    XFillPolygon(Dsply, DrawArea, Draw_gc, points, *nf, Complex,
                 CoordModeOrigin);
    free(points);
  }
...
int *dash, *pattern;
{
  ...
  */
  ...
  if (dashstyle != NULL && Draw_gc)
    XSetDashes(Dsply, Draw_gc, 0, dashstyle, len);
  if (Draw_gc)
 ...
 *****************************************************************/
Translate_u_to_win(xu, yu, xwin, ywin) float xu, yu;
int *xwin, *ywin;
{
  float tmpx, tmpy;
...
result =
    XAllocColorCells(mydisplay, cmapnew, False, NULL, 0, pixels, MAX_COLOURS);
if (result == 0)
  printf("2 Error in allocating the color cells...\n");
...
for (k = 0; k < *nstep; k++) {
  ...
  if (ncr1 > 0) {
    ...
    if (ncr1 > 2) {
      for (i = 1; i <= ncr1; i++) {
        ip = i + nc0;
        color[ip].red = (rgbv[icolr + 3] * i + rgbv[icolr] * (ncr1 - i)) / ncr1;
        color[ip].green =
            (rgbv[icolg + 3] * i + rgbv[icolg] * (ncr1 - i)) / ncr1;
        color[ip].blue =
            (rgbv[icolb + 3] * i + rgbv[icolb] * (ncr1 - i)) / ncr1;
        color[ip].flags = DoRed | DoGreen | DoBlue;
        color[ip].pixel = pixels[ip];
      }
    }
...
float *val, *start, *interval;
{
  ...
  if (*justify == J_CENTRE)
    ...
    offset = 0;
  if (*justify == J_TOP || *justify == J_RIGHT)
    *size = -(*size);
  if (*horizontal) {
...
int *colour, *outline;
{
  ...
  */
  ...
  if (*outline < 2) {
    ...
    XFillArc(Dsply, DrawArea, Draw_gc, ixv, iyv, idiam, idiam, 0, 64 * 360);
  }
...
formatnumber_(str, num, format) char *str;
float *num;
int *format;
{
  char form[7];
