...
void InitSettings() { InitOptions(&Settings); }
...
void SetOptionValue(struct plot_params *opts, int id, XtPointer val) {
  ...
  case O_REGION:
    ...
    break;
  case O_OVERLAY:
    opts->overlay = *((int *)val);
    break;
  case O_DRWUNODETYP:
    opts->draw_nodes = *((int *)val);
    break;
  case O_NODEATTRIB:
    opts->node_attrib = *((int *)val);
    ElleFindNodeAttributeRange(opts->node_attrib, &amin, &amax);
    opts->node_limits[0] = amin;
    opts->node_limits[1] = amax;
    break;
  case O_NODEATTRMIN:
    opts->node_limits[0] = *((double *)val);
    break;
  case O_NODEATTRMAX:
    opts->node_limits[1] = *((double *)val);
    break;
  case O_UNODEATTRIB:
    opts->unode_attrib = *((int *)val);
    ElleFindUnodeAttributeRange(opts->unode_attrib, &amin, &amax);
    opts->unode_limits[0] = amin;
    opts->unode_limits[1] = amax;
    break;
  case O_UNODEATTRMIN:
    opts->unode_limits[0] = *((double *)val);
    break;
  case O_UNODEATTRMAX:
    opts->unode_limits[1] = *((double *)val);
    break;
  case O_LBLREGION:
    opts->label_region = *((int *)val);
    break;
  case O_FLYNNATTRIB:
    opts->flynn_attrib = *((int *)val);
    ElleFindFlynnAttributeRange(opts->flynn_attrib, &amin, &amax);
    opts->flynn_limits[0] = amin;
    opts->flynn_limits[1] = amax;
    break;
  case O_FLYNNATTRMIN:
    opts->flynn_limits[0] = *((double *)val);
    break;
  case O_FLYNNATTRMAX:
    opts->flynn_limits[1] = *((double *)val);
    break;
  case O_BNDATTRIB:
    opts->boundary_attrib = *((int *)val);
    if (opts->boundary_attrib == MISORIENTATION)
      ElleFindBndAttributeRange(opts->boundary_attrib, &amin, &amax);
    opts->bnd_limits[0] = amin;
    opts->bnd_limits[1] = amax;
    break;
  case O_BNDATTRMIN:
    opts->bnd_limits[0] = *((double *)val);
    break;
  case O_BNDATTRMAX:
    opts->bnd_limits[1] = *((double *)val);
    break;
  case O_BNDLEVEL:
    opts->boundary_level = *((int *)val);
    break;
  case O_UPDFREQ:
    opts->update_freq = *((int *)val);
    break;
  default:
    break;
  }
...
void GetOptionValue(struct plot_params *opts, int id, XtPointer val) {
  ...
  case O_REGION:
    *((int *)val) = opts->region;
    break;
  case O_OVERLAY:
    *((int *)val) = opts->overlay;
    break;
  case O_DRWUNODETYP:
    *((int *)val) = opts->draw_nodes;
    break;
  case O_NODEATTRIB:
    *((int *)val) = opts->node_attrib;
    break;
  case O_NODEATTRMIN:
    *((double *)val) = opts->node_limits[0];
    break;
  case O_NODEATTRMAX:
    *((double *)val) = opts->node_limits[1];
    break;
  case O_UNODEATTRIB:
    *((int *)val) = opts->unode_attrib;
    break;
  case O_UNODEATTRMIN:
    *((double *)val) = opts->unode_limits[0];
    break;
  case O_UNODEATTRMAX:
    *((double *)val) = opts->unode_limits[1];
    break;
  case O_LBLREGION:
    *((int *)val) = opts->label_region;
    break;
  case O_BNDATTRIB:
    *((int *)val) = opts->boundary_attrib;
    break;
  case O_BNDATTRMIN:
    *((double *)val) = opts->bnd_limits[0];
    break;
  case O_BNDATTRMAX:
    *((double *)val) = opts->bnd_limits[1];
    break;
  case O_BNDLEVEL:
    *((int *)val) = opts->boundary_level;
    break;
  case O_FLYNNATTRIB:
    *((int *)val) = opts->flynn_attrib;
    break;
  case O_FLYNNATTRMIN:
    *((double *)val) = opts->flynn_limits[0];
    break;
  case O_FLYNNATTRMAX:
    *((double *)val) = opts->flynn_limits[1];
    break;
  case O_UPDFREQ:
    *((int *)val) = opts->update_freq;
    break;
  default:
    break;
  }
...
void GetCForegroundOption(int *val) { *val = Settings.fg; }

void GetColourmapOption(int *val) { *val = Settings.colourmap; }
...
void SetColourmapOption(int val) { Settings.colourmap = val; }

void SetMaxColoursOption(int val) { Settings.max_colours = val; }
...
int ValidColmapOption(char *str) { return (name_match(str, colmap_terms)); }
        ...
        case O_FLYNNATTRIB:
          ...
          if (i != NONE) {
            ...
            if ((err = ReadMinMax(str, &min, &max)) == 0) {
              SetOptionValue(plot, O_FLYNNATTRMIN, (XtPointer)(&min));
              SetOptionValue(plot, O_FLYNNATTRMAX, (XtPointer)(&max));
            } else {
              dump_comments(fp);
              err = 0;
            }
        ...
        case O_NODEATTRIB:
          if (fscanf(fp, "%s", str) != 1)
            ...
            if (i != NONE && i != NUM_NB && i != TRPL_ONLY) {
              ...
              ELLE_UNUSED(scan_ret);
              if ((err = ReadMinMax(str, &min, &max)) == 0) {
                SetOptionValue(plot, O_NODEATTRMIN, (XtPointer)(&min));
                SetOptionValue(plot, O_NODEATTRMAX, (XtPointer)(&max));
              } else {
                dump_comments(fp);
                err = 0;
              }
        ...
        case O_UNODEATTRIB:
          if (fscanf(fp, "%s", str) != 1)
            ...
            if (i != NONE && i != U_LOCATION) {
              ...
              ELLE_UNUSED(scan_ret);
              if ((err = ReadMinMax(str, &min, &max)) == 0) {
                SetOptionValue(plot, O_UNODEATTRMIN, (XtPointer)(&min));
                SetOptionValue(plot, O_UNODEATTRMAX, (XtPointer)(&max));
              } else {
                dump_comments(fp);
                err = 0;
              }
        ...
        case O_BNDATTRIB:
          if (fscanf(fp, "%s", str) != 1)
            ...
            if (i != NONE && i != STANDARD) {
              ...
              ELLE_UNUSED(scan_ret);
              if ((err = ReadMinMax(str, &min, &max)) == 0) {
                SetOptionValue(plot, O_BNDATTRMIN, (XtPointer)(&min));
                SetOptionValue(plot, O_BNDATTRMAX, (XtPointer)(&max));
              } else {
                dump_comments(fp);
                err = 0;
              }
          ...
          break;
        default:
          warning_msg(VAR_ERR, str);
          scan_ret = fscanf(fp, "%[^A-Za-z0-9]", str);
          ELLE_UNUSED(scan_ret);
          scan_ret = fscanf(fp, "%[^ \n\t]", str);
          ELLE_UNUSED(scan_ret);
          break;
    ...
    if ((str[0] = getc(fp)) == '#')
      ...
      if (indx == -1) {
        ...
        case O_FLYNNATTRIB:
          ...
          if (i != NONE) {
            if ((err = ReadMinMax(fp, str, &min, &max)) == 0) {
              SetOptionValue(&Settings.plot_opts, O_FLYNNATTRMIN,
                             (XtPointer)(&min));
              SetOptionValue(&Settings.plot_opts, O_FLYNNATTRMAX,
                             (XtPointer)(&max));
            } else
              dump_comments(fp);
          }
        ...
        case O_NODEATTRIB:
          ...
          if (i != NONE) {
            if ((err = ReadMinMax(fp, str, &min, &max)) == 0) {
              SetOptionValue(&Settings.plot_opts, O_NODEATTRMIN,
                             (XtPointer)(&min));
              SetOptionValue(&Settings.plot_opts, O_NODEATTRMAX,
                             (XtPointer)(&max));
            } else
              dump_comments(fp);
          }
        ...
        case O_BNDATTRIB:
          ...
          if (i != NONE && i != STANDARD) {
            if ((err = ReadMinMax(fp, str, &min, &max)) == 0) {
              SetOptionValue(&Settings.plot_opts, O_BNDATTRMIN,
                             (XtPointer)(&min));
              SetOptionValue(&Settings.plot_opts, O_BNDATTRMAX,
                             (XtPointer)(&max));
            } else
              dump_comments(fp);
          }
...
int WriteSettings(FILE *fp) {
  ...
  while (display_terms[i].name != 0 && !err) {
    ...
    case O_PAPERSZ:
      ...
      break;
    case O_FONTHGT:
      fprintf(fp, "%d", opts->dfltfonthgt);
      break;
    case O_PG_XMARG:
      fprintf(fp, "%.3f", opts->page_xmargin);
      break;
    case O_PG_YMARG:
      fprintf(fp, "%.3f", opts->page_ymargin);
      break;
    case O_ZOOM:
      fprintf(fp, "%f", opts->zoom);
      break;
    case O_LINEWDTH:
      fprintf(fp, "%.3f", opts->linewidth);
      break;
    case O_PGSCL:
      fprintf(fp, "%.3f", opts->linewidth);
      break;
    case O_FOREGRND:
      fprintf(fp, "%d", opts->fg);
      break;
    case O_LINESTYLE:
    ...
    case O_COLMAP:
      ...
      break;
    case O_RESCALE:
      fprintf(fp, "%d", opts->rescale);
      break;
    case O_VERBOSE:
      fprintf(fp, "%d", opts->verbose);
      break;
    case O_TEXTBG:
      fprintf(fp, "%d", opts->text_bg);
      break;
    }
    if (fprintf(fp, "\n") < 0)
      err = WRITE_ERR;
