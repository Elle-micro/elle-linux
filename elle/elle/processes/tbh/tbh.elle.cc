  ...
  for (i = 0; i < EllemaxStages(); i++) {
    if (!ElleUnodesActive()) {
      ...
      for (j = 0; j < max; j++) {
    	                   ...
    	                   ElleFlynnHasAttribute(j,E_XX)*/)
        {
          ...
          if (mintype == QUARTZ || mintype == FELDSPAR) {
            ElleGetFlynnEuler3(j, &curra, &currb,
                               &currc); // retrieve 3 Euler angles

            curra = curra * PI / 180.0;
            currb = currb * PI / 180.0;
            currc = currc * PI / 180.0;

            euler(rmap, curra, currb,
                  currc); // calculate equivalent rotation tensor
            ...
            if (TBH_ONLY) {
              ...
              ElleGetFlynnRealAttribute(
                  j, &(etensor[0]), E_XX); // retrieve averaged strain for Flynn
            ...
            if (mintype == QUARTZ)
              tbh_(etensor, ermap, &ework, &err, fncrss[0],
                   fnxlinfo); // calculate quartz lattice rotations
            else
              tbh_(etensor, ermap, &ework, &err, fncrss[1],
                   fnxlinfo); // calculate fsp lattice rotations
            ...
            ElleSetFlynnEuler3(j, curra, currb,
                               currc); // store new Euler angles
          }
        }
      }
    } else {
      ...
      for (j = 0; j < max; j++) {
        ...
        err = 1;
        ElleGetUnodeAttribute(j, &curra, &currb, &currc,
                              EULER_3); // retrieve 3 Euler angles

        curra = curra * PI / 180.0;
        currb = currb * PI / 180.0;
        currc = currc * PI / 180.0;

        euler(rmap, curra, currb, currc); // calculate equivalent rotation
                                          // tensor
        ...
        if (TBH_ONLY) {
          ...
          ElleGetUnodeAttribute(j, &(etensor[0]),
                                E_XX); // retrieve averaged strain for Flynn
        ...
        ElleSetUnodeAttribute(j, curra, currb, currc,
                              EULER_3); // store new Euler angles
      }
    }
    ElleUpdate();
  }
