...
double dislocationdensityscaling =
    10e13; // in m-2    (ie 1.5 in elle file =1e13 in real worlds=
  ...
  for (i = 0; i < EllemaxStages(); i++) {
    ...
    for (j = 0; j < max; j++) {
      ...
      if (ElleFlynnIsActive(j) && !isParent(j)) {
        ...
        if (QUARTZ == mintype) {
          critical_energy = (GetMineralAttribute(mintype, CRITICAL_RX_ENERGY) *
                             (1000 / (temp + 1000)) * fluid_factor);
          energyofdislocations = GetMineralAttribute(mintype, DD_ENERGY);
        }
        ...
        energy = dislocden * energyofdislocations * dislocationdensityscaling;
        ...
        if (test > critical_energy) {
          ...
          if (testE == 1) {
            ...
            ElleSetFlynnEuler3(j, olda, oldb, oldc);
        }
      }
    }
