...
void Fluid_Lattice::TestPhreeqc(double timestep, double space) {
  ...
  for (PhreeqC::FLUID ::iterator it = iphreeqc_obj.Conc_inf.begin();
    ...
    if (it->first != "pH") {
      ...
      if (type == "temp") {
        SolveAdvectionLax_Wenddroff_Temperature(timestep * 10000, space, 2);
        SolveTemperatureDiffusionImplicit(constant, timestep, space);
      } else {
        SolveAdvectionLax_Wenddroff(timestep * 10000, space, 2);
        SolveDiffusionImplicit(constant, timestep, space);
      }
...
#pragma omp for collapse(2)
    for (int i = 0; i < dimX; i++) {
      for (int j = 0; j < dimY; j++) {
        ...
        react[i][j] = si_calcite;
        Si[i][j] = si_calcite; // fill the matrix
        ++(*show_progress);
      }
    }
  }
