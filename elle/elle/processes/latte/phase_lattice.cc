  ...
  do {
            ...
            if (!runParticle->neigP[j]->isHole) {
              if (runParticle->is_boundary && use_grains != 0) {
                if (runParticle->neigP[j]->grain != runParticle->grain ||
                  ...
                  // alen = runParticle->radius + runParticle->neigP[j]->radius;
                  alen = sqrt(
                      runParticle->neigpos[i][0] * runParticle->neigpos[i][0] +
                      runParticle->neigpos[i][1] * runParticle->neigpos[i][1]);
              ...
              while (neig) {
                ...
                if (!neig->done) // if not already done
                {
                  if (neig->xpos) // just in case
                  {
                    ...
                    rep_constant = (runParticle->E + neig->E) / 2.0;
    ...
    for (i = 0; i < numParticles; i++) {
      if (runParticle->isHoleBoundary) {
        ...
        lame1 = runParticle->E * runParticle->poisson_ratio /
                (1.0 - (2.0 * runParticle->poisson_ratio) *
                           (1.0 + runParticle->poisson_ratio));
        lame2 = runParticle->E / (2.0 + 2.0 * runParticle->poisson_ratio);

        //-----------------------------------------------------------------------
        // getting strain tensor
        //-----------------------------------------------------------------------

        uxy = ((1 + runParticle->poisson_ratio) / runParticle->young) *
              runParticle->sxy;
        uxx = (1 / runParticle->E) *
              (runParticle->sxx -
               (runParticle->syy * runParticle->poisson_ratio));
        uyy = (1 / runParticle->E) *
              (runParticle->syy -
               (runParticle->sxx * runParticle->poisson_ratio));
        ...
        //---------------------------------------------------------
        // scaling energy  pressure_scale * pascal_scale *
        //---------------------------------------------------------
    ...
    for (i = 0; i < numParticles; i++) {
      if (runParticle->isHoleBoundary) {
        ...
        } else // use the repulsion box concentration
        {
          ...
          {
            rate = rate_constant *
                   (1 - ((runParticle->conc / (5416)) /
                         exp((sum_Energy) / (gas_const * (temp + 273)))));
            if (rate < 0.0) {
              rate =
                  -rate_constant *
                  (1 - ((5416 * exp(sum_Energy / (gas_const * (temp + 273)))) /
                        runParticle->conc));
            }
          }
    ...
    if (preRunParticle->prob > 1.0) // dissolution
    {
...

      {
        ...
        } else                        // use box
        {
          ...
          concBox[preRunParticle->box_pos] =
              1 / preRunParticle->mV; // set concentration of that box
        }
