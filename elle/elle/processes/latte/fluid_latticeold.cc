  ...
  for (k = 0; k < iteration; k++) {
    for (i = 0; i < dim; i++) // x loop
    {
      for (j = 1; j < dim - 1; j++) // y loop
      {
        ...
        {
          if (boundary == 1) {
            C_for_x = 0.5 * (conc[i][j] + conc[i + 1][j]) -
                      0.5 * vel_darcyx[i][j] * dt / dx *
                          (conc[i + 1][j] - conc[i][j]);
            C_back_x = 0.5 * (conc[i][j] + conc[dim - 1][j]) +
                       0.5 * vel_darcyx[i][j] * dt / dx *
                           (conc[dim - 1][j] - conc[i][j]);

            // step 2 x
            dxc[i][j] =
                conc[i][j] - vel_darcyx[i][j] * dt / dx * (C_for_x - C_back_x);
          }
          if (boundary == 2) //
            dxc[i][j] = conc[i][j];
        } else if (i == dim - 1) // wrapping right boundary
        {
          if (boundary == 1) {
            C_for_x =
                0.5 * (conc[i][j] + conc[0][j]) -
                0.5 * vel_darcyx[i][j] * dt / dx * (conc[0][j] - conc[i][j]);
            C_back_x = 0.5 * (conc[i][j] + conc[i - 1][j]) +
                       0.5 * vel_darcyx[i][j] * dt / dx *
                           (conc[i - 1][j] - conc[i][j]);
          ...
          // step 1 x
          C_for_x =
              0.5 * (conc[i][j] + conc[i + 1][j]) -
              0.5 * vel_darcyx[i][j] * dt / dx * (conc[i + 1][j] - conc[i][j]);
          C_back_x =
              0.5 * (conc[i][j] + conc[i - 1][j]) +
              0.5 * vel_darcyx[i][j] * dt / dx * (conc[i - 1][j] - conc[i][j]);
        ...
        // step 1 y
        C_for_y =
            0.5 * (conc[i][j] + conc[i][j + 1]) -
            0.5 * vel_darcyy[i][j] * dt / dy * (conc[i][j + 1] - conc[i][j]);
        C_back_y =
            0.5 * (conc[i][j] + conc[i][j - 1]) +
            0.5 * vel_darcyy[i][j] * dt / dy * (conc[i][j - 1] - conc[i][j]);
        ...
        // step 2 y
        dyc[i][j] =
            conc[i][j] - vel_darcyy[i][j] * dt / dy * (C_for_y - C_back_y);
      }
    }
    for (i = 0; i < dim; i++) // x loop
    {
...
void Fluid_Lattice::multiplication_inv_Phreeqc() {
  ...
  for (i = 0; i < dim; i++) {
    for (j = 0; j < dim; j++) {
      ...
      for (k = 0; k < dim; k++)
        conc[i][j] += a_invl[j][i][k] * ddyc[k][j];
    }
  }
}
...
void Fluid_Lattice::TestPhreeqc(double timestep, double space) {
  ...
  for (PhreeqC::FLUID ::iterator it = iphreeqc_obj.Conc_inf.begin();
    ...
    if (it->first != "ph") {
      ...
      SolveAdvectionLax_Wenddroff_Phreeqc(timestep, space, 1);

      // then diffusion

      SolveDiffusionImplicit_Phreeqc(constant, timestep, space);
