...
void Lattice::UpdateElle() {
  ...
  for (i = 0; i < numParticles; i++) // and loop again
  {
    if (runParticle->p_Unode) {
      ...
      runParticle->p_Unode->setAttribute(U_ATTRIB_A, runParticle->metal_conc);

      // plot the fracture time
      runParticle->p_Unode->setAttribute(U_ATTRIB_B, runParticle->IronOxide);

      runParticle->p_Unode->setAttribute(U_ATTRIB_C, runParticle->Jarosite);

      runParticle->p_Unode->setAttribute(U_DISLOCDEN,
      ...
      runParticle->p_Unode->setAttribute(U_VISCOSITY, runParticle->accSolIndex);

      // runParticle->p_Unode->setAttribute(U_DENSITY, runParticle->metal_conc);
      ...
      runParticle->p_Unode->setAttribute(U_DENSITY, runParticle->fluid_P);
...
void Lattice::DeformLatticeExtension(float move, int plot, float dx,
  ...
  runParticle = lastParticle; // go back one
...
/**************************************************************************************
 * function that dumps the solution index along a horizontal profile
 *
 **************************************************************************************/

void Lattice::DumpSolIndex(double yrow) {
  FILE *stat;               // file pointer
  int i, j, jj, p_counter;  // counters
  float mean, differential; // mean stress, differential
  // stress, pressure
  ...
  stat = fopen("SolIndex.txt", "a"); // open statistic output
  // append file
                                     ...
                                     // box
  {
    ...
    if (runParticle->nb < (yrow * 200 + 200)) {
      if (runParticle->nb > (yrow * 200)) {
        // and dump the data
        mean = runParticle->calcite;
        if (mean < 0.0)
          mean = 0.0;

        fprintf(stat, " %f", mean); // one row
        // of y
        // coordinates
      }
    }
  ...
  fprintf(stat, " %f\n", runParticle->metal_conc);

  fclose(stat); // close file
}

/**************************************************************************************
 * function that dumps the solution index along a vertical profile
 *
 **************************************************************************************/

void Lattice::DumpSolIndexvertical(double xrow) {
  FILE *stat;               // file pointer
  int i, j, jj, p_counter;  // counters
  float mean, differential; // mean stress, differential
  // stress, pressure
  ...
  stat = fopen("SolIndexV.txt", "a"); // open statistic output
  // append file
                                     ...
                                     // box
  {
    ...
    if (runParticle->xpos > xrow) {
      if (runParticle->xpos < xrow + 2 * runParticle->radius) {
...
void Lattice::RelaxSheet(float t_step) {
  ...
  for (i = 0; i < numParticles; i++) // loop
  {
    ...
    ddx = dx - dx * exp(-runParticle->sheet_young * t_step /
                        (runParticle->sheet_visc / runParticle->SheetT));
...
void Lattice::Gravity(float depth, float density) {
  ...
  for (j = 0; j < numParticles; j++) {
    runParticle->fg =
        ...
        9.81 / (runParticle->real_young / 10); // pascal_scale*pressure_scale
    runParticle->fg = runParticle->fg * runParticle->radius * 2.0;
    if (runParticle->is_top_lattice_boundary)
  ...
  move = 9.81 * (runParticle->real_radius * 2.0 * particlex) *
         runParticle->real_density / (runParticle->real_young / 10);
    ...
    if (runParticle->conc > 0.0) {
      if (runParticle->metal_conc > 0.0) {
        if ((runParticle->conc * runParticle->metal_conc) >
            runParticle->calcite) {
          change = (runParticle->conc * runParticle->metal_conc) -
                   runParticle
          ...
          if (change > 0.0) {
            ...
            react =
                -1 * reaction_time * runParticle->mV * rate_constant *
                (1 - ((runParticle->conc * runParticle->metal_conc) / 0.01));
...
void Lattice::Calculate_Fluid_Pressure(double upper_pressure,
                                       ...
                                       double additional) {
  {
    ...
    for (k = 0; k < iter; k++) {
      ...
      Fluid->test_pressure_with_source(1); // adding the source term
...
double Lattice::Calculate_Fluid_Pressure_timeadjust(double upper_pressure,
  ...
  Fluid->matrices_set_1();
  Fluid->multiplication();
  Fluid->test_pressure_with_source(4); // adding the source term
  Fluid->transpose_trans_P();
  Fluid->invert();
  Fluid->multiplication_inv();
  ...
  Fluid->Get_Fluid_Velocity(nonlinear_time);
...
void Lattice::Fluid_Insert_Random_Node(double ymin, double ymax,
  ...
  // Fluid->Fluid_Input(5000000, 20, 20);
}
...
void Lattice::Calculate_Advection(double time, double space, int boundary,
                                  int method) {
  if (method == 1)
    Fluid->SolveAdvectionLax_Wenddroff(time, space / particlex, boundary, 0.5);
  else if (method == 2)
...
//---------------------------------------------------------------------------------------
// Code written by uli to incorporate phreeqc
// Gary 2020
//---------------------------------------------------------------------------------------

void Lattice::SetHydroChem(const char *pore_fluid, const char *inf_fluid,
                           const char *d_base) {
  Fluid->SetFluidPhreeqc(pore_fluid, inf_fluid, d_base);
}

void Lattice::SetResultMaps(std::string parameters) {
  cout << "Building result vectors for particles containing:" << endl;
  char delimiter = ',';
  std::vector<std::string> splits;
  std::istringstream ss(parameters);
  std::string parameter;
  while (std::getline(ss, parameter, delimiter)) {
    splits.push_back(parameter);
    cout << " " << parameter << endl;
  }

  std::vector<std::pair<string, double>> parameter_map;
  for (int I = 0; I < splits.size(); I++) {
    parameter_map.push_back(make_pair(splits[I], 0));
    Fluid->outputs.push_back(splits[I]);
  }

  for (i = 0; i < numParticles; i++) {
    runParticle->results_map = parameter_map;
    runParticle = runParticle->nextP;
  }
}

void Lattice::TransportPhreeqc(double timestep, double space, int adv) {
  cout << Fluid->outputs[0] << endl;
  Fluid->Transport(timestep, space, adv);
}

void Lattice::GetOutputPhreeqc() {
  Fluid->GetOutput(Fluid->outputs);
  Fluid->Pass_Back_Gradients(repBox, 0);
}

void Lattice::TestPhreeqc(double time, double space) {
  Fluid->TestPhreeqc(time, space, 1);
}

// void Lattice::TestPhreeqc(double time, double space, int adv)
//{
// Fluid->TestPhreeqc(time, space, adv);
//}

void Lattice::TestInitPhreeqc() { Fluid->InitConcPhreeqc(); }

void Lattice::AccumulateSolIndex() {
  double index;

  for (i = 0; i < numParticles; i++) {
    if (runParticle->calcite > 0.0)
      index = runParticle->calcite;
    else
      index = 0.0;

    runParticle->accSolIndex += index;

    runParticle = runParticle->prevP;
  }
  for (i = 0; i < numParticles; i++) {
    if (runParticle->Hamatite > 0.0)
      index = runParticle->Hamatite;
    else
      index = 0.0;

    runParticle->accSolIndex2 += index;
...
void Lattice::Initialize_Heat(double xpos, double xpos2) {
  Fluid->SetHeatSheet(xpos, xpos2);

  Fluid->Pass_Back_SheetT(repBox);
}

void Lattice::InitHeat(int T) {
  int i, j, count;
...
void Lattice::Adjust_break() {
  ...
  for (i = 0; i < numParticles; i++) {
    for (j = 0; j < 8; j++) {
      if (runParticle->neigP[j])
        runParticle->break_Str[j] =
            runParticle->break_Str[j] / (runParticle->SheetT / 200);
    }
    runParticle = runParticle->prevP;
  }
}
      ...
      if (pos > 0) {
        if (pos == boxpos) {
          ...
          while (neig) // as long as there is somebody
          {
            ...
            dd = sqrt(((neig->xpos - dx) * (neig->xpos - dx)) +
                      ((neig->ypos - dy) * (neig->ypos - dy)));
                  ...
                  if (dd > runParticle->radius * 1.7) {
                    ...
                    // if (ran_nb < prob) // now we are actually healing
                    {
                      if (connected_neighbors < 8) {
                        ...
                        // good, average_springs = 0.05; //from 0.01 to 0.02,
                        // 0.02 is good, average_break_Str = 0.005;
                        //}
  ...
  for (x = 0; x < particlex - 10; x++) {
    for (y = 0; y < particlex - 1; y++) {
      ...
      if (repBox[xx])
        ...
        // do check if its really a hole and not just extended
        if (repBox[xx + 1]) {
          if (repBox[xx + 1]->draw_break)
            broken1 = true;
        } else
          broken1 = true;
        if (repBox[xx - 1]) {
  ...
  for (i = 0; i < numParticles; i++) {
    ...
    for (kk = 0; kk < 8; kk++) {
      if (runParticle->neigP[kk]) {
        ...
        if (runParticle->SheetT > 250) {
          runParticle->break_Str[kk] =
              runParticle->break_Str[kk] *
              (1 - (runParticle->Tdif / ref)); // tdif/ref*ref
          runParticle->springf[kk] = runParticle->springf[kk] *
                                     (1 - (runParticle->Tdif / ref)); // 25*ref
          runParticle->springs[kk] =
              runParticle->springs[kk] *
              (1 - (runParticle->Tdif /
                    ref)); // Breaking Strength, goes up when cooling

          if (runParticle->break_Str[kk] < 0.00001) {
            runParticle->break_Str[kk] = 0.00001;
            cout << "oh oh " << endl;
          }

          if (runParticle->springf[kk] < 0.00001)
            runParticle->springf[kk] = 0.00001;
          if (runParticle->springs[kk] < 0.00001)
            runParticle->springs[kk] = 0.00001;
        } else {
          runParticle->break_Str[kk] = average_break; // tdif/ref*ref
          runParticle->springf[kk] = runParticle->springf[kk] *
                                     (1 - (runParticle->Tdif / ref)); // 25*ref
          runParticle->springs[kk] =
              runParticle->springs[kk] * (1 - (runParticle->Tdif / ref));
        }
