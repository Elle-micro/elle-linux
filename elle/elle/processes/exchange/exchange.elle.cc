...
void SetVoronoi(int tmp, int unode1, int unode2, int index2, int unode3,
                int index3);
...
double GrtBtCalcMinEnergy(double init_Bt_conc, double init_Grt_conc, double T,
                          double grt_mole, double Bt_mole, double *sgc,
                          double *sbc); // mwj*** 23/06/2003
...
int InitSetUnodes() {
  ...
  if (strlen(infile) > 0) {
     ...
     */
    if (ElleNodeAttributeActive(CONC_A) && ElleUnodeAttributeActive(CONC_A) &&
...
void SetMargins() {
  ...
  for (i = 0; i < max_flynns; i++) // cycle through stages
  {
    if (ElleFlynnIsActive(i)) {
      ...
      for (j = 0; j < count; j++) {
        val =
            ElleNodeMarginConcEstimate(bdnodelist[j], CONC_A, &unodelist, roi);
        ElleSetNodeMarginAttribute(bdnodelist[j], val, CONC_A, i);
      }
      numunodes = unodelist.size();
      for (j = 0; j < count; j++)
        bdnodelist.pop_back();
      for (j = 0; j < numunodes; j++)
        unodelist.pop_back();
    }
  }
}
...
int CalcConc() {
  ...
  ElleSetTemperature(ElleTemperature() +
                     CoolingRate / 2.0); // this is for cooling rate
  ...
  for (i = 0; i < max_flynns; i++) // cycle through flynns
  {
    ...
    if (ElleFlynnIsActive(i) && ReactionIsActive(i)) {
      ...
      ElleGetFlynnIntAttribute(
          i, &mintype, MINERAL); // mwj*** 20/06/2003
      ...
      if (EllemaxStages() == 1) {
        ...
        if ((Minlen * (Minlen / 2) / kappa_m) < 1.0)
          fprintf(stderr,
                  "Warning:Kappa, node separation or stages need adjustment. "
                  "Minlen: %le Kappa: %le\n",
                  Minlen, Kappa);
      }
...
void SetupConc(int flynnid, int attr_id, double *minlen, int *max_nbs) {
  ...
  *minlen = 1.0;
  ...
  ElleGetFlynnUnodeList(flynnid,
                        unodelist); // get the list of unodes for a flynn
  count = unodelist.size();

  for (i = 0; i < count; i++) {
    std::vector<int> nbnodes1;
    std::vector<int> node_index1;
    ElleGetTriPtNeighbours(unodelist[i], nbnodes1, node_index1,
                           1); // get the  list of neighbours for a unode
    ...
    while (j < tmp) {
      ...
      if (j < ii && nbnodes[j] != NO_NB) {
        ...
        if (j + 1 == ii)
          ...
          SetVoronoi(i * tmp + k, unodelist[i], nbnodes[j], node_index[j],
                     nbnodes[0], node_index[0]);
        else

          SetVoronoi(i * tmp + k, unodelist[i], nbnodes[j], node_index[j],
                     nbnodes[j + 1], node_index[j + 1]);
...
void SetupBdConc(int flynnid, int attr_id, int *bd_max_nbs) {
  ...
  for (i = 0; i < count; i++) {
    std::vector<int> nbnodes1;
    std::vector<int> node_index1;
    ElleGetNodeTriPtNeighbours(bdnodelist[i], nbnodes1, node_index1,
                               bdnodelist[i + 1], 1);
    if (num_nbs > tmp) {
      tmp = num_nbs;
    }
    ...
    while (j < tmp - 1) {
      ...
      if (j < ii && nbnodes[j] != NO_NB) {
        ...
        if (j + 1 < ii) {
          ...
          SetBdVoronoi(i * tmp + k, bdnodelist[i], nbnodes[j], node_index[j],
                       nbnodes[j + 1], node_index[j + 1]);
        }

        if (node_index[j] == 1)
          BdSegLength[i * tmp + k] =
              ElleNodeSeparation(bdnodelist[i], nbnodes[j]);
        else
          BdSegLength[i * tmp + k] = CalcUnodeSep(nbnodes[j], bdnodelist[i], 1);
...
void CalcConcAtUnode(int flynnid, double dt, int num_nbs) {
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    NewConc[n] = OldConc[n] * (1 - double_kappa * dt * (tmp3 / area)) +
                 (double_kappa * dt * tmp1 / area);
  }
}
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    for (i = 0; i < num_nbs - 1; i++) {
      ...
      if (j != NO_NB) {
        ...
        tmp1 +=
            xx * BdSegWidth[ii * num_nbs + i] / BdSegLength[ii * num_nbs + i];
    ...
    BdNewConc[n] = BdOldConc[n] * (1 - double_kappa * dt * (tmp3 / area)) +
                   (double_kappa * dt * tmp1 / area);
  }
}
...
void UpdateBdConc(int flynnid) {
  ...
  for (j = 0; j < count; j++) {
    ElleSetNodeMarginAttribute(bdnodelist[j], BdNewConc[bdnodelist[j]], CONC_A,
                               flynnid);
  }
}
...
void SetBdVoronoi(int tmp, int node1, int node2, int index2, int node3,
                  int index3) {
  Coords xy1, xy2, xy3;
...
void ExchangeReaction() {
  ...
  double l_val, b_val, min_energy, bt_min_energy, grt_min_energy, grt_mole,
      bt_mole;
  double init_Bt_conc, init_Grt_conc, T, T_K, k;
  std::vector<int> bdnodelist;
  int mintype;                                // mwj*** 20/06/2003
  int randflynns[10000], targetflynns[10000]; // arrays for randomised flynn
                                              // numbers mwj*** 21/06/2003
  static int first_ran =
      0; // flag to check for first time through function mwj*** 21/06/2003
  int test_ran, no_active_flynns = 0; // test of random flynn, number of
                                      // reactive flynns  mwj*** 21/06/2003
  ...
  for (i = 0; i < max_flynns; i++) {
    if (ElleFlynnIsActive(i) &&
        ReactionIsActive(
            i)) //  need to store flynn in random list mwj*** 21/06/2003
    {
      targetflynns[i] = -1;
      no_active_flynns++;
    } else
  ...
  for (i = 0; i < no_active_flynns;
       i++) //  create random flynn id list mwj*** 21/06/2003
  {
    do {
  ...
  init_Bt_conc = CalcInitMinConc(
      MICA); // calculate the average conc. of Bt margin mwj*** 21/06/2003
  init_Grt_conc = CalcInitMinConc(
      GARNET); // calculate the average conc. of Grt margin mwj*** 21/06/2003
  ...
  GrtBtCalcMinEnergy(init_Bt_conc, init_Grt_conc, T_K, grt_mole, bt_mole,
                     &grt_min_energy, &bt_min_energy); // mwj*** 21/06/2003
...
double GrtBtCalcMinEnergy(double init_Bt_conc, double init_Grt_conc, double T,
                          double grt_mole, double Bt_mole, double *sgc,
                          double *sbc) {
  double ln_KD, KD, grt_change, bt_change, Bt_mf, Grt_mf, term_1, term_2,
      term_3, a, b;
  ...
  a = (init_Grt_conc - (init_Grt_conc - grt_change)) /
      (init_Bt_conc - (init_Bt_conc - bt_change));
  ...
  stable_Bt_conc1 = (-term_2 + sqrt(term_2 * term_2 - 4.0 * term_1 * term_3)) /
                    (2.0 * term_1);
  stable_Bt_conc2 = (-term_2 - sqrt(term_2 * term_2 - 4.0 * term_1 * term_3)) /
                    (2.0 * term_1);
