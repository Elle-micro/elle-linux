...
int InitVM() {
  ...
  if (reset) {
    if ((err = ElleResetNodeConc((int)udata[NodeId], (int)udata[ResetNum],
      ...
      cout << "Resetting " << (int)udata[ResetNum] * 2 + 1 << " nodes to "
           << udata[AttribValue] << endl;
    else
...
int Calc_Conc() {
  ...
  {
    for (fid2 = 0; fid2 < maxf;
         fid2++) // loop through all flynns and rest prox map to zero
    {
      if (ElleFlynnIsActive(fid2)) {
        ElleFlynnNodes(fid2, &nodeids, &numnodes);
        for (nid = 0; nid < numnodes;
             nid++) // reset all boundary concentrations to 0
        {
    ...
    if (ElleFlynnIsActive(fid)) {
      ...
      if (mintype == 1) // only for garnets
      {
        ...
        for (nid = 0; nid < numnodes;
             nid++) // reset all garnet boundary concentrations to 1
        {
          if (ElleNodeIsActive(nodeids[nid]))
        ...
        ElleWriteData(fname);
        for (fid2 = 0; fid2 < max;
             fid2++) // loop through all nodes and add to total proximity map
                     // (assumes starting file had zero N_ATTRIB_A)
        {
    ...
    for (j = 0; j < max; j++) {
      if (ElleNodeIsActive(j)) {
        ...
        if (xy.x > 0.25 && xy.x < 0.75)
          ...
          ElleSetNodeAttribute(j, 0.8, Attrib_id);
    ...
    if (ElleNodeIsActive(j)) {
      ...
      while (i < 3) {
        ...
        if (nbnodes[i] != NO_NB) {
          ...
          SegLength[j * 3 + k] = ElleNodeSeparation(j, nbnodes[i]);
...
void writeprofile(int cnt) {
  ...
  //    static int ordering[160]={159, 10, 145, 110, 50, 19, 83, 73, 133, 130,
  //    137, 32, 67, 30, 122, 84, 125, 44, 153, 147, 151, 69, 64, 108, 141, 89,
  //    101, 107, 29, 98, 91, 88, 155, 115, 11, 23, 68, 74, 77, 128, 3, 13, 127,
  //    102, 61, 75, 15, 94, 1, 131, 48, 16, 80, 58, 95, 7, 148, 99, 56, 24, 20,
  //    117, 111, 54, 121, 146, 152, 76, 139, 21, 34, 5, 138, 150, 60, 38, 86,
  //    2, 18, 28, 12, 103, 65, 157, 90, 79, 114, 33, 43, 124, 100, 140, 46, 87,
  //    17, 126, 35, 112, 27, 62, 119, 40, 134, 116, 135, 97, 9, 22, 66, 70,
  //    106, 4, 25, 6, 55, 118, 143, 142, 37, 113, 81, 72, 14, 158, 132, 31, 36,
  //    96, 42, 78, 45, 104, 0, 51, 129, 71, 8, 59, 149, 47, 109, 123, 53, 154,
  //    120, 156, 82, 57, 136, 52, 63, 26, 144, 93, 85, 41, 39, 92, 105, 49};
  //    static int ordering[160]={18, 2, 17, 5, 76, 51, 75, 27, 52, 41, 68, 13,
  //    67, 40, 54, 29, 77, 53, 78, 6, 16, 1, 87, 0};
  static int ordering[160] = {
      81,  72,  14,  158, 132, 31,  36,  96,  42,  78,  45,  104, 0,   51,  129,
      71,  8,   59,  149, 47,  109, 123, 53,  154, 120, 156, 82,  57,  136, 52,
      63,  26,  144, 93,  85,  41,  39,  92,  105, 49,  159, 10,  145, 110, 50,
      19,  83,  73,  133, 130, 137, 32,  67,  30,  122, 84,  125, 44,  153, 147,
      151, 69,  64,  108, 141, 89,  101, 107, 29,  98,  91,  88,  155, 115, 11,
      23,  68,  74,  77,  128, 3,   13,  127, 102, 61,  75,  15,  94,  1,   131,
      48,  16,  80,  58,  95,  7,   148, 99,  56,  24,  20,  117, 111, 54,  121,
      146, 152, 76,  139, 21,  34,  5,   138, 150, 60,  38,  86,  2,   18,  28,
      12,  103, 65,  157, 90,  79,  114, 33,  43,  124, 100, 140, 46,  87,  17,
      126, 35,  112, 27,  62,  119, 40,  134, 116, 135, 97,  9,   22,  66,  70,
      106, 4,   25,  6,   55,  118, 143, 142, 37,  113};
  UserData udata;
