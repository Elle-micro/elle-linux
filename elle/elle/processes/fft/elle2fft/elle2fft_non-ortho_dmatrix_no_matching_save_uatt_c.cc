...
int FindUnode(Coords* ptxy, Coords* origin, double dx, double dy, int numperrow);
// int FindUnode(Coords *ptxy, int start);
int FindUnode_alb(Coords* ptxy, int start);
void check_error();
// double **dmatrix_alb(long nrl,long nrh,long ncl,long nch); using dmatrix in
// mat.h
...
main(int argc, char** argv) {
  ...
  if ((err = ParseOptions(argc, argv))) // modernize: intentional assignment in
                                        // condition (behavior-neutral)
    OnError("", err);
...
int Elle2PPC() {
  ...
  SQ_pattern = (int)userdata[SQ_patterns]; // a value of 0 force irregular pattern of
                                           // unodes, to avoid error with FindSQ to
                                           // models with low resolution
  rad_euler = (int)userdata[Rad_euler];    // a value of 0 euler angles in input
                                           // elle file are in radians
  ...
  if (ElleSSOffset() != 0.0)
    Log(0, "Export to a non orthogonal grid ");

  for (n = 0, r = 0; r < rows; r++) { // cycle through fft points
    fft_xy.x = 0.0;

    if (ElleSSOffset() != 0.0)
      fft_xy.x = fft_xy.x + ElleSSOffset() * fft_xy.y;

    for (c = 0; c < cols; c++) {
      // printf("r and c: %d\t%d\n",r,c);

      if (!sq_grid)
        i = FindUnode_alb(&fft_xy, 0);
      else
        i = FindUnode(&fft_xy, &origin, elle_dx, elle_dy, elle_cols);
      ...
      if (i != NO_VAL) {
        ...
        ElleGetFlynnRealAttribute(jgrain, &jphase,
                                  DISLOCDEN); // Flynn dislocation density is used as phase indicative
        pts_i[n] = (int)jphase;
  ...
  for (i = 0, j = 0; i < max && outf; i++) {
    if (ElleFlynnIsActive(i)) {
      ...
      outf << val[0] << '\t' << val[1] << '\t' << val[2] << '\t' << val[0] << '\t' << '0' << '\t' << i << endl;
      flynn_ids[j] = i;
      j++;
    }
  }
  ...
  for (r = 1; r <= rows; r++) { // cycle through fft points
    i = (r - 1) * cols;
    for (c = 1; c <= cols; c++, i++) {
      outf << pts[i][0] << '\t' << pts[i][1] << '\t' << pts[i][2] << '\t' << c << '\t' << r << '\t' << '1' << '\t';
      j = 0;
