...
void SetBnodeStrain(vector<int>& attriblist) {
  ...
  max_unodes = ElleMaxUnodes();                     // maximum unode number used
  roi = sqrt(1.0 / (double)max_unodes / 3.142) * 3; // VIP, ROI controls number of unodes used to interpolate the local
                                                    // displacement field
  ...
  for (i = 0; i < ElleMaxFlynns(); i++) {
    if (ElleFlynnIsActive(i)) {
      ...
      for (j = 0; j < count; j++) {
        ...
        // change the tracer parameter .. F_ATTRIB_B & U_VISCOSITY ?¿ constant
        // not interval, double convert to int the tracer layer in gbm is
        // U_ATTRIB_C but in elle2fft & fft2elle is U_ATTRIB_A !! caution unify
        // attrib in the code!! modified trace in next piece of program
...
int SetUnodeAttributesFromFile2(char* fname) {
  ...
  */
  ...
  while (datafile) {
    ...
    datafile >> val[0] >> val[1] >> val[2] >> val[3] >> val[4] >> val[5] >> val[6] >> val[7] >> val[8] >> val[9] >>
        val[10] >> val[11];
    // iwork=val[4]*val[5];
