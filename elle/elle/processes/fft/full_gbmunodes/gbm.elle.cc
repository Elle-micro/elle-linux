...
double afactor = 0; // 0 non redistribution of DD inside flynn <default>, 1
                    // full-automatic redistribution
int TNcheckopt = 0; // ==1 non check TN

// Update information
/*	10/12/2008 Modified to add a variable to select flynns or unodes
information to calculate the misorientation and anisotropy of boundary energy

/* 	GBM driven by surface energy and strain stored energy. The volumetric
energy is calculated using data from flynn or unodes layer
* 	The process verify the condition of grain boundary with thickness = 0;
* 	if GB_thickness != 0 then need to modify ebergy surface of both
interfaces *	The nodes are moved along the direction of maximum reduction of
energy similar than described in Becker et al. (2008).
*
* 	Volumetric energy is calculated using clipping function, gradient of
dislocation density is interpolated.
* 	DataUser parameters:
* 	1) The local gradient of energy is calculated using 4-dummy nodes or
8-dummy nodes
...
General equation is E=en(l1+l2+l3)*lengthscale with E=energy, en=surface energy
and l1/l2/l3 the length of the segments next to the node adjusted to the
lengthscale */
...
double GetNodeEnergy(int node, Coords *xy) {
  ...
  // and put them into variables. In case of a double node, one is NO_NB and we
  ...
  node2 = nbnode[n];
  n = 0;
  // see if the neighbouring nodes are active. Do we need that? Don't think
  // so...
  ...
  // if the node is a triple, we have to get the third node and the length of
  // the segment
  ...
  // Each node can belong to more than one flynn, each flynn can have a
  // different SURFACE_ENERGY We add them all up
...
ElleRelPosition(xy,nbnode[n],&relxy,&len);
        ...
        energyofsurface *= GetOrientationEnergy(rgn1,rgn2,&relxy,len,id);
         // printf("UNODES rgn[n],nbrgn,energy  %i %i
...
double density_unodes(int node, Coords *xy) {
  ...
  // Brute routine with large list of the unodes of the flynns that belongs the
  // bnode of reference Area of search is limited by ROI npts=sqrt(max_unodes);
  ...
  roi = sqrt(1.0 / (double)max_unodes / 3.142) *
        3; // aprox. limit at 2nd neighbours
  ...
  if (jj == 0) {
    jj = 0;
    e[0] = dist_total = 0.0;
    // roi = sqrt(1.0/(double)max_unodes/3.142)*4;	// aprox. limit at 2nd
    // neighbours
  ...
  /*
            unodelist.clear();
                    ElleRegions(node,rgn); //find the neighbours flynns of the
     bnode
            ...
            for(j=0;j<3;j++) {
              if (rgn[j] != NO_NB) {
                ...
                for(k=0;k<unodelist.size();k++) {
                  ...
                  ElleGetUnodePosition(unodelist[k],&xy_unode); // xy es del
     node !! ElleCoordsPlotXY (&xy_unode, xy); dist =
     pointSeparation(xy,&xy_unode);
...
double GetOrientationEnergy(int rgn1, int rgn2, Coords *nbxy, double len,
  ...
  // For columnar models, sufficient; General three-dimensional or high symmetry
  // material use full-misorientation
...
int GGMoveNode_all(int node, Coords *xy) {
  ...
  // 04/12/2008
  // Default version with 4-pivot points, but added the possibility to use
  // diagonal positions to increase stability of triple nodes 4-pivot points +
  // small time steps seems that TNs are normally stable but some drift if
  // strain stored energy >>> boundary energy
...
int write_data(int stage) {
  ...
  //~ //write out number of grains and numbers of sides if stage is multiple of
  //1000 ~ //F_ATTRIB_A has area at last test ~ if(fmod(stage,500)==0) ~ { ~
  //f2=fopen("grains-sides.csv","a"); ~ f1=fopen("growth-sides.csv","a"); ~
  //for(c=0;c<ElleMaxFlynns();c++) ~ { ~ sides=0; ~ if(ElleFlynnIsActive(c)) ~ {
  //~ numberflynns++;
...
/* !/brief Runs through all the nodes (in random order) and moves them also
writes out some statistics, turn this on or off using STATS*/
int GBMGrowth() {
  ...
  */
  ...
  for (i = 0; i < EllemaxStages(); i++) {
    ...
    max = ElleMaxNodes();
    // to prevent moving a single node always at the same time, we shuffel them
...
int update_dislocden() {
    ...
    if (ElleFlynnIsActive(j)) {
      ...
      if (valold != j) {
        ...
        for (i = 0; i < count; i++) {
          ...
          flynnold = int(old_flynn);
          if (flynnold == valold)
            ElleSetUnodeAttribute(i, U_ATTRIB_C, double(j));
        }
        unodelist.clear();
  ...
  for (i = 0; i < max_unodes; i++) {
    ...
    if (flynnold != flynnid) {
      ...
      // Update euler angles. Nearest unode, use quaternion for interpolate the
      // Euler orientation
  ...
  double avdensity; // a_factor=0.0 redistribution factor; 1.0 full while 0.0
                    // non redistribution
