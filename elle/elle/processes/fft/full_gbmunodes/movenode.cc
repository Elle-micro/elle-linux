/*Modified 11/12/2008 to add boundary mobility as function of misorientation
 * angle (AGA)
 */
...
int GetNewPos(int node, double e1, double e2, double e3, double e4,
              Coords *newpos, double trialdist) {
  Coords npos, pvec;
  int t;
...
int MoveDNode(int node, Coords pvec, Coords *movedir) {
  ...
  double density[2], len_body, bodyenergy, diff_density[4], len_sup1, len_sup2,
      len_sup3;
  ...
  id = 0; // if id != 1 working with flynn, id ==1 working with unodes.. define
          // as a user value
  ...
  if (ElleNodeIsActive(node)) {
    ...
    // Get Boundary mobility
    mob1 = mob2 = GetBoundaryMobility(
        node, node1); /// 2; //in Elle, boundaries belong to two flynns,
    ...
    if (ElleFlynnAttributeActive(EULER_3) || ElleFlynnAttributeActive(CAXIS)) {
      ...
      while (n < 2) {
        ...
        if (id != 1) { // the flynn version ..
          ...
          mob[l] = GetBoundaryMobility(node, nnbnode[n]) *
                   Get2BoundaryMobility(misorientation);
          l++;
        }

        else { // this is the unodes version ..
          // First version, take information of the nearest unode in both
          // regions
    ...
    if (cosalpha1 < 0.01745)
      cosalpha1 = 0.01745;
    if (cosalpha2 < 0.01745)
      cosalpha2 = 0.01745;
    ...
    if (flength * ElleTimestep() * ElleSpeedup() > ElleSwitchdistance()) {
      // sprintf( logbuf, "Distance too big to move double, angles:
      // %lf,%lf,%lf,Distance: %lf, 2*Switchdistance: %lf\n", acos( cosalpha1 )
      // * RTOD, acos( cosalpha2 ) * RTOD, acos( DEGCos( vec1, vec2 ) ) * RTOD,
      // flength * ElleTimestep(), 2 * ElleSwitchdistance() ); Log( 2, logbuf );
...
int MoveTNode(int node, Coords pvec, Coords *movedir) {
  double l1, l2, l3, len, cosalpha1, cosalpha2, cosalpha3, mob1, mob2, mob3,
      flength;
  ...
  if (ElleNodeIsActive(node)) {
    ...
    if (ElleFlynnAttributeActive(EULER_3) || ElleFlynnAttributeActive(CAXIS)) {
      ...
      while (n < 3) {
        ...
        if (id != 1) { // this is the flynn version ..
          misorientation = GetMisorientation(rgn[0], rgn[1], id);
          mob[n] = GetBoundaryMobility(node, nbnode[n]) *
                   Get2BoundaryMobility(misorientation);
        }
    ...
    if (cosalpha2 < 0.01745)
      cosalpha2 = 0.01745;
    if (cosalpha3 < 0.01745)
      cosalpha3 = 0.01745;

    aa = (((l1 * pow(cosalpha1, 2)) / mob1) +
          ((l2 * pow(cosalpha2, 2)) / mob2) + (l3 * pow(cosalpha3, 2) / mob3));
    ...
    dt = ElleTimestep() * ElleSpeedup();
    if (flength * ElleTimestep() * ElleSpeedup() > ElleSwitchdistance()) {
      // sprintf( logbuf, "Distance too big to move triple! angles: %lf,%lf,%lf
      // Distance: %lf, 2*Switchdistance: %lf\n", acos( cosalpha1 ) * RTOD,
      // acos( cosalpha2 ) * RTOD, acos( cosalpha3 ) * RTOD, flength *
      // ElleTimestep(), 2 * ElleSwitchdistance() );
      // Log( 2, logbuf );
      // sprintf( logbuf, "reduced movement of node %d! ", node );
      // Log( 0, logbuf );
      sprintf(logbuf,
...
double GetBoundaryMobility(int node, int nb) {
  double mobility = 0, mobil, arg2, arg1, arg, T, R = 8.314472, Q, H = 145,
...
//we need to know which flynn it is and get the boundary mobility for the
segments. ElleNeighbourRegion( node, nb, &rgn[0] ); ElleNeighbourRegion( nb,
node, &rgn[1] );
...
if (mob2<mobility) mobility=mob2;
  ...
  */
  ...
  // Caution !! Data of Mo from grain boundary Motion of pure ice bicrystals!!
  // with first and second thermal groove!!
  // The intrinsic mobility change 2 order of magnitude (0.023 to 2.3 m4 J-1 s-1
  // !). Using first, perhaps if driving force is high change to second ?¿
...
int GetBodyNodeEnergy2(int n, double *total_energy) {
  ...
  ElleNodeSameMineral(n, &same,
                      &themin); // only allow GBM between same mineral type
  /*!
  ...
  for (i = 0; i < 2; i++) {
    if (rgn[i] != NO_NB) {
      ...
      // energy[i] =
      // energyofdislocations*area[i]*density[i]*dislocationdensityscaling;
...
double GetMineralAttribute_ice(int attribute) {
  ...
  case GB_MOBILITY_ice:
    val = 0.023; // m4/s·J or m2/Kg·s Nasello et al. 2005 Intrinsic Mobility Mo;
                 // 1st stage of groove movement-low velocity
    break;

  case GB_MOBILITY_2_ice:
    val = 2.3; // m4/s·J or m2/Kg·s Nasello et al. 2005 Intrinsic Mobility Mo;
               // 2nd stage of groove movement- fast
    break;

  case DD_ENERGY_ice:
    val = 3.5e-10; // Core energy dislocations Gb**2/2, 3500 (MPa) *4.52e-10
                   // (m)**2 /2; Jm-1
    break;
...
double GetMisorientation(int rgn1, int rgn2, int id) {
  ...
  dotprod =
      (xyz[0].x * xyz[1].x) + (xyz[0].y * xyz[1].y) + (xyz[0].z * xyz[1].z);
...
int SearchUnode(int rgn, Coords *xy) {
  ...
  roi = sqrt(1.0 / (double)max_unodes / 3.142) *
        5; // aprox. limit at 3nd neighbours
