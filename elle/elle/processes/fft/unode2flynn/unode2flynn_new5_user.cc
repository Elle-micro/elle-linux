...
bool ElleCoordRelativePosition(Coords* current, Coords* bnd, int num_bnd_nodes, int* xflags, int* yflags,
                               Coords* rel_pos);
int FindBnode(std::vector<int> nodes, Coords* xy);
int splitFlynn(int flynnindex, int* child1, int* child2, int first, int last, vector<PointData>& boundary);
...
int Unode2Flynn() {
  ...
  *	1) Critical threshold of dislocation density. Only if dislocation
  density of unode >> d_crit. Last parameter is a user data. The scalar critical
  dislocation density can be regarded as an average threshold of low-high angle
  boundary. *	2) Location of critical unode with regard to the boundary nodes.
  Only allowed unodes near to grain boundaries!!
  * 	3) Critical area of prexistent flynn to split. If flynn area <<
  area_crit, grain split is aborted
  *   4) Unstable process. Need a computer with large free RAM memory, if not
  nucleation fails because bas memory assignation. Also, unknown overflow
  errors. *	5) Assume a new grain free of dislocations. The new grain's
  orientation is the same than the critical unode.
  *   6) Assumption of grain size of nucleated grain. By default take 1st
  neighbours unodes, but 2nd or 3rd neighbours unodes can be selected. At this
  moment unrelated to local stress, work, stress strain rate, etc. A
  single-unode is assumed as unstable grain and shrink.
  *

  CAUTION! A tricky part is an adequate ratio between switchdistance of bnodes
  and unodes.
  */
    ...
    if (d_den > d_crit && area_k > area_crit) {
      ...
      if (unode_id != NO_VAL && UnodesActive() && unode_id < ElleMaxUnodes()) {
        ...
        for (lit = nbflynns.begin(); lit != nbflynns.end(); lit++) {
          ...
          for (vit = tmplist.begin(); vit != tmplist.end() && !err; vit++) {
      ...
      if (ElleVoronoiActive() && !err) {
        ...
        copy(unodenbs.begin(), unodenbs.end(), inserter(total_unodenbs, total_unodenbs.end()));
        ...
        if (n_neighbours > 1) {
          ...
          for (i = 0; i < unodenbs.size(); i++) {
            ...
            copy(tmpnbs.begin(), tmpnbs.end(), inserter(total_unodenbs, total_unodenbs.end()));
          }
        }
        // 3rd neighbours
        // The closest nbs of the unode's closest nbs
        ...
        if (n_neighbours > 2) {
          for (i = 0; i < tmpnbs.size(); i++) {
            ...
            copy(tmpnbs2.begin(), tmpnbs2.end(), inserter(total_unodenbs, total_unodenbs.end()));
          }
        ...
        if (n_neighbours > 3) {
          for (i = 0; i < tmpnbs2.size(); i++) {
            ...
            copy(tmpnbs3.begin(), tmpnbs3.end(), inserter(total_unodenbs, total_unodenbs.end()));
          }
        ...
        for (i = 0; i < nodelist.size(); i++) {
          ...
          if (ElleFindVoronoiPt(xy, &j) == false)
            j = -1;
        ...
        std::cout << std::endl;
        for (it2 = total_list.begin(); it2 != total_list.end(); it2++) {
          if (std::count(total_list.begin(), total_list.end(), *it2) == 1)
            std::cout << *it2 << ' ';
        }
        std::cout << std::endl;
#endif
        ...
        while (i < nodelist.size() && first == NO_NB) {
          ...
          if (ElleFindVoronoiPt(xy, &curr) == true) {
            if (find(total_list.begin(), total_list.end(), curr) != total_list.end()) {
              first = curr;
              startindex = i;
            }
          }
        ...
        while (index < numunodes && !found) {
          if ((it = find(vpt_array[index].begin(), vpt_array[index].end(), next)) == vpt_array[index].end())
            index++;
          else
            found = true;
        }
        ...
        if (error_alb == 0) {
          ...
          while (*it != first) {
            if (std::count(total_list.begin(), total_list.end(), *it) == 1) {
              V_data->vpoints[*it].getPosition(&xy);
              bndnode.setvalues(xy.x, xy.y, i++, 1);
              boundary.push_back(bndnode);
              it++;
              ...
              if (!found)
                OnError("Did not find next node", 0);
              it++;
              if (it == vpt_array[index].end())
                it = vpt_array[index].begin();
            }
          ...
          err = splitFlynn(k, &child1, &child2, nodelist[endindex], nodelist[startindex], boundary);
          ...
          // Add a check area to prevent reset of old grains, with changes of
          // flynn number smallest flynn is always the new flynn RX
...
int splitFlynn(int flynnindex, int* child1, int* child2, int elle_first, int elle_last, vector<PointData>& boundary) {
...
int FindBnode(std::vector<int> nodes, Coords* xy) {
  ...
  for (it = nodes.begin(); it != nodes.end() && !matched; it++) {
    if (ElleNodeIsActive(*it))
      ElleNodePlotXY(*it, &nodexy, xy);
    else
      OnError("Bnode no. invalid", RANGE_ERR);
