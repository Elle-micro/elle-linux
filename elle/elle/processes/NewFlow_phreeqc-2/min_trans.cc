...
// ---------------------------------------------------------------
Min_Trans_Lattice::Min_Trans_Lattice() { transition = true; }
...
void Min_Trans_Lattice::Change_Particle() {
...
void Min_Trans_Lattice::Reaction_Two() {
  ...
  float merker1, // multipurpose
      merker2,   // dito
      numerator, denominator, molareVolume,
  ...
  double rate_constant, rate, rate2, exponent, N, k;

  double delta_Ev_pos, delta_Ev_neg, delta_Ed_pos, delta_Ed_neg, delta_surf_pos,
      delta_surf_neg, deltaG_pos, deltaG_neg;
  ...
  for (i = 0; i < numParticles; i++) {
    if (runParticle->phase_change_required) {
      ...
      for (j = 0; j < 6; j++) {
        deltaG_pos = (runParticle->dir_rate[j]) *
                     (runParticle->mV - runParticle->previous_mV) *
                     mole_per_particle;
        deltaG_neg = 0.0;
        ...
        if (runParticle->dir_rate[j] != 0.0) {
          runParticle->dir_rate[j] =
              162000.0 * (runParticle->temperature + 273.0) *
              (exp(-(Activation_Energy() / N +
                     (deltaG_neg + delta_Ed_neg) / (N * mole_per_particle)) /
                   (k * (runParticle->temperature + 273.0))) -
               exp(-(Activation_Energy() / N -
                     (deltaG_pos + delta_Ed_pos) / (N * mole_per_particle)) /
                   (k * (runParticle->temperature + 273.0))));
        }
      }
    }
...
void Min_Trans_Lattice::Energy_Calculation(Particle *partikel) {
  ...
  // scaling:
  partikel->eEl *=
      partikel->mV * mole_per_particle * pressure_scale * pascal_scale;

  bulk = partikel->young / (3 * (1 - 2 * pois));
  // alt:
  // eEl_distortion = lame2 * ((uxx-uyy)*(uxx-uyy)+(6*uxy*uxy));
  // daniel versuch:
  eEl_distortion = lame2 * (4 * uxy * uxy + 2 * uxx * uxx + 2 * uyy * uyy -
                            ((4 / 3) * (uxx * uxx + uyy * uyy)));
  eEl_dilatation = (bulk / 2) * (uxx + uyy) * (uxx + uyy);

  partikel->eEl_distortion = eEl_distortion * partikel->mV * mole_per_particle *
                             pressure_scale * pascal_scale;
  partikel->eEl_dilatation = eEl_dilatation * partikel->mV * mole_per_particle *
                             pressure_scale * pascal_scale;
    ...
    // if (!partikel->is_lattice_boundary)
    {
      if (1) {
        ...
        if (partikel->fluid_neig_count) {
          ...
          // partikel->surfE = (partikel->surfE * (1-surface_area_factor)) +
        ...
        if (partikel->fluid_neig_count) {
          ...
          for (j = 0; j < partikel->fluid_neig_count; j++) {
            bound_stress =
                bound_stress + partikel->fluid_neigP[j]->bound_stress;
          }

          bound_stress = bound_stress / partikel->fluid_neig_count;

          // partikel->bound_stress = (partikel->bound_stress +
          // bound_stress)/2.0;
        }

        partikel->bound_stress = -1.0 * partikel->bound_stress;

        partikel->bound_stress = partikel->bound_stress * pressure_scale *
                                 pascal_scale * partikel->mV;
      }
    }
  }
}
...
//   for (i = 0; i < numParticles; i++)
//     {
//       //cout << "Partikel-temperature (save heat): " <<
//       runParticle->temperature << endl; runParticle->previous_temperature =
//       runParticle->temperature; runParticle = runParticle->nextP;
//     }
//
...
//       //cout << "Partikel-temperature (invert heat): " <<
//       runParticle->temperature << endl; runParticle = runParticle->nextP;
//     }
//
  ...
  for (i = 0; i < numParticles; i++) {
    if (runParticle->prob >= 1.0) {
      ...
      case 1:
        ...
        runParticle->young =
            runParticle->young * 1.2; // change repulsion constant
      ...
      case 2:
        ...
        runParticle->young =
            runParticle->young * 0.833333333; // change repulsion constant
  ...
  for (i = 0; i < numParticles; i++) {
    if (runParticle->grain != beware_grain1 &&
      ...
      runParticle->real_radius =
          pow(((3.0 * mole_per_particle * runParticle->mV) / (4.0 * 3.1415)),
  ...
  for (i = 0; i < numParticles; i++) { // and loop through particles
    for (j = 0; j < 8; j++) {          // loop through neighbours of particle
      if (runParticle->neigP[j]) {     // if there is a neighbour
        if (runParticle->neigP[j]->mineral !=
            runParticle->mineral) { // if I am a boundary
...
void Min_Trans_Lattice::Gbm() {
  ...
  float rate, time2, prob, lowest_time, lowest_time2, local_time, v, t,
      time_interval2, dd;
...
void Min_Trans_Lattice::Call_Reaction_Two() {
  ...
  for (i = 0; i < numParticles; i++) {
    ...
    if (runParticle->mineral == 2 && runParticle->is_phase_boundary &&
        !runParticle->is_lattice_boundary) {
      runParticle->phase_change_required = true;
    } else {
      runParticle->phase_change_required = false;
...
// called in gbm()
float Min_Trans_Lattice::CalcArea(float lowest_time) {
  int i;
...
//  called in gbm()
float Min_Trans_Lattice::CalcTime() {
...
float Min_Trans_Lattice::Latent_Heat_Release() {
  int i, j;
  float heat_capacity, latent_heat, temperature;

  if (runParticle->mineral == 1) {
    heat_capacity = 200.2 -
                    1037.0 * pow(double(runParticle->temperature), -0.5) -
                    1320000.0 * pow(double(runParticle->temperature), -2.0) -
                    492000000.0 * pow(double(runParticle->temperature), -3.0);
    temperature = -39000.0 / heat_capacity;
    // cout << runParticle->temperature << endl;
  } else if (runParticle->mineral == 2) {
    heat_capacity = 217.8 -
                    1408.0 * pow(double(runParticle->temperature), -0.5) -
                    526000000.0 * pow(double(runParticle->temperature), -3.0);
    temperature = 39000.0 / heat_capacity;
    // cout << runParticle->temperature << endl;
  }
...
void Min_Trans_Lattice::Provisoric_Mineral_Transformation() {
  ...
  // flag (=condition) to perform a provisoric phase-change for each
  // particle:=phase_change_required

  // make it a loop through all particles, to built the new configuration of
  // particles at ones:
  ...
  for (i = 0; i < numParticles; i++) {
    ...
    if (runParticle->phase_change_required == true) {
      if (runParticle->mineral == 1) {
        ...
        runParticle->young =
            runParticle->young * 0.8333333333; // change repulsion constant
  ...
  // here, finally, the new molare volume & radius for the particle will be
  // calculated:
  for (i = 0; i < numParticles; i++) {
    // if (runParticle->grain != beware_grain1 && runParticle->grain !=
    // beware_grain2 && runParticle->grain != beware_grain3) {
...
void Min_Trans_Lattice::Heat_Flow(float Time_Step_Heat) {
  int p_counter, i, col;
  float delta_t, wall_pos, delta_x, delta_y, density, specific_heat, lambda,
      highest_temp;
...
float Min_Trans_Lattice::Activation_Energy() {
  float activation_energy, activation_volume, activation_enthalpy, mean_stress;
  ...
  for (i = 0; i < numParticles; i++) {
    if (runParticle->mineral == 1 && !runParticle->is_lattice_boundary) {
      if (runParticle->prob >= 1.0 && runParticle != nucleus) {
        buffer = pow((mole_per_particle * runParticle->mV) /
...
float Min_Trans_Lattice::Energy_Barrier() {
  float prefactor, surface_energy, driving_force, molare_volume, energy_barrier,
      mean_stress;
...
float Min_Trans_Lattice::Excess_Pressure() {
  ...
  float delta_P, // differenz to the equilibrium pressure for the given
                 // temperature (akaogie, daessler)
      P, // at phase boundary with temperature T
      T, mean_stress;

  float clapeyron_constant, enthalpy_differenz, volume_differenz;
...
float Min_Trans_Lattice::Undercooling() {
  ...
  float delta_T, // differenz to the equilibrium temperature for the given
...
float Min_Trans_Lattice::Driving_Force() {
  float driving_force, entropy_change, mean_stress, mean_stress_change,
...
void Min_Trans_Lattice::DumpDiffStressAndTime(Particle *particle, float time,
                                              float eEl_diff) {
  FILE *stat;
