...
// Suppress hard error in deprecated <boost/progress.hpp>/<boost/timer.hpp>
// used by this legacy process. We opt into keeping these headers until
// the code is modernized to new Boost timer facilities.
...
// Prefer system IPhreeqc headers if available, otherwise fall back to bundled stub
...
public:
  ...
  typedef vector<std::pair<const char*, double>> FLUID; // vector to hold concentrations of a fluid
  ...
  void SetInfFluid(const char* f2);       // check and set the file of the infiltrating fluid
  void SetDatabase(const char* database); // check and set the database
  double ReturnValue(VAR vs,
  ...
  // Main------------------------------------------------------------------
  void Equilibrate(vector<std::pair<const char*, double>>& fluid, string phase);
  double SI(FLUID fluid, string phase, bool write_file);
  void React(FLUID fluid, double k, double n, double A0, double V, double m0, double time_step, double tot_time,
             string mineral, string formula, bool write_file);
  double dissCalcite(vector<double> Fluid, double radius, double porosity, int NB, double& AdCalcite);
};
#endif
