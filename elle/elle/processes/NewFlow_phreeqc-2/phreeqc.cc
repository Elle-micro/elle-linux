...
void PhreeqC::UpdateVector(VAR v,
  ...
  for (it = vec.begin(); it != vec.end(); it++) {
    if (it->first != "Alkalinity") {
      cout << it->first << " " << ReturnValue(v, it->first) << endl;
      it->second = ReturnValue(v, it->first);
    } else {
      cout << "alkalinity " << ReturnValue(v, "Alk(eq/kgw)") << endl;
...
it != Conc_matrices.end(); it++)
                        {
                                if(boost::iequals(it->first, name))
                                {
                                        boost::array<double, dimX * dimY>
conc_1D ; for(int ii = 0; ii < dimX*dimY; ii++) conc_1D[ii] = value;
                                        (*it).second = conc_1D;
                                }
                        }
...
void PhreeqC::GetConcentration(const char *type, double (&Conc)[dimX][dimY]) {
  boost::array<double, (dimY * dimX)> Conc_matrix;
  for (FLUID_array ::iterator it = Conc_matrices.begin();
       it != Conc_matrices.end(); it++) {
    if (it->first == type)
...
void PhreeqC::SetConcentration(const char *type, double Conc[dimX][dimY]) {
  for (FLUID_array ::iterator it = Conc_matrices.begin();
    ...
    if (it->first == type) {
      for (int x = 0; x < dimY; x++)
        for (int y = 0; y < dimX; y++) {
          it->second[y * dimX + x] = Conc[x][y];
        }
    }
  }
}

double PhreeqC::FindConcentration(const char *type) {
  double value = 0;
  for (FLUID ::iterator it = Conc_inf.begin(); it != Conc_inf.end(); it++) {
...
PhreeqC::FLUID PhreeqC::GetFluid(int x, int y) {
  FLUID fluid;
  for (FLUID_array ::iterator it = Conc_matrices.begin();
       it != Conc_matrices.end(); it++)
    fluid.push_back(make_pair(it->first, it->second[y * dimX + x]));
  return (fluid);
}
...
// calculate SI=========================================================
double PhreeqC::SI(vector<std::pair<const char *, double>> fluid, string phase,
                   bool write_file) {
  vector<std::pair<const char *, double>> fluid2 = fluid;
  VAR vs;
  double SI = 0;
  string si = "si_" + phase;
  ...
  for (FLUID ::iterator it = fluid2.begin(); it != fluid2.end(); it++)
    oss << it->first << " " << ConvertToString(it->second).c_str() << endl;
...
void PhreeqC::Equilibrate(vector<std::pair<const char *, double>> &fluid,
  ...
  iphreeqc.SetOutputFileName("Equi");
  std::ostringstream oss;
  oss << "Solution 1_mod " << endl;
  oss << "units " << units.c_str() << endl;
