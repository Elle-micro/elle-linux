...
void Lattice::UpdateElle() {
  ...
  for (i = 0; i < numParticles; i++) // and loop again
  {
    ...
    runParticle->p_Unode->setAttribute(U_ATTRIB_A,
    ...
    runParticle->p_Unode->setAttribute(U_ATTRIB_C,
    ...
    // runParticle->p_Unode->setAttribute(U_DENSITY, runParticle->real_density);
    runParticle->p_Unode->setAttribute(U_DENSITY, runParticle->react_time);
    ...
    runParticle->p_Unode->setAttribute(U_ATTRIB_A, runParticle->calcite);
...
void Lattice::DeformLatticeExtension(float move, int plot, float dx,
  ...
  if (plot == 1)
    UpdateElle(1); // and update the interface of Elle if it
  // is wanted
}
// DEFORM_LATTICE
/*******************************************************************************
...
void Lattice::Fluid_Parameters(int boundary, double grain_size) {
  Fluid->Read_Porosity_Movement_Matrix(0, repBox, boundary);
...
void Lattice::Calculate_Fluid_Pressure(double upper_pressure,
                                       ...
                                       double additional) {
  {
    ...
    Fluid->Set_Boundaries(upper_pressure, lower_pressure, sides, 1.0,
                          additional);
    ...
    for (k = 0; k < iter; k++) {
      ...
      Fluid
          ->test_pressure_with_source(); // adding the source term (test please)
...
void Lattice::TestPhreeqc() { Fluid->TestPhreeqc(); }
