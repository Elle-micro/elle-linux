...
void stats::SaveMeltStats(int mineral, int Count) {
  ...
  if (rest.rem ==
      0) // only writes stuff every tenth stage,change to what you like
  {
    statsout = fopen("melt-statistics.csv", "a+");
    // printf("\n%d",Count);       //make linefeed before printing current
    // number of runs
...
void stats::CalcAreaMineral(int mineral, int iflynn) {
  ...
  fprintf(statsout, "%d,%lf,", iflynn,
          area); // print area of melt pockets in file
...
void stats::getFlynnCircumference(int mineral, int iflynn) {
  ...
  {
    if (n == num - 1) // if n=last Flynn in array, calculate distance between
                      // last and first Node
    {
...
void stats::calculateAngle(int mineral, int iflynn) {
  ...
  memoryzn = zn; // I dunno why but somewhere along the line in the for-cycle zn
                 // always gets set to the value of j[2] ?!?!?!?!?!?!?
  ...
  for (z = 0; z < zn; z++) // cycle through all nodes
  {
    if (ElleNodeIsActive(zarr[z])) // only do stuff if node exists
    {
      ...
      {
        // tpnode[counttpnodes]=zarr[z];//put all triplenodes in this array -
        // ATTENTION: IF THERE ARE MORE THEN 21 TRIPLENODES THE PROGRAM WILL
        // CRASH! counttpnodes++; //that is for the calculation of the curvature
        // between triplenodes
        nbnode[check] = zarr[z];
        check++;
        ...
        zn = memoryzn;
        c = ElleNodeSeparation(nbnode[1],
                               nbnode[2]); // and these should be the lengths of
                                           // the lines a, b and c
        ...
        angle =
            acos(cosgamma) *
            (180 /
             3.1415926535); // now of course acos() is in radiants!
                            // 1degree=phi/180radiant--1radiant=180/phi degrees
        // printf("Node %d is triple, neighbouring nodes are %d and
        // %d,",nbnode[0], nbnode[1],nbnode[2]); printf("a=%f, b=%f, c=%f,
        // Angle:%lf\n",a,b,c,angle);
