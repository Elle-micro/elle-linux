...
int Melt() {
  ...
  ElleCheckFiles();
  sprintf(logbuf, "This version is a special version, it lets only nodes move "
                  "that belong to at least one  solid flynn");
  // Log(0,logbuf);
  max = ElleMaxNodes();
  for (k = 0; k < max; k++)
  ...
  // Log( 2, logbuf );
  // setting the fudge factor to a negative value will keep it fixed at that
  // value This gives problems when restarting the simulation, the fudge factor
  // is positive then and will be altered
  if (CurrData[Magic] < 0) {
    alterfudge = false;
    // make fudge positive
  ...
  for (i = 0; i < EllemaxStages(); i++) {
    ...
    /****************************************************/
    // we only adjust the fudge factor if it is positive. If it is negative, it
    // will not be altered
    if (alterfudge) {
      // if we have decreasing melt or increasing melt we have to keep the fudge
      // factor stable so calculate the disequilibirum
      test0 = (arealiq - CurrData[EquilMeltFrac]) / CurrData[EquilMeltFrac];
      test1 = fabs(test0);
      // if the difference between the actual melt area and the user defined one
      // (represented by the disequilibrium) is bigger than upper
      if (test1 > upper) //.005
      {
        // make it fit later in Get SurfaceEnergy by increasing the surface
        // energy but only if it is not too small (or big). Depending on the
        // starting value of the fudge factor, cdmagic can reach very high/low
        // values. We do not allow that here
        if (cdmagic < 10 * CurrData[Magic])
          cdmagic = cdmagic * 1.0075;
        // and log a warning
        sprintf(logbuf, "upper: stages:%d arealiq:%lf %%diseq:%lf fudge:%lf equi:%lf\n", i, arealiq,
                100 * (fabs(arealiq - CurrData[EquilMeltFrac]) / CurrData[EquilMeltFrac]), cdmagic,
                CurrData[EquilMeltFrac]); //
                                          // Log( 1, logbuf );
      }
      // if the difference between the actual melt area and the user defined one
      // is smaller than lower, make it fit
      else if (test1 < lower) {
    ...
    max = ElleMaxNodes();
    // to prevent always using the same sequence of nodes, we randomly shuffle
    // them
    ran.clear();
    for (j = 0; j < max; j++)
      if (ElleNodeIsActive(j))
        ran.push_back(j);
    // Modern C++: replace deprecated random_shuffle with std::shuffle
    {
      static std::mt19937 rng(static_cast<unsigned>(std::random_device{}()));
      std::shuffle(ran.begin(), ran.end(), rng);
    }
    ...
    for (j = 0; j < ran.size(); j++) {
      if (ElleNodeIsActive(ran.at(j))) {
        ...
        /*******************************************************************
         *this lets only a specific grain move, all other grains are fixed. If
         *other grains share a common boundary, that boundary can move. This is
         *only for the age relationship of a growing crystal.
         ******************************************************************/
        /*if(rgn[0]==8)
                val0=0;
        else
        ...
        /******************************************************************/
        /*This moves a single, predefined node in a predefined direction (here
         *half the switchdistance upward), all other nodes move according to
         *their energy values.
         *******************************************************************/
        /*te=ran.at(j);
        ...
        for (int q = 0; q < 3; q++)
          if (rgn[q] != NO_NB) {
            Melt_MoveNode4Pos(rgn[q], ran.at(j), &incr, &same, arealiq, CurrData[Mobility], cdmagic,
                              CurrData[EquilMeltFrac], CurrData[XlXlEnergy], CurrData[LiqXlEnergy],
        ...
        /******************************************************************
         * This belongs to the if(val0==0 || etc.) for irregular melt stuff, see
         *above
         *******************************************************************/
        //}
        /*******************************************************************/
...
int Melt_MoveNode4Pos(int flynn, int n, Coords* movedir, int* same, double arealiq, double mobfac, double fudge,
                      double areaequil, double energyxlxl, double energyliqxl, double energyliqliq) {
  int i, l;
  ...
  ElleSetPosition(n, &newxy);
  // calculate the direction and distance the node has moved (needed for the
  // crossings check)
...
int Melt_MoveNode3Pos(int flynn, int n, Coords* movedir, int* same, double arealiq, double mobfac, double fudge,
                      double areaequil, double energyxlxl, double energyliqxl, double energyliqliq) {
  int i, l;
  ...
  while (go) {
    ...
    for (l = 0; l < 3; l++) {
      ...
      dblarealiq = CalcAreaLiq(MeltId);
      // get the energy of the node at the new position
      anisotropy += GetSurfaceNodeEnergy(n, MeltId, &xyz[l], &E[l], dblarealiq, fudge, areaequil, energyxlxl,
                                         energyliqxl, energyliqliq);
    }
    ElleSetPosition(n, &oldxy);
    ElleSetPrevPosition(n, &prev);
...
int GetBodyNodeEnergy(int n, double* total_energy) {
  ...
  for (i = 0; i < 3; i++) {
    // if there are only two surrounding flynns, one value of rgn is NO_NB
    // (NO_NeighBour)
    if (rgn[i] != NO_NB) {
...
/*!This is a bit weired since it calculates the surface energy for a segment,
 * not actually for the node (which is correct).*/
double GetSurfaceNodeEnergy(int n, int meltmineral, Coords* xy, double* energy, double arealiq, double fudge,
                            double areaequil, double energyxlxl, double energyliqxl, double energyliqliq) {
  int i, nb[3];
  Coords xynb;
  ...
  for (i = 0; i < 3; i++) {
    if (nb[i] != NO_NB) {
      ...
      //  with the rg2 and rg3 values, the flynns do not have to have a default
      //  anymore. Either they have an orientation or they don't, if they don't
      //  fine too.*/
      if (rgn2 != NO_NB && rgn3 != NO_NB && ElleFlynnHasAttribute(rgn2, CAXIS) && ElleFlynnHasAttribute(rgn3, CAXIS)) {
        ...
        ElleGetFlynnCAxis(rgn3, &xyz[0]);
        csl_factor = 1.0;
      } else if (rgn2 != NO_NB && rgn3 != NO_NB && ElleFlynnHasAttribute(rgn2, EULER_3) &&
                 ElleFlynnHasAttribute(rgn3, EULER_3)) {
        rg3 = rg2 = true;
      ...
      if (len == 0.0)
        len = 1.0;
      // only do this if at least one of them actually has values and also do it
      ...
      // if they both have values, do also this because there is something in
      // xyz[1]
      if (rg3 && rg2) {
      ...
      rg2 = rg3 = false;
      // if there is no angle energy for one or more, it is still 1 so it does
      // not count in the calculations below
  ...
  // If arealiq is smaller than areaequil, this calculation adds energy to the
  // surface energy. If arealiq is bigger than areaequil, this calculations adds
  // just a little bit of energy to the surface energy. So depending on the melt
  // fraction, the surface energy returned here gets an extra boost or not. This
  // also means that the direction the node is eventually going to move towards
  // will be in a direction so that the melt fraction stays stable or is
  // adjusted to the equilibrium melt fraction).
  //
  //
  //
  // We have to add the pressure-energy term here. If it is negative, this means
  // that the melt pocket needs to get melt so each node movement that increases
  // the area of the melt pocket should be favored (or vice versa). We could
  // still use the fudge factor, but I think this is not very usefull anymore.

  /*!Used like this, there is a tricky balancing problem between the surface
   * energies, the fudge-factor and the angle energies. I have not solved it
   * yet, but it works if fudge =surface energy=1.
   */
...
double GetDistanceP1P2(int nextnode, int prevnode) {
  ...
  // if there is wrapping between the two points, we have to adjust the length
  // here
  if (len >= 0.5)
    // ChangeLength is defined in meltmovenode.cc
    len = ChangeLength(xy1, xy2);
  // return the distance to the next melt pocket corrected for the lengthscale
  // of this model
