...
int GetBodyNodeEnergy(int n, double *total_energy) {
  ...
  for (i = 0; i < 3; i++) {
    if (rgn[i] != NO_NB) {
      ...
      area[i] =
          fabs(ElleRegionArea(rgn[i])) * (ElleUnitLength() * ElleUnitLength());
...
/* !/brief this is called to when it is time to move a node (and also to check
 * if a node has to be moved) */
...
int GBMGrowth() {
  ...
  if (ElleFlynnAttributeActive(DISLOCDEN) ||
      ElleFlynnAttributeActive(EULER_3) || ElleFlynnAttributeActive(CAXIS))
    surface_only = false;
