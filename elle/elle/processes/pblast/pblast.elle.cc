...
void RotatePosition(int node, Coords *curr, int nb1, int nb2) {
  ...
  angle(xy.x, xy.y, xy_nb2.x, xy_nb2.y, curr->x, curr->y, &ang2);
  angle(xy.x, xy.y, xy_nb1.x, xy_nb1.y, curr->x, curr->y, &ang1);
  /*
  ...
  if (ang2 < 0)
    ang2 += PI * 2;
  ang = (ang1 + ang2) * 0.5;
  rotate_coords(curr->x, curr->y, xy.x, xy.y, &xnew, &ynew, ang);
  curr->x = xnew;
  curr->y = ynew;
}
#if XY
