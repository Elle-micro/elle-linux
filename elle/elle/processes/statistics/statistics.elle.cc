...
int ProcessFunction() {
  ...
  for (i = 0; i < EllemaxStages(); i++) {
    ...
    max = ElleMaxFlynns();
    if (Flynn_Area == 1 && ElleFlynnAttributeActive(MINERAL) &&
        ElleFlynnAttributeActive(AGE) && ElleFlynnAttributeActive(CYCLE)) {
      for (j = 0; j < max; j++) {
    ...
    if (ElleFlynnAttributeActive(MINERAL) && ElleFlynnAttributeActive(AGE) &&
        ElleFlynnAttributeActive(CYCLE)) {
      for (j = 0; j < max; j++) {
        if (ElleFlynnIsActive(j) && !isParent(j)) {
          ElleGetFlynnRealAttribute(j, &cycle, CYCLE);
    ...
    if (ElleFlynnAttributeActive(EULER_3)) {
      ...
      if (Max_Orient == 1) {
        ...
        fprintf(tmpstats,
                "<=5\t%f\n<=10\t%f\n<=20\t%f\n<=40\t%f\n<=60\t%f\n<=90\t%f\n<="
                "180\t%f\n",
                max_length[1], max_length[2], max_length[3], max_length[4],
    ...
    /*
            if (Write_Elle == 0) {
               ...
               for (j=0;j<max;j++) {
                   if (ElleNodeIsActive(j) &&
       (ElleNodeAttribute(j,N_ATTRIB_B)==1)) { ElleDeleteDoubleJNoCheck(j);
...
void BinOrientMinMax(int node, double orient, double new_length,
  ...
  if (orient <= 5)
    b_length[1] += (float)new_length;
  else if (orient > 5 && orient <= 10)
    b_length[2] += (float)new_length;
  else if (orient > 10 && orient <= 20)
    b_length[3] += (float)new_length;
  else if (orient > 20 && orient <= 40)
    b_length[4] += (float)new_length;
  else if (orient > 40 && orient <= 60)
    b_length[5] += (float)new_length;
  else if (orient > 60 && orient <= 90)
    b_length[6] += (float)new_length;
  else if (orient > 90 && orient <= 180)
    b_length[7] += (float)new_length;
  /*
     ...
     for (j=0;j<3;j++) {
          ...
          if (nbnodes[j]!=NO_NB && nbnodes[j]>node &&
     ElleNodeIsTriple(nbnodes[j])){ if (orient<=5)
     b_length[1]+=(float)new_length; else if (orient>5 && orient<=10)
     b_length[2]+=(float)new_length; else if (orient>10 && orient<=20)
     b_length[3]+=(float)new_length; else if (orient>20 && orient<=40)
     b_length[4]+=(float)new_length; else if (orient>40 && orient<=60)
     b_length[5]+=(float)new_length; else if (orient>60 && orient<=90)
     b_length[6]+=(float)new_length; else if (orient>90 && orient<=180)
     b_length[7]+=(float)new_length;
          }
...
void BinOrientMax(int node, double orient, double new_length,
  ...
  if (orient <= 5)
    bmax_length[1] = bmax_length[1] + new_length;
  if (orient <= 10)
    bmax_length[2] += new_length;
  if (orient <= 20)
    bmax_length[3] += new_length;
  if (orient <= 40)
    bmax_length[4] += new_length;
  if (orient <= 60)
    bmax_length[5] += new_length;
  if (orient <= 90)
    bmax_length[6] += new_length;
  if (orient <= 180)
    bmax_length[7] += new_length;
  /*
     float new_length;
     ElleNeighbourNodes(node,nbnodes);
     for (j=0;j<3;j++) {
