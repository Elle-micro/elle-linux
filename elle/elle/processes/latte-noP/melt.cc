...

{
  ...
  userdata[EquilMeltFrac] =
      0.1; // equilibrium melt fraction (not currently used)
...
* /
  ...
  for (i = 0; i < EllemaxStages(); i++) {
    ...
    if ((fabs(arealiq - CurrData[EquilMeltFrac])) / CurrData[EquilMeltFrac] >
        upper) //.00003 orig
      CurrData[Magic] = CurrData[Magic] * 1.0075;
    else if ((fabs(arealiq - CurrData[EquilMeltFrac])) /
                 CurrData[EquilMeltFrac] <
             lower)
    ...
    if ((fabs(arealiq - CurrData[EquilMeltFrac])) / CurrData[EquilMeltFrac] >
        upper)
      printf("i arealiq %%diseq fudge equil: %d %lf %lf %lf %lf\n", i, arealiq,
             100 * (fabs(arealiq - CurrData[EquilMeltFrac]) /
                    CurrData[EquilMeltFrac]),
             CurrData[Magic],
             CurrData[EquilMeltFrac]); // output to screen to check melt area
                                       // changes
    max = ElleMaxNodes();
    if (i % 2) {
      for (k = 0; k < max; k++) {
        if (ElleNodeIsActive(k)) {
          GBM_MoveNode(k, &incr, &same, arealiq, CurrData[Mobility],
                       CurrData[Magic], CurrData[EquilMeltFrac],
                       CurrData[XlXlEnergy], CurrData[LiqXlEnergy],
...
int Melt::GBM_MoveNode(int n, Coords *movedir, int *same, double arealiq,
  ...
  GetSurfaceNodeEnergy(n, MeltId, &newxy, &startsurfaceenergy, dblarealiq,
                       fudge, areaequil, energyxlxl, energyliqxl, energyliqliq);
  ...
  for (i = 0; i < 4; i++) {
    ...
    GetSurfaceNodeEnergy(n, MeltId, &newxy, &surfaceenergy, dblarealiq, fudge,
                         areaequil, energyxlxl, energyliqxl, energyliqliq);
...
int Melt::GetBodyNodeEnergy(int n, double *total_energy) {
  int i, nb[3];
  ERegion rgn[3];
  double sum_energy;
...
int Melt::GetCSLFactor(Coords_3D xyz[3], float *factor) {
...
int Melt::ElleGetFlynnEulerCAxis(int flynn_no, Coords_3D *dircos) {
...
int Melt::GetSurfaceNodeEnergy(int n, int meltmineral, Coords *xy,
                               double *energy, double arealiq, double fudge,
                               double areaequil, double energyxlxl,
                               double energyliqxl, double energyliqliq) {
  int i, nb[3];
  Coords xynb;
  ...
  for (i = 0; i < 3; i++) {
    if (nb[i] != NO_NB) {
      ...
      if (ElleFlynnHasAttribute(rgn2, CAXIS) &&
          ElleFlynnHasAttribute(rgn3, CAXIS)) {
  ...
  E += fabs((arealiq - areaequil) / areaequil) *
...
double Melt::GetAngleEnergy(double angle) {
  int angdeg;
...
double Melt::CalcAreaLiq(int meltmineral) {
  int i;
...
void Melt::ElleNodeSameAttrib(int node, int *same, double *sameval,
                              int attrib_id) {
  int i, j;
  double theval = 0;
  double dval[3];
