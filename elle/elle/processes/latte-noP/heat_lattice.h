...
public:
  ...
  ~Heat_Lattice(){}; // Destructor   (does nothing)
  ...
  void set_cell_temperature(
      float x, float y,
      float t); // ̹bergibt x- und y-position, mit delta temp f̹r die zelle

  float set_particle_temperature(float x,
                                 float y); // gibt zell-temperatur an x/y-stelle
  ...
  void SetHeatFlowEnabled(bool);

private:
  double a[800][800], a_save[800][800], a_inv[800][800], b[800][800],
      ...
      beta, adiabatic_temperature, pressure, wall_pos, time, factor;

  float boundary_condition;

  int col,
      counter; // number of rows and columns (of lattice/matrix), and a counter
  ...
  void multiplication_two(); // multiplication von invertierter matrix mit
                             // aktuelle w�rmeverteilung (=implizite l̦sung)
