        ...
        if ((i > 0 && i < dim - 1) && (j > 0 && j < dim - 1)) {
          source_c[i][j] = kappa[i][j] * time_a /
                           (2 * mu * (1 - solid_fraction[i][j]) *
                            pow(width * scale_constt, 2.0));
          source_c[i][j] = 1000 * source_c[i][j] *
                           ((Pf[i][j + 1] - Pf[i][j - 1]) +
...
void Fluid_Lattice::SolveAdvectionFromm(double space, double time) {
  ...
  for (kk = 0; kk < iteration; kk++) {
    ...
    for (i = 2; i < dim - 2; i++) // x loop
    {
      ...
      {
        dxc[i][j] = 0.5 * (vel_darcyx[i][j] + vel_darcyx[i + 1][j]) *
                    (0.5 * (Con[i + 1][j] + Con[i][j]) +
                     0.125 * (Con[i][j] - Con[i - 1][j]) -
                     0.125 * (Con[i + 2][j] - Con[i + 1][j]));
        dxc[i][j] -= fabs(0.5 * (vel_darcyx[i][j] + vel_darcyx[i + 1][j])) *
        ...
        dxc[i][j] += 0.5 * (vel_darcyy[i][j] + vel_darcyy[i][j + 1]) *
                     (0.5 * (Con[i][j + 1] + Con[i][j]) +
                      0.125 * (Con[i][j] - Con[i][j - 1]) -
                      0.125 * (Con[i][j + 2] - Con[i][j + 1]));
        dxc[i][j] -= fabs(0.5 * (vel_darcyy[i][j] + vel_darcyy[i][j + 1])) *
...
//-------------------------------------------------------------------------------------------------------------
// this function connects the particles of the lattice with the fluid lattice
// matrix the particle lattice has a repulsion box to track particles that is on
// the scale of the particles The fluid lattice matrix is larger, so one
// diffusion lattice grid point contains 4 boxes of the repulsion box that
// stores the particles. The particles are arranged in a triangular grid, so
// there may be more than one in the repulsion box. The repulsion box is a
// direct pointer to a particle that is stored in there. That particle points to
// a potential second one in the box and so on. The end is a NULL pointer. In
// order to talk forth and back between fluid lattice and lattice we need to
// find the respective particles in the fluid box.
//
// the function is passed a grain number (flynn number from the interface), an
// index for what grain this is supposed to be and the the box as Particle
// **list. This is the repulsion box stored in the lattice. Now fluid_lattice
// can directly acces the particles through that box in this function
//
// if the i,j loop goes from 1 to dim-1 the boundaries are not included.
// Otherwise it goes from 0 to dim/1 its important to remember that c and c++
// loops and matrices/vectors start with 0!
//
//-------------------------------------------------------------------------------------------------------------
...
void Fluid_Lattice::Set_Mineral_Fluid(int gr, int min, Particle **list) {
  ...
  for (i = 1; i < dim - 1; i++) // loop in x = i and y = j except for boundaries
  {
    for (j = 1; j < dim - 1; j++) {
      ...
      //--------------------------------------------------------------------------------------------
      // Now define counters for all 4 repulsion boxes that we have to look into
      // to define the solid fraction for the pressure node as a function of i
      // and j.
      //
      // the repulsion box is a one-dimensional list starting in the lower left
      // hand corner of the lattice with 0 and running towards the right with
      // TWICE the lattice width (so for a 100 lattice up to 199). The box in
      // the second horizontal row above 0 is then 200 and so on.
      //
      // at the moment this routine is not using the boundaries because the
      // boxes there are either not completely full or are empty. There is
      // already the try to double up full boxes for the boundaries, but that
      ...
      for (k = 0; k < 4; k++) // four repulsion boxes
      {
        ...
        {
          help =
              list[box]; // help now points at particle, help is a dummy pointer

          if (help->grain ==
              gr) // the particles have a parameter 'grain' that has the number
          {
            if (min == 1) // garnet?
            {
          ...
          while (help->next_inBox) // now do the same stuff for additional
...
//
// Fluid_Boundary[i][j] = sets the diffusion coefficient for the fluid that
// potentially can be different everywhere Fluid[i][j] is just an int with 0 for
// no fluid and 1 for I am fluid
//
//----------------------------------------------------------------------------------------------------------

void Fluid_Lattice::Set_Boundary_Fluid(double background, double bound,
                                       Particle **list) {
  int i, j, particle_in_box, k;
  ...
  for (i = 0; i < dim; i++) // loop in x = i and y = j except for boundaries
  {
    for (j = 0; j < dim; j++) {
      ...
      for (k = 0; k < 4;
           k++) // again the four repulsion boxes in your fluid node
      {
        if (k == 0)
          box = (((j * 2) - 1) * (2 * pl_size)) + 2 * pl_size + 2 +
                ((i - 1) * 2); // lower left
        ...
        if (list[box]) // is there a particle in the box (there should be one..)
        {
          ...
          if (help->is_boundary ==
              1) // if the particle is_boundary is 1 its a grain boundary
          {
            Fluid_Boundary[i][j] =
                bound;              // diffusion coefficient for the boundary
...
//------------------------------------------------------------------------------------------
// Passing back the values to the solid lattice , so the concentrations of Mg
// and Fe for solid grains and the grain boundaries
//
// This takes each pressure node and looks into its 4 repulsion boxes and gives
// the values to the particles in there.
//
...
void Fluid_Lattice::Pass_Back_Concentration(
    Particle **
    ...
    for (j = 0; j < dim; j++) {
      ...
      for (k = 0; k < 4; k++) // go to four quadrants of pressure node
      {
        ...
        if (list[box]) {
          ...
          help->Fluid =
              Fluid_Boundary[i][j]; // visualize the grain boundaries or fluid
...
void Fluid_Lattice::SolveDiffusionImplicitSolid(double time, double space) {
  ...
  multiplication_solid_Mg(); // multiply concentration matrix with first matrix,
                             // explicit step
...
void Fluid_Lattice::alpha_set_solid(double time_dif) // need only alpha
{
  ...
  for (i = 0; i < dim; i++) {
    for (j = 0; j < dim; j++) {
      ...
      {
        alpha[i][j] =
            time_dif * Mineral_Diffusion[i][j] / pow(width * scale_constt, 2.0);
        if (alpha[i][j] > max)
          max = alpha[i][j];
      }
    }
...
void Fluid_Lattice::SolveDiffusionImplicitFluid(double time, double space) {
  ...
  alpha_set_dif_por(
      time); // this was setting different coefficients for x and y
...
void Fluid_Lattice::alpha_set_dif_por(double time_dif) // alpha is x
{
  ...
  for (i = 1; i < dim - 1; i++) {
    for (j = 1; j < dim - 1; j++) {
      {
        dif = (Fluid_Boundary[i][j] + Fluid_Boundary[i - 1][j] +
               Fluid_Boundary[i + 1][j]) /
              3;
...
void Fluid_Lattice::beta_set_dif_por(double time_dif) // beta is y
{
  ...
  for (i = 1; i < dim - 1; i++) {
    for (j = 1; j < dim - 1; j++) {
      {
        dif = (Fluid_Boundary[i][j] + Fluid_Boundary[i][j - 1] +
               Fluid_Boundary[i][j + 1]) /
              3;
...
//--------------------------------------------------------------------------------------------
// This is the new explict solver for the grain boundaries. It solves only in
// the boundaries. It also takes care whether or not diffusion is one, 1.5 or
// two dimensional.
//
//
//---------------------------------------------------------------------------------------------
...
void Fluid_Lattice::SolveDiffusionExplicitFluid(double time, double space) {
  ...
  for (i = 1; i < dim - 1; i++) {
    for (j = 1; j < dim - 1; j++) {
      ...
      {
        dif = Fluid_Boundary[i][j]; // diffusion coefficient for fluid. These
                                    // are the same for Fe and Mg now, but we
        ...
        if (Fluid[i + 1][j] == 0 &&
          ...
          ddyc[i][j] =
              Mg_conc_b[i][j] +
              (time * dif / pow(width * scale_constt, 2.0)) *
                  (Mg_conc_b[i - 1][j] -
                   Mg_conc_b[i][j]); // backwards a fluid and forward not
          ...
          ddyc[i][j] =
              ...
              (time * dif / pow(width * scale_constt, 2.0)) *
                  (Mg_conc_b[i + 1][j] -
                   Mg_conc_b[i][j]); // forward a fluid and backward not

        else if (Fluid[i - 1][j] == 1 && Fluid[i + 1][j] == 1)
          ddyc[i][j] =
              Mg_conc_b[i][j] +
              (time * dif / pow(width * scale_constt, 2.0)) *
                  (Mg_conc_b[i - 1][j] - 2 * Mg_conc_b[i][j] +
                   Mg_conc_b[i + 1][j]); // normal case, both neighbours fluid
        ...
        if (Fluid[i][j + 1] == 0 &&
          ...
          ddyc[i][j] = ddyc[i][j] + Mg_conc_b[i][j] +
                       (time * dif / pow(width * scale_constt, 2.0)) *
                           (Mg_conc_b[i][j - 1] -
                            Mg_conc_b[i][j]); // backwards fluid and forward not

        else if (Fluid[i][j - 1] == 0 && Fluid[i][j + 1] == 1)
          ddyc[i][j] = ddyc[i][j] + Mg_conc_b[i][j] +
                       (time * dif / pow(width * scale_constt, 2.0)) *
                           (Mg_conc_b[i][j + 1] -
                            Mg_conc_b[i][j]); // forward fluid and backward not

        else if (Fluid[i][j - 1] == 1 && Fluid[i][j + 1] == 1)
          ddyc[i][j] =
              ddyc[i][j] + Mg_conc_b[i][j] +
              (time * dif / pow(width * scale_constt, 2.0)) *
                  (Mg_conc_b[i][j - 1] - 2 * Mg_conc_b[i][j] +
                   Mg_conc_b[i][j + 1]); // normal case, both neighbours fluid
  ...
  for (i = 0; i < dim; i++) {
    for (j = 0; j < dim; j++) {
      ...
      if (Fluid[i][j] == 1)
        ddyc[i][j] = 0.04; // define boundaries as two times background
                           // concentration (later divided by 2)
    }
  }
      ...
      if (Fluid[i][j] == 1) // this is a grain boundary with fluid
      {
        ...
        if (Fluid[i + 1][j] == 0 &&
          ...
          ddyc[i][j] =
              ...
              (time * dif / pow(width * scale_constt, 2.0)) *
                  (Fe_conc_b[i - 1][j] -
                   Fe_conc_b[i][j]); // backwards a fluid and forward not
          ...
          ddyc[i][j] =
              ...
              (time * dif / pow(width * scale_constt, 2.0)) *
                  (Fe_conc_b[i + 1][j] -
                   Fe_conc_b[i][j]); // forward a fluid and backward not

        else if (Fluid[i - 1][j] == 1 && Fluid[i + 1][j] == 1)
          ddyc[i][j] =
              Fe_conc_b[i][j] +
              (time * dif / pow(width * scale_constt, 2.0)) *
                  (Fe_conc_b[i - 1][j] - 2 * Fe_conc_b[i][j] +
                   Fe_conc_b[i + 1][j]); // normal case, both neighbours fluid
        ...
        if (Fluid[i][j + 1] == 0 &&
          ...
          ddyc[i][j] = ddyc[i][j] + Fe_conc_b[i][j] +
                       (time * dif / pow(width * scale_constt, 2.0)) *
                           (Fe_conc_b[i][j - 1] -
                            Fe_conc_b[i][j]); // backwards fluid and forward not

        else if (Fluid[i][j - 1] == 0 && Fluid[i][j + 1] == 1)
          ddyc[i][j] = ddyc[i][j] + Fe_conc_b[i][j] +
                       (time * dif / pow(width * scale_constt, 2.0)) *
                           (Fe_conc_b[i][j + 1] -
                            Fe_conc_b[i][j]); // forward fluid and backward not

        else if (Fluid[i][j - 1] == 1 && Fluid[i][j + 1] == 1)
          ddyc[i][j] =
              ddyc[i][j] + Fe_conc_b[i][j] +
              (time * dif / pow(width * scale_constt, 2.0)) *
                  (Fe_conc_b[i][j - 1] - 2 * Fe_conc_b[i][j] +
                   Fe_conc_b[i][j + 1]); // normal case, both neighbours fluid
...
// This is the exchange reaction that we have to work on, this is only a dummy
// at the moment
...
void Fluid_Lattice::Exchange(double time, double space, Particle **list) {
  ...
  for (i = 0; i < dim; i++) {
    for (j = 0; j < dim; j++) {
      ...
      for (k = 0; k < 4; k++) // go to four quadrants of pressure node to find
...
void Fluid_Lattice::SolveHeatSheet(double space, double time,
                                   double dif) // time in years
{
  int i, j;
  double factor;
  ...
  for (i = 1; i < (200) - 1; i++) {
    for (j = 1; j < dim - 1; j++) {
      ...
      // HeatSheet[i][j+1]);
      IntHeatSheet[i][j] = HeatSheet[i][j] + (factor) * (HeatSheet[i - 1][j] -
                                                         2 * HeatSheet[i][j] +
                                                         HeatSheet[i + 1][j]);
      IntHeatSheet[i][j] =
          IntHeatSheet[i][j] + HeatSheet[i][j] +
          (factor) *
              (HeatSheet[i][j - 1] - 2 * HeatSheet[i][j] + HeatSheet[i][j + 1]);
    }
  }
  ...
  for (i = 1; i < (200) - 1; i++) {
    for (j = 1; j < dim - 1; j++) {
      HeatSheet[i][j] =
          0.5 * IntHeatSheet[i][j] - ((HeatSheet[i][j] - 200) * 1 / 100);
    }
  }
  for (i = 0; i < (200); i++) {
...
void Fluid_Lattice::Pass_Back_SheetT(
    Particle **list, double temp,
    double critAge) // again gets the replusion box as direct pointer from the
                    // lattice
{
  int i, j, k;
  Particle *help; // used as pointer to reach particles
    ...
    for (j = 0; j < dim; j++) {
      ...
      for (k = 0; k < 4; k++) // go to four quadrants of pressure node
      {
        ...
        if (list[box]) {
          ...
          help->Tdif =
              help->SheetT - HeatSheet[i][j]; // other: sqrt((help->SheetT -
                                              // HeatSheet[i][j])*(help->SheetT
                                              // - HeatSheet[i][j]))
