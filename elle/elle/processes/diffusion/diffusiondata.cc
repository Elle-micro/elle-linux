...
double DiffusionTimestep() { return (Diffdat.dt); }

void SetDiffusionTimestep(double dt) { Diffdat.dt = dt; }

double DiffusionConstant() { return (Diffdat.DIFF); }

void SetDiffusionConstant(double DIFF) { Diffdat.DIFF = DIFF; }

double DiffusionDefaultBCond() { return (Diffdat.CBND); }

void SetDiffusionDefaultBCond(double CBND) { Diffdat.CBND = CBND; }
...
void InitDiffusionArrays(int size) {
  ...
  for (i = 0; i < size; i++)
    Diffdat.Area[i] = Diffdat.Len0[i] = Diffdat.Len1[i] = Diffdat.Len2[i] =
        Diffdat.DeltaC[i] = 0.0;
}
...
int SetupDiffusionArrays(struct triangulateio *tri) {
  ...
  for (i = 0; i < ne; i++) {
    ...
    Diffdat.Len0[i] = sqrt((ex[1] - ex[2]) * (ex[1] - ex[2]) +
                           (ey[1] - ey[2]) * (ey[1] - ey[2]));
    Diffdat.Len1[i] = sqrt((ex[2] - ex[0]) * (ex[2] - ex[0]) +
                           (ey[2] - ey[0]) * (ey[2] - ey[0]));
    Diffdat.Len2[i] = sqrt((ex[0] - ex[1]) * (ex[0] - ex[1]) +
                           (ey[0] - ey[1]) * (ey[0] - ey[1]));
    tris = (Diffdat.Len0[i] + Diffdat.Len1[i] + Diffdat.Len2[i]) / 2;
    Diffdat.Area[i] = sqrt(tris * (tris - Diffdat.Len0[i]) *
