...
int InitDiffusion() {
  ...
  if (MeshData.tri->triangleattributelist == NULL) {
    ...
    if ((rgnattribs = (double *)malloc((maxrgns + 1) * sizeof(double))) == NULL)
      return (1);
    dflt = 0.0;
    for (i = 0; i < maxrgns; i++)
      rgnattribs[i] = dflt;
    ...
    if (attribute_set(infile, FLYNN_REAL_ATTRIB_KEY, maxrgns + 1, rgnattribs,
                      &dflt)) {
      for (i = 0; i < MeshData.tri->numberoftriangles; i++) {
        k = (int)MeshData.tri->triangleattributelist[i];
