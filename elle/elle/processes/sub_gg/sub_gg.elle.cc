...
void CalcMisorientEuler(double curra1, double currb1, double currc1,
                        double curra2, double currb2, double currc2,
                        double *orient);
...
int SetUnodes() {
  ...
  // srand48((long) now); // initialise random number generator from current
  // time
...
void SetUnodeAttributeFromNbs(int flynnid, int attr_id) {
  ...
  for (j = 0; j < 6; j++) // set up transition probabilities based on number of
                          // similar neighbours
  {
  ...
  for (loops = 0; loops < 1; loops++) {
    for (i = 0; i < count; i++) {
      ...
      ii = (int)(ElleRandomD() *
                 count); // randomly select a unode from within flynn

      vector<int> nbnodes, bndflag;
      ElleGetTriPtNeighbours(unodelist[ii], nbnodes, bndflag,
                             0); // get the  list of neighbours for a unode
      num_nbs = nbnodes.size();
      if (num_nbs == 0)
        continue;
      // printf("num_nbs: %d\n",num_nbs);

      ElleGetUnodeAttribute(unodelist[ii], &k[0], &k[1], &k[2],
                            attr_id); //  get the unodes attribute

      // printf("kvals: %lf %lf %lf\n",k[0],k[1],k[2]);

      for (j = 0, flag = 0; j < num_nbs;
           j++) // check to see if all neighbours are same
      {
        ElleGetUnodeAttribute(
            nbnodes[j], &e[0], &e[1], &e[2],
            attr_id); // get neighbouring unode attribute value
        if (fabs(e[0] - k[0]) > eps || fabs(e[1] - k[1]) > eps ||
            fabs(e[2] - k[2]) > eps)
          flag = 1;
      }
      ...
      for (j = 0, total1 = 0; j < num_nbs;
           j++) // loop through all neighbouring unodes
      {
        ElleGetUnodeAttribute(
            nbnodes[j], &e[0], &e[1], &e[2],
            attr_id); // get neighbouring unode attribute value
        ...
        if ((fabs(orient) > max_angle || fabs(orient) == max_angle) &&
            slope == 1) {
          misorientenergy = 1.0;
          ...
          misorientenergy =
              fabs(orient / max_angle) *
              (1 - (log(orient / max_angle))); // printf(" energy_shockly:
                                               // %lf\n",misorientenergy);
        }
      ...
      ElleGetUnodeAttribute(nbnodes[randnb], &k[0], &k[1], &k[2],
                            attr_id); // get neighbouring unode attribute value

      for (j = 0, total2 = 0; j < num_nbs;
           j++) // loop through all neighbouring unodes
      {
        ElleGetUnodeAttribute(
            nbnodes[j], &e[0], &e[1], &e[2],
      ...
      if (m < prob)
        ElleSetUnodeAttribute(unodelist[ii], k[0], k[1], k[2],
                              attr_id); // set new unode attribute value
    }
  }
}
...
void CalcMisorientEuler(double curra1, double currb1, double currc1,
  ...
  euler(rmap1, (double)curra1, (double)currb1,
        (double)currc1); // gives rotation matrix
  euler(rmap2, (double)curra2, (double)currb2,
        (double)currc2); // gives rotation matrix
  ...
  // fprintf(stdout,"CalcMisorient - adjusting val for acos from %lf for bnd %d,
  // %d\n",val,rgn1,rgn2);
  if (val > 1.0)
    val = 1.0;
  else if (val < -1.0)
  ...
  val = (a11 + a22 + a33 - 1) / 2;
  //  if (val > 1.0+eps || val < -1.0-eps )
  //  fprintf(stdout,"CalcMisorient - adjusting val for acos from %lf for bnd
  //  %d, %d\n",val,rgn1,rgn2);
  if (val > 1.0)
    val = 1.0;
  else if (val < -1.0)
