...
// for c++
#include "attrib.h" // struct coord (hier: xy)
...
Lattice::Lattice()
    :
      ...
      def_time_u(0),    // deformation time zero (uniaxial compression)
...
void Lattice::InitiateGrainParticle() {
  ...
  double fx, fy, a; // more conversion for box
  ...
  for (j = 0; j < maxFlynn; j++) // loop through flynns
  {
    if (ElleFlynnIsActive(j)) // use only active flynns
    {
      ...
      ElleGetFlynnUnodeList(j, unodelist); // just gets the
                                           // Unodelist for
                                           // flynn j
      ElleGetFlynnRealAttribute(j, &a, F_ATTRIB_A);
                                          ...
                                          // be)
          {
                                                   ...
                                                   // particle
            {
                                                        ...
                                                        // this
              {
                ...
                runParticle->next_inBox->grain_attribute = a;
                // particle
                // grain
                // number
              }
...
float Lattice::DeformLatticeSimpleShear(float move, int plot) {
  ...
  for (i = 0; i < numParticles; i++) // go through all particles
                                     // starting from back
  {
    runParticle->xpos = runParticle->xpos + (move * (runParticle->ypos));
...
float Lattice::DeformLatticedrift6(float move, int time, float changeone,
  ...
  // starting from back
  {
    ...
    if (runParticle->xpos >
        0.3) // everything right of the always fixed position
    {
      if (driftlist[int(
              (runParticle->ypos) *
      ...
      {
        if (runParticle->xpos < (0.7 + (move * time * -1.0))) // average
        {
          // cout << "average" << endl;
          runParticle->xpos =
              runParticle->xpos - ((runParticle->xpos - 0.3) * 2.0 * move);
          runParticle->sheet_x = runParticle->sheet_x -
                                 ((runParticle->sheet_x - 0.3) * move * 2.0);
        } else {
          runParticle->xpos = runParticle->xpos - move;
          runParticle->sheet_x = runParticle->sheet_x - move;
        }
...
void Lattice::WeakenGrainAttribute(double nb, float constant, float visc,
                                   float break_strength) {
  ...
  for (i = 0; i < numParticles; i++) // loop through particles
  {
    if (runParticle->grain_attribute == nb) // if the right grain
    {
      // --------------------------------------------------------
      // adjust youngs modulus of particle, default is for
      // spring constant of 1.0
  ...
  porosity = (count * 100) / numParticles;
  ...
  for (x = 0; x < particlex; x++) // was -10
  {
    for (y = 0; y < particlex - 1; y++) {
      ...
      if (repBox[xx])
        ...
        //	broken1 = true;
        // else if (repBox[xx+1]->SheetT > 600)
        // broken1 = true;
        //}
        // else
        // broken1 = true;
...
void Lattice::ChangePoisson(double poisson) {
  int i;
  default_Poisson = poisson;
...
void Lattice::ChangeRelaxMax(int max) { relaxmax = max; }
