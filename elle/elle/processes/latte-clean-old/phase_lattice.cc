...
Phase_Lattice::Phase_Lattice()
    :
      ...
      water_compres(0.0000000005), count_time(0.0)
 ...
 * rate constants or youngs moduli. 1.0 is 100 percent for the second phase
 *relative to the original phase, 0.01 1 percent. factor gives the percentage
 *that the original rate will change. 1.0 is no change. elastic is the same,
 *changes the spring constants of particles and the youngs modulus (for the
 *repulsion) which particle is in the second distribution is determined using
...
void Phase_Lattice::Set_Rate_Two_Phase(float ratio, float factor,
                                       float elastic) {
  int i, j;     // counter
  float ran_nb; // pseudorandom number
  ...
  {
    ran_nb =
        rand() / (float)RAND_MAX; // pic pseudorandom from 0 to 1.0 for grains

    if (ran_nb < ratio) // in the second distribution ?
    {

      runParticle->rate_factor =
          runParticle->rate_factor * factor; // change rate constant
      runParticle->young =
          runParticle->young * elastic; // change youngs modulus
...
void Phase_Lattice::Set_Rate_Three_Phase(float ratio1, float factor1,
                                         float elastic1, float ratio2,
                                         float factor2, float elastic2) {
  int i, j;     // counter
  float ran_nb; // pseudorandom number
  ...
  for (i = 0; i < numParticles; i++) // loop through particles
  {
    ...
    if (ran_nb < ratio1) // in the second distribution ?
    {

      runParticle->rate_factor =
          runParticle->rate_factor * factor1; // change rate constant
      runParticle->young =
          runParticle->young * elastic1; // change youngs modulus

      for (j = 0; j < 8; j++) {
        if (runParticle->neigP[j]) // and change the spring constants
        {
          runParticle->springf[j] = runParticle->springf[j] * elastic1;
        }
      }

    } else if (ran_nb < ratio1 + ratio2) // in the second distribution ?
    {

      runParticle->rate_factor =
          runParticle->rate_factor * factor2; // change rate constant
      runParticle->young =
          runParticle->young * elastic2; // change youngs modulus

      for (j = 0; j < 8; j++) {
        if (runParticle->neigP[j]) // and change the spring constants
        {
          runParticle->springf[j] = runParticle->springf[j] * elastic2;
        }
      }
    }
    runParticle = runParticle->nextP;
...
void Phase_Lattice::Set_Rate_Two_Phase_Layer(float ratio, float factor,
                                             float elastic, float ymin,
                                             float ymax) {
  int i, j;     // counter
  float ran_nb; // pseudorandom number
  ...
  {
    if (runParticle->ypos > ymin) {
      if (runParticle->ypos < ymax) {

        ran_nb = rand() /
                 (float)RAND_MAX; // pic pseudorandom from 0 to 1.0 for grains
        ...
        if (ran_nb < ratio) // in the second distribution ?
        {
          ...
          runParticle->young =
              runParticle->young * elastic; // change youngs modulus
          runParticle->E = runParticle->E * elastic;
          runParticle->springconst = runParticle->springconst * elastic;

          for (j = 0; j < 8; j++) {
            if (runParticle->neigP[j]) // and change the spring constants
            {
              runParticle->springf[j] = runParticle->springf[j] * elastic;
              runParticle->springs[j] = runParticle->springs[j] * elastic;
            }
          }
        }
      }
...
// function to give different Elle grains different dissolution constants.

void Phase_Lattice::Set_Rate_Two_Grains(float ratio, float factor) {
  int j, i;
  float ran_nb, prob; // rand number

  for (j = 0; j < grain_counter; j++) {
    ...
    if (prob < ratio) {
      ...
      for (i = 0; i < numParticles; i++) // loop through particles
      {
        if (runParticle->grain == j) // if the right grain
        {
          // --------------------------------------------------------
          // adjust dissolution constant
          // --------------------------------------------------------

          runParticle->rate_factor = runParticle->rate_factor * factor;
        }
...
void Phase_Lattice::Make_Circular_Hole(float radius, float shift) {
  ...
  {
    help = (runParticle->ypos * (particlex) - (particlex / 2.0)) *
           (runParticle->ypos * (particlex) - (particlex / 2.0));
    help = sqrt(
        help +
        (runParticle->xpos * (particlex) - ((particlex + shift) / 2.0)) *
            (runParticle->xpos * (particlex) - ((particlex + shift) / 2.0)));
...
void Phase_Lattice::Make_Elliptical_Hole(float height, float ratio,
                                         float shift) {
  int i, j; // counters
  float help;
  ...
  for (i = 0; i < numParticles; i++) // loop through all particles
  {
    help = (runParticle->ypos * (particlex) - (particlex / 2.0)) *
           (runParticle->ypos * (particlex) - (particlex / 2.0));
    help = sqrt(help + ((runParticle->xpos + shift) * (particlex / ratio) -
                        ((particlex / ratio) / 2.0)) *
                           ((runParticle->xpos + shift) * (particlex / ratio) -
                            ((particlex / ratio) / 2.0)));
...
void Phase_Lattice::Set_Distribution_Surface_Energy(float distribution,
                                                    float springdis) {
  int i;
  float ran_nb;
  ...
  for (i = 0; i < numParticles; i++) {
    ...
    ran_nb = ran_nb * distribution /
             springdis; // apply distribution of surface free energy

    runParticle->surf_free_E =
        runParticle->surf_free_E + (runParticle->surf_free_E * ran_nb);
...
void Phase_Lattice::DissolveYRow(float ymin, float ymax, bool remove) {
  ...
  for (i = 0; i < numParticles; i++) {
    ...
    if (runParticle->ypos >= ymin && runParticle->ypos <= ymax) {
      ...
      for (j = 0; j < 8; j++) {
        if (runParticle->neigP[j]) {
          runParticle->springf[j] = 0.0; // set spring constant
          runParticle->springs[j] = 0.0; // set spring to 1.0
        }
      }
    }
...
void Phase_Lattice::DissolveYRowSinus(float ymin, float ymax, bool remove) {
  ...
  for (i = 0; i < numParticles; i++) {
    ...
    if (runParticle->ypos >= (ymin + sin(runParticle->xpos * 8) / 8) &&
        runParticle->ypos <= (ymax + sin(runParticle->xpos * 8) / 8)) {
      runParticle->isHole = true; // is a hole
      runParticle->young = 0.0;   // youngs modulus zero
      if (remove) {
        runParticle->ypos = -0.1;
...
void Phase_Lattice::Set_Concentration(int yes, double concentration) {
  int i;
  ...
  for (i = 0; i < numParticles; i++) {
    if (yes == 0) {
      ...
      if (runParticle->isHole)
        runParticle->left_conc = runParticle->conc = concentration;
      else
        runParticle->left_conc = runParticle->conc = 0.0;
    }
    runParticle = runParticle->nextP;
  }
}
...
void Phase_Lattice::SetGaussianRateDistribution(double g_mean, double g_sigma) {
  ...
  for (i = 0; i < numParticles; i++) // loop through the particles now
  {
    do {
      k_spring =
          rand() / (float)RAND_MAX; // pic a pseudorandom float between 0 and 1
      ...
      //--------------------------------------------------------------
      // if the number is smaller or the same as the probability
...
void Phase_Lattice::Dissolution_Strain(int dump) {
  ...
  do {
    ...
    for (i = 0; i < numParticles; i++) // and loop
    {
      ...
      {
        // if (!runParticle->is_lattice_boundary)  // dont dissolve lattice
        // boundaries
        {
          //-----------------------------------------
            ...
            if (runParticle->neigP[j]) // if spring is active
            {
              ...
              if (runParticle->neigP[j]->isHoleBoundary) {
                // if (!runParticle->neigP[j]->is_lattice_boundary)
                {
                  ...
                  runParticle->fluid_neigP[runParticle->fluid_neig_count] =
                      runParticle->neigP[j];
                  ...
                  runParticle->fluid_neig_count =
                      runParticle->fluid_neig_count + 1;
                }
              }
            }
          }
          ...
          runParticle->surfE =
              runParticle->surf_free_E * 2.0 / (runParticle->radius * the_size);

          runParticle->surfE =
              runParticle->surfE * ((runParticle->open_springs - 2.0) / 4.0);
    ...
    for (i = 0; i < numParticles; i++) {
      if (runParticle->isHoleBoundary) {
        if (runParticle->fluid_neig_count > 1) {
          runParticle->surfE = (runParticle->newsurfE +
                                ((runParticle->fluid_neigP[0]->newsurfE +
                                  runParticle->fluid_neigP[1]->newsurfE) *
                                 0.5)) *
                               0.5;
        } else if (runParticle->fluid_neig_count > 0) {
          runParticle->surfE =
              (runParticle->newsurfE + runParticle->fluid_neigP[0]->newsurfE) *
              0.5;
        }
      }
    ...
    for (i = 0; i < numParticles; i++) {
      if (runParticle
              ->isHoleBoundary) // if the particle is on a boundary of the hole
      {
        // if (!runParticle->is_lattice_boundary)     // if its not part of the
        // lattice boundary
        {
          ...
          lame1 =
              runParticle->young * pois / (1.0 - (pois * pois) * (1.0 + pois));
          lame2 = runParticle->young / (2.0 + pois);

          //------------------------------------------------------------------------
          // calculate the components of the infinitesimal strain tensor from
          // the components of the stress tensor for the particle
          //------------------------------------------------------------------------
          ...
          //------------------------------------------------------------------------
          // and now calculate the elastic energy for a single particle using
          // the two lame constants and the components of the infinitesimal
          // strain tensor of each particle
          //------------------------------------------------------------------------
          ...
          //-------------------------------------------------------------------------
          // and multiply by the molecular volume plus a scaling parameter for
          // the stress. The stress scaling is set with the youngsmodulus of the
          // material
          //--------------------------------------------------------------------------

          runParticle->eEl = runParticle->eEl * runParticle->mV *
                             pressure_scale * pascal_scale;
          ...
          if (runParticle->fluid_neig_count) {
            ...
            if (runParticle->leftNeighbour) // if I have a left neighbour
            {
              ...
              for (j = 0; j < 40; j++) // do that for 10 neighbours
              {
                ...
                {
                  surface_left[j] =
                      help->surfE; // write surface energy of that particle
                  help = help->leftNeighbour; // set help pointer
                } else
                  surface_left[j] = 0.0; // no particle means zero surface
                                         // energy = no influence
              }
            }
            ...
            {
              surfE = surfE + (surface_left[j] /
                               ((((j + 1) * 2) + 1) * (((j + 1) * 2) + 1)));
            }

            runParticle->newsurfE =
                runParticle->surfE + surfE; // and add particle itself
          } else
          ...
          rate = rate_constant *
                 (1 - exp(-((runParticle->eEl + runParticle->newsurfE) /
                            (gas_const * (temp + 273)))));
          ...
          rate =
              rate * internal_time * runParticle->mV * runParticle->rate_factor;

          runParticle->prob = rate * (runParticle->open_springs / 6.0) /
                              sqrt(runParticle->area / Pi);
    ...
    //---------------------------------------------------------------------------------
    // Now we went through all the particles and calculated their probabilities
    // to dissolve in this case. Now we have to check if a particle is
    // dissolving in the given time - probability is larger than 1 and dissolve
    // it and shrink all other particles or shrink all particles according to
    // the current time step and their probabilities.
    //---------------------------------------------------------------------------------

    if (preRunParticle->prob >
        1.0) // fastest particle dissolves in time step given
    {
      //--------------------------------------
      ...
      for (j = 0; j < 8; j++) {
        if (preRunParticle->neigP[j]) // check if spring is active
        {
          ...
          {
            preRunParticle->grain =
                preRunParticle->neigP[j]->grain; // now I am part of that hole
          }
        }
      }
      //--------------------------------------------------------------------------------------
      // now shrink all the other particles (shrinking is virtual with
      // particle->area, does not affect the particle->radius that is used for
      // the elastic model. Elastic model only sees if particle is totally gone
      // or not. time step for shrinking is determined by the time needed to
      // dissolve the previous particle with the highest probability
      //---------------------------------------------------------------------------------------

      for (i = 0; i < numParticles; i++) // check all particles
      {
        if (runParticle->isHoleBoundary) // If I am along a hole boundary -> can
                                         // react/dissolve
        {
          // if (!runParticle->is_lattice_boundary)		// if I am not a
          // lattice boundary
          {
            //------------------------------------------------
            // shrink the area
            ...
            runParticle->area =
                runParticle->area -
                (runParticle->area * runParticle->prob / preRunParticle->prob);
          }
        }
      ...
      internal_time =
          internal_time * (preRunParticle->prob - 1.0) / preRunParticle->prob;
    ...
    //------------------------------------------------------------------------
      ...
      for (i = 0; i < numParticles; i++) {
        ...
        {
          // if (!runParticle->is_lattice_boundary)  // I am not a lattic
          // boundary
          {
            //-----------------------------------------------------------
            ...
            runParticle->area =
                runParticle->area - (runParticle->area * runParticle->prob);
          }
        }
    ...
    for (i = 0; i < numParticles; i++) {
      ...
      {
        // if (!runParticle->is_lattice_boundary)   // I am not a lattice
        // boundary
        {
          //--------------------------------------------------------------------
          ...
          if (runParticle->area < (runParticle->radius * runParticle->radius *
                                   the_size * the_size * 0.00001)) {
            for (j = 0; j < 8; j++) {
              if (runParticle->neigP[j]) // if spring is active
              {
                ...
                if (runParticle->neigP[j]->isHole) {
                  runParticle->grain =
                      runParticle->neigP[j]->grain; // and now part of the hole
                }
              }
            }
  ...
  do {
          ...
          if (1) // dummy
          {
            ...
            for (j = 0; j < 8; j++) {
              ...
              if (runParticle->neigP[j]) {
                ...
                spring_count =
                    spring_count + 1; // count number of active connections
                ...
                if (runParticle->grain == runParticle->neigP[j]->grain) {
                  ...
                  if (runParticle->neigP[j]->isHole) {
                    //-----------------------------------------------------------
                    // this is a boundary spring, add it to the number of
                    // springs
                    //-----------------------------------------------------------
                    ...
                    //----------------------------------------------------------------
                    // and go through the whole monty of calculating the
                    // stresses on that boundary spring (spring has only normal
                    ...
                    // get distance parallel to spring and equilibrium distance
                    // (alen)
                    ...
                    fn = fn * 2.0 *
                         runParticle->radius; // scaling to particle size

                    //-------------------------------------------------------------
                    // scale normal force by particle area divided by six
                    // because the open surface is now only for one spring i.e.
                    // 1/6 of the particles circumference or area
                    //-------------------------------------------------------------

                    fn =
                        fn / ((runParticle->radius * runParticle->radius * Pi) /
                              6.0);
            ...
            if (1) {
              runParticle->open_springs =
                  runParticle->open_springs + 6 - spring_count;
              ...
              for (ii = 0; ii < 3; ii++) {
                for (jj = 0; jj < 3; jj++) {
                  ...
                  if (jj == 0)
                    box = 2 * particlex * (-1); // lower row of boxes
                  if (jj == 1)
                    box = 0; // middle row of boxes
                  if (jj == 2)
                    box = 2 * particlex; // upper row of boxes

                  pos = runParticle->box_pos + (ii - 1) +
                        box; // define box position
                    ...
                    while (neig) {
                      ...
                      if (!neig->done) // if not already done
                      {
                        if (neig->xpos) // just in case
                        {
                          ...
                          //---------------------------------------------------------------
                          // determine the force on particle = constant times
                          // strain
                          //---------------------------------------------------------------
                          ...
                          rep_constant =
                              (runParticle->young + neig->young) / 2.0;
                          if (neig->young == 0.0)
                            rep_constant = 0.0;
                          ...
                          fn = rep_constant *
                               (dd - alen); // normal" force per length"
                                            // (pressure times length)
                          fx = fn * uxi;    //" force x"
                          fy = fn * uyi;    //" force y"
                          ...
                          if (fn < 0.0) // if compressive
                          {
                            ...
                            fn = fx * uxi +
                                 fy * uyi; // force in direction of spring
                            ...
                            //-------------------------------------------------------------
                            // scale normal force by particle area divided by
                            // six because the open surface is now only for one
                            // spring i.e. 1/6 of the particles circumference or
                            // area
                            //-------------------------------------------------------------

                            fn = fn / ((runParticle->radius *
                                        runParticle->radius * Pi) /
                                       6.0);
                            ...
                            runParticle->bound_stress =
                                runParticle->bound_stress + fn;

                            cout << "repulsion stress"
                                 << runParticle->bound_stress << endl;
                          }
                        }
                      }
            ...
            runParticle->surfE = runParticle->surf_free_E * 2.0 /
                                 (runParticle->radius * the_size);
      ...
      if (runParticle->isHoleBoundary) {
        if (!runParticle->is_lattice_boundary) {
          if (1) {
            ...
            lame1 = runParticle->young * pois /
                    (1.0 - (pois * pois) * (1.0 + pois));
            lame2 = runParticle->young / (2.0 + pois);
            ...
            if (runParticle->fluid_neig_count) {
              ...
              for (j = 0; j < runParticle->fluid_neig_count; j++) {
                bound_stress =
                    bound_stress + runParticle->fluid_neigP[j]->bound_stress;
              }

              bound_stress = bound_stress / runParticle->fluid_neig_count;

              // runParticle->bound_stress = (runParticle->bound_stress +
              // bound_stress)/2.0;
            }
            ...
            sum_Energy = runParticle->eEl + runParticle->surfE +
                         runParticle->bound_stress;
            ...
            rate = rate_constant *
                   (1 - (exp(0.002) /
                         exp((sum_Energy) / (gas_const * (temp + 273)))));
    ...
    if (preRunParticle->prob > 1.0) // particle dissolves completely
    {
      ...
      for (i = 0; i < numParticles;
           i++) // and shrink the rest according to time used to dissolve
                // fastest particle
      {
        if (runParticle->isHoleBoundary) {
          if (!runParticle->is_lattice_boundary) {
            //------------------------------------------------------------------------
            // and shrink, area is virtual only for dissolution not for elastic
            // part
            //------------------------------------------------------------------------
            runParticle->area =
                runParticle->area -
      ...
      Adjust_Hole_List();
    } else // in this case all particles shrink according to the given time,
           // none dissolves completely
    {
      for (i = 0; i < numParticles; i++) {
        if (runParticle->isHoleBoundary) {
          if (!runParticle->is_lattice_boundary) {
  ...
  do {
              ...
              while (neig) {
                ...
                if (!neig->done) // if not already done
                {
                  if (neig->xpos) // just in case
                  {
                    ...
                    fn =
                        rep_constant * (dd - alen); // normal" force per length"
                                                    // (pressure times length)
                    fx = fn * uxi;                  //" force x"
                    fy = fn * uyi;                  //" force y"
                    ...
                    if (fn < 0.0) // if compressive
                    {
                      ...
                      //-------------------------------------------------------------
                      // scale normal force by particle area divided by six
                      // because the open surface is now only for one spring
                      // i.e. 1/6 of the particles circumference or area
                      //-------------------------------------------------------------

                      fn = fn /
                           ((runParticle->radius * runParticle->radius * Pi) /
                            6.0);
                      ...
                      // cout << "repulsion stress" << runParticle->bound_stress
                      // << endl;
                    }
                  }
                }
    ...
    for (i = 0; i < numParticles; i++) // second loop through all particles
    {
      if (runParticle->isHoleBoundary) {
          ...
          if (runParticle->leftNeighbour) // if I have a left neighbour
          {
            ...
            for (j = 0; j < 10; j++) // do that for 10 neighbours
            {
              if (help->leftNeighbour) // if there is somebody on the left
              {
                ...
                surface_left[j] =
                    0.0; // no particle means zero surface energy = no influence
            }
          }
        ...
        runParticle->bound_stress = runParticle->bound_stress * pressure_scale *
                                    pascal_scale * runParticle->mV;
        ...
        rate = rate_constant *
               (1.0 - (1.0 / exp((sum_Energy) / (gas_const * (temp + 273.0)))));
    ...
    if (preRunParticle->prob > 1.0) // particle dissolves completely
    {
      ...
      if (shrink == 1) // shrink means shrink the other particles according to
                       // time of fastest particle
      {
      ...
      //----------------------------------------------------------------------------------------
      // now subtract the time needed to dissolve the fastest particle form the
      // internal time
      //----------------------------------------------------------------------------------------
    ...
    } else // if no particle dissolves in the given time step
    {
      if (shrink) // if we shrink particles
      {
        for (i = 0; i < numParticles; i++) {
          if (runParticle->isHoleBoundary) {
            //------------------------------------------------------------------------------
            // shrink particles according to the internal time step and their
            // dissolution rates
            //------------------------------------------------------------------------------
    ...
    Relaxation(); // and relax (something changed in the model if a particle is
                  // dissolved ! )
    ...
    if (dump > 0) {
      if (dump_pict >= dump) {
        ...
        dump_pict = 0;
           ...
           0.0); // use up all the time given by the external step
}
...
void Phase_Lattice::Dissolution_StylosII(int dump, int stress, int use_grains,
                                         int shrink, int sizex) {
  int i, j, ii, jj; // counters for loops
  int dump_pict;
  ...
  float step_model;
  int count_open; // counter for neighbour particles with only two springs,
                  // deletes floating triangles

  internal_time = time;
  dump_pict = 0;

  step_model = (dump / 2.0) - int(dump / 2.0);
  ...
  do {
    ...
    if (runParticle->disconst == 0.0)
      rate_constant = 0.0000001; // rate constant for quartz at 200¡C
    else
      rate_constant = runParticle->disconst;
    ...
    for (i = 0; i < numParticles; i++) {
      ...
      if (runParticle->isHoleBoundary) // if its a hole boundary
      {
            ...
            if (!runParticle->neigP[j]->isHole) {
              if (runParticle->is_boundary && use_grains != 0) {
                if (runParticle->neigP[j]->grain != runParticle->grain ||
                  ...
                  // on that boundary spring (spring has only normal force in
                  // this direction..
                  //----------------------------------------------------------------
                  ...
                  fn = fn /
                       ((runParticle->radius * runParticle->radius * Pi) / 6.0);
              ...
              while (neig) {
                ...
                if (!neig->done) // if not already done
                {
                  if (neig->xpos) // just in case
                  {
                    ...
                    rep_constant =
                        (runParticle->springconst + neig->springconst) / 2.0;
        ...
        runParticle->surfE =
            runParticle->surf_free_E * 2.0 / (runParticle->real_radius);
    ...
    for (i = 0; i < numParticles; i++) {
      if (runParticle->isHoleBoundary) {
        ...
        lame1 = runParticle->E * pois / (1.0 - (2.0 * pois) * (1.0 + pois));
        lame2 = runParticle->E / (2.0 + 2.0 * pois);
        ...
        // runParticle->eEl = 0.5 * lame1 *
        // (runParticle->exx+runParticle->eyy)*(runParticle->exx+runParticle->eyy);
        // runParticle->eEl =
        // runParticle->eEl+(lame2*((runParticle->exx*runParticle->exx)+(runParticle->eyy*runParticle->eyy)+2*(runParticle->exy*runParticle->exy)));

        //---------------------------------------------------------
        // scaling energy
        //---------------------------------------------------------

        runParticle->eEl = runParticle->eEl * runParticle->mV * pressure_scale *
                           pascal_scale * runParticle->radius *
        ...
        if (runParticle->fluid_neig_count) {
          ...
          // runParticle->surfE = (runParticle->surfE * (1-surface_area_factor))
          // + (surfE * surface_area_factor);
        } else
          runParticle->newsurfE = 0.0;
        ...
        /* bound_stress = 0.0;
         ...
         runParticle->fluid_neigP[j]->bound_stress;
             }
        ...
        if (stress == 5) {
          if (count != 0) {
            runParticle->bound_stress =
                runParticle->bound_stress +
                ((meansurfstress / count) * runParticle->surfcount);
        ...
        if (stress == 0)
          sum_Energy = runParticle->eEl + runParticle->newsurfE +
                       runParticle->bound_stress;
        // sum_Energy = runParticle->newsurfE + runParticle->bound_stress;
        else if (stress == 1)
          sum_Energy = runParticle->bound_stress;
        else if (stress == 2)
        ...
        rate = rate_constant *
               (1 - exp(-((sum_Energy) / (gas_const * (temp + 273)))));

        // rate =
        // rate_constant*(1-(1/exp((sum_Energy)/(gas_const*(temp+273))))); //
        // killed exp(0.002)
    ...
    if (preRunParticle->prob > 1.0) {
      preRunParticle->young = 0.0;
      preRunParticle->E = 0.0;
      preRunParticle->rate_factor = 0.0;
      preRunParticle->isHole = true;
      preRunParticle->isHoleBoundary = false;
      ...
      ElleRemoveUnodeFromFlynn(preRunParticle->p_Unode->flynn(),
                               preRunParticle->p_Unode->id());
      ...
      if (shrink == 1) {
        for (i = 0; i < numParticles; i++) {
          if (runParticle->isHoleBoundary) {
            // if (!runParticle->is_lattice_boundary)
            {
              // runParticle->area=runParticle->area -
              // (runParticle->area*runParticle->prob/preRunParticle->prob);
              if (runParticle->area <= 0.0)
                cout << runParticle->area << "negative ARea !" << endl;
            }
      ...
      // delete particles with one spring only and 3 particle triangles

      for (i = 0; i < numParticles; i++)

      {
        if (runParticle->isHoleBoundary) {
          if (runParticle->open_springs > 4) {
            runParticle->young = 0.0;
            runParticle->E = 0.0;
            preRunParticle->rate_factor = 0.0;
            runParticle->isHole = true;
            runParticle->isHoleBoundary = false;
          ...
          } else if (runParticle->open_springs == 4) // check the neighbours
          {
            count_open = 0;
            for (j = 0; j < 8; j++) {
              if (runParticle->neigP[j]) {
                if (runParticle->neigP[j]->open_springs == 4) {
                  count_open++;
                }
              }
            }
      ...
      // end remove particle with one spring

      internal_time = internal_time *
                      (preRunParticle->prob - (1.0 / (particlex / 1.0))) /
                      preRunParticle->prob;
...
void Phase_Lattice::GrowthDissolution(int dump, int use_Box, int change,
                                      int experiment_time) {
  ...
  do {
    ...
    for (i = 0; i < numParticles; i++) {
      if (runParticle->isHoleBoundary) {
        ...
        if (use_Box == 0) // if the box index is zero use the fluid particles
          ...
          for (j = 0; j < 8; j++) {
            if (runParticle->neigP[j]) {
              if (runParticle->neigP[j]->isHole) {
                runParticle->conc =
                    runParticle->neigP[j]
                        ->conc; // get concentration from particle in fluid
                break;
              }
            }
          }
        } else // use the repulsion box concentration
        {
          ...
          {
            rate = rate_constant *
                   (1 - ((runParticle->conc / (2000)) /
                         exp((sum_Energy) / (gas_const * (temp + 273)))));
            if (rate < 0.0) {
              rate =
                  -rate_constant *
                  (1 - ((2200 * exp(sum_Energy / (gas_const * (temp + 273)))) /
                        runParticle->conc));
            }
          }
          ...
          abs_prob = sqrt(runParticle->prob *
                          runParticle->prob); // in order to get highest rate
    ...
    if (preRunParticle->prob > 1.0) // dissolution
    {
...

      {
        ...
        {
          preRunParticle->conc =
              1 / preRunParticle->mV; // concentration at that particle
        } else                        // use box
        {
          if (concBox[preRunParticle->box_pos] ==
              0.0) // if this box is not yet fluid
            old_fl_vol =
                old_fl_vol + 1; // make it fluid without changing fluid volume

          // concBox[preRunParticle->box_pos] = 1/preRunParticle->mV; // set
          // concentration of that box
          concBox[preRunParticle->box_pos] = 20000;
        }
      ...
      internal_time = internal_time *
                      (sqrt(preRunParticle->prob * preRunParticle->prob) -
                       (1.0 / react_factor)) /
                      sqrt(preRunParticle->prob * preRunParticle->prob);
    ...
    else if (preRunParticle->prob < -1.0) // growth
    {
      ...
      if (found) {
        preRunParticle->neigP[j]->isHole =
            false; // and activate that neighour (grows)
        ...
        for (jj = 0; jj < 8; jj++) // loop around the neighbours neighbours
        {
          if (preRunParticle->neigP[j]->neigP[jj]) {
            if (preRunParticle->neigP[j]->neigP[jj]->young > 0.0) {
              //------------------------------------------------------------------------------
              // now activate that particle, give it a youngs modulus and a
              // spring constant
              //------------------------------------------------------------------------------

              preRunParticle->neigP[j]->young =
                  preRunParticle->neigP[j]->neigP[jj]->young;
              preRunParticle->neigP[j]->springf[jj] =
                  preRunParticle->neigP[j]->young;
              ...
              } else // use the box for the concentration
              {
                if (concBox[preRunParticle->neigP[j]->box_pos] != 0.0) {
                  ...
                  concBox[preRunParticle->neigP[j]->box_pos] =
                      0.0; // this is now solid - must dissolve other boxes
                }
              }
              preRunParticle->neigP[j]->dis_time =
                  overall_dis_time; // set damping value
              break;
            }
          }
        }
        ...
        if (use_Box ==
            0) // dont use the box, use the particles for the concentration
        {
          for (jj = 0; jj < 8; jj++) {
            if (preRunParticle->neigP[j]->neigP[jj]) {
              if (preRunParticle->neigP[j]
                      ->neigP[jj]
                      ->isHole) // find a neighbour in the fluid
              {
                // and subtract the concentration used to grow the particle

                preRunParticle->neigP[j]->neigP[jj]->conc =
                    preRunParticle->neigP[j]->neigP[jj]->conc -
                    (1 / preRunParticle->neigP[j]->mV);
                break;
              }
            }
          }
            ...
            for (jj = 0; jj < 3; jj++) {
              ...
              if (concBox[pos]) // if this is active
              {
                if (concBox[pos] > 0.0 && done == false) // if it is fluid
                {
                  ...
                  // concBox[pos] = concBox[pos] + concBox[pos] -
                  // (1/preRunParticle->neigP[j]->mV);
                  concBox[pos] = concBox[pos] + concBox[pos] - 5000;
    ...
    if (use_Box == 0) {
      ...
      Adjust_Conc_Box(fluidchange, 1,
                      internal_time); // adjust concentration of background box
    }
...
void Phase_Lattice::GrowthDissolutionTwo(int dump, int use_Box, int change,
                                         int experiment_time) {
  ...
  do {
    ...
    for (i = 0; i < numParticles; i++) // and loop first time
    {
      ...
      if (runParticle->isHoleBoundary) // if its a hole boundary
      {
        ...
        //    fn = rep_constant * (dd - alen);    // normal" force per length"
        //    (pressure times length) fx = fn * uxi;                    //"
        //    force x" fy = fn * uyi;                    //" force y"
        ...
        //	fn = fn * 2.0 * runParticle->radius;    // scaling to particle
        //size
    ...
    //	{
    //		runParticle->surfE = (runParticle->newsurfE +
    //((runParticle->fluid_neigP[0]->newsurfE+runParticle->fluid_neigP[1]->newsurfE)*0.5))*0.5;
    //	}
    //	else if (runParticle->fluid_neig_count > 0)
    //	{
    //		runParticle->surfE = (runParticle->newsurfE +
    //runParticle->fluid_neigP[0]->newsurfE)*0.5;
    //}
    //}
    ...
    for (i = 0; i < numParticles; i++) {
      if (runParticle->isHoleBoundary) {
        ...
        //     for (j = 0; j<40; j++)  // do that for 10 neighbours
        //      {
        //          if (help->leftNeighbour)   // if there is somebody on the
        //          left
        //         {
        //              surface_left[j] = help->surfE;  // write surface energy
        //              of that particle help = help->leftNeighbour;     // set
        //              help pointer
        //          }
        //          else
        //              surface_left[j] = 0.0;   // no particle means zero
        //              surface energy = no influence
        //       }
        // }
        ...
        //   for (j = 0; j < 4; j++)  // next ten
        //   {
        //       surfE = surfE + surface_left[j] + surface_right[j];  // add all
        //       up
        //      surface_left[j] = surfE;  // and put them in surface_left array
        //  }
        //  surfE = 0.0;
        //  for (j = 0; j < 4; j++)  // and do mean and scaling
        //  {
        //     surfE = surfE + (surface_left[j]/((((j+1)*2)+1)*(((j+1)*2)+1)));
        //  }

        //   runParticle->newsurfE = runParticle->surfE + surfE; // and add
        //   particle itself
        ...
        // runParticle->bound_stress = runParticle->bound_stress *
        // pressure_scale * pascal_scale * runParticle->mV;
    ...
    for (i = 0; i < numParticles; i++) {
      if (runParticle->isHoleBoundary) {
        if (runParticle->prob >= 0.0 && full[runParticle->box_pos] == 0) {
          concBox[runParticle->box_pos] =
              concBox[runParticle->box_pos] + (8000 * runParticle->prob);

          full[runParticle->box_pos] = 1;

          cout << runParticle->prob << endl;
          cout << concBox[runParticle->box_pos] << endl;
        }
      }
    ...
    max_y = int(2 * particlex * 2) * particlex; // size of box
    ...
    if (preRunParticle->prob > 1.0) // dissolution
    {
...

      {
        ...
        } else                        // use box
        {
          ...
          // concBox[preRunParticle->box_pos] = 1/preRunParticle->mV; // set
          // concentration of that box
          concBox[preRunParticle->box_pos] = 8000;
        }
    ...
    // if (found)
    //{
    // preRunParticle->neigP[j]->isHole = false;  // and activate that neighour
    // (grows)
    ...
    //{
    ////------------------------------------------------------------------------------
    //// now activate that particle, give it a youngs modulus and a spring
    ///constant
    ////------------------------------------------------------------------------------
    ...
    // must dissolve other boxes
    //}
    //}
    // preRunParticle->neigP[j]->dis_time = overall_dis_time; // set damping
    // value break;
    //}
    ...
    // if (use_Box == 0)  // dont use the box, use the particles for the
    // concentration
    //{
    // for (jj = 0; jj<8; jj++)
    //{
    // if (preRunParticle->neigP[j]->neigP[jj])
    //{
    // if (preRunParticle->neigP[j]->neigP[jj]->isHole) // find a neighbour in
    // the fluid
    //{
    //// and subtract the concentration used to grow the particle
    ...
    // preRunParticle->neigP[j]->neigP[jj]->conc =
    // preRunParticle->neigP[j]->neigP[jj]->conc -
    // (1/preRunParticle->neigP[j]->mV); break;
    //}
    //}
    //}
...
void Phase_Lattice::Adjust_Conc_Box(int change, int diffusion, double timedif) {
  int i, ii, jj;      // loop counter
  ...
  if (diffusion) {
    for (ii = 0; ii < 400; ii++) {
      for (jj = 0; jj < 200; jj++) {
        ...
        if (repBox[ii + (jj * particlex)]) {
          if (repBox[ii + (jj * particlex)]->isHole ||
              repBox[ii + (jj * particlex)]
                  ->isHoleBoundary) // if first particle in box is hole
          {
            conc_f[ii][jj] =
                concBox[ii + (jj * particlex)]; // concbox is also hole->give
                                                // concentration
            fluid[ii][jj] = 1;
          } else {
            help_P =
                repBox[ii +
                       (jj * particlex)]; // loop through tail particles in box
            while (help_P->next_inBox)    // as long as there is a tail
            {
              if (help_P->next_inBox->isHole ||
                  help_P->next_inBox->isHoleBoundary) // is that fluid ?
              {
                conc_f[ii][jj] =
                    concBox[ii + (jj * particlex)]; // concbox is also
                                                    // hole->give concentration
                fluid[ii][jj] = 1;
                break; // and exit
              }
    ...
    // SolveDiffusionExplicit(timedif, runParticle->radius*the_size,
    // 0.000000001);
    SolveDiffusionExplicit(1, 1, 0.01);
          ...
          if (repBox[ii + (jj * particlex)]) {
            if (repBox[ii + (jj * particlex)]->isHole ||
                repBox[ii + (jj * particlex)]
                    ->isHoleBoundary) // if first particle in box is hole
            {
              concBox[ii + (jj * particlex)] =
                  conc_f[ii][jj]; // concbox is also hole->give concentration
              repBox[ii + (jj * particlex)]->conc =
                  concBox[ii + (jj * particlex)];
            } else {
              help_P = repBox[ii + (jj * particlex)]; // loop through tail
                                                      // particles in box
              while (help_P->next_inBox) // as long as there is a tail
              {
                if (help_P->next_inBox->isHole ||
                    help_P->next_inBox->isHoleBoundary) // is that fluid ?
                {
                  concBox[ii + (jj * particlex)] =
                      conc_f[ii]
                            [jj]; // concbox is also hole->give concentration
                  help_P->next_inBox->conc = concBox[ii + (jj * particlex)];
  ...
  if (change == 1) {
    ...
    delta_p =
        float(((float)old_fl_vol - (float)fluid_volume) / (float)fluid_volume);
...
void Phase_Lattice::SolveDiffusionExplicit(double time, double space,
                                           double difconstant) {
  int i, j;
  double dif;
  ...
  //
  // first define background and fix boundaries. Boundaries we may treat
  ...
  for (i = 0; i < dim; i++) {
    for (j = 0; j < dim; j++) {
      ...
      // if 	(fluid[i][j]==1)
      // ddyc[i][j] = 0.08; // define boundaries as two times background
      // concentration (later divided by 2)
    }
  }
  ...
  for (i = 1; i < dim - 1; i++) {
    for (j = 1; j < dim - 1; j++) {
      ...
      {
        dif =
            difconstant; // diffusion coefficient for fluid. These are the same
                         // for Fe and Mg now, but we can easily change that

        // first do x which is i

        if (fluid[i + 1][j] == 0 &&
            fluid[i - 1][j] == 0) // both neighbours in x are not fluid
          ddyc[i][j] = conc_f[i][j];

        else if (fluid[i + 1][j] == 0 && fluid[i - 1][j] == 1)
          ddyc[i][j] = conc_f[i][j] +
                       (time * dif / pow(space, 2.0)) *
                           (conc_f[i - 1][j] -
                            conc_f[i][j]); // backwards a fluid and forward not

        else if (fluid[i - 1][j] == 0 && fluid[i + 1][j] == 1)
          ddyc[i][j] = conc_f[i][j] +
                       (time * dif / pow(space, 2.0)) *
                           (conc_f[i + 1][j] -
                            conc_f[i][j]); // forward a fluid and backward not

        else if (fluid[i - 1][j] == 1 && fluid[i + 1][j] == 1)
          ddyc[i][j] =
              conc_f[i][j] +
              (time * dif / pow(space, 2.0)) *
                  (conc_f[i - 1][j] - 2 * conc_f[i][j] +
                   conc_f[i + 1][j]); // normal case, both neighbours fluid
        ...
        if (fluid[i][j + 1] == 0 &&
            fluid[i][j - 1] == 0) // both neighbours in y are not fluid
          ddyc[i][j] = conc_f[i][j] + ddyc[i][j];

        else if (fluid[i][j + 1] == 0 && fluid[i][j - 1] == 1)
          ddyc[i][j] = ddyc[i][j] + conc_f[i][j] +
                       (time * dif / pow(space, 2.0)) *
                           (conc_f[i][j - 1] -
                            conc_f[i][j]); // backwards fluid and forward not

        else if (fluid[i][j - 1] == 0 && fluid[i][j + 1] == 1)
          ddyc[i][j] = ddyc[i][j] + conc_f[i][j] +
                       (time * dif / pow(space, 2.0)) *
                           (conc_f[i][j + 1] -
                            conc_f[i][j]); // forward fluid and backward not

        else if (fluid[i][j - 1] == 1 && fluid[i][j + 1] == 1)
          ddyc[i][j] =
              ddyc[i][j] + conc_f[i][j] +
              (time * dif / pow(space, 2.0)) *
                  (conc_f[i][j - 1] - 2 * conc_f[i][j] +
                   conc_f[i][j + 1]); // normal case, both neighbours fluid
  ...
  for (i = 1; i < dim - 1; i++) {
    for (j = 1; j < dim - 1; j++) {
      conc_f[i][j] = 0.5 * ddyc[i][j]; // fill the original matrix
    }
  }
}
...
void Phase_Lattice::Set_Dis_Time(int time) { overall_dis_time = time; }
...
void Phase_Lattice::Make_Concentration_Box(float ymax, float concentration) {
  ...
  for (i = 0; i < max_y; i++) // run through box
  {
    ...
    {
      if (repBox[i]->isHole || repBox[i]->isHoleBoundary) // if that is a hole
      {
      ...
      } else                             // if not go along tail
      {
        ...
        while (
            help_P->next_inBox) // as long as there is somebody along the tail
        {
          if (help_P->next_inBox->isHole ||
              help_P->next_inBox->isHoleBoundary) // if that is a hole particle
          {
...
void Phase_Lattice::Fill_Fluid(float ymax, float concentration) {
  int ii, jj;
  Particle *help_P;
  ...
  for (ii = 0; ii < 200; ii++) // run through box
  {
    for (jj = 0; jj < 200; jj++) // run through box
    {
      //--------------------------------------------------------
      // concBox is the concentration box
      // repBox is the repulsion box that contains pointers
      ...
      if (repBox[ii +
                 (jj * particlex)]) // if there is someone in the Repulsion box
      {
        if (repBox[ii + (jj * particlex)]->isHole) // if that is a hole
        {
          conc_f[ii][jj] = box_conc; // concbox is also hole->give concentration
          fluid[ii][jj] = 1;
        } else // if not go along tail
        {
          help_P = repBox[ii + (jj * particlex)]; // helper for the loop
          while (
              help_P->next_inBox) // as long as there is somebody along the tail
          {
            ...
            {
              conc_f[ii][jj] =
                  box_conc; // concbox is also hole->give concentration
              fluid[ii][jj] = 1;
              break;
            }
            help_P = help_P->next_inBox;
...
void Phase_Lattice::DumpStressRow(double y_box_min, double y_box_max,
  ...
  // finite_strain = strain * (def_time_u + def_time_p);   // calculate finite
  // strain
              ...
              if (!runParticle->isHole) {
                if (!runParticle->isHoleBoundary) {
                  ...
                  if (runParticle->young != 0.0) {
                    //-------------------------------------------------------------------
                    // first calculate the two lame constants from the youngs
                    // modulus of a particle and the poisson number of the model
                    //-------------------------------------------------------------------
                    ...
                    //------------------------------------------------------------------------
                    // calculate the components of the infinitesimal strain
                    // tensor from the components of the stress tensor for the
                    // particle
                    //------------------------------------------------------------------------
                    ...
                    //------------------------------------------------------------------------
                    // and now calculate the elastic energy for a single
                    // particle using the two lame constants and the components
                    // of the infinitesimal strain tensor of each particle
                    //------------------------------------------------------------------------
                    ...
                    //-------------------------------------------------------------------------
                    // and multiply by the molecular volume plus a scaling
                    // parameter for the stress. The stress scaling is set with
...
void Phase_Lattice::DumpYStressRow(double y_box_min, double y_box_max,
                                   double x_box_min, double x_box_max,
                                   int xrow) {
  FILE *stat;                      // file pointer
  int i, j, jj, p_counter;         // counters
            ...
            if (!runParticle->is_lattice_boundary) {
              if (!runParticle->isHole) {
                ...
                if (runParticle->young != 0.0) {
                  ...
                  lame1 = runParticle->young * pois /
                          (1.0 - (2.0 * pois) * (1.0 + pois));
                  lame2 = runParticle->young / (2.0 + 2.0 * pois);

                  //------------------------------------------------------------------------
                  // calculate the components of the infinitesimal strain tensor
                  // from the components of the stress tensor for the particle
                  //------------------------------------------------------------------------
                  ...
                  //------------------------------------------------------------------------
                  // and now calculate the elastic energy for a single particle
                  // using the two lame constants and the components of the
                  // infinitesimal strain tensor of each particle
                  //------------------------------------------------------------------------
...
void Phase_Lattice::Make_Circular_Pore(float radius, float shift) {
  int i, j; // counters
  float help;
...
void Phase_Lattice::Grow(float prob) {
  int i, j, c; // counters
  float help;
  float ran_nb;
  ...
  for (i = 0; i < numParticles; i++) // loop through all particles
  {
    if (runParticle->fluid_particles > 0) {
      help = 1 / runParticle->fluid_particles;
    } else
      help = 8;
    ...
    for (j = 0; j < 6; j++) {
      if (runParticle->neigP[j]) {
        ...
        if (runParticle->neigP[j]->fluid_particles == 8)
          c = 1;
      }
    }
    if (c) {
      ...
      if (help < (ran_nb * prob)) // if particle is in the hole
      {
        runParticle->nofluid = true;
        runParticle->fluid_particles = 8;
...
void Phase_Lattice::UpdateFluidLatticeGas() {
  ...
  for (i = 0; i < numParticles; i++) {
    for (j = 0; j < 6; j++) {
      if (runParticle->neigP[j]) {
        runParticle->fluid_particle[j] =
            runParticle->neigP[j]
                ->fluid_particle_new[runParticle->neig_spring[j]];
      }
    }
...
void Phase_Lattice::UpdateFluidLatticeGasGravity() {
  int i, j, count;
  bool done;
  float ran_nb;
      ...
      for (j = 0; j < 6; j++) {
        if (runParticle->neigP[j]) {
          if (runParticle->fluid_particle[j] == 1) {
            ...
            if (ran_nb > 0.5)
            // if (runParticle->phase == 1)
            {
              if (j > 0) {
                if (runParticle->neigP[4]) {
                  if (!runParticle->neigP[4]->nofluid) {
                    runParticle->fluid_particle_new[4] = 1;
                    done = true;
                  }
                } else if (runParticle->neigP[5]) {
                  if (!runParticle->neigP[5]->nofluid) {
                    runParticle->fluid_particle_new[5] = 1;
                    done = true;
                  }
                } else if (runParticle->neigP[j - 1]) {
                  if (!runParticle->neigP[j - 1]->nofluid) {
                    runParticle->fluid_particle_new[j - 1] = 1;
                    done = true;
                  }
