...
void Experiment::SetUpFromFile() {
  ...
  for (i = 0; i < 50; i++) {
    if (setdata.exec[i]) {
      switch (i) {
      case 0:
        SetPhase(setdata.parameters[0][0], setdata.parameters[0][1],
                 setdata.parameters[0][2], setdata.parameters[0][3]);
        break;

      case 1:
        SetGaussianSpringDistribution(setdata.parameters[1][0],
                                      setdata.parameters[1][1]);
        break;

      case 2:
        SetGaussianRateDistribution(setdata.parameters[2][0],
                                    setdata.parameters[2][1]);
        break;

      case 3:
        SetGaussianStrengthDistribution(setdata.parameters[3][0],
                                        setdata.parameters[3][1]);
        break;
      ...
      case 6:
        SetWallBoundaries(int(setdata.parameters[6][0]),
                          setdata.parameters[6][1]);
        break;

      case 7:
        WeakenAll(setdata.parameters[7][0], int(setdata.parameters[7][1]),
                  setdata.parameters[7][2]);
        break;

      case 8:
        WeakenHorizontalParticleLayer(
            setdata.parameters[8][0], setdata.parameters[8][1],
            setdata.parameters[8][2], setdata.parameters[8][3],
            setdata.parameters[8][4]);

        // do it twice, if necessary
        if (setdata.parameters[8][5] != 0.0)
          WeakenHorizontalParticleLayer(
              setdata.parameters[8][5], setdata.parameters[8][6],
              setdata.parameters[8][7], setdata.parameters[8][8],
              setdata.parameters[9][9]);
        break;
      ...
      case 14:
        DissolveYRow(setdata.parameters[14][0], setdata.parameters[14][1],
                     true);
        break;

      case 16:
        WeakenHorizontalParticleLayer(
            setdata.parameters[16][0], setdata.parameters[16][1],
            setdata.parameters[16][2], setdata.parameters[16][3],
            setdata.parameters[16][4]);
        break;
      ...
      case 17:
        if (!rundata.exec[15])
          ...
          SetGaussianYoungDistribution(setdata.parameters[17][0],
                                       setdata.parameters[17][1]);
        else
          // if transition is activated. see above.
          SetGaussianYoungDistribution_2(setdata.parameters[17][0],
                                         setdata.parameters[17][1]);
        break;

      case 18:
        WeakenGrain(int(setdata.parameters[18][3]), setdata.parameters[18][0],
                    setdata.parameters[18][1], setdata.parameters[18][2]);
        break;
      ...
      case 21:
        for (j = 0; j < 100; j++) {
          SetFluidLatticeGasRandomGrain(setdata.parameters[21][0],
                                        j * int(setdata.parameters[21][1]));
        }
        break;
      case 22:
        for (j = 0; j < 500; j++) {
      ...
      case 25:
        ...
        HeatGrain(int(setdata.parameters[25][1]),
                  int(setdata.parameters[25][0]));
        heat_distribution.SetHeatFlowParameters(4.2, 1005.0, 0.000001,
                                                1000.0); // rho, c, diffusivity,
        SetHeatLatticeHeatFlowExample();
        Set_Absolute_Box_Size(setdata.parameters[11][0]);
        break;
      case 30:
          // SetSinAnisotropy( int( setdata.parameters[ 30 ][ 0 ] ),
          // setdata.parameters[ 30 ][ 1 ] );
          ;
        break;
      case 31:
...
void Experiment::RunFromFile(int experiment_time) // i == timestep
{
  ...
  for (k = 0; k < 50; k++) {
    if (rundata.exec[k]) {
      ...
      case 5:
        ...
        break;
      case 7:
        Dissolution_StylosII(int(rundata.parameters[28][0]), 0,
                             int(rundata.parameters[7][0]),
                             int(rundata.parameters[7][1]), 1);
        break;
      case 16:
  ...
  for (k = 0; k < 50; k++) {
    if (statdata.exec[k]) {
      switch (k) {
      case 0:
        DumpStatisticStressBox(
            statdata.parameters[0][0], statdata.parameters[0][1],
            statdata.parameters[0][2], statdata.parameters[0][3], strain);
        break;
      case 1:
        DumpStatisticStressGrain(strain, int(statdata.parameters[1][0]));
        break;
      case 2:
        DumpStatisticStressTwoGrains(strain, int(statdata.parameters[2][0]),
                                     int(statdata.parameters[2][1]));
        break;
      case 3:
        DumpTimeStatisticSurface(strain, (statdata.parameters[3][0]),
                                 (statdata.parameters[3][1]));
        break;
      case 4:
        DumpStressRow(statdata.parameters[0][0], statdata.parameters[0][1],
                      statdata.parameters[0][2], statdata.parameters[0][3],
                      int(statdata.parameters[0][4]));
        break;
      case 5:
        DumpYStressRow(statdata.parameters[0][0], statdata.parameters[0][1],
                       statdata.parameters[0][2], statdata.parameters[0][3],
                       int(statdata.parameters[0][4]));
        break;
      case 6:
