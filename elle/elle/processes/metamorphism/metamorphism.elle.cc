...
int min_names[6] = {0, 5, 4, 1,
                    2, 3}; // switch mineral names to match modes.txt order (Grt
                           // Opx	Cpx	Pl	q )
double modal[1000][6];     // data from modes.txt
double
    modal_out[6]; // linear interpolation of modal data for current temperature
double surface_energy[5][3] = {.05,  .05, 0.05, .05,  .05, 0.05, .05, .05,
                               0.05, .05, .05,  0.05, .05, .05,  0.05};
...
int InitMet() {
...
#ifdef DEBUG
  tmp = 1028731772; // uncomment define of DEBUG to allow specified random
                    // number seed
#endif
  cout << "seed= " << tmp << endl;
  srand(tmp);
   ...
   * It would probably be nicer to generate PT modal grids for each phase so we
   * can wander aimlessly around PT space
   */
  modes = fopen("modes.txt", "r");

  i = 0;
  do {
    err = fscanf(modes, "%lf %lf %lf %lf %lf %lf", &modal[i][0], &modal[i][1],
                 &modal[i][2], &modal[i][3], &modal[i][4], &modal[i][5]);
    i++;
  } while (err > 0);
...
/*!
 \brief	principal calculation of multiple phase reactions based on input
 thermodynaic data from Perplex \par	Description:

                This calculation is based on the melt code but includes the
 possibility of evolving multiple phases based on the calculated equilibrium
 modal % based on Perplex calculations, which at present have to be done before
 hand, The code loops through each phase indivisually to try and move two-phase
 boundaries so as to try and minimise the area misfit compared to the
 equilibrium area. As present the surface energies are set by an internal
 surface energy array, however this should be externalised. The PID control
 using the fudge factor also needs to be made more robust. The behaviour of this
 system as a phase is about to disappear is not very stable and needs to be
 modified so that multiple smaller timesteps replace the single large one. The
 method of defining which phases are present is all very ad hoc, but we need to
 define the nature of relationship with Perplex before we can improve this.

                usage elle_metamorphism -i filename -s number of time steps -f
 interval to save elle files -u mobility_of_grain_boundaries fudge_factor
 incremental_Temp incremental_Pressure

                e.g.

                elle_metamorphism -i eqm_900.elle -s 300 -f 25 -u 1e-10 .001 0.5
 0
...
int Metamorphism() {
  ...
  int MaxPhase = 85; // ID of highest phase used (assumes all phases betweeen 81
                     // & MaxPhase are present, will have to be redone!)
  double phasephaseEnergy, hostphaseEnergy,
      hosthostEnergy; // (surface energy pairs three types of boundary)
  ...
  if (ElleCount() == 0)
    ElleAddDoubles(); // add double nodes if gaps too large
  if (ElleDisplay())
    EllePlotRegions(ElleCount()); // display input file
  ...
  for (k = 0; k < max;
       k++) { // checks to make sure double node & triple node spacing OK
  ...
  modal_stats =
      fopen("modal_stats.txt", "w"); // file to store output modal % stats
                                     // (reset each time process is run)

  for (i = 0; i < EllemaxStages(); i++) // loop through for all time steps
  {
    temp = ElleTemperature();    // get current T conditions from Elle file
    for (j = 0; j < no_pts; j++) // calc modal % for current PT conditions using
                                 // linear interpolation
    {
      if ((modal[j][0] > temp && modal[j + 1][0] <= temp) ||
          (modal[j][0] < temp && modal[j + 1][0] >= temp)) {
        ratio = (modal[j][0] - temp) / (modal[j][0] - modal[j + 1][0]);

        for (k = 1; k <= no_modes; k++)
          modal_out[k] =
              (ratio * (modal[j + 1][k] - modal[j][k])) + modal[j][k];
        break;
      }
    }
    ...
    for (PhaseId = 81; PhaseId <= MaxPhase;
         PhaseId++) // for each phase, update gb positions
    {

      EquilPhaseFrac =
          modal_out[min_names[PhaseId - 80]] /
          100; // convert predicted interpolated modal % to decimal form
      areaphase = Calcareaphase(PhaseId); // get current area of specified phase

      phasephaseEnergy =
          surface_energy[PhaseId - 81]
                        [0]; // read surface energies from table, this will need
                             // to read an external database
      hostphaseEnergy = surface_energy[PhaseId - 81][1];
      hosthostEnergy = surface_energy[PhaseId - 81][2];

      if ((fabs(areaphase - EquilPhaseFrac)) / EquilPhaseFrac >
          upper) // update damping factor up or down
      ...
      if ((fabs(areaphase - EquilPhaseFrac)) / EquilPhaseFrac > upper)
        printf("T phase i areaphase equil %%diseq fudge : %lf %d %d %lf %lf "
               "%lf %lf\n",
      ...
      fprintf(modal_stats, " %lf %lf", EquilPhaseFrac,
              areaphase); // dump predicted vs actual data for this phase

      max = ElleMaxNodes();

      random_shuffle(seq.begin(),
                     seq.end()); // randomise order of node selection
      ...
      for (n = 0; n < max; n++) {
        ...
        {
          Phase_MoveNode(k, &incr, &same, areaphase, CurrData[Mobility],
                         CurrData[Magic], EquilPhaseFrac, phasephaseEnergy,
                         hostphaseEnergy, hosthostEnergy); // calc node movement

          ElleCrossingsCheck(
              k, &incr); // actually move the node, and check for crossings
        }
      }

      for (j = 0; j < max;
           j++) { // checks to make sure double node & triple node spacing OK
...
int Phase_MoveNode(int n, Coords *movedir, int *same, double areaphase,
  ...
  // so far there are no body energies in this model, so these values are always
  // zero
  ...
  for (i = 0; i < 4; i++) {
    ...
    tryenergy =
        (bodyenergy - startbodyenergy) + (surfaceenergy - startsurfaceenergy);
  ...
  if (dEdX > 0.0) {
    ...
    // printf("io=%le temp=%le dE=%le mob=%le velocity=%le distance=%le x=%le
    // y=%le\n",
...
int GetBodyNodeEnergy(int n,
                      double *total_energy) // not used in this process (yet)
{
  int i, nb[3];
  ERegion rgn[3];
  double sum_energy;
...
int GetSurfaceNodeEnergy(int n, int meltmineral, Coords *xy, double *energy,
  ...
  for (i = 0; i < 3; i++) {
    if (nb[i] != NO_NB) {
      ...
      if (ElleFlynnHasAttribute(rgn2, CAXIS) &&
          ElleFlynnHasAttribute(rgn3,
                                CAXIS)) // calculate misorientations between
                                        // grains (currently not used)
      {
      ...
      if (cosalpha >= 0.0)
        ...
        alpha = acos(-cosalpha);
      E += len * GetAngleEnergy(alpha) * csl_factor * energyofsurface *
           lengthscale;

      // if(val2 != PhaseId || val3 != PhaseId)
      //	printf("alpha xyz[0].x xyz[0].y :%lf %lf
  ...
  E += fabs((areaphase - areaequil) / areaequil) *
       fudge; // area balancing fudge to ensure global melt fraction is roughly
              // stable
