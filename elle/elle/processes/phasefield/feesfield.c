...
double **
dmatrix(long nrl, long nrh, long ncl,
        long nch); /* added from numerical recipies to avoid >32k error */
...
main(int argc, char *argv[]) {
  ...
  /* initial configuration: T=0 and theta=1 in a cercle of radius 3H (0
   * elsewhere). */
  ...
  for (i = 0; i < NX; i++)
    for (j = 0; j < NY; j++) {
      T[i][j] = 0.0;
      theta[i][j] =
          ((i - NX / 2) * (i - NX / 2) + (j - NY / 2) * (j - NY / 2) < 10)
              ? 1.0
              : 0.0;
    }
  ...
  for (t = 0; t < NTIME; t++) {
    ...
    for (i = 0; i < NX; i++) {
      for (j = 0; j < NY; j++) {
        ...
        lap_theta[i][j] =
            (2.0 * (theta[ip][j] + theta[im][j] + theta[i][jp] + theta[i][jm]) +
             theta[ip][jp] + theta[im][jm] + theta[im][jp] + theta[ip][jm] -
             12.0 * theta[i][j]) /
            (3.0 * H * H);
        lap_T[i][j] =
            (2.0 * (T[ip][j] + T[im][j] + T[i][jp] + T[i][jm]) + T[ip][jp] +
             T[im][jm] + T[im][jp] + T[ip][jm] - 12.0 * T[i][j]) /
            (3.0 * H * H);
    ...
    for (i = 0; i < NX; i++) {
      for (j = 0; j < NY; j++) {
        ...
        scal =
            grad_eps2.x * grad_theta[i][j].x + grad_eps2.y * grad_theta[i][j].y;

        /* Euler step */
        theta[i][j] += (dxdy + dydx + eps2[i][j] * lap_theta[i][j] + scal +
                        tet * (1.0 - tet) * (tet - 0.5 + m)) *
                       DT / TAU;
   ...
   * for (i=0;i<NX;i++) for (j=0;j<NY;j++) fprintf(fp," %d %d
   * %e\n",i,j,theta[i][j]);	fclose(fp);	sprintf(file,"T.dat"); fp =
   * fopen(file,"w");	for (i=0;i<NX;i++) for (j=0;j<NY;j++) fprintf(fp," %d %d
   * %e\n",i,j,T[i][j]);	fclose(fp); */

  sprintf(file, "theta.txt");
  fp = fopen(file, "w");
  for (i = 0; i < NX; i++) {
    for (j = 0; j < NY; j++)
