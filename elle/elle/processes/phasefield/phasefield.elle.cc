...
double get_angle(double x, double y) {
  double angle;
  if (x == 0.0)
    angle = (y > 0.0 ? 0.5 * PI : -0.5 * PI);
  else if (x > 0.0)
    angle = (y > 0.0 ? atan(y / x) : 2.0 * PI + atan(y / x));
  else
    angle = PI + atan(y / x);
  return angle;
};
...
// Renamed epsilon -> epsilon_pf and epsilon_prime -> epsilon_prime_pf to
// avoid multiple definition clash with global symbol 'epsilon' in core (triangle.c)
double **ax, **ay, epsilon_pf, epsilon_prime_pf, anglex, tet, dydx, dxdy, scal;
...
int InitSetUnodes() {
  ...
  if (strlen(infile) > 0) {
    ...
    /* initial configuration: T=0 and theta=1 in a circle of radius 3H (0
     * elsewhere). */
  ...
  {
    {
      ...
      for (i = 0; i < NX; i++) {
        for (j = 0; j < NY; j++) {
          ...
          /* gradient and laplacians (order H^2, 9 points isotropic version for
           * laplacians) */
          grad_theta[i][j].x = (theta[ip][j] - theta[im][j]) / H;
          grad_theta[i][j].y = (theta[i][jp] - theta[i][jm]) / H;

          lap_theta[i][j] = (2.0 * (theta[ip][j] + theta[im][j] + theta[i][jp] + theta[i][jm]) + theta[ip][jp] +
                             theta[im][jm] + theta[im][jp] + theta[ip][jm] - 12.0 * theta[i][j]) /
                            (3.0 * H * H);
          lap_T[i][j] = (2.0 * (T[ip][j] + T[im][j] + T[i][jp] + T[i][jm]) + T[ip][jp] + T[im][jm] + T[im][jp] +
                         T[ip][jm] - 12.0 * T[i][j]) /
                        (3.0 * H * H);

          /* angular dependence of the interfacial width "espilon" */
          anglex = get_angle(grad_theta[i][j].x, grad_theta[i][j].y);
          epsilon_pf = EPS * (1.0 + DELTA * cos(ANISO * (anglex - ANGLE0)));
          epsilon_prime_pf = -EPS * ANISO * DELTA * sin(ANISO * (anglex - ANGLE0));

          /* auxiliary quantities appearing in the phase field equation */
          ay[i][j] = -epsilon_pf * epsilon_prime_pf * grad_theta[i][j].y;
          ax[i][j] = epsilon_pf * epsilon_prime_pf * grad_theta[i][j].x;
          eps2[i][j] = epsilon_pf * epsilon_pf;
        }
      }
      ...
      for (i = 0; i < NX; i++) {
        for (j = 0; j < NY; j++) {
          ...
          /* Euler step */
          theta[i][j] +=
              (dxdy + dydx + eps2[i][j] * lap_theta[i][j] + scal + tet * (1.0 - tet) * (tet - 0.5 + m)) * DT / TAU;
