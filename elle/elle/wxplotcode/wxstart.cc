...
// modernize: include <locale.h> for setlocale usage (analysis fast-track)
#include <locale.h>
  ...
  if (strlen(ElleFile()) > 0) {
    ...
    if (wxFileExists(filename)) {
      ...
      if (wxFileExists(filename)) {
        ...
        Log(0,
            "Could not load zip-archive, because it does not exist... Sorry!");
    }
...
int FillPolygon(int index, float *x, float *y, int *num, int *col,
                int *outline) {
  char logbuf[4096];
  ...
  if (index != -1) {
      ...
      /*        else
              {
                  ...
                  CartesianToPolar( coordval.x, coordval.y, coordval.z, & theta,
         & phi ); sprintf(logbuf,"flynn:%d
         theta:%lf,phi:%lf\n",index,theta,phi); Log(2,(const char *)logbuf);
              }*/
    }
    if (!color || index == -1)
    ...
    // else
    //    wxGetApp().mframe->panel->GLDrawPolygon( coords, * num, r, g, b );
    //    //fill
  }
  ...
  if (rainbow) {
    if (display_options->GetRainStages() != ElleCount())
      if (display_options->GetRainStages() + display_options->GetRainChange() ==
          ElleCount()) {
...
lb ); //boundary attribute//and outline*/
  ...
  case 2:
    wxGetApp().mframe->panel->GLDrawLines(coords, *num, lr, lg,
                                          lb); // boundary attribute//outline
    break;
  case 3:
    wxGetApp().mframe->panel->GLDrawLinesBound(coords, *num, r, g,
                                               b); // boundary attribute
    break;
  default: // do nothing
    break;
  }
...
int drawcircle_(int dd, float *x, float *y, float *size, int *col,
                int *outline) {
  int r, g, b, set;
  double xx, yy;
  xx = *x;
...
int plotu_(float *x, float *y, int *w) { return (0); }

int setpencolor_(int *col) { return (0); }

int drawlabel_(float *x, float *y, int *x_justify, int *y_justify, int *mode,
               char *label, int *len) {
  // No-op label drawing in this backend; return success to avoid warnings.
  return 0;
}
...
void SetStage(int stages) {
  ...
  if (display_options->GetSavePic()) {
    ...
    if (rest.rem == 0) {
      ...
      // modernize: convert wxString to UTF-8 for printf-style formatting
      // (analysis fast-track)
      {
        auto fnUtf8 = filename.ToUTF8();
        snprintf(cc, sizeof(cc), "Filename pics: %s", fnUtf8.data());
      }
      Log(0, (const char *)cc);
...
void update_() {
  ...
  if (wxGetApp().IsMainLoopRunning())
#else // replacement code for old version
  if (wxGetApp().Initialized() && wxGetApp().mframe && wxGetApp().mframe->panel)
#endif
  {
...
void warning_msg(int i, const char *msg) {
  ...
  mesg = msg;
  wxLogMessage(
      wxT("This comes from warning_msg in wxstart.cc!!!!!!!!!!!!!!!!"));
  wxLogMessage(wxT("%s"), msg);
}

Settings *GetUserOptions() { return (user_options); }

DSettings *GetDisplayOptions() { return (display_options); }

// NOTE: Do not define Log() here; it is provided by elle_core (NODISP stub).
...
void SavePic() {
  ...
  if (file != wxT("")) {
    ...
    name.Prepend(file);
    path.Append(name);
    // modernize: convert wxString to UTF-8 when writing to C buffer (analysis
    // fast-track)
    {
      auto pathUtf8 = path.ToUTF8();
      snprintf(f, sizeof(f), "%s", pathUtf8.data());
    }
    // GUI uses core's Log() implementation; do not define another here.
  }
  // Nothing else to do here; function kept for compatibility
}
