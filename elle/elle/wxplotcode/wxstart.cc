...
void SetStage( int stages )
{
    ...
    if(display_options->GetSavePic())
    {
        ...
        if(rest.rem==0)
        {
            ...
            sprintf(cc,"Filename pics: %s",(const char*)filename.mb_str());
            Log(0,(const char *)cc);
...
void SavePic()
{
    ...
    if ( file != wxT("" ))
    {
        ...
        name.Prepend( file );
        path.Append( name );
        snprintf( f, sizeof(f), "%s", (const char*)path.mb_str() );
        Log( 1, (const char *)f );
                  ...
                  int decimate)
{
    ...
    i=pixmap.LoadFile(wxString::FromUTF8(fname),wxBITMAP_TYPE_ANY);
	if ( i== 0)
    {
