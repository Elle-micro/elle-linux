...
BEGIN_EVENT_TABLE(TableData, wxFrame)
EVT_TOOL(Tsave, TableData::OnSaveData)
EVT_TOOL(Tmpoly, TableData::OnMarkPolygon)
EVT_TOOL(TSaveSelected, TableData::OnSaveSelected)
EVT_TOOL(Tedit, TableData::OnEdit) EVT_TOOL(Tdselect, TableData::OnDeselectRows)
    EVT_GRID_LABEL_LEFT_CLICK(TableData::OnClickLabel)
        EVT_GRID_CELL_LEFT_CLICK(TableData::OnCellEdit)
            EVT_CLOSE(TableData::EndSession) END_EVENT_TABLE()

                TableData::TableData(wxWindow *parent, int id, wxString title)
    : wxFrame(parent, id, title, wxPoint(100, 100), wxSize(430, 380)) {
  ...
  toolbar->AddTool(Tsave, "Save data", *(toolbaricon[0]),
                   "Save data to seperate file", wxITEM_NORMAL);
  toolbar->AddTool(TSaveSelected, "Save selected data", *(toolbaricon[1]),
                   "Save only selected data", wxITEM_NORMAL);
  toolbar->AddTool(Tmpoly, "Mark polygon", *(toolbaricon[2]), "Mark polygon",
                   wxITEM_NORMAL);
  toolbar->AddTool(Tedit, "Edit selected", *(toolbaricon[3]), "Edit selected",
                   wxITEM_NORMAL);
  toolbar->AddTool(Tdselect, "Deselect all", *(toolbaricon[4]), "Deselect all",
                   wxITEM_NORMAL);
  toolbar->Realize();
}

void TableData::EndSession(wxCloseEvent &event) { this->Destroy(); }

void TableData::OnEdit(wxCommandEvent &event) {
  wxMessageBox("You will change all marked values.\nClick on the colum you "
               "want to change",
               "Message", wxOK | wxICON_ERROR);
  editselcell = true;
}
...
void TableData::EditSelCells(int col) {
  ...
  if (col < 3 && GetTitle() != "Flynn-Data")
    wxMessageBox("You can not change these values", "Message",
                 wxOK | wxICON_ERROR);
...
int TableData::GetColHeaderInt(int col) {
  ...
  id = name_match(const_cast<char *>(welche.ToUTF8().data()), FileKeys);
  if (id == -1)
    id = name_match(const_cast<char *>(welche.ToUTF8().data()), BoundaryKeys);
  if (id == -1)
    id = name_match(const_cast<char *>(welche.ToUTF8().data()), MineralKeys);
  if (id == -1)
    id = name_match(const_cast<char *>(welche.ToUTF8().data()), VelocityKeys);
  if (id == -1)
    id = name_match(const_cast<char *>(welche.ToUTF8().data()), StressKeys);
  if (id == -1)
    id = name_match(const_cast<char *>(welche.ToUTF8().data()), StrainKeys);
  if (id == -1)
    id = name_match(const_cast<char *>(welche.ToUTF8().data()),
                    FiniteStrainKeys);
  if (id == -1)
    id = name_match(const_cast<char *>(welche.ToUTF8().data()), FlynnAgeKeys);
  if (id == -1)
    id =
        name_match(const_cast<char *>(welche.ToUTF8().data()), FlynnStrainKeys);
  if (id == -1)
    id = name_match(const_cast<char *>(welche.ToUTF8().data()), Leftovers);
  return (id);
}
...
int TableData::GetRowHeaderInt(int row) {
  ...
  id = atoi(val.ToUTF8().data());
  return (id);
}

void TableData::AddRow(void) { tgrid->AppendRows(1); }

void TableData::AddCol(void) { tgrid->AppendCols(1); }
...
void TableData::OnSaveData(wxCommandEvent &event) {
  ...
  filename = wxFileSelector(
      "Choose File", "", "", "",
      "CSV files (*.csv)|Text files (*.txt)|*.txt|All Files (*)|*", wxFD_SAVE);
  if (!filename.IsEmpty()) {
    FILE *fp;
    fp = fopen(filename.ToUTF8().data(), "w");
    for (col = 0; col < tgrid->GetNumberCols(); col++) {
      val = tgrid->GetColLabelValue(col);
      fprintf(fp, "%s", val.ToUTF8().data());
      fprintf(fp, ",");
    }
    fprintf(fp, "\n");
...
// Test which node is inside the polygon. if it is, the index of it will be
// stored
...
// Test which unode is inside the polygon. if it is, the index of it will be
// stored
...
// Test if each node of a flynn is inside the polygon, if so it's index will be
// stored if not it is dismissed. returns the number of flynns in the index
...
void TableData::CloseTable(wxCommandEvent &event) { Close(true); }

void TableData::AutoSize(void) { tgrid->AutoSizeColumns(true); }
