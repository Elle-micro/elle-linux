...
DSettings::DSettings(void) {
  ...
  raincolor = 0;
  rainchange = 10;
  fclampcolor = bclampcolor = nclampcolor = uclampcolor = sclampcolor =
      ucmdargs = false;
  bccmin = bccmax = nccmin = nccmax = uccmin = uccmax = sccmax = sccmin =
      fccmax = fccmin = 0.0;
}

void DSettings::SetUCmdArgs(bool t) { ucmdargs = t; }

void DSettings::SetRainStages(int s) { cstages = s; }

int DSettings::GetRainStages() { return cstages; }
...
int DSettings::GetRainColor() { return raincolor; }

void DSettings::SetRainChange(int s) { rainchange = s; }

int DSettings::GetRainChange() { return rainchange; }

void DSettings::SetSavePicFName(wxString t) { savepicfname = t; }
wxString DSettings::GetSavePicFName() { return savepicfname; }

void DSettings::SetSavePic(bool t) { savepics = t; }
bool DSettings::GetSavePic() { return savepics; }
void DSettings::SetLogLevel(int l) { loglevel = l; }
...
int DSettings::GetLogLevel() { return loglevel; }
void DSettings::SetUnodesRangeFlag(bool var) { rangeflagunodes = var; }
void DSettings::SetSegmentsRangeFlag(bool var) { rangeflagsegments = var; }

void DSettings::ShowNodeNumbers(bool c) { shownodenumbers = c; }

void DSettings::ShowFlynnNumbers(bool c) { showflynnnumbers = c; }

bool DSettings::ShowFlynnNumbers() { return showflynnnumbers; }

bool DSettings::ShowNodeNumbers() { return shownodenumbers; }
...
void DSettings::LoadArchiveRunOpts(wxTextInputStream *text) {
  ...
  line = line.AfterLast(' ');
  {
    wxScopedCharBuffer buf = line.ToUTF8();
...
void DSettings::SaveDSettings() {
  ...
  filename = wxFileSelector(
      "Choose filename to save preferences", GetSaveFileDir(), filename, "zip",
      "Preference-Files (*.zip)|*.zip|All Files (*)|*", wxFD_SAVE);
  FILE *out;
...
bool DSettings::LoadDSettings() {
  ...
  filename = wxFileSelector("Choose filename to load preferences", "", "", "",
                            "Preference-Files (*.zip)|*.zip|All Files (*)|*",
                            wxFD_OPEN);
  if (!filename.IsEmpty() && filename.AfterLast('.') == "zip") {
...
bool DSettings::GetUnodesRangeFlag() { return rangeflagunodes; }
...
void DSettings::SetUnodesNotRangeFlag(bool var) { notrangeflagunodes = var; }

bool DSettings::GetUnodesNotRangeFlag() { return notrangeflagunodes; }
...
bool DSettings::GetSegmentsRangeFlag() { return rangeflagsegments; }
...
bool DSettings::GetSegmentsNotRangeFlag() { return notrangeflagsegments; }
...
int DSettings::GetNodeSize() { return nodesize; }

void DSettings::SetNodeSize(int size) { nodesize = size; }
void DSettings::SetUNodeSize(int size) { unodesize = size; }

int DSettings::GetUNodeSize() { return unodesize; }
void DSettings::SetSegmentSize(int size) { segmentsize = size; }

int DSettings::GetSegmentSize() { return segmentsize; }

int DSettings::GetLineSize() { return linesize; }

void DSettings::SetLineSize(int size) { linesize = size; }
    ...
    if (state) {
      ...
      */
      ...
      if (options->boundary_attrib == MISORIENTATION) {
        ElleFindBndAttributeRange(options->boundary_attrib, &amin, &amax);
...
void DSettings::SetLoadFileDir(wxString filed) { loadfiledir = filed; }

wxString DSettings::GetLoadFileDir() { return loadfiledir; }

void DSettings::SetSaveFileDir(wxString filed) { savefiledir = filed; }

wxString DSettings::GetSaveFileDir() { return savefiledir; }
