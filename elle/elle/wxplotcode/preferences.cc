...
EVT_TEXT(ID_psegmentnotrangeflagmaxval,
         Preferences::OnSegmentsNotRangeFlagTxtMax)
...
EVT_TEXT(ID_psegmentnotrangeflagminval,
         Preferences::OnSegmentsNotRangeFlagTxtMin)
...
LinePane::LinePane(wxWindow *parent, const wxSize linesize, int wdth,
                   wxColor col)
    : wxPanel(parent, -1, wxDefaultPosition, linesize) {
  linewdth = wdth;
  linecol = col;
}

DotPane::DotPane(wxWindow *parent, const wxSize panesize, int size, wxColor col)
    : wxPanel(parent, -1, wxDefaultPosition, panesize) {
  dotsize = size;
  dotcol = col;
}

// Initialize everyting. Set all default values for colors and diameters and so
// on and show the preferences window. Oh, the buttons and stuff are defined
// here too!
Preferences::Preferences(wxWindow *parent)
    : wxDialog(parent, PREFSWIN, wxT("Preferences"), wxPoint(100, 100),
               wxSize(500, 480), wxCAPTION | wxSYSTEM_MENU) {
  ...
  wxButton *ok =
      new wxButton(this, ID_POk, wxT("OK"), wxDefaultPosition, wxDefaultSize);
  wxButton *apply = new wxButton(this, ID_PApply, wxT("Apply"),
                                 wxDefaultPosition, wxDefaultSize);
  wxButton *cancel = new wxButton(this, ID_PCancel, wxT("Cancel"),
                                  wxDefaultPosition, wxDefaultSize);
  wxButton *save = new wxButton(this, ID_PSave, wxT("Save"), wxDefaultPosition,
                                wxDefaultSize);
  wxButton *load = new wxButton(this, ID_PLoad, wxT("Load"), wxDefaultPosition,
                                wxDefaultSize);
...
void Preferences::OnLoadPrefs(wxCommandEvent &event) {
  ...
  if (doptions->LoadDSettings()) {
    ...
    EllePlotRegions(0);
    wxMessageBox(wxT("Preferences loaded successfully!"),
...
void Preferences::OnClose(wxCloseEvent &event) { Destroy(); }
...
wxPanel *Preferences::CreateUNodePage(void) {
  ...
  wxSpinCtrl *unodespin =
      new wxSpinCtrl(pppanel, ID_Punodesize, mm, wxDefaultPosition,
                     wxDefaultSize, wxSP_ARROW_KEYS, 1, 10, 2);
  cunode = new wxListBox(pppanel, ID_punodelistbox, wxDefaultPosition,
  ...
  if (welche)
    free(welche);
  ///////////////////////////////////////////////////////////
  wxCheckBox *utriangle =
      new wxCheckBox(pppanel, ID_Punodetriangles, wxT("Triangulate unodes"),
                     wxDefaultPosition, wxDefaultSize);
  utriangle->SetValue(triangulate);
  // wxButton * single = new wxButton( pppanel, ID_Unodes3d, wxT("Show
  // UNode-surface"), wxDefaultPosition, wxDefaultSize );
  ...
  wxCheckBox *flagunodes = new wxCheckBox(
      pppanel, ID_Prangeflagunodes, wxT("Only draw unodes with values between"),
      wxDefaultPosition, wxDefaultSize);
  flagunodes->SetValue(prangeflagunodes);
  mm = wxT("");
  mm.Printf("%-10.3lg", urangevalmin);
  uvalmin =
      new wxTextCtrl(pppanel, ID_punoderangeflagminval, mm, wxDefaultPosition,
                     wxDefaultSize, wxTE_PROCESS_ENTER);
  uvalmin->SetValue(mm);
  mm = wxT("");
  mm.Printf("%-10.3lg", urangevalmax);
  uvalmax =
      new wxTextCtrl(pppanel, ID_punoderangeflagmaxval, mm, wxDefaultPosition,
                     wxDefaultSize, wxTE_PROCESS_ENTER);
  uvalmax->SetValue(mm);
  ...
  wxCheckBox *notflagunodes =
      new wxCheckBox(pppanel, ID_Pnotrangeflagunodes,
                     wxT("Do NOT draw unodes with values between"),
                     wxDefaultPosition, wxDefaultSize);
  notflagunodes->SetValue(pnotrangeflagunodes);
  mm = wxT("");
  mm.Printf("%-10.3lg", unotrangevalmin);
  wxTextCtrl *notuvalmin =
      new wxTextCtrl(pppanel, ID_punodenotrangeflagminval, mm,
                     wxDefaultPosition, wxDefaultSize, wxTE_PROCESS_ENTER);
  notuvalmin->SetValue(mm);
  mm = wxT("");
  mm.Printf("%-10.3lg", unotrangevalmax);
  wxTextCtrl *notuvalmax =
      new wxTextCtrl(pppanel, ID_punodenotrangeflagmaxval, mm,
                     wxDefaultPosition, wxDefaultSize, wxTE_PROCESS_ENTER);
  ...
  wxCheckBox *ccolor =
      new wxCheckBox(pppanel, ID_UClampColor, wxT("Clamp color between"),
                     wxDefaultPosition, wxDefaultSize);
  ccolor->SetValue(uclampcolor);
  Uccmin = new wxTextCtrl(pppanel, ID_UCCMin, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
  mm.Printf("%-10.3lg", uccmin);
  Uccmin->SetValue(mm);
  Uccmax = new wxTextCtrl(pppanel, ID_UCCMax, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
...
void Preferences::OnUnodesNotRangeFlagTxtMax(wxCommandEvent &event) {
  (((wxTextCtrl *)event.GetEventObject())->GetValue())
      .ToDouble(&unotrangevalmax);
}

void Preferences::OnUnodesNotRangeFlagTxtMin(wxCommandEvent &event) {
  (((wxTextCtrl *)event.GetEventObject())->GetValue())
      .ToDouble(&unotrangevalmin);
}
...
wxPanel *Preferences::CreateSegmentPage(void) {
  ...
  wxSpinCtrl *segmentspin =
      new wxSpinCtrl(pppanel, ID_Psegmentsize, mm, wxDefaultPosition,
                     wxDefaultSize, wxSP_ARROW_KEYS, 1, 10, 2);
  csegment = new wxListBox(pppanel, ID_psegmentlistbox, wxDefaultPosition,
                           wxDefaultSize, 0, NULL, wxLB_SINGLE);
  ///////////////////////////////////////////////////////
  ...
  wxCheckBox *flagsegments =
      new wxCheckBox(pppanel, ID_Prangeflagsegments,
                     wxT("Only draw segments with values between"),
                     wxDefaultPosition, wxDefaultSize);
  flagsegments->SetValue(prangeflagsegments);
  mm = wxT("");
  mm.Printf("%-10.3lg", srangevalmin);
  svalmin =
      new wxTextCtrl(pppanel, ID_psegmentrangeflagminval, mm, wxDefaultPosition,
                     wxDefaultSize, wxTE_PROCESS_ENTER);
  svalmin->SetValue(mm);
  mm = wxT("");
  mm.Printf("%-10.3lg", srangevalmax);
  svalmax =
      new wxTextCtrl(pppanel, ID_psegmentrangeflagmaxval, mm, wxDefaultPosition,
                     wxDefaultSize, wxTE_PROCESS_ENTER);
  svalmax->SetValue(mm);
  wxStaticText *tx = new wxStaticText(pppanel, -1, wxT("and"));
  ...
  wxCheckBox *notflagsegments =
      new wxCheckBox(pppanel, ID_Pnotrangeflagsegments,
  ...
  wxTextCtrl *notsvalmin =
      new wxTextCtrl(pppanel, ID_psegmentnotrangeflagminval, mm,
                     wxDefaultPosition, wxDefaultSize, wxTE_PROCESS_ENTER);
  notsvalmin->SetValue(mm);
  mm = wxT("");
  mm.Printf("%-10.3lg", snotrangevalmax);
  wxTextCtrl *notsvalmax =
      new wxTextCtrl(pppanel, ID_psegmentnotrangeflagmaxval, mm,
                     wxDefaultPosition, wxDefaultSize, wxTE_PROCESS_ENTER);
  ...
  wxBoxSizer *v2 = new wxBoxSizer(wxHORIZONTAL);
  wxCheckBox *ccolor =
      new wxCheckBox(pppanel, ID_SClampColor, wxT("Clamp color between"),
                     wxDefaultPosition, wxDefaultSize);
  ccolor->SetValue(sclampcolor);
  Sccmin = new wxTextCtrl(pppanel, ID_SCCMin, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
  mm.Printf("%-10.3lg", sccmin);
  Sccmin->SetValue(mm);
  Sccmax = new wxTextCtrl(pppanel, ID_SCCMax, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
...
wxPanel *Preferences::CreateFlynnPage(void) {
  ...
  wxBoxSizer *v2 = new wxBoxSizer(wxHORIZONTAL);
  wxCheckBox *pfshownumbers =
      new wxCheckBox(fpanel, ID_Pflynnshownumbers, wxT("Show Numbers"),
                     wxDefaultPosition, wxDefaultSize);
  pfshownumbers->SetValue(false);
  pflynnshownumbers = false;
  wxButton *single =
      new wxButton(fpanel, ID_Pcolorsingleflynn, wxT("Color Single\nFlynn"),
                   wxDefaultPosition, wxDefaultSize);
  wxButton *rsingle = new wxButton(fpanel, ID_PResetSingleFlynnColor,
                                   wxT("Reset Single\nFlynn"),
                                   wxDefaultPosition, wxDefaultSize);
  cflynn = new wxListBox(fpanel, ID_pflynnlistbox, wxDefaultPosition,
                         wxDefaultSize, 0, NULL, wxLB_SINGLE);
  //////////////////////////////////////////////////////////
  int x, count, *welche = 0;
  ...
  wxString mm;
  wxCheckBox *ccolor =
      new wxCheckBox(fpanel, ID_FClampColor, wxT("Clamp color between"),
                     wxDefaultPosition, wxDefaultSize);
  ccolor->SetValue(fclampcolor);
  Fccmin = new wxTextCtrl(fpanel, ID_FCCMin, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
  mm.Printf("%lf", fccmin);
  Fccmin->SetValue(mm);
  Fccmax = new wxTextCtrl(fpanel, ID_FCCMax, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
  mm.Printf("%lf", fccmax);
  Fccmax->SetValue(mm);
...
wxPanel *Preferences::CreateLinePage(void) {
  ...
  wxButton *linecolor = new wxButton(lpanel, ID_PLColor, wxT("Line-Color"),
                                     wxDefaultPosition, wxDefaultSize);
  ...
  wxSpinCtrl *linespin =
      new wxSpinCtrl(lpanel, ID_Plinesize, ls, wxDefaultPosition, wxDefaultSize,
                     wxSP_ARROW_KEYS, 1, 10, wdth);
  ...
  cline = new wxListBox(lpanel, ID_plinelistbox, wxDefaultPosition,
                        wxDefaultSize, 0, NULL, wxLB_SINGLE);
  ...
  wxString name =
      GetAttributeName(doptions->GetShowArgs(BOUNDARIES), BOUNDARIES);
  ...
  wxString mm;
  wxStaticText *raintx =
      new wxStaticText(lpanel, -1, wxT("Change line color every xth stage"));
  mm.Printf("%d", rainchange);
  wxSpinCtrl *rchange =
      new wxSpinCtrl(lpanel, ID_Prainchange, mm, wxDefaultPosition,
                     wxDefaultSize, wxSP_ARROW_KEYS, 1, 1000, 10);
  v1->Add(raintx, 0, wxALIGN_LEFT | wxALL, 5);
  v1->Add(rchange, 0, wxALIGN_LEFT | wxALL, 5);

  wxCheckBox *ccolor =
      new wxCheckBox(lpanel, ID_BClampColor, wxT("Clamp color between"),
                     wxDefaultPosition, wxDefaultSize);
  ccolor->SetValue(bclampcolor);
  Bccmin = new wxTextCtrl(lpanel, ID_BCCMin, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
  mm.Printf("%lf", bccmin);
  Bccmin->SetValue(mm);
  Bccmax = new wxTextCtrl(lpanel, ID_BCCMax, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
  mm.Printf("%lf", bccmax);
  Bccmax->SetValue(mm);
...
wxPanel *Preferences::CreateBNodePage(void) {
  ...
  wxButton *dcolor = new wxButton(bpanel, ID_PDColor, wxT("D-Node-Color"),
                                  wxDefaultPosition, wxDefaultSize);
  z1->Add(dcolor, 0, wxALIGN_LEFT | wxALL, 5);
  wxButton *tcolor = new wxButton(bpanel, ID_PTColor, wxT("T-Node-Color"),
                                  wxDefaultPosition, wxDefaultSize);
  z1->Add(tcolor, 0, wxALIGN_LEFT | wxALL, 5);
  mm.Printf("%d", nodesize);
  wxSpinCtrl *bnodespin =
      new wxSpinCtrl(bpanel, ID_Pbnodesize, mm, wxDefaultPosition,
                     wxDefaultSize, wxSP_ARROW_KEYS, 1, 10, size);
  z1->Add(bnodespin, 0, wxALIGN_LEFT | wxALL, 5);
  wxCheckBox *bnumshow =
      new wxCheckBox(bpanel, ID_Pnodeshownumbers, wxT("Show Nodenumbers"),
                     wxDefaultPosition, wxDefaultSize);
  z1->Add(bnumshow, 0, wxALIGN_LEFT | wxALL, 5);
  pnodeshownumbers = false;
  bnumshow->SetValue(false);
  cbnode = new wxListBox(bpanel, ID_pbnodelistbox, wxDefaultPosition,
                         wxDefaultSize, 0, NULL, wxLB_SINGLE);
  ...
  ///////////////////////////////////////////////////////////
  wxCheckBox *ccolor =
      new wxCheckBox(bpanel, ID_NClampColor, wxT("Clamp color between"),
                     wxDefaultPosition, wxDefaultSize);
  ccolor->SetValue(nclampcolor);
  Nccmin = new wxTextCtrl(bpanel, ID_NCCMin, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
  mm.Printf("%lf", nccmin);
  Nccmin->SetValue(mm);
  Nccmax = new wxTextCtrl(bpanel, ID_NCCMax, mm, wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
  mm.Printf("%lf", nccmax);
  Nccmax->SetValue(mm);
...
void Preferences::OnColorSingleFlynn(wxCommandEvent &event) {
  wxTextEntryDialog *count = new wxTextEntryDialog(
      this, wxT("Please enter number of Flynn"), wxT("HEY THERE!"));
  wxColour col;
  count->ShowModal();
  GetUserColor(&col);
  if (col.Ok()) {
    ElleFlynnSetColor(atoi(count->GetValue()), true, col.Red(), col.Green(),
                      col.Blue());
    EllePlotRegions(0);
  }
}
...
void Preferences::OnApply(wxCommandEvent &event) {
  ...
  if (uclampcolor && useunodea != NONE && useunodea != U_LOCATION &&
      useunodea != LOCATION) {
  ...
  if (sclampcolor && usesegmenta != NONE && usesegmenta != S_LOCATION &&
      usesegmenta != LOCATION) {
...
wxString Preferences::GetAttributeName(int welche, int type) {
  ...
  switch (type) {
  case FLYNNS:
    if (id_match(region_terms, welche, clabel))
      valid = true;
    break;
  case BOUNDARIES:
    if (id_match(boundary_terms, welche, clabel))
      valid = true;
    break;
  case BNODES:
    if (id_match(node_terms, welche, clabel))
      valid = true;
    break;
  case UNODES:
    if (id_match(unode_terms, welche, clabel))
      valid = true;
    break;
  case SEGMENTS:
    if (id_match(segment_terms, welche, clabel))
      valid = true;
    break;
  default:
    break;
...
int Preferences::GetAttributeInt(wxString welche, int type) {
  int id = -1;
  // modernize: convert wxString to UTF-8 char* for legacy helpers
  const wxScopedCharBuffer buf = welche.ToUTF8();
  char *name_c = const_cast<char *>(buf.data());
  switch (type) {
  case FLYNNS:
    id = name_match(name_c, region_terms);
    break;
  case BOUNDARIES:
    id = name_match(name_c, boundary_terms);
    break;
  case BNODES:
    id = name_match(name_c, node_terms);
    break;
  case UNODES:
    id = name_match(name_c, unode_terms);
    break;
  case SEGMENTS:
    id = name_match(name_c, segment_terms);
    break;
  default:
    break;
  }
#if XY
  id = name_match(name_c, FileKeys);
  if (id == -1)
    id = name_match(name_c, BoundaryKeys);
  if (id == -1)
    id = name_match(name_c, MineralKeys);
  if (id == -1)
    id = name_match(name_c, VelocityKeys);
  if (id == -1)
    id = name_match(name_c, StressKeys);
  if (id == -1)
    id = name_match(name_c, StrainKeys);
  if (id == -1)
    id = name_match(name_c, FiniteStrainKeys);
  if (id == -1)
    id = name_match(name_c, FlynnAgeKeys);
  if (id == -1)
    id = name_match(name_c, FlynnStrainKeys);
  if (id == -1)
    id = name_match(name_c, Leftovers);
#endif
  return (id);
}
...
void Preferences::ColormapOnLoad(wxCommandEvent &event) {
  DSettings *doptions = GetDisplayOptions();
  wxString sel = wxFileSelector(
      wxT("Choose Colormap-File"), "", "", "",
      "Colormap-Files  (*.cmap)|*.cmap|All Files (*)|*", wxFD_OPEN);
  if (!sel.IsEmpty()) {
    doptions->CmapLoad(std::string(sel.ToUTF8()));
    cbar->paintNow();
  }
}
...
void Preferences::ColormapOnSave(wxCommandEvent &event) {
  DSettings *doptions = GetDisplayOptions();
  wxString sel = wxFileSelector(
      "Choose filename to save Colormap", "", "", "",
      "Colormap-Files (*.cmap)|*.cmap|All Files (*)|*", wxFD_SAVE);
  if (!sel.IsEmpty()) {
    doptions->CmapSave(std::string(sel.ToUTF8()));
    Refresh(true, NULL);
  }
}
...
wxPanel *Preferences::ColormapCreateControls(void) {
  ...
  wxTextCtrl *item4 =
      new wxTextCtrl(cpanel, ID_TEXTCTRL, _(""), wxDefaultPosition,
                     wxDefaultSize, wxTE_PROCESS_ENTER);
  ...
  wxButton *item8 = new wxButton(cpanel, ID_DEFAULT, _("Default"),
  ...
  wxButton *save = new wxButton(cpanel, ID_CSAVE, _("Save cmap"),
                                wxDefaultPosition, wxDefaultSize, 0);
  item6->Add(save, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);

  wxButton *load = new wxButton(cpanel, ID_CLOAD, _("Load cmap"),
                                wxDefaultPosition, wxDefaultSize, 0);
  ...
  int x, count, *welche;
  wxString choices1[] = {_T( "Flynns" )};
  wxString choices2[] = {_T( "BNodes" )};
  wxString choices3[] = {_T( "UNodes" )};
  fselect = new wxChoice(cpanel, ID_sflynn, wxDefaultPosition, wxDefaultSize, 1,
                         choices1);
  bselect = new wxChoice(cpanel, ID_sbnode, wxDefaultPosition, wxDefaultSize, 1,
                         choices2);
  uselect = new wxChoice(cpanel, ID_sunode, wxDefaultPosition, wxDefaultSize, 1,
                         choices3);
  ElleFlynnDfltAttributeList(&welche, &count);
  for (x = 0; x < count; x++) {
  ...
  wxStaticText *c1 =
      new wxStaticText(cpanel, -1, wxT("Change colour at value:"));
  wxTextCtrl *c2 = new wxTextCtrl(cpanel, ID_CTEXT, _(""), wxDefaultPosition,
                                  wxDefaultSize, wxTE_PROCESS_ENTER);
...
void Preferences::GetUserColor(wxColour *col) { *col = wxGetColourFromUser(); }
...
void Preferences::ColormapOnChangeColorValue(wxCommandEvent &event) {
  ...
  if (tmp > vmax || tmp < vmin)
    wxMessageBox(wxT("Value out of range!"), wxT("Attention"), wxOK, this);
  else {
    dp = (vmax - vmin) / 100;
    tmp = tmp / dp;
