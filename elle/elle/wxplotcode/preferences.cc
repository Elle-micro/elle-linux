...
int Preferences::GetAttributeInt( wxString welche,int type )
{
    ...
    switch(type) {
    case FLYNNS:
	id = name_match( (const char *)welche.mb_str(), region_terms );
		break;
    case BOUNDARIES:
	id = name_match( (const char *)welche.mb_str(), boundary_terms );
		break;
    case BNODES:
	id = name_match( (const char *)welche.mb_str(), node_terms );
		break;
    case UNODES:
	id = name_match( (const char *)welche.mb_str(), unode_terms );
		break;
    case SEGMENTS:
	id = name_match( (const char *)welche.mb_str(), segment_terms );
		break;
    default:     break;
    }
#if XY
	id = name_match( (const char *)welche.mb_str(), FileKeys );
	if ( id == -1 )
		id = name_match( (const char *)welche.mb_str(), BoundaryKeys );
	if ( id == -1 )
		id = name_match( (const char *)welche.mb_str(), MineralKeys );
	if ( id == -1 )
		id = name_match( (const char *)welche.mb_str(), VelocityKeys );
	if ( id == -1 )
		id = name_match( (const char *)welche.mb_str(), StressKeys );
	if ( id == -1 )
		id = name_match( (const char *)welche.mb_str(), StrainKeys );
	if ( id == -1 )
		id = name_match( (const char *)welche.mb_str(), FiniteStrainKeys );
	if ( id == -1 )
		id = name_match( (const char *)welche.mb_str(), FlynnAgeKeys );
	if ( id == -1 )
		id = name_match( (const char *)welche.mb_str(), FlynnStrainKeys );
	if ( id == -1 )
		id = name_match( (const char *)welche.mb_str(), Leftovers );
#endif
	return ( id );
}
...
void Preferences::ColormapOnLoad( wxCommandEvent & event )
{
    string filename;
    DSettings * doptions = GetDisplayOptions();
    filename = wxFileSelector( wxT("Choose Colormap-File"), "", "", "", "Colormap-Files  (*.cmap)|*.cmap|All Files (*)|*",wxFD_OPEN );
