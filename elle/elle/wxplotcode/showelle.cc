...
void ShowelleFrame::OnSaveAs(wxCommandEvent & event)
{
    ...
    DSettings * doptions = GetDisplayOptions();
    sdir = doptions->GetSaveFileDir();
    filename = wxFileSelector( "Choose File", "", "", "", "ELLE files (*.elle)|*.elle|All Files (*)|*" ,wxFD_SAVE);
    if ( !filename.IsEmpty() )
    {
        if(filename.Find('.')==-1)
            filename.Append(".elle");
...
void ShowelleFrame::OnOpen( wxCommandEvent & event )
{
    ...
    filename = wxFileSelector( "Choose File", filedir, "", "",
                               "ELLE files (*.elle)|*.elle|ELLE-Archive files (*.zip)|*.zip|All Files (*)|*" ,wxFD_OPEN);
    if ( !filename.IsEmpty() )
    {
        ...
        ElleSetFile((char *)(const char *)filename.mb_str());       //<<<<<----
        if ( fileend != "zip" )
            LoadFile( filename );
        else if(fileend=="zip")
...
void ShowelleFrame::OnSaveRun( wxCommandEvent & event )
{
    ...
    int n;
    FILE * fp;
    wxString dird = wxFileSelector( "Choose file", dir.GetName(), "", "","ELLE files (*.elle)|*.elle|All Files (*)|*" ,wxFD_OPEN);
    ...
    if ( !dird.IsEmpty() )
    {
		...
		ElleReinit(); //loescht die ganzen Arrays
                    ElleReadData( ( char * )(const char *)(f.Item(n)).mb_str() );
                    EllePlotRegions( 0 );
                    panel->Refresh( false, NULL );
                    panel->Update();
...
void ShowelleFrame::OnSaveSingle( wxCommandEvent & event )
{
    wxString savefilename = wxFileSelector( "Choose filename to save the pic", "", "", "",
                                            "PNG files (*.png)|*.png|JPEG files (*.jpg)|*.jpg|PCX files (*.pcx)|*.pcx|Ps files (*.ps)|*.ps|BMP files (*.bmp)|*.bmp|All Files (*)|*",wxFD_SAVE );
...
#if XY
    ElleReinit(); //loescht die ganzen Arrays
    ElleReadData( (char *)(const char *)filename.mb_str() );
    //panel->Refresh(false,NULL);
#endif
    filename=filename.BeforeLast('.');
    filename.Append(".zip");
