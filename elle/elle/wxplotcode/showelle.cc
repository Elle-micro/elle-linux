...
ShowelleFrame::ShowelleFrame(const wxString &title, const wxPoint &pos,
                             const wxSize &size)
  ...
  menuGraphic->Append(ID_Overlay_always, "Always &overlay",
                      "Always overlay stages", true);
  ...
  b->Add(panel, 0, wxALIGN_LEFT | wxALIGN_TOP | wxALL,
         5); // CENTER_HORIZONTAL | wxALL, 5 );
...
void ShowelleFrame::OnShowLog(wxCommandEvent &event) { logframe->Show(true); }
...
void ShowelleFrame::OnSaveStereo(wxCommandEvent &event) {
  ...
  filename =
      wxFileSelector("Choose File", sdir, "", ".ps",
                     "Postscript files (*.ps)|*.ps|All Files (*)|*", wxFD_SAVE);
  if (filename.Find('.') == -1)
    filename.Append(".ps");
  PlotAxes(filename);
}
...
void ShowelleFrame::OnSave(wxCommandEvent &event) {
  ...
  if (!filename.IsEmpty()) {
    ...
    if (filename.AfterLast('.') != "zip") {
      ElleWriteData(filename.ToUTF8().data()); // modernize: wxString->UTF-8
...
void ShowelleFrame::OnSaveAs(wxCommandEvent &event) {
  ...
  sdir = doptions->GetSaveFileDir();
  filename =
      wxFileSelector("Choose File", "", "", "",
                     "ELLE files (*.elle)|*.elle|All Files (*)|*", wxFD_SAVE);
  if (!filename.IsEmpty()) {
    if (filename.Find('.') == -1)
      filename.Append(".elle");
...
void ShowelleFrame::OnOnline(wxCommandEvent &event) {
  wxFrame *frame =
      new wxFrame(this, 18, "Help-Window", wxDefaultPosition, wxDefaultSize);
  wxHtmlWindow *help =
      new wxHtmlWindow(frame, 19, wxDefaultPosition, wxSize(640, 600),
                       wxHW_DEFAULT_STYLE, "Help-Window");
...
void ShowelleFrame::CreateToolbarZoom(void) {
  tool =
      new wxFrame(this, ZOOMWIN, "Zoom", wxDefaultPosition, wxSize(250, 150));
  tooltext = new wxTextCtrl(tool, -1, "", wxDefaultPosition, wxSize(180, 50),
  ...
  toolbar->AddTool(Tool_zoomin, "Zoom in", *(toolbaricon[0]), "Zoom in",
                   wxITEM_NORMAL);
  toolbar->AddTool(Tool_zoomout, "Zoom out", *(toolbaricon[1]), "Zoom out",
                   wxITEM_NORMAL);
  toolbar->AddTool(Tool_stop, "Stop zooming", *(toolbaricon[2]), "Stop zooming",
                   wxITEM_NORMAL);
  toolbar->AddTool(Tool_getbnode, "Get info for Bnode", *(toolbaricon[3]),
                   "Get info for Bnode", wxITEM_NORMAL);
  toolbar->AddTool(Tool_getunode, "Get info for Unode", *(toolbaricon[4]),
                   "Get info for Unode", wxITEM_NORMAL);
  toolbar->AddTool(Tool_label, "Label", *(toolbaricon[5]),
                   "Label nodes and flynns", wxITEM_NORMAL);
  toolbar->Realize();
  tool->Show(true);
...
void ShowelleFrame::OnToolIn(wxCommandEvent &event) { panel->ChangeView(5); }

void ShowelleFrame::OnToolOut(wxCommandEvent &event) { panel->ChangeView(4); }
...
void ShowelleFrame::OnOpen(wxCommandEvent &event) {
  ...
  filename = wxFileSelector("Choose File", filedir, "", "",
                            "ELLE files (*.elle)|*.elle|ELLE-Archive files "
                            "(*.zip)|*.zip|All Files (*)|*",
                            wxFD_OPEN);
  if (!filename.IsEmpty()) {
    ...
    ElleSetFile(
        const_cast<char *>(filename.ToUTF8().data())); // modernize: UTF-8
    if (fileend != "zip")
      LoadFile(filename);
    else if (fileend == "zip")
    ...
    Log(0, title.ToUTF8().data()); // modernize: wxString->UTF-8
  } else {
...
void ShowelleFrame::OnSaveRun(wxCommandEvent &event) {
  ...
  FILE *fp;
  wxString dird =
      wxFileSelector("Choose file", dir.GetName(), "", "",
                     "ELLE files (*.elle)|*.elle|All Files (*)|*", wxFD_OPEN);
  ...
  if (!dird.IsEmpty()) {
    if (dir.Open(dird)) {
      ...
      message.Printf("You are about to make pictures out of %ld files! Is that "
                     "what you want?",
                     f.GetCount());
      if (wxMessageBox(message, _T("Interesting question..."), wxYES | wxNO) ==
          wxYES) {
        wxBeginBusyCursor();
        for (n = 0; n < f.GetCount(); n++) {
          ...
          ElleReadData(const_cast<char *>(f.Item(n).ToUTF8().data()));
          EllePlotRegions(0);
          panel->Refresh(false, NULL);
          panel->Update();
          ...
          {
            const wxScopedCharBuffer base =
                savefilename.AfterLast(E_DIR_SEPARATOR).ToUTF8();
            fprintf(fp, "%s,\\\n", base.data());
          }
        }
        fprintf(fp, "\" -mf type=png -ovc lavc -lavcopts vcodec=wmv2 -oac copy "
                    "-o movie.wmf");
        fclose(fp);
        wxEndBusyCursor();
      }
...
void ShowelleFrame::OnSaveSingle(wxCommandEvent &event) {
  wxString savefilename =
      wxFileSelector("Choose filename to save the pic", "", "", "",
                     "PNG files (*.png)|*.png|JPEG files (*.jpg)|*.jpg|PCX "
                     "files (*.pcx)|*.pcx|Ps files (*.ps)|*.ps|BMP files "
                     "(*.bmp)|*.bmp|All Files (*)|*",
                     wxFD_SAVE);
...
void ShowelleFrame::OnPSPrint(wxCommandEvent &event) { PSPrint(); }
void ShowelleFrame::PSPrint(void) {
  ...
  wxPrintPreview *preview = new wxPrintPreview(
      new PrintPS(panel), new PrintPS(panel), &printDialogData);
  if (!preview->Ok()) {
    delete preview;
    wxMessageBox(_T("There was a problem previewing.\nPerhaps your current ")
                 _T("printer is not set correctly?"),
                 _T("Previewing"), wxOK);
  } else {
    wxPreviewFrame *psframe =
        new wxPreviewFrame(preview, this, _T("Isn't this great: A preview"),
...
bool PrintPS::OnPrintPage(int p) {
  ...
  if (unitcell.xlength >= unitcell.ylength) {
    pscan->psDC->SetUserScale((1 / unitcell.xlength) * unitcell.xlength,
                              (1 / unitcell.xlength) * unitcell.ylength);
    pscan->psDC->SetClippingRegion(0, 0, 300, 300);
  } else {
    pscan->psDC->SetUserScale((1 / unitcell.ylength) * unitcell.xlength,
                              (1 / unitcell.ylength) * unitcell.ylength);
    pscan->psDC->SetClippingRegion(0, 0, 300, 300);
  }
  if (pscan->psDC) {
    pscan->DrawToPS(true);
  ...
  EllePlotRegions(0);
  return true; // modernize: ensure non-void function returns a value
}

void PrintPS::GetPageInfo(int *minPage, int *maxPage, int *pageFrom,
                          int *pageTo) {
  *minPage = 1;
  *maxPage = 1;
  *pageFrom = 1;
...
bool PrintPS::HasPage(int pageNum) { return (pageNum == 1); }
