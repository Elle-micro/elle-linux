...
public: /*!Constructor for new Canvas. The panel has OpenGL capabilities.
           ::Canvas */
  Canvas(wxWindow *parent, int vsize, int hsize);
  wxDC *psDC;
  /*!Bool variables to store lots of settings */
  bool enablezoom, mzoom, bnodeshow, lineshow, flynnshow, flynnshownumbers,
      getinfobnode, getinfounode;
  ...
  /*!Draws all the node numbers. Only usefull if in zooming. See the code
   * Canvas.cc */
...
private:
  ...
  void GLDrawTransparentPolygon(double coords[][3], int num, int r, int g,
                                int b);
