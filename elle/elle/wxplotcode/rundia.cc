...
BEGIN_EVENT_TABLE(RunDia, wxDialog)
EVT_BUTTON(ID_OK, RunDia::OnOk)
EVT_BUTTON(ID_RDEFAULT, RunDia::OnReset)
EVT_BUTTON(ID_Cancel, RunDia::OnCancel)
EVT_BUTTON(ID_SaPic, RunDia::OnSavePics)
    EVT_CHECKBOX(ID_logfile, RunDia::OnLogFile)
        EVT_CHECKBOX(ID_loglevel0, RunDia::OnLogLevel0)
            EVT_CHECKBOX(ID_loglevel1, RunDia::OnLogLevel1)
                EVT_GRID_CELL_LEFT_CLICK(RunDia::OnCellEdit)
                    EVT_CHECKBOX(ID_loglevel2, RunDia::OnLogLevel2)
                        END_EVENT_TABLE()

                            RunDia::RunDia(wxWindow *parent, wxWindowID id,
...
void RunDia::OnCellEdit(wxGridEvent &event) {
  ...
  if (col == 0)
    wxMessageBox(wxT("You can not change these values"),
                 wxT("This is an important message!"), wxOK | wxICON_ERROR);
  else {
...
void RunDia::CreateControls() {
  ...
  wxStaticBoxSizer *item6 =
      new wxStaticBoxSizer(wxVERTICAL, item1, wxT("Run option settings"));
  ...
  wxStaticText *l1 = new wxStaticText(item1, -1, wxT("Stages"),
                                      wxDefaultPosition, wxSize(100, 28));
  item3->Add(l1, 0, wxALIGN_LEFT | wxALL, 5);

  stages =
      new wxTextCtrl(item1, ID_stages, _(""), wxDefaultPosition, wxDefaultSize);
  item4->Add(stages, 0, wxALIGN_RIGHT | wxALL, 5);

  wxStaticText *l2 = new wxStaticText(item1, -1, wxT("Save interval"),
                                      wxDefaultPosition, wxSize(100, 25));
  item3->Add(l2, 0, wxALIGN_LEFT | wxALL, 5);

  sinterv = new wxTextCtrl(item1, ID_sinterv, _(""), wxDefaultPosition,
                           wxDefaultSize);
  item4->Add(sinterv, 0, wxALIGN_RIGHT | wxALL, 5);

  wxStaticText *l3 = new wxStaticText(item1, -1, wxT("Save file root"),
                                      wxDefaultPosition, wxSize(100, 25));
  item3->Add(l3, 0, wxALIGN_LEFT | wxALL, 5);

  sfiler =
      new wxTextCtrl(item1, ID_sfiler, _(""), wxDefaultPosition, wxDefaultSize);
  item4->Add(sfiler, 0, wxALIGN_RIGHT | wxALL, 5);

  wxStaticText *l14 = new wxStaticText(item1, -1, wxT("Save pics root"),
                                       wxDefaultPosition, wxSize(130, 25));
  item3->Add(l14, 0, wxALIGN_LEFT | wxALL, 5);

  wxButton *spic = new wxButton(item1, ID_SaPic, _("SavePics"),
                                wxDefaultPosition, wxDefaultSize, 0);
  item4->Add(spic, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);

  dfreq =
      new wxTextCtrl(item1, ID_dfreq, _(""), wxDefaultPosition, wxDefaultSize);
  item4->Add(dfreq, 0, wxALIGN_RIGHT | wxALL, 5);

  wxStaticText *l15 = new wxStaticText(item1, -1, wxT("Display frequency"),
                                       wxDefaultPosition, wxSize(130, 25));
  item3->Add(l15, 0, wxALIGN_LEFT | wxALL, 5);

  wxStaticText *l4 = new wxStaticText(item1, -1, wxT("Switch distance"),
                                      wxDefaultPosition, wxSize(130, 25));
  item3->Add(l4, 0, wxALIGN_LEFT | wxALL, 5);

  switchd = new wxTextCtrl(item1, ID_switchd, _(""), wxDefaultPosition,
                           wxDefaultSize);
  item4->Add(switchd, 0, wxALIGN_RIGHT | wxALL, 5);

  wxStaticText *l5 = new wxStaticText(item1, -1, wxT("Speed up"),
                                      wxDefaultPosition, wxSize(100, 25));
  ...
  logfile = new wxCheckBox(item1, ID_logfile, wxT("Save Logfile"),
                           wxDefaultPosition, wxDefaultSize);
  logf = new wxCheckBox(item1, ID_logfileappend, wxT("Append to old Logfile"),
                        wxDefaultPosition, wxDefaultSize);

  loglevel0 = new wxCheckBox(item1, ID_loglevel0, wxT("Loglevel: quiet"),
                             wxDefaultPosition, wxDefaultSize);
  loglevel1 = new wxCheckBox(item1, ID_loglevel1, wxT("Loglevel: medium"),
                             wxDefaultPosition, wxDefaultSize);
  loglevel2 = new wxCheckBox(item1, ID_loglevel2, wxT("Loglevel: full"),
                             wxDefaultPosition, wxDefaultSize);
  ...
  wxButton *bdef = new wxButton(item1, ID_RDEFAULT, _("Restore Run options"),
                                wxDefaultPosition, wxDefaultSize, 0);
  main2->Add(bdef, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);

  wxButton *bok =
      new wxButton(item1, ID_OK, _("OK"), wxDefaultPosition, wxDefaultSize, 0);
  main2->Add(bok, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);

  wxButton *bcan = new wxButton(item1, ID_Cancel, _("Cancel"),
                                wxDefaultPosition, wxDefaultSize, 0);
  ...
  wxStaticBoxSizer *vsizer =
      new wxStaticBoxSizer(wxVERTICAL, item1, wxT("UserData"));
  ElleUserData(ud);
  ...
  wxStaticText *tx = new wxStaticText(
      item1, -1, wxT("Not all of the displayed values are necessarily used."));
  wxStaticText *tx1 = new wxStaticText(
      item1, -1, wxT("This depends on the process. Therefore, undefined"));
  wxStaticText *tx2 = new wxStaticText(
      item1, -1, wxT("values (usually very high or very low) might show up"));
  wxStaticText *tx3 = new wxStaticText(
      item1, -1, wxT("in the table.  Just pretend they are not there!"));
...
void RunDia::OnOk(wxCommandEvent &event) {
  ...
  {
    wxScopedCharBuffer buf = vsfiler.ToUTF8();
    ElleSetSaveFileRoot(const_cast<char *>(buf.data()));
  }
...
void RunDia::OnCancel(wxCommandEvent &event) { EndModal(wxID_CANCEL); }

void RunDia::OnSavePics(wxCommandEvent &event) {
  DSettings *display_options = GetDisplayOptions();
  display_options->SetSavePic(true);
  display_options->SetSavePicFName(wxFileSelector(
      wxT("Choose filename to save the pic"), wxT(""), wxT(""), wxT(""),
      wxT("PNG files (*.png)|*.png|JPEG files (*.jpg)|*.jpg|PCX files "
          "(*.pcx)|*.pcx|BMP files (*.bmp)|*.bmp|All Files (*)|*"),
      wxFD_SAVE));
}
