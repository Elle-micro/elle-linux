...
int cppFile::LoadZIPData(gzFile in) {
  ...
  while (!gzeof(in) && !finished) {
    ...
    case PARENTS:
      ...
      break;
    case UNODES:
      /* * read the order of attribute vals * old code (pre 2.3.4) had
       * attributes on * same line as position data */
      i = 0;
