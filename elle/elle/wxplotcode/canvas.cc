...
Canvas::Canvas(wxWindow *parent, int vsize, int hsize)
    : wxScrolledWindow(parent, CANVASWIN, wxDefaultPosition,
                       wxSize(hsize, vsize), wxHSCROLL | wxVSCROLL) {
  zoomfactor = 1;
  lineshow = true;
  bnodeshow = true;
  flynnshow = false;
  ...
  // initial colors of lines and bnodes, these can be changed if the users wants
  // to
...
void Canvas::CheckUnitCell() {
  if (isinit) {
    ...
    if (unitcell.xlength != 1 || unitcell.ylength != 1 || w != orig_width ||
        h != orig_height) {
       ...
       */
      image = new wxBitmap(im.Rescale((int)(orig_width * unitcell.xlength),
...
void Canvas::ResetScreen() { EllePlotRegions(0); }

void Canvas::DrawToPS(bool t) { printps = t; }
...
void Canvas::EndSession(wxCloseEvent &event) { this->Destroy(); }
...
void Canvas::PSDrawLines(double coords[][3], int num, int rr, int gg, int bb) {
  ...
  for (int n = 0; n < num; n++) {
    ...
    points[n].y = (int)(size.GetHeight() -
                        ((coords[n][1] / unitcell.ylength) * size.GetHeight()));
  }
  pen = psDC->GetPen();
  DSettings *dset = GetDisplayOptions();
  ...
  psDC->DrawLine(points[0].x, points[0].y, points[num - 1].x,
                 points[num - 1].y);
}
...
void Canvas::IMDrawLines(double coords[][3], int num, int rr, int gg, int bb) {
  ...
  temp_dc.SetPen(pen);
  for (n = 0; n < num - 1; n++)
    temp_dc.DrawLine(points[n].x, points[n].y, points[n + 1].x,
                     points[n + 1].y);
  temp_dc.DrawLine(points[0].x, points[0].y, points[num - 1].x,
                   points[num - 1].y);
  temp_dc.SelectObject(wxNullBitmap);
}

void Canvas::GLDrawLinesBound(double coords[][3], int num, int r, int g,
                              int b) {
  if (!printps)
...
void Canvas::IMDrawLinesBound(double coords[][3], int num, int r, int g,
                              int b) {
  int n, linesize;
  wxPen pen;
  wxPoint points[num];
...
void Canvas::PSDrawLinesBound(double coords[][3], int num, int r, int g,
                              int b) {
  int n, linesize;
  wxPen pen;
  wxPoint points[num];
...
void Canvas::IMDrawSingleBNode(double x, double y, int r, int g, int b) {
  ...
  point.y =
      (int)(size.GetHeight() - ((y / unitcell.ylength) * size.GetHeight()));
  DSettings *dset = GetDisplayOptions();
  pen.SetWidth(1);
...
void Canvas::IMDrawUNodes(int unattrib) {
  ...
  if (useunodeattrib == EULER_RGB) {
    ...
    for (n = 0; n < count; n++) {
      ...
      point.y = (int)(size.GetHeight() -
                      ((xy.y / unitcell.ylength) * size.GetHeight()));
    ...
    if (ElleUnodesActive()) {
      ...
      for (n = 0; n < count; n++) {
        ...
        if (dset->GetUnodesRangeFlag()) {
          if (value > dset->GetUnodesRangeFlagMaxValue() ||
              value < dset->GetUnodesRangeFlagMinValue()) {
            drawunode = false;
          }
        }
        if (dset->GetUnodesNotRangeFlag()) {
          if (value < dset->GetUnodesNotRangeFlagMaxValue() &&
              value > dset->GetUnodesNotRangeFlagMinValue()) {
            drawunode = false;
          }
        }
        if (drawunode) {
    ...
    if (ElleUnodesActive()) {
      ...
      for (n = 0; n < count; n++) {
        ...
        if (drawunode) {
          ...
          psDC->SetPen(pen);
          psDC->SetBrush(wxBrush(wxColour(r, g, b), wxSOLID));
          psDC->DrawCircle((int)(xy.x * size.GetWidth()),
                           (int)(size.GetHeight() - (xy.y * size.GetHeight())),
                           unodesize);
        }
      }
    }
    ...
    if (ElleMaxSegments() > 0) {
      ...
      for (n = 0; n < count; n++) {
        ...
        if (dset->GetSegmentsRangeFlag()) {
          if (value > dset->GetSegmentsRangeFlagMaxValue() ||
              value < dset->GetSegmentsRangeFlagMinValue()) {
            drawsegment = false;
          }
        }
        if (dset->GetSegmentsNotRangeFlag()) {
          if (value < dset->GetSegmentsNotRangeFlagMaxValue() &&
              value > dset->GetSegmentsNotRangeFlagMinValue()) {
            drawsegment = false;
          }
        }
        if (drawsegment) {
...
/*! Draws every fifth nodenumber of a flynn. *Muss ich nochmal bei, die Schrift
 * ist zu gross, oder nur jeden dritten... */
...
/*! Draws a transparent polygon. This is used during marking a set of flynns or
 * nodes in the data windows */
void Canvas::GLDrawTransparentPolygon(double coords[][3], int num, int r, int g,
                                      int b) {
  int n;
  wxPen pen;
  pen.SetColour(r, g, b);
...
void Canvas::IMShowFlynnNumbers(int index, float *x, float *y, int num) {
  ...
  temp_dc.DrawText(
      number, (int)(xy.x / unitcell.xlength * size.GetWidth()) - xoff,
      (int)(size.GetHeight() - (xy.y / unitcell.ylength * size.GetHeight())) -
          yoff);
  temp_dc.SelectObject(wxNullBitmap);
  number = "";
}
...
void Canvas::PSShowFlynnNumbers(int index, float *x, float *y, int num) {
  ...
  psDC->DrawText(number, (int)(xy.x * size.GetWidth()),
                 (int)(size.GetHeight() - (xy.y * size.GetHeight())));
  number = "";
}
...
void Canvas::OnOverlay() { EllePlotRegions(0); }

/*!Save the current file as a single picture. *Strictly WYSIWYG! Handy function
 * to use during runs, so each stage is saved as a picture immediately instead
 * of saving elle-files and converting them later.*/
void Canvas::OnSaveSingle(wxString filename) {
  ...
  if (!filename.IsEmpty()) {
    ...
    if (suffix == filename || suffix.IsEmpty()) {
      ...
      filename.Append(".png");
      wxMessageBox(
          "You did not supply a file-type. Will save the picture as png.",
          "Info", wxOK);
    }
    if (suffix == "jpg")
...
/*! dir = direction 0 - up, 1 - down, 2 - left, 3 - right, 4 = just zoom in, 5 -
just zoom out lots of stuff changed because now the zoom is moved using mouse
dragging*/
void Canvas::ChangeView(int dir) {
  ...
  wxImage im;
  im = image->ConvertToImage();
  im.Rescale((int)(im.GetWidth() * zoomfactor),
             (int)(im.GetHeight() * zoomfactor));
  image = new wxBitmap(im);
  SetVirtualSize(image->GetWidth(), image->GetHeight());
  SetScrollbarsTicks();
  EllePlotRegions(0);
...
wxBitmap *Canvas::GetImage() { return image; }
