...
int PlotAxes(wxString filename) {
  ...
  /*This comes from the Init-function above. Needs to be in here so that this
   * function can be called from within the GUI initialise any necessary
   * attributes which may not have been in the elle file ValidAtt is one of:
   * CAXIS, ENERGY, VISCOSITY, MINERAL, STRAIN, SPLIT, GRAIN*/
  ...
  psout = fopen(filename.ToUTF8().data(), "w");
  pstartps(psout);
  ...
  if (!ElleUnodeAttributeActive(EULER_3)) {
    ...
    for (i = 0; i < NumPlots; i++) {
      if (caxis_output && i < 1) {
        polarout[i] = fopen(polarfile[i].ToUTF8().data(), "w");
        if (fprintf(polarout[i], "%s %s %s\n", "ID", "Azimuth", "Dip") < 0)
          OnError((char *)polarfile[i].ToUTF8().data(), WRITE_ERR);
      }
      ...
      for (j = 0; j < max; j++) {
        ...
        if (ElleFlynnIsActive(j)) {
          ...
          if (mintype == QUARTZ || mintype == CALCITE) {
            ...
            eulerZXZ(rmap, curra * M_PI / 180, currb * M_PI / 180,
                     currc * M_PI / 180);
    ...
    for (i = 0; i < NumPlots; i++) {
      ...
      for (j = 0; j < max; j += step) {
        ...
        if (ElleMaxFlynns() > 0 && ElleFlynnAttributeActive(MINERAL)) {
          ...
          if (mintype == QUARTZ || mintype == CALCITE)
            plotonept(axis_hexagonal[i], rmap, center[i], radius, psout, 0);
          else // assume cubic
            plotonept(axis_cubic[i], rmap, center[i], radius, psout, 0);
        } else {
          if (caxis_output && i < 1) {
            x = j % cols;
            y = j / rows;
...
void startsteronet(double *center, double radius, FILE *psout, char *title,
  ...
  fprintf(psout, "newpath\n");
  fprintf(psout, "%lf %lf moveto\n", center[0] + 0.85 * radius,
          center[1] + 0.85 * radius);
