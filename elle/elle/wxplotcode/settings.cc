...
void Settings::SetLogFile(bool c) { logfile = c; }
bool Settings::GetLogFile() { return logfile; }
void Settings::SetLogAppend(bool c) { logfileappend = c; }
bool Settings::GetLogAppend() { return logfileappend; }
void Settings::SetRainbow(bool s) { rainbow = s; }
bool Settings::GetRainbow() { return rainbow; }
/*
...
void Settings::SetOptionValue(int id, XtPointer val) {
  ...
  case O_NODEATTRIB:
    node_attrib = *((int *)val);
    ElleFindNodeAttributeRange(node_attrib, &amin, &amax);
    node_limits[0] = amin;
    node_limits[1] = amax;
    break;
  case O_NODEATTRMIN:
    node_limits[0] = *((double *)val);
    break;
  case O_NODEATTRMAX:
    node_limits[1] = *((double *)val);
    break;
  case O_UNODEATTRIB:
    unode_attrib = *((int *)val);
    ElleFindUnodeAttributeRange(unode_attrib, &amin, &amax);
    unode_limits[0] = amin;
    unode_limits[1] = amax;
    break;
  case O_UNODEATTRMIN:
    unode_limits[0] = *((double *)val);
    break;
  case O_UNODEATTRMAX:
    unode_limits[1] = *((double *)val);
    break;
  case O_SEGMENTATTRIB:
    segment_attrib = *((int *)val);
    ElleFindSegmentAttributeRange(segment_attrib, &amin, &amax);
    segment_limits[0] = amin;
    segment_limits[1] = amax;
    break;
  case O_SEGMENTATTRMIN:
    segment_limits[0] = *((double *)val);
    break;
  case O_SEGMENTATTRMAX:
    segment_limits[1] = *((double *)val);
    break;
  case O_LBLREGION:
    label_region = *((int *)val);
    break;
  case O_FLYNNATTRIB:
    flynn_attrib = *((int *)val);
    ElleFindFlynnAttributeRange(flynn_attrib, &amin, &amax);
    flynn_limits[0] = amin;
    flynn_limits[1] = amax;
    break;
  case O_FLYNNATTRMIN:
    flynn_limits[0] = *((double *)val);
    break;
  case O_FLYNNATTRMAX:
    flynn_limits[1] = *((double *)val);
    break;
  case O_BNDATTRIB:
    boundary_attrib = *((int *)val);
    if (boundary_attrib == MISORIENTATION)
      ElleFindBndAttributeRange(boundary_attrib, &amin, &amax);
    bnd_limits[0] = amin;
    bnd_limits[1] = amax;
    break;
  case O_BNDATTRMIN:
    bnd_limits[0] = *((double *)val);
    break;
  case O_BNDATTRMAX:
    bnd_limits[1] = *((double *)val);
    break;
  case O_BNDLEVEL:
    boundary_level = *((int *)val);
    break;
  case O_UPDFREQ:
    update_freq = *((int *)val);
    break;
...
void Settings::GetOptionValue(int id, XtPointer val) {
  ...
  case O_NODEATTRIB:
    *((int *)val) = node_attrib;
    break;
  case O_NODEATTRMIN:
    *((double *)val) = node_limits[0];
    break;
  case O_NODEATTRMAX:
    *((double *)val) = node_limits[1];
    break;
  case O_UNODEATTRIB:
    *((int *)val) = unode_attrib;
    break;
  case O_UNODEATTRMIN:
    *((double *)val) = unode_limits[0];
    break;
  case O_UNODEATTRMAX:
    *((double *)val) = unode_limits[1];
    break;
  case O_SEGMENTATTRIB:
    *((int *)val) = segment_attrib;
    break;
  case O_SEGMENTATTRMIN:
    *((double *)val) = segment_limits[0];
    break;
  case O_SEGMENTATTRMAX:
    *((double *)val) = segment_limits[1];
    break;
  case O_LBLREGION:
    *((int *)val) = label_region;
    break;
  case O_BNDATTRIB:
    *((int *)val) = boundary_attrib;
    break;
  case O_BNDATTRMIN:
    *((double *)val) = bnd_limits[0];
    break;
  case O_BNDATTRMAX:
    *((double *)val) = bnd_limits[1];
    break;
  case O_BNDLEVEL:
    *((int *)val) = boundary_level;
    break;
  case O_FLYNNATTRIB:
    *((int *)val) = flynn_attrib;
    break;
  case O_FLYNNATTRMIN:
    *((double *)val) = flynn_limits[0];
    break;
  case O_FLYNNATTRMAX:
    *((double *)val) = flynn_limits[1];
    break;
  case O_UPDFREQ:
    *((int *)val) = update_freq;
    break;
...
void Settings::GetForegroundOption(int *val) { *val = fg; }

void Settings::GetColourmapOption(int *val) { *val = colourmap; }
...
void Settings::SetOverlayOption(int p) { overlay = p; }
...
void Settings::SetColourmapOption(int val) { colourmap = val; }

void Settings::SetMaxColoursOption(int val) { max_colours = val; }
      ...
      if (indx == -1) {
        ...
        case O_LINEWDTH:
          if (fscanf(fp, "%f", &tmp) != 1)
            return (NUM_ERR);
          linewidth = tmp;
          // if (!initial) setlinewidth_(&tmp); disabled weil die von mir
          // gespeichert wird...
          break;
        case O_PGSCL:
          if (fscanf(fp, "%f", &tmp) != 1)
        ...
        case O_BNDATTRIB:
          ...
          break;
        default:
          warning_msg(VAR_ERR, (const char *)str);
...
int Settings::WriteSettings(FILE *fp) {
  ...
  while (display_terms[i].name != 0 && !err) {
    ...
    case O_PAPERSZ:
      ...
      break;
    case O_FONTHGT:
      fprintf(fp, "%d", dfltfonthgt);
      break;
    case O_PG_XMARG:
      fprintf(fp, "%.3f", page_xmargin);
      break;
    case O_PG_YMARG:
      fprintf(fp, "%.3f", page_ymargin);
      break;
    case O_LINEWDTH:
      fprintf(fp, "%.3f", linewidth);
      break;
    case O_PGSCL:
      fprintf(fp, "%.3f", linewidth);
      break;
  ...
  while (option_terms[i].name != 0 && !err) {
    ...
    case O_UPDFREQ:
      GetUpdFreqOption(&val);
      fprintf(fp, " %d", val);
      break;
    case O_DRWUNODETYP:
      GetUnodePlotOption(&val);
      fprintf(fp, " %d", val);
      break;
    case O_DRWSEGMENTTYP:
      GetSegmentPlotOption(&val);
      fprintf(fp, " %d", val);
      break;
    case O_LBLREGION:
      GetLblRgnOption(&val);
      ...
      break;
    case O_OVERLAY:
      GetOverlayOption(&val);
      fprintf(fp, " %d", val);
      break;
    case O_FLYNNATTRIB:
      GetFlynnAttribOption(&val);
      ...
      break;
    case O_BNDLEVEL:
      GetBndLevelOption(&val);
      fprintf(fp, " %d", val);
      break;
    case O_BNDATTRIB:
      GetBndAttribOption(&val);
