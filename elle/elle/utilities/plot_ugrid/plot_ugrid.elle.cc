  ...
  for (i = 0; i < ugrid_size; i++) {
    for (j = 0; j < ugrid_size; j++) {
      if (j == 0) {
        ...
        fprintf(
            psout, "%lf %lf moveto\n",
            offset + scale * upt[(int)(fmod(i + (di * j), ugrid_size))][j].x,
            offset + scale * upt[(int)(fmod(i + (di * j), ugrid_size))][j].y);
      }

      if (sqrt(((upt[(int)(fmod(i + (di * j), ugrid_size))][j].x -
                 upt[(int)fmod(i + (di * j) + di, ugrid_size)]
        ...
        fprintf(psout, "newpath\n");
        fprintf(psout, "%lf %lf moveto\n",
                offset + scale * upt[(int)fmod(i + (di * j) + di, ugrid_size)]
                                    [(int)fmod(j + dj, ugrid_size)]
                                        .x,
                offset + scale * upt[(int)fmod(i + (di * j) + di, ugrid_size)]
                                    [(int)fmod(j + dj, ugrid_size)]
      ...
      for (j = 0; j < ugrid_size; j++) {
        if (j == 0) {
          ...
          fprintf(
              ...
              offset +
                  scale *
                      upt[(int)(fmod(ugrid_size - i + (di * j), ugrid_size))][j]
                          .x,
              offset +
                  scale *
                      upt[(int)(fmod(ugrid_size - i + (di * j), ugrid_size))][j]
        ...
        if (sqrt(
                ((upt[(int)(fmod(ugrid_size - i + (di * j), ugrid_size))][j].x -
                  upt[(int)fmod(ugrid_size - i + (di * j) + di, ugrid_size)]
                     [(int)fmod(j + dj, ugrid_size)]
                         .x) *
                 (upt[(int)(fmod(ugrid_size - i + (di * j), ugrid_size))][j].x -
                  upt[(int)fmod(ugrid_size - i + (di * j) + di, ugrid_size)]
