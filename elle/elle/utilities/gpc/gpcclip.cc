...
int gpcclip(std::vector<Coords> &pv2, std::vector<Coords> &pv1,
  ...
  MALLOC(poly1.contour, poly1.num_contours * sizeof(gpc_vertex_list),
         gpc_vertex_list *, "contour creation");
  poly1.contour[0].num_vertices = pv1.size();
  MALLOC(poly1.contour[0].vertex,
         poly1.contour[0].num_vertices * sizeof(gpc_vertex), gpc_vertex *,
         "vertex creation");
  ...
  MALLOC(poly2.contour, poly2.num_contours * sizeof(gpc_vertex_list),
         gpc_vertex_list *, "contour creation");
  poly2.contour[0].num_vertices = pv2.size();
  MALLOC(poly2.contour[0].vertex,
         poly2.contour[0].num_vertices * sizeof(gpc_vertex), gpc_vertex *,
         "vertex creation");
