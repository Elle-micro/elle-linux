...
void NewFile::SetButtons() {
  ...
  if (userdata->set_fluid_lattice_gas_grain_proc ||
      userdata->set_walls_lattice_gas_proc) {
...
NewFile::~NewFile() { delete userdata; }
...
void NewFile::SetDefaults(wxCommandEvent &WXUNUSED(event)) {
  ...
  case 6:
    ...
    // XRCCTRL(*this, "ID_TEXT_NAME",
    // wxStaticText)->SetLabel(_T("Phase-transition"));
    break;
  case 7:
  ...
  case 8:
    ...
    // XRCCTRL(*this, "ID_TEXT_NAME", wxStaticText)->SetLabel(_T("Grain
    // growth"));
    break;
  case 9:
  ...
  case 11:
    ...
    // XRCCTRL(*this, "ID_TEXT_NAME", wxStaticText)->SetLabel(_T("Viscoelastic
    // relax"));
    break;
  case 12:
    userdata->SetGrainGrowthFracture();
    // userdata->name = _T("Graingrowth with fracturing");
    // XRCCTRL(*this, "ID_TEXT_NAME", wxStaticText)->SetLabel(_T("Graingrowth
    // with fracturing"));
    break;
  }
}
...
void NewFile::OnOk(wxCommandEvent &WXUNUSED(event)) {
  userdata->SaveData();
  // XRCCTRL(*master, "ID_TEXT_MAIN_NAME",
  // wxStaticText)->SetLabel(userdata->name);
  EndModal(true);
}

void NewFile::OnCancel(wxCommandEvent &WXUNUSED(event)) { Destroy(); }
