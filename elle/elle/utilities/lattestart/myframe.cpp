...
MyFrame::MyFrame(wxWindow *parent) {
  ...
  // pfade setzen fuer:
  // showelleps, mencoder, latte (mit disablen der control und warnhinweis),
  // imagemaker showelleps_path, mencoder_path, latte_path, convert_path
...
void MyFrame::OnIdle(wxIdleEvent &event) {
  ...
  if (CIsRunning && convert_callback) {
    if (convert_callback->IsInputAvailable()) {
      ...
      XRCCTRL(*this, "ID_TEXTCTRL_CONVERSIONSTDOUT", wxTextCtrl)
          ->AppendText(msg);
    }
  }
}
...
void MyFrame::SelectElleFile(wxCommandEvent &WXUNUSED(event)) {
  wxFileDialog dialog(this, wxT("Choose elle-file"), appdirectory,
                      wxEmptyString, _T( "elle-files (*.elle)|*.elle" ),
                      wxOPEN);
    ...
    if (!userselectedarchive) {
      ...
      if (dir.HasFiles(archive_out)) {
        if (CheckArchive(archive_out_path)) {
          XRCCTRL(*this, "ID_TEXTCTRL_ARCHIVEDIR", wxTextCtrl)->Clear();
          XRCCTRL(*this, "ID_TEXTCTRL_ARCHIVEDIR", wxTextCtrl)
              ->AppendText(archive_out_path);
          // wxMessageDialog dialog( this, wxT( "An archive of this name already
          // exists \n you may edit existing preferences \n or replace them " ),
          // wxT( "Info" ), wxID_OK ); dialog.ShowModal();
          ...
          XRCCTRL(*this, "ID_TEXTCTRL_ARCHIVEDIR", wxTextCtrl)->Clear();
          XRCCTRL(*this, "ID_TEXTCTRL_ARCHIVEDIR", wxTextCtrl)
              ->AppendText(archive_path);
          wxMessageDialog dialog(this,
                                 wxT("An archive already exists \n but doesnt "
                                     "contain preferences."),
                                 wxT("Info"), wxID_OK);
          dialog.ShowModal();
        ...
        // wxMessageDialog dialog( this, archive_path + wxT( " will be created
        // in current elle-directory" ), wxT( "Create new archive" ), wxID_OK );
        // dialog.ShowModal();
      ...
      XRCCTRL(*this, "ID_TEXTCTRL_ARCHIVEOUT", wxTextCtrl)
          ->AppendText(archive_out_path);
    }
  }
}
...
void MyFrame::SelectArchive(wxCommandEvent &WXUNUSED(event)) {
  wxFileDialog dialog(this, wxT("Choose zip-archive"), archive_dir,
                      wxEmptyString, _T( "ZIP archives (*.zip)|*.zip" ),
                      wxOPEN);
  int cont;
  ...
  if (cont == wxID_OK) {
    ...
    if (!CheckArchive(archive_path)) {
      wxMessageDialog message(
          this, wxT("This archive doesnt contain preferences to import!"),
          wxT("Warning"), wxID_OK);
      message.ShowModal();
    } else {
      XRCCTRL(*this, "ID_TEXTCTRL_ARCHIVEDIR", wxTextCtrl)->Clear();
...
void MyFrame::BrowseZip(wxCommandEvent &WXUNUSED(event)) {
  ...
  wxFileDialog dialog(this, _T( "Choose a ZIP archive" ), wxGetCwd(),
                      wxEmptyString, _T( "ZIP archives (*.zip)|*.zip" ),
                      wxOPEN);
  ...
  if (dialog.ShowModal() == wxID_OK) {
    ...
    if (!CheckArchive(archive_path)) {
      wxMessageDialog dialog(
          this, wxT("zip-file doesn't contain pref-file to import!"),
          wxT("Error importing!"), wxID_OK);
      dialog.ShowModal();
      ...
      if (archive_dir.Trim() == wxGetCwd().Trim()) {
        XRCCTRL(*this, "ID_CHOICE_ZIP_FILE", wxChoice)
            ->SetStringSelection(archive);
      } else {
        XRCCTRL(*this, "ID_CHOICE_ZIP_FILE", wxChoice)
            ->SetStringSelection(archive_path);
      }
    }
    XRCCTRL(*this, "ID_BUTTON_EDITEXISTINGFILE", wxButton)->Enable(true);
  }
...
void MyFrame::ShowCreateNewFileDialog(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (archive_out_path.IsEmpty()) {
    if (!elle_path.IsEmpty())
      ...
      wxMessageDialog dialog(
          this,
          _T( "Choose an elle-file from this directory first!\n Otherwise an archive can't be created." ),
          _T( "Error" ), wxOK);
      dialog.ShowModal();
      return;
    }
  ...
  if (wxFileExists(archive_out_path)) {
    if (CheckArchive(archive_out_path)) {
      wxMessageDialog message(
          this, _T( "Really overwrite settings\n in an existing archive?" ),
          _T( "An archive already exists!" ), wxOK | wxCANCEL);
...
void MyFrame::ShowEditExistingDialog(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (archive != wxEmptyString) {
    ...
    wxMessageDialog dialog(
        this,
        _T( "A pref-file doesn't yet exist!\nYou'll have to create one." ),
        _T( "Error!" ), wxOK);
    dialog.ShowModal();
    return;
  }
...
void MyFrame::ChooseElleFile(wxCommandEvent &WXUNUSED(event)) {
  if (XRCCTRL(*this, "ID_CHOICE_ELLE_FILE", wxChoice)->GetSelection() != 0) {
    ellefile =
        XRCCTRL(*this, "ID_CHOICE_ELLE_FILE", wxChoice)->GetStringSelection();
    elle_dir = wxGetCwd();
    ...
    if (XRCCTRL(*this, "ID_CHOICE_ZIP_FILE", wxChoice)->GetSelection() == 0) {
      ...
      XRCCTRL(*this, "ID_TEXT_OUT_ARCHIVE", wxStaticText)
          ->SetLabel(wxT("none"));
    }
  }
...
void MyFrame::OpenArchiveDescription(wxString temp) {
  ...
  while (entry = zip.GetNextEntry(), entry != NULL) {
    if (entry->GetName() == wxT("latte_desc")) {
      if (zip.OpenEntry(*entry)) {
        while (!zip.Eof())
          XRCCTRL(*this, "ID_TEXTCTRL_DESC", wxTextCtrl)
              ->AppendText(text.ReadLine() + wxT("\n"));
      }
    }
  }
}
...
void MyFrame::StartRun(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (!XRCCTRL(*this, "ID_CHECKBOX_LOCAL_DIR", wxCheckBox)->IsChecked()) {
    if (!wxFileExists(latte_path)) {
      // cout << latte_path.ToAscii() << endl;
      wxMessageDialog dialog(this,
                             wxT("No global latte-executable found! \nAdd it "
                                 "in the 'misc-settings' dialoge."),
                             wxT("Error"), wxID_OK);
      dialog.ShowModal();
      return;
    }
    ...
    if (!wxFileExists(wxT("elle_latte"))) {
      wxMessageDialog dialog(
          this,
          wxT("No local latte-executable found!\nIf it exists in this "
              "directory, \nplease rename it to 'elle_latte'"),
          wxT("Error"), wxID_OK);
      dialog.ShowModal();
      return;
    }
  ...
  if (!IsRunning) {
    ...
    if (!ellefile) {
      wxMessageDialog dialog(this, wxT("No *.elle file!"),
                             wxT("You must open an elle data-file first!"));
      dialog.ShowModal();
      return;
    }

    // check, if it should run in the local dir, and additionally if it should
    // be running in a shell
    if (XRCCTRL(*this, "ID_CHECKBOX_LOCAL_DIR", wxCheckBox)->IsChecked()) {
      cmd = _T( "./elle_latte" );
    } else {
      cmd = latte_path;
    ...
    cmd.Append(_T( " -f " ));
    s.Printf(_T( "%d" ),
             int(XRCCTRL(*this, "ID_SPINCTRL_SAVE_INTERVAL", wxSpinCtrl)
                     ->GetValue()));
    cmd.Append(s);
...
void MyFrame::SaveDesc() {
  ...
  if (archive_out_path != wxEmptyString) {
    ...
    wxRenameFile(wxGetCwd() + wxFILE_SEP_PATH + wxT("tmp.zip"),
                 archive_out_path);
  }
}

void MyFrame::SaveStdout(wxCommandEvent &WXUNUSED(event)) {
  wxFileDialog dialog(this, wxT("Choose elle-file"), elle_dir, wxEmptyString,
                      _T( "elle-files (*.elle)|*.elle" ), wxSAVE);
  ...
  fout
      << XRCCTRL(*this, "ID_TEXTCTRL_STDOUT", wxTextCtrl)->GetValue().ToAscii();

  fout.close();
}

void MyFrame::SelectExecutable(wxCommandEvent &WXUNUSED(event)) {
  wxDirDialog dialog(this, wxT("Choose directory of latte-executable"),
                     wxGetCwd());
  dialog.ShowModal();
  ...
  if (wxFileExists(latte_path)) {
    ...
    XRCCTRL(*this, "ID_TEXTCTRL_LATTEEX", wxTextCtrl)
        ->AppendText(latte_path.BeforeLast(wxFILE_SEP_PATH));
  } else
    latte_path = wxEmptyString;
}
...
void MyFrame::MakeMovie(wxCommandEvent &WXUNUSED(event)) {
  ...
  wxString k, buffer, startnumber = wxEmptyString, endnumber = wxEmptyString,
                      save_movie_file; // buffer

  if (start_file_dir != end_file_dir) {
    wxMessageDialog dialog(this,
                           wxT(" Start- and end-files for the \nmovie are in "
  ...
  if (save_file_dir != end_file_dir) {
    wxMessageDialog dialog(
        this,
        wxT(" Save-path has to be in the \nsame directory as elle-files! "),
        wxT("Error"), wxID_OK);
    dialog.ShowModal();
  ...
  // definieren des elle_filename (==rumpf) und addition der start-nr und
  // .elle-endung
  elle_stepsize =
      XRCCTRL(*this, "ID_SPINCTRL_MOVIEINTERVAL", wxSpinCtrl)->GetValue();

  start_file = wxFileNameFromPath(
      XRCCTRL(*this, "ID_TEXTCTRL_SELECTSTART", wxTextCtrl)->GetValue());
  end_file = wxFileNameFromPath(
      XRCCTRL(*this, "ID_TEXTCTRL_SELECTEND", wxTextCtrl)->GetValue());
  save_movie_file = wxFileNameFromPath(
      XRCCTRL(*this, "ID_TEXTCTRL_SELECTSAVENAME", wxTextCtrl)->GetValue());

  if (end_file == start_file) {
    wxMessageDialog dialog(
        this, wxT(" Start- and end-files for the \nmovie are identical "),
        wxT("Error"), wxID_OK);
    dialog.ShowModal();
  ...
  if (save_movie_file.Trim(1).Trim(0) == wxEmptyString) {
    wxMessageDialog dialog(this, wxT(" Set a filename for the movie! "),
                           wxT("Error"), wxID_OK);
    dialog.ShowModal();
  ...
  if (start_file.Trim(1).Trim(0).IsEmpty()) {
    wxMessageDialog dialog(this, wxT(" Set a start-file! "), wxT("Error"),
                           wxID_OK);
    dialog.ShowModal();
  ...
  if (end_file.Trim(1).Trim(0).IsEmpty()) {
    wxMessageDialog dialog(this, wxT(" Set a end-file! "), wxT("Error"),
                           wxID_OK);
    dialog.ShowModal();
  ...
  if (end_file != start_file) {
    wxMessageDialog dialog(
        this, wxT(" Start- and end-file are \nof different series! "),
        wxT("Error"), wxID_OK);
    dialog.ShowModal();
  ...
  if (gif_save_file.Trim().AfterLast('.') != wxT("gif")) {
    ...
    XRCCTRL(*this, "ID_TEXTCTRL_SELECTSAVENAME", wxTextCtrl)
        ->AppendText(save_movie_file);
  }

  // überprüfen, ob alle files verfügbar sind, evt stepsize anpassen, evt erneut
  // überprüfen
...
void MyFrame::MakeGif() {
  ...
  cmd =
      wxT("convert -loop ") + word.MakeString() + wxT(" *.ps ") + gif_save_file;
...
void MyFrame::MakeGifTerminated(wxProcessEvent &WXUNUSED(event)) {
  ...
  SPSIsRunning = false;
  XRCCTRL(*this, "ID_TEXTCTRL_MAKEMOVIE", wxTextCtrl)
      ->AppendText(wxT("\nConversion to gif finished!\n"));
  XRCCTRL(*this, "ID_TEXTCTRL_MAKEMOVIE", wxTextCtrl)
      ->AppendText(wxT("File saved as ") + gif_save_file);
...
void MyFrame::ChooseMovieStartFile(wxCommandEvent &WXUNUSED(event)) {
  wxFileDialog dialog(this, wxT("Choose elle-file"), moviemake_dir,
                      wxEmptyString, _T( "elle-files (*.elle)|*.elle" ),
                      wxOPEN);
  ...
  if (cont == wxID_OK) {
    if (!(dialog.GetDirectory() != end_file_dir &&
          end_file_dir != wxEmptyString)) {
      XRCCTRL(*this, "ID_TEXTCTRL_SELECTSTART", wxTextCtrl)->Clear();
      XRCCTRL(*this, "ID_TEXTCTRL_SELECTSTART", wxTextCtrl)
          ->AppendText(dialog.GetPath());

      start_file_dir = dialog.GetDirectory();
      moviemake_dir = start_file_dir;
    } else {
      wxMessageDialog dialog(
          this, wxT("Start- and end-files must be in the same directories!"),
          wxT("Sorry"), wxID_OK);
      dialog.ShowModal();
    }
  }
...
void MyFrame::ChooseMovieEndFile(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (cont == wxID_OK) {
    if (!(dialog.GetDirectory() != start_file_dir &&
          start_file_dir != wxEmptyString)) {
      XRCCTRL(*this, "ID_TEXTCTRL_SELECTEND", wxTextCtrl)->Clear();
      XRCCTRL(*this, "ID_TEXTCTRL_SELECTEND", wxTextCtrl)
          ->AppendText(dialog.GetPath());
...
void MyFrame::ChooseSaveFileName(wxCommandEvent &WXUNUSED(event)) {
  wxFileDialog dialog(this, wxT("Choose filename"), moviemake_dir,
                      wxEmptyString, _T( "gif-files (*.gif)|*.gif" ), wxSAVE);
...
void MyFrame::ChooseConversionFile(wxCommandEvent &WXUNUSED(event)) {
  wxFileDialog dialog(
      this, wxT("Choose movie-file"), elle_dir, wxEmptyString,
      _T( "movie-files (*.gif;*.avi; *.mpg)|*.gif;*.avi;*.mpg" ), wxOPEN);
  ...
  if (cont == wxID_OK) {
    ...
    if (convert_file != wxEmptyString) {
      ...
      XRCCTRL(*this, "ID_TEXTCTRL_CONVERTFILE", wxTextCtrl)
          ->AppendText(convert_file);
    } else
      return;
...
void MyFrame::StartConversion(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (to_gif || to_avi || to_mpeg) {
    if (is_mpeg || is_avi || is_gif) {
      file1 = XRCCTRL(*this, "ID_TEXTCTRL_CONVERTFILE", wxTextCtrl)
                  ->GetValue()
                  .Trim();
      ...
      wxMessageDialog dialog(
          this,
          wxT("You didn't choose a file to convert, \nor it's of the wrong "
              "format.\nNote that movie-files are recognized by the "
              "file-ending! "),
          wxT("Problem"), wxID_OK);
      dialog.ShowModal();
    ...
    wxMessageDialog dialog(
        this, wxT(" You didn't choose a file-type \n to convert to. "),
        wxT("Problem"), wxID_OK);
    dialog.ShowModal();
...
void MyFrame::GifChecked(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (XRCCTRL(*this, "ID_CHECKBOX_GIF", wxCheckBox)->IsChecked()) {
    ...
    if (XRCCTRL(*this, "ID_TEXTCTRL_CONVERTFILE", wxTextCtrl)
            ->GetValue()
            .Trim()
            .AfterLast('.') == wxT("mpg"))
      XRCCTRL(*this, "ID_CHECKBOX_AVI", wxCheckBox)->Enable(true);
    else if (XRCCTRL(*this, "ID_TEXTCTRL_CONVERTFILE", wxTextCtrl)
                 ->GetValue()
                 .Trim()
                 .AfterLast('.') == wxT("avi"))
      XRCCTRL(*this, "ID_CHECKBOX_MPEG", wxCheckBox)->Enable(true);
