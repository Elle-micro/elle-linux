...
LatticeGas::LatticeGas(wxWindow *parent, process_latticegas prc,
                       usdat *userdata, bool CalledFromNewFile) {
...
LatticeGas::~LatticeGas() {
  ...
  if (!CalledByNewFile) {
    if (userdat->set_fluid_lattice_gas_grain_proc ||
        userdat->set_walls_lattice_gas_proc) {
...
void LatticeGas::Cancel(wxCommandEvent &WXUNUSED(event)) { this->Destroy(); }
...
void LatticeGas::CloseLattice() {
  ...
  XRCCTRL(*this, "ID_TEXTCTRL_GAS_PERCENT", wxTextCtrl)
      ->GetValue()
      .ToDouble(&userdat->fluid_lattice_gas_random[0]);
  userdat->fluid_lattice_gas_random_proc = false;

  // setfunc
  userdat->set_fluid_lattice_gas_grain_proc =
      XRCCTRL(*this, "ID_CHECKBOX_DIFFUSION", wxCheckBox)->IsChecked();
  // runfunc
  // userdat->lattice_fluid_gas_percent_proc();
  XRCCTRL(*this, "ID_TEXTCTRL_GAS_POROSITY", wxTextCtrl)
      ->GetValue()
      .ToDouble(&userdat->set_fluid_lattice_gas_grain[0]);
  userdat->set_fluid_lattice_gas_grain[1] =
      int(XRCCTRL(*this, "ID_SPINCTRL_GRAIN_AMOUNT", wxSpinCtrl)->GetValue());

  // only runfuncs necessary
  userdat->set_walls_lattice_gas_proc =
      XRCCTRL(*this, "ID_CHECKBOX_FLUID_FLOW", wxCheckBox)->IsChecked();
  XRCCTRL(*this, "ID_TEXTCTRL_GAS_PUMP", wxTextCtrl)
      ->GetValue()
      .ToDouble(&userdat->set_walls_lattice_gas[0]);
...
void LatticeGas::SetLatticeGas() {
  ...
  XRCCTRL(*this, "ID_CHECKBOX_DIFFUSION", wxCheckBox)
      ->SetValue(userdat->set_fluid_lattice_gas_grain_proc);
  word = userdat->set_fluid_lattice_gas_grain[0];
  XRCCTRL(*this, "ID_TEXTCTRL_GAS_POROSITY", wxTextCtrl)->SetValue(word);
  XRCCTRL(*this, "ID_SPINCTRL_GRAIN_AMOUNT", wxSpinCtrl)
      ->SetValue(int(userdat->set_fluid_lattice_gas_grain[1]));

  XRCCTRL(*this, "ID_CHECKBOX_FLUID_FLOW", wxCheckBox)
      ->SetValue(userdat->set_walls_lattice_gas_proc);
...
void LatticeGas::UnCheckOne(wxCommandEvent &WXUNUSED(event)) {
  if (XRCCTRL(*this, "ID_CHECKBOX_DIFFUSION", wxCheckBox)->IsChecked() &&
      XRCCTRL(*this, "ID_CHECKBOX_FLUID_FLOW", wxCheckBox)->IsChecked())
