...
EVT_BUTTON(XRCID("EDIT_GAUSSIAN_YOUNG"),
           WizardDialogSetFunc::EditGaussYoungParas)
...
// gaussian checkboxes
// EVT_UPDATE_UI(XRCID( "GAUSS_CHECK" ),
// WizardDialogSetFunc::OnUpdateCheckboxGauss ) EVT_UPDATE_UI(XRCID( "LIN_CHECK"
// ), WizardDialogSetFunc::OnUpdateCheckboxLin )
END_EVENT_TABLE()
...
WizardDialogSetFunc::WizardDialogSetFunc(wxWindow *parent, process_set prc,
                                         usdat *userdata) {
...
// initialise
void WizardDialogSetFunc::EditGaussStrengthParas(
  ...
  if (XRCCTRL(*this, "STRENGTH_DISTRI", wxRadioBox)->GetSelection() == 2) {
    ...
    dialog.ShowModal();
  } else if (XRCCTRL(*this, "STRENGTH_DISTRI", wxRadioBox)->GetSelection() ==
             1) {
    ...
    wxMessageDialog dialog(this, _T("Choose a type first!"), _T("Error!"),
                           wxOK);
    dialog.ShowModal();
  }
}
...
void WizardDialogSetFunc::EditGaussSpringParas(
    wxCommandEvent &WXUNUSED(event)) {
  wxVariant word;
...
void WizardDialogSetFunc::EditWalls(wxCommandEvent &WXUNUSED(event)) {
  ...
  XRCCTRL(*this, "ID_RADIOBOX_SW_BOTH_EDIT", wxRadioBox)
      ->SetSelection(int(userdat->walls[0]));
...
void WizardDialogSetFunc::WeakenHoriz(wxCommandEvent &WXUNUSED(event)) {
  ...
  XRCCTRL(*this, "ID_CHECKBOX_2ND_LAYER", wxCheckBox)
      ->SetValue(userdat->extra_hor_layer_proc);
...
void WizardDialogSetFunc::SetSettings() {
  ...
  // spring distrib
  XRCCTRL(*this, "SPRING_DISTRI", wxCheckBox)
      ->SetValue(userdat->gauss_spring_proc);

  // young distrib
  XRCCTRL(*this, "YOUNG_DISTRI", wxCheckBox)
      ->SetValue(userdat->gauss_young_proc);

  //   //Relaxthreshold

  word = userdat->multiply_relax[0];
  XRCCTRL(*this, "MULT_RELAX", wxTextCtrl)->SetValue(word);
  XRCCTRL(*this, "CHECK_MULT_RELAX", wxCheckBox)
      ->SetValue(userdat->multiply_relax_proc);

  //   //timestep
  word = userdat->timestep[0];
  XRCCTRL(*this, "TIMESTEP", wxTextCtrl)->SetValue(word);
  XRCCTRL(*this, "CHECK_TIMESTEP", wxCheckBox)
      ->SetValue(userdat->timestep_proc);

  //   //set walls
  XRCCTRL(*this, "CHECK_SET_WALLS", wxCheckBox)->SetValue(userdat->walls_proc);

  //   //wweaken horizontal
  XRCCTRL(*this, "CHECK_WEAKEN_HORIZ", wxCheckBox)
      ->SetValue(userdat->weaken_hor_proc);

  //   //weaken all
  XRCCTRL(*this, "CHECK_WEAKEN_ALL", wxCheckBox)
      ->SetValue(userdat->weaken_all_proc);

  // weaken grain
  XRCCTRL(*this, "ID_CHECKBOX_WEAKEN_GRAIN", wxCheckBox)
      ->SetValue(userdat->weaken_grain_proc);
  XRCCTRL(*this, "ID_SPINCTRL_WEAKEN_GRAIN", wxSpinCtrl)
      ->SetValue(int(userdat->weaken_grain[3]));

  //   //make gb
  XRCCTRL(*this, "CHECK_MAKE_GB", wxCheckBox)->SetValue(userdat->make_gb_proc);

  //   //mineral
  XRCCTRL(*this, "ID_MINERAL", wxRadioBox)
      ->SetSelection(int(userdat->mineral[0]));
  ...
  // box size
  XRCCTRL(*this, "CHECK_BOX_SIZE", wxCheckBox)
      ->SetValue(userdat->set_box_size_proc);
  ...
  XRCCTRL(*this, "ID_CHECKBOX_TRANSITION", wxCheckBox)
      ->SetValue(userdat->transition_proc);
  XRCCTRL(*this, "ID_CHECKBOX_TRANSITION_HEAT", wxCheckBox)
      ->SetValue(userdat->transition_heat_proc);

  XRCCTRL(*this, "ID_CHECKBOX_HEAT_CONDUCTION", wxCheckBox)
      ->SetValue(userdat->heat_conduction_proc);
  XRCCTRL(*this, "ID_SPINCTRL_HEAT_CONDUCTION_NB", wxSpinCtrl)
      ->SetValue(int(userdat->heat_conduction[0]));
  XRCCTRL(*this, "ID_SPINCTRL_HEAT_CONDUCTION_T", wxSpinCtrl)
      ->SetValue(int(userdat->heat_conduction[1]));
}
...
void WizardDialogSetFunc::CloseGaussParams() {
  ...
  if (XRCCTRL(*master, "STRENGTH_DISTRI", wxRadioBox)->GetSelection() == 2) {
    if (XRCCTRL(*this, "GAUSS_MEAN", wxTextCtrl)
            ->GetValue()
            .ToDouble(&userdat->gauss_strength[0]) &&
        XRCCTRL(*this, "GAUSS_SIGMA", wxTextCtrl)
            ->GetValue()
            .ToDouble(&userdat->gauss_strength[1])) {
      EndModal(true);
    } else {
      wxMessageDialog msgDlg(this, _("Numbers only!"), _("Error!"), wxOK);
      msgDlg.ShowModal();
    }
  } else if (XRCCTRL(*master, "STRENGTH_DISTRI", wxRadioBox)->GetSelection() ==
             1) {
    a = XRCCTRL(*this, "ID_TEXTCTRL_LINEAR_MEAN1", wxTextCtrl)
            ->GetValue()
            .ToDouble(&userdat->linear_strength[0]);
    b = XRCCTRL(*this, "ID_TEXTCTRL_LINEAR_SIZE1", wxTextCtrl)
            ->GetValue()
            .ToDouble(&userdat->linear_strength[1]);
    c = XRCCTRL(*this, "ID_TEXTCTRL_LINEAR_MEAN2", wxTextCtrl)
            ->GetValue()
            .ToDouble(&userdat->linear_strength[2]);
    d = XRCCTRL(*this, "ID_TEXTCTRL_LINEAR_SIZE2", wxTextCtrl)
            ->GetValue()
            .ToDouble(&userdat->linear_strength[3]);
    if (a && b && c && d) {
      EndModal(true);
    } else {
      wxMessageDialog msgDlg(this, _("Numbers only!"), _("Error!"), wxOK);
...
void WizardDialogSetFunc::CloseSpringGauss() {
  if (XRCCTRL(*this, "GAUSS_MEAN", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->gauss_spring[0]) &&
      XRCCTRL(*this, "GAUSS_SIGMA", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->gauss_spring[1]))
    EndModal(true);
  else {
    wxMessageDialog msgDlg(this, _("Numbers only!"), _("Error!"), wxOK);
    msgDlg.ShowModal();
...
void WizardDialogSetFunc::CloseRateGauss() {
  if (XRCCTRL(*this, "GAUSS_MEAN", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->gauss_rate[0]) &&
      XRCCTRL(*this, "GAUSS_SIGMA", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->gauss_rate[1]))
    EndModal(true);
  else {
    wxMessageDialog msgDlg(this, _("Numbers only!"), _("Error!"), wxOK);
    msgDlg.ShowModal();
...
void WizardDialogSetFunc::CloseYoungGauss() {
  if (XRCCTRL(*this, "GAUSS_MEAN", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->gauss_young[0]) &&
      XRCCTRL(*this, "GAUSS_SIGMA", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->gauss_young[1]))
    EndModal(true);
  else {
    wxMessageDialog msgDlg(this, _("Numbers only!"), _("Error!"), wxOK);
    msgDlg.ShowModal();
...
void WizardDialogSetFunc::CloseWallEdit() {
  userdat->walls[0] =
      XRCCTRL(*this, "ID_RADIOBOX_SW_BOTH_EDIT", wxRadioBox)->GetSelection();

  if (XRCCTRL(*this, "SW_CONST_EDIT", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->walls[1]))
    EndModal(true);
  else {
    wxMessageDialog msgDlg(this, _("Numbers only!"), _("Error!"), wxOK);
    msgDlg.ShowModal();
...
void WizardDialogSetFunc::CloseWeakenEdit() {
  ...
  a = XRCCTRL(*this, "WH_Y_MIN", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_hor[0]);
  b = XRCCTRL(*this, "WH_Y_MAX", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_hor[1]);
  c = XRCCTRL(*this, "WH_CONST", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_hor[2]);
  d = XRCCTRL(*this, "WH_VISC", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_hor[3]);
  e = XRCCTRL(*this, "WH_BREAKSTRENGTH", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_hor[4]);

  userdat->extra_hor_layer_proc =
      XRCCTRL(*this, "ID_CHECKBOX_2ND_LAYER", wxCheckBox)->IsChecked();
...
void WizardDialogSetFunc::CloseWeakenAllEdit() {
  ...
  a = XRCCTRL(*this, "WA_CONST", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_all[0]);
  b = XRCCTRL(*this, "WA_VISC", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_all[1]);
  c = XRCCTRL(*this, "WA_BREAKSTRENGTH", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_all[2]);
...
void WizardDialogSetFunc::CloseGrainBoundaryEdit() {
  if (XRCCTRL(*this, "GB_CONST", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->make_gb[0]) &&
      XRCCTRL(*this, "GB_BREAKSTRENGTH", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->make_gb[1]))
    EndModal(true);
  else {
    wxMessageDialog msgDlg(this, _("Numbers only!"), _("Error!"), wxOK);
    msgDlg.ShowModal();
...
void WizardDialogSetFunc::CloseEditMikeSettings() {
  ...
  // start reactions at ... GPa (convert to pascal on save!)
  k = XRCCTRL(*this, "ID_TEXTCTRL_START_GPA", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->transition[1]);
  userdat->transition[1] *= 1e9;
  // activation energy
  i = XRCCTRL(*this, "ID_TEXTCTRL_ACTIVATION_ENERGY", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->transition[0]);
  ...
  // rate distrib
  userdat->gauss_rate_proc =
      XRCCTRL(*this, "RATE_DISTRI", wxCheckBox)->IsChecked();
  // spring distrib
  userdat->gauss_spring_proc =
      XRCCTRL(*this, "SPRING_DISTRI", wxCheckBox)->IsChecked();
  // young distrib
  userdat->gauss_young_proc =
      XRCCTRL(*this, "YOUNG_DISTRI", wxCheckBox)->IsChecked();

  // Relaxthreshold
  a = XRCCTRL(*this, "MULT_RELAX", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->multiply_relax[0]);
  // cout << userdat->multiply_relax[0] << endl;
  userdat->multiply_relax_proc =
      XRCCTRL(*this, "CHECK_MULT_RELAX", wxCheckBox)->IsChecked();

  // timestep
  b = XRCCTRL(*this, "TIMESTEP", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->timestep[0]);
  userdat->timestep_proc =
      XRCCTRL(*this, "CHECK_TIMESTEP", wxCheckBox)->IsChecked();

  // set walls
  userdat->walls_proc =
      XRCCTRL(*this, "CHECK_SET_WALLS", wxCheckBox)->IsChecked();

  // wweaken horizontal
  userdat->weaken_hor_proc =
      XRCCTRL(*this, "CHECK_WEAKEN_HORIZ", wxCheckBox)->IsChecked();

  // weaken all
  userdat->weaken_all_proc =
      XRCCTRL(*this, "CHECK_WEAKEN_ALL", wxCheckBox)->IsChecked();

  // weaken grain
  userdat->weaken_grain_proc =
      XRCCTRL(*this, "ID_CHECKBOX_WEAKEN_GRAIN", wxCheckBox)->IsChecked();
  userdat->weaken_grain[3] =
      XRCCTRL(*this, "ID_SPINCTRL_WEAKEN_GRAIN", wxSpinCtrl)->GetValue();

  // make gb
  userdat->make_gb_proc =
      XRCCTRL(*this, "CHECK_MAKE_GB", wxCheckBox)->IsChecked();
  ...
  // x-row
  userdat->dissolve_x_proc =
      XRCCTRL(*this, "CHECK_X_ROW", wxCheckBox)->IsChecked();
  c = XRCCTRL(*this, "DIS_UPPER", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->dissolve_x[0]);
  d = XRCCTRL(*this, "DIS_LOWER", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->dissolve_x[1]);

  // y-row
  userdat->dissolve_y_proc =
      XRCCTRL(*this, "CHECK_Y_ROW", wxCheckBox)->IsChecked();
  e = XRCCTRL(*this, "DIS_LEFT", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->dissolve_y[0]);
  f = XRCCTRL(*this, "DIS_RIGHT", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->dissolve_y[1]);

  // box size
  userdat->set_box_size_proc =
      XRCCTRL(*this, "CHECK_BOX_SIZE", wxCheckBox)->IsChecked();
  g = XRCCTRL(*this, "BOX_SIZE", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->set_box_size[0]);

  // plot frac
  userdat->plot_frac_proc =
      XRCCTRL(*this, "CHECK_PLOT", wxCheckBox)->IsChecked();
  userdat->plot_frac[0] = XRCCTRL(*this, "PLOT_NB", wxSpinCtrl)->GetValue();

  userdat->transition_proc =
      XRCCTRL(*this, "ID_CHECKBOX_TRANSITION", wxCheckBox)->GetValue();
  userdat->transition_heat_proc =
      XRCCTRL(*this, "ID_CHECKBOX_TRANSITION_HEAT", wxCheckBox)->GetValue();

  userdat->heat_conduction_proc =
      XRCCTRL(*this, "ID_CHECKBOX_HEAT_CONDUCTION", wxCheckBox)->GetValue();
  userdat->heat_conduction[0] =
      XRCCTRL(*this, "ID_SPINCTRL_HEAT_CONDUCTION_NB", wxSpinCtrl)->GetValue();
  userdat->heat_conduction[1] =
      XRCCTRL(*this, "ID_SPINCTRL_HEAT_CONDUCTION_T", wxSpinCtrl)->GetValue();
...
void WizardDialogSetFunc::ExtraHorLayer(wxCommandEvent &WXUNUSED(event)) {
  ...
  XRCCTRL(*this, "ID_TEXTCTRL_2ND_LAY_BREAK_STRENGTH", wxTextCtrl)
      ->SetValue(word);
...
void WizardDialogSetFunc::CloseAddLayer() {
  ...
  a = XRCCTRL(*this, "ID_TEXTCTRL_2ND_LAY_Y_MIN", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->extra_hor_layer[0]);
  b = XRCCTRL(*this, "ID_TEXTCTRL_2ND_LAY_Y_MAX", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->extra_hor_layer[1]);
  c = XRCCTRL(*this, "ID_TEXTCTRL_2ND_LAY_CONSTANT", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->extra_hor_layer[2]);
  d = XRCCTRL(*this, "ID_TEXTCTRL_2ND_LAY_VISCOSITY", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->extra_hor_layer[3]);
  e = XRCCTRL(*this, "ID_TEXTCTRL_2ND_LAY_BREAK_STRENGTH", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->extra_hor_layer[4]);
...
void WizardDialogSetFunc::CloseWeakenGrain() {
  ...
  a = XRCCTRL(*this, "WA_CONST", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_grain[0]);
  b = XRCCTRL(*this, "WA_VISC", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_grain[1]);
  c = XRCCTRL(*this, "WA_BREAKSTRENGTH", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->weaken_grain[2]);
