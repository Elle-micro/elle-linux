...
EVT_BUTTON(XRCID("ID_BUTTON_EDIT_RUNFUNCTIONS"),
           EditFile::ShowRunFunctionsDialog)
...
EditFile::EditFile(wxWindow *parent, wxString archive_path,
                   wxString archive_out_path) {
  master = parent;

  wxXmlResource::Get()->LoadDialog(this, parent,
                                   wxT("BasicSettingsExistingFile"));
...
EditFile::~EditFile() { delete userdata; }
...
void EditFile::OnOk(wxCommandEvent &WXUNUSED(event)) {
  ...
  // wxStaticText)->SetLabel(userdata->name); userdata->SaveData();
  EndModal(true);
}
