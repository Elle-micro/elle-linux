...
// WizardDialogRunFunc::OnUpdateCheckboxGauss ) EVT_UPDATE_UI(XRCID( "LIN_CHECK"
// ), WizardDialogRunFunc::OnUpdateCheckboxLin )
END_EVENT_TABLE()
...
WizardDialogRunFunc::WizardDialogRunFunc(wxWindow *parent, process_run prc,
                                         usdat *userdata) {
...
void WizardDialogRunFunc::SetRunFunc() {
  ...
  XRCCTRL(*this, "ID_CHECKBOX_COMPACTION", wxCheckBox)
      ->SetValue(userdat->def_unilateral_proc);
  XRCCTRL(*this, "ID_CHECKBOX_PURESHEAR", wxCheckBox)
      ->SetValue(userdat->def_pureshear_proc);
  XRCCTRL(*this, "ID_CHECKBOX_BILATERAL", wxCheckBox)
      ->SetValue(userdat->def_bilateral_proc);
  XRCCTRL(*this, "ID_CHECKBOX_VISCOSITY_ENABLE", wxCheckBox)
      ->SetValue(userdat->viscosity_proc);
  ...
  XRCCTRL(*this, "ID_RADIOBOX_DISSOLUTION_STRESS", wxRadioBox)
      ->SetSelection(int(userdat->stylolithe_stress_shrink[0]));
  XRCCTRL(*this, "ID_RADIOBOX_DIS_SHRINK", wxRadioBox)
      ->SetSelection(int(userdat->stylolithe_stress_shrink[1]));
  XRCCTRL(*this, "ID_CHECKBOX_STYLOLITHEDISSOLUTION", wxCheckBox)
      ->SetValue(userdat->stylolithe_dissolution);
  XRCCTRL(*this, "ID_SPINCTRL_DISSOLUTION_PLOT", wxSpinCtrl)
      ->SetValue(int(userdat->dis_plot[0]));

  XRCCTRL(*this, "ID_CHECKBOX_DISSOLUTIONSTRAIN", wxCheckBox)
      ->SetValue(userdat->dissolution_strain);

  XRCCTRL(*this, "ID_CHECKBOX_SHRINK_GRAIN", wxCheckBox)
      ->SetValue(userdat->shrink_grain_proc);
  XRCCTRL(*this, "ID_SPINCTRL_GRAINNB", wxSpinCtrl)
      ->SetValue(int(userdat->shrink_grain[0]));
  word = userdat->shrink_grain[1];
  XRCCTRL(*this, "ID_TEXTCTRL_GRAINSHRINK", wxTextCtrl)->SetValue(word);

  XRCCTRL(*this, "ID_CHECKBOX_SHRINK_BOX", wxCheckBox)
      ->SetValue(userdat->shrink_box);
  word = userdat->box_shrinkage[0];
  XRCCTRL(*this, "ID_TEXTCTRL_SHRINK_BOX", wxTextCtrl)->SetValue(word);

  XRCCTRL(*this, "ID_CHECKBOX_SINOSOIDAL", wxCheckBox)
      ->SetValue(userdat->sin_anisotropy_proc);
...
void WizardDialogRunFunc::CloseRunFunc() {
  userdat->def_unilateral_proc =
      (XRCCTRL(*this, "ID_CHECKBOX_COMPACTION", wxCheckBox)->GetValue());
  userdat->def_pureshear_proc =
      (XRCCTRL(*this, "ID_CHECKBOX_PURESHEAR", wxCheckBox)->GetValue());
  userdat->def_bilateral_proc =
      (XRCCTRL(*this, "ID_CHECKBOX_BILATERAL", wxCheckBox)->GetValue());
  userdat->viscosity_proc =
      (XRCCTRL(*this, "ID_CHECKBOX_VISCOSITY_ENABLE", wxCheckBox)->GetValue());

  XRCCTRL(*this, "ID_TEXTCTRL_STEPSIZE", wxTextCtrl)
      ->GetValue()
      .ToDouble(&userdat->stepsize[0]);

  userdat->viscosity_proc =
      XRCCTRL(*this, "ID_CHECKBOX_VISCOSITY_ENABLE", wxCheckBox)->GetValue();
  XRCCTRL(*this, "ID_TEXTCTRL_VISCOSITY", wxTextCtrl)
      ->GetValue()
      .ToDouble(&userdat->viscosity[0]);

  userdat->stylolithe_dissolution =
      XRCCTRL(*this, "ID_CHECKBOX_STYLOLITHEDISSOLUTION", wxCheckBox)
          ->GetValue();
  userdat->dis_plot[0] =
      XRCCTRL(*this, "ID_SPINCTRL_DISSOLUTION_PLOT", wxSpinCtrl)->GetValue();
  userdat->stylolithe_stress_shrink[0] =
      XRCCTRL(*this, "ID_RADIOBOX_DISSOLUTION_STRESS", wxRadioBox)
          ->GetSelection();
  userdat->stylolithe_stress_shrink[1] =
      XRCCTRL(*this, "ID_RADIOBOX_DIS_SHRINK", wxRadioBox)->GetSelection();
  userdat->dis_plot[0] =
      XRCCTRL(*this, "ID_SPINCTRL_DISSOLUTION_PLOT", wxSpinCtrl)->GetValue();

  userdat->dissolution_strain =
      XRCCTRL(*this, "ID_CHECKBOX_DISSOLUTIONSTRAIN", wxCheckBox)->GetValue();

  userdat->shrink_grain_proc =
      XRCCTRL(*this, "ID_CHECKBOX_SHRINK_GRAIN", wxCheckBox)->GetValue();
  userdat->shrink_grain[0] =
      XRCCTRL(*this, "ID_SPINCTRL_GRAINNB", wxSpinCtrl)->GetValue();
  XRCCTRL(*this, "ID_TEXTCTRL_GRAINSHRINK", wxTextCtrl)
      ->GetValue()
      .ToDouble(&userdat->shrink_grain[1]);

  userdat->shrink_box =
      XRCCTRL(*this, "ID_CHECKBOX_SHRINK_BOX", wxCheckBox)->GetValue();
  XRCCTRL(*this, "ID_TEXTCTRL_SHRINK_BOX", wxTextCtrl)
      ->GetValue()
      .ToDouble(&userdat->box_shrinkage[0]);

  userdat->sin_anisotropy_proc =
      XRCCTRL(*this, "ID_CHECKBOX_SINOSOIDAL", wxCheckBox)->GetValue();
  XRCCTRL(*this, "ID_TEXTCTRL_SIN_WAVELENGTH", wxTextCtrl)
      ->GetValue()
      .ToDouble(&userdat->sin_anisotropy[0]);
  XRCCTRL(*this, "ID_TEXTCTRL_SIN_AMPLITUDE", wxTextCtrl)
      ->GetValue()
      .ToDouble(&userdat->sin_anisotropy[1]);
...
void WizardDialogRunFunc::ExtraDeform(wxCommandEvent &WXUNUSED(event)) {
  ...
  XRCCTRL(*this, "ID_SPINCTRL_EXTRATIMESTEP", wxSpinCtrl)
      ->SetValue(int(userdat->extra_deformation[1]));
  XRCCTRL(*this, "ID_CHECKBOX_EXTRADEFORM", wxCheckBox)
      ->SetValue(userdat->extra_deform);
...
void WizardDialogRunFunc::CloseExtraDeform() {
  if (XRCCTRL(*this, "ID_TEXTCTRL_EXTRASTEPSIZE", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat->extra_deformation[0])) {
    userdat->extra_deformation[1] =
        XRCCTRL(*this, "ID_SPINCTRL_EXTRATIMESTEP", wxSpinCtrl)->GetValue();

    userdat->extra_deform =
        XRCCTRL(*this, "ID_CHECKBOX_EXTRADEFORM", wxCheckBox)->GetValue();

    switch (XRCCTRL(*this, "ID_RADIOBOX_EXTRA_DEFORM", wxRadioBox)
                ->GetSelection()) {
    case 0:
