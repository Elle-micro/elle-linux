...
void WizardDialog::Cancel(wxCommandEvent &WXUNUSED(event)) { this->Destroy(); }
...
void WizardDialog::CloseWallEdit() {
  if (XRCCTRL(*this, "SW_BOTH_EDIT", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat.walls[0]) &&
      XRCCTRL(*this, "SW_CONST_EDIT", wxTextCtrl)
          ->GetValue()
...
void WizardDialog::CloseEditMikeSettings() {
  ...
  h = XRCCTRL(*this, "PLOT_NB", wxTextCtrl)
          ->GetValue()
          .ToDouble(&userdat.plot_frac[0]);
