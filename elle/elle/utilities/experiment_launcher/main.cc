...
bool mainwinapp::OnInit() {
  ...
  mainwinFrame *frame = new mainwinFrame(
      wxT("Elle-book examples"), wxPoint(50, 50), wxSize(300, 300), arg0);
  SetTopWindow(frame);
  frame->Init();
  frame->Show(true);
...
mainwinFrame::mainwinFrame(const wxString &title, const wxPoint &pos,
  ...
  wxStaticText *l1 = new wxStaticText(this, -1, "Examples", wxDefaultPosition,
                                      wxDefaultSize, wxALIGN_LEFT);
  wxStaticText *l2 = new wxStaticText(
      this, -1, "Description", wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
  lb = new wxListBox(this, ID_ListBox, wxDefaultPosition, wxDefaultSize, 0,
                     NULL, wxLB_SINGLE);
  for (int n = 0; n < tfile.size(); n++)
    lb->Append(tfile.at(n).name);
  // now the textfield. First, we put the description of the textfield into the
  // variable description
  description = wxT("Welcome to the experiment launcher.\n\nPlease choose an "
                    "example from the list on the left, then click Go");
  text = new wxTextCtrl(this, wxID_ANY, description, wxDefaultPosition,
                        wxSize(250, 100), wxTE_MULTILINE | wxTE_READONLY);
  ...
  wxButton *button1 =
      new wxButton(this, ID_BUTTONGO, "Go", wxDefaultPosition, wxDefaultSize);
  // wxButton * button3 = new wxButton( this, ID_BUTTONSTOP, "Stop",
  // wxDefaultPosition, wxDefaultSize); and now we add it to the base sizer
  ...
  wxStaticText *ld1 =
      new wxStaticText(this, -1, "Processes/utilities", wxDefaultPosition,
                       wxDefaultSize, wxALIGN_LEFT);
  wxStaticText *ld2 = new wxStaticText(
      this, -1, "Description", wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
  lbd = new wxListBox(this, ID_ListBox2, wxDefaultPosition, wxDefaultSize, 0,
                      NULL, wxLB_SINGLE);
  ...
  // variable description
  description = wxT(
      "Start utilities from here.\n\nNo input file will be loaded on startup.");
  textd = new wxTextCtrl(this, wxID_ANY, description, wxDefaultPosition,
                         wxSize(250, 100), wxTE_MULTILINE | wxTE_READONLY);
  ...
  // everything is sorted left to right in this sizer, so the button first
  wxButton *button2 =
      new wxButton(this, ID_BUTTONGO2, "Go", wxDefaultPosition, wxDefaultSize);
...
void mainwinFrame::OnQuit(wxCommandEvent &WXUNUSED(event)) { Close(TRUE); }

void mainwinFrame::OnAbout(wxCommandEvent &WXUNUSED(event)) {
  wxMessageBox(wxT("To Run an Experiment, select it from the list and click on "
                   "Go.\nThis thing was written by\n\nDr. J.K. "
                   "Becker\nbecker@jkbecker.de"),
               wxT("About"), wxOK | wxICON_INFORMATION, this);
}

void mainwinFrame::Init() { Refresh(true, NULL); }

/*!This sets the name of the script and executes it by calling the OnExecute
 * function.
 */
long pid = 0;
void mainwinFrame::OnButtonGo(wxCommandEvent &event) {
  if (!escript.Contains("null")) // obvious hack to prevent top level
                                 // experiments giving an error, but could blow
                                 // up if process or path has null in it!
...
void mainwinFrame::OnButtonGo2(wxCommandEvent &event) {
  if (!escript2.Contains("null")) // obvious hack to prevent top level
                                  // experiments giving an error, but could blow
                                  // up if process or path has null in it!
