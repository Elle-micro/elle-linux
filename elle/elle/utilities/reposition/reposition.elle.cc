...
int ProcessFunction() {
  int j;
  int st_interval = 0, err = 0, max, maxu;
  CellData data;
  Coords corners[4], xy;
  double eps = 1e-5; /* basil uses float */

  EllesaveInterval(); // retain call; return unused

  st_interval = ES_statsInterval();
  if (st_interval > 0)
    ES_WriteStatistics(0);
  ...
  if (fabs(data.ylength - 1.0) < eps) {
    corners[BASELEFT].x = 0;
    corners[BASELEFT].y = 0;
    corners[BASERIGHT].x = 1.0;
    corners[BASERIGHT].y = 0;
    corners[TOPRIGHT].x = 1.0;
    corners[TOPRIGHT].y = 1.0;
    corners[TOPLEFT].x = 0;
    corners[TOPLEFT].y = 1.0;
  } else {
    corners[BASELEFT].x = 0;
    corners[BASELEFT].y = 0;
    corners[BASERIGHT].x = data.xlength;
    corners[BASERIGHT].y = 0;
    corners[TOPRIGHT].x = data.xlength;
    corners[TOPRIGHT].y = data.ylength;
    corners[TOPLEFT].x = 0;
    corners[TOPLEFT].y = data.ylength;
  }
  /*
          else {
  ...
  */
  ...
  for (j = 0; j < max; j++) {
    if (ElleNodeIsActive(j)) {
      ...
      if (xy.x < corners[0].x || xy.x > corners[1].x || xy.y < corners[0].y ||
          xy.y > corners[2].y)
    ...
    if (ElleFlynnIsActive(curr) && !ElleRegionIsSimple(curr, &isect)) {
      ...
      if (ElleReplaceIntersection(&isect, &fnew) == 0) {
        ...
        if (fnew != NO_NB) {
          ...
          if (ElleRegionArea(fnew) < min_area)
            small_flynn = fnew;
...
void TopologyCheck() {
  ...
  for (i = 0; i < ElleMaxFlynns(); i++) {
    while (ElleFlynnIsActive(i) && !ElleRegionIsSimple(i, &isect)) {
      if (ElleNodeIsActive(isect.node1a))
        writeNodeLocalPolyFile(isect.node1a);
      fprintf(stderr, " %d not simple, node %d\n", i, isect.node1a);
