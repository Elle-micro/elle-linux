...
int Do_Shift() {
  ...
  shift_state =
      fopen("shift_state", "r"); // temp file that shows previous offset state
  shift_hist = fopen("shift_hist", "a"); // file that records all displacements

  if (!shift_state) // no tmp file, so first time encountered in experiment, so
                    // apply random shift
  {
    ...
    fprintf(shift_state, "%12.8lf %12.8lf %12.8lf", dy, cum_offset,
            unitcell.ylength);
    fclose(shift_state);
  } else // need to check if in shifted state or not
  {
    if ((n = fscanf(shift_state, "%lf %lf %lf", &dy, &last_cum_offset,
                    &last_ylength) < 3))
    ...
    if (dy > 0.0) // shifted state, so unshift, and remove horizontal offset too
    {
      ...
      */
      ...
      fclose(shift_state);
      shift_state = fopen("shift_state", "w");
      fprintf(shift_state, "%12.8lf %12.8lf %12.8lf", 0.0, cum_offset,
              unitcell.ylength);
      fclose(shift_state);
  ...
  ElleSetCellBBox(&unitcell.cellBBox[BASELEFT], &unitcell.cellBBox[BASERIGHT],
                  &unitcell.cellBBox[TOPRIGHT], &unitcell.cellBBox[TOPLEFT]);
  ElleCellBBox(&unitcell);
  ...
  if (fabs(unitcell.ylength - 1.0) < eps) {
    ...
    if (unitcell.xlength < offset)
      offset = offset - unitcell.xlength;
  }
