          ...
          if (ang < min_ang && (EllePhaseBoundary(k, nb[1]) == 1)) {
            if (EllePhaseBoundary(k, nb[2]) == 1) {
              ...
              do {
                ...
                dist = sqrt(dist);
                count++;
              } while (ang < min_ang && (dist > ElleSwitchdistance() * 0.1));
              if (count > 1) {
            ...
            do {
              ...
              xy[0].x += movedist.x;
              xy[0].y += movedist.y;
              angle0(xy[0].x, xy[0].y, xy[2].x, xy[2].y, xy[1].x, xy[1].y,
                     &currang);
              // if (currang==0)   OnError("angle error",0);
              ang = fabs(currang);
            ...
            if (ang < min_ang && (EllePhaseBoundary(k, nbnodes[j]) == 1)) {
              if ((EllePhaseBoundary(k, nbnodes[j]) == 1) &&
                ...
                j = 3;
                       ...
                       min_ang /*&& (EllePhaseBoundary(k,nbnodes[i])==1)*/) {
              count = 0;
              do {
                IncreaseAngle(&xy[0], &xy[2], &xy[1], &movedist);
                xy[0].x += movedist.x;
...
void CheckArea(double min_area) {
  ...
  if (ElleUnodesActive())
    UnodeAttributeList(&attribs, &num_attribs);
          ...
          for (k = 0, merged = 0; its != rgn.end() && !merged; k++, its++) {
            ...
            if (min[0] == min[1] && ElleMergeFlynnsNoCheck(*its, j) == 0) {
              ...
              if (num_attribs > 0 && reassigned.size() > 0)
                err = UpdateAttributes(reassigned, unodeset[0], density_min);
              delete unodeset[0];
            }
          }
          if (!merged)
            printf("Could not merge %d - two phases\n", j);
        }
      }
    }
  }
    ...
    for (j = 0, removed = 0; j < max; j++) {
      if (ElleFlynnIsActive(j)) {
        ...
        if (fdata.ratio > max_ratio) {
          ...
          if (!merged)
            printf("Could not merge %d - different attributes\n", j);
        }
      }
    }
  }
...
/*!
        ...
        \param          reassigned - vector of unodes to be updated
                        ...
                        density_min - updated disloc density value (use
default?) \return         \a err - this is not reset in the function
