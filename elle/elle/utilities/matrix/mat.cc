...
void uneuler(double rmap[3][3], double *a, double *b, double *g) {
   ...
   * cosg*cosb*cosa - sing*sina	-sina*cosb*cosg - sing*cosa
   * -sinb*cosg sing*cosa*cosb + cosg*sina	-sina*sing*cosb + cosg*cosa
   * -sinb*sing sinb*cosa					-sina*sinb
   * cosb
   */
...
void matmult(double a[3][3], double b[3][3], double ab[3][3]) {
  ...
  cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, 3, 3, 3, 1.0, &a[0][0],
              3, &b[0][0], 3, 0.0, &tmp[0][0], 3);
  for (p1 = &tmp[0][0], p2 = &ab[0][0], i = 0; i < 9; i++, p1++, p2++)
    *p2 = *p1;
}
...
void mataxismult(double a[3][3], double ax[3], double y[3]) {
  ...
  cblas_dgemv(CblasRowMajor, CblasTrans, 3, 3, 1.0, &a[0][0], 3, ax, 1, 0.0,
              tmp, 1);
  for (p1 = &tmp[0], p2 = &y[0], i = 0; i < 3; i++, p1++, p2++)
    *p2 = *p1;
}
...
void matvecmult(double a[3][3], double x[3], double y[3]) {
  ...
  double tmp[3], *p1, *p2;
  /* use a tmp array as y may be the same address as x */
  cblas_dgemv(CblasRowMajor, CblasNoTrans, 3, 3, 1.0, &a[0][0], 3, x, 1, 0.0,
              tmp, 1); // assumes x is 3x1
  for (p1 = &tmp[0], p2 = &y[0], i = 0; i < 3; i++, p1++, p2++)
    *p2 = *p1;
}
...
void free_imatrix(int **im, long rowl, long rowh, long coll, long colh) {
  ELLE_UNUSED(rowl); // modernize: unused params (A26)
  ELLE_UNUSED(rowh); // modernize: unused params (A26)
  ELLE_UNUSED(coll); // modernize: unused params (A26)
  ELLE_UNUSED(colh); // modernize: unused params (A26)
  if (im) {
    if (im[0]) {
      delete[] im[0];
      im[0] = 0;
...
double **dmatrix(long rowl, long rowh, long coll, long colh) {
  ...
  if (d) {
    if (((d[0] = (double *)malloc(nrows * ncols * sizeof(double))) == 0)) {
      free(d);
      d = 0;
    } else
      /* set up the array mapping */
 ...
 */
double ***d3tensor(long rowl, long rowh, long coll, long colh, long depthl,
                   long depthh) {
  ...
  if (m3t) {
    if (((m3t[0] = (double **)malloc(nrows * ncols * sizeof(double))) == 0) ||
        ((m3t[0][0] = (double *)malloc(nrows * ncols * ndepth *
                                       sizeof(double))) == 0)) {
      if (m3t[0]) {
        free(m3t[0]);
        m3t[0] = 0;
      ...
      /* set up the array mapping */
      for (i = 1; i < ncols; i++)
        m3t[0][i] = m3t[0][i - 1] + ndepth;
      for (i = 1; i < nrows; i++) {
        m3t[i] = m3t[i - 1] + ncols;
        m3t[i][0] = m3t[i - 1][0] + ncols * ndepth;
        for (j = 1; j < ncols; j++)
          m3t[i][j] = m3t[i][j - 1] + ndepth;
      }
    }
  }
  return m3t;
...
void free_d3tensor(double ***d3t, long rowl, long rowh, long coll, long colh,
  ...
  ELLE_UNUSED(coll);   // modernize: unused params (A26)
  ELLE_UNUSED(colh);   // modernize: unused params (A26)
  ELLE_UNUSED(depthl); // modernize: unused params (A26)
  ELLE_UNUSED(depthh); // modernize: unused params (A26)
  if (d3t) {
    if (d3t[0][0]) {
      free(d3t[0][0]);
      d3t[0][0] = 0;
...
void matdeterm(double a[3][3], double *da) {
  ...
  *da = -a[0][2] * a[1][1] * a[2][0] - a[0][0] * a[1][2] * a[2][1] -
