...
#define OPTIMAL(v, i, n)                                                       \
  (NE(v[PREV_INDEX(i, n)].y, v[i].y) || NE(v[NEXT_INDEX(i, n)].y, v[i].y))

#define FWD_MIN(v, i, n)                                                       \
  (GE(v[PREV_INDEX(i, n)].vertex.y, v[i].vertex.y) &&                          \
...
#define REV_MIN(v, i, n)                                                       \
  (GT(v[PREV_INDEX(i, n)].vertex.y, v[i].vertex.y) &&                          \
   GE(v[NEXT_INDEX(i, n)].vertex.y, v[i].vertex.y))

#define NOT_RMAX(v, i, n) GT(v[PREV_INDEX(i, n)].vertex.y, v[i].vertex.y)

#define VERTEX(e, s, x, y)                                                     \
  {                                                                            \
    add_vertex(&((e)->outp->v[(s)]), x, y);                                    \
    (e)->outp->active++;                                                       \
  }

#define P_EDGE(d, e, i, j)                                                     \
  {                                                                            \
    (d) = (e);                                                                 \
    do {                                                                       \
      (d) = (d)->prev;                                                         \
    ...
    if (GE(xtop, (*st)->xtop) || EQ(edge->delx, (*st)->e->delx)) {
      ...
      if (EQ(den, 0.0)) {
        ...
        r = (((*st)->e->bot.y - edge->bot.y) * (edge->top.x - edge->bot.x) -
             ((*st)->e->bot.x - edge->bot.x) * (edge->top.y - edge->bot.y)) /
            den;
  ...
  for (edge = aet; edge; edge = edge->next) {
    if (edge->bundle[ABOVE][CLIP] || edge->bundle[ABOVE][SUBJ] || edge->head) {
      if (EQ(edge->top.y, yt))
        ...
        add_st_edge(&st, it, edge,
                    edge->bot.x + edge->delx * (yt - edge->bot.y));
    }
  }
  while (st) {
    stp = st->prev;
...
void gpc_polygon_clip(gpc_op op, gpc_polygon *subject, gpc_polygon *clip,
  ...
  while (scanbeam < sbt_entries) {
    ...
    for (next_edge = aet->next; next_edge; next_edge = next_edge->next) {
      ...
      next_edge->bundle[BELOW][next_edge->type] =
          NE(next_edge->bot.y, yb) || next_edge->pred;
      ...
      if (next_edge->bundle[BELOW][next_edge->type]) {
        tndx =
            EQ(next_edge->bot.y, yb) ? next_edge->pred->delx : next_edge->delx;
          ...
          while (search) {
            ...
            if (prev_edge) {
              if (prev_edge->bundle[ABOVE][CLIP] ||
                  prev_edge->bundle[ABOVE][SUBJ] || prev_edge->head)
                search = FALSE;
            } else
              search = FALSE;
          }
  ...
  if (result->num_contours > 0) {
    ...
    while (poly) {
      npoly = poly->next;
      if (poly->active) {
        result->contour[c].num_vertices = count_vertices(poly->proxy[LEFT]);
