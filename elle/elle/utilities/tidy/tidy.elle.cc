...
/*!
 ...
 \par	Description:
                Code for tidying up elle files, the following activities may be
 performed:

                1) Change bnode spacing
                2) Randomise EULER_3 angles for flynns with MINERAL attribute
 set to QUARTZ 3) Convert flynns with specific F_ATTRIB_A to be mineral MICA 4)
 Set MICA grains to have c0axis normal to long axis of grain 5) Strip unodes
 from file 6) Add unodes to file in one of 4 patterns (hex, square, random, or
 random within regular cells)
                ...
                B = 1 for randomising orientations [default = 0 leaves them
 unchanged]

                C = F_ATTRIB_A for flynns to be set to MICA  [default = 0 leaves
 them unchanged]

                D = 1 for calculating MICA crystal axis orientations [default =
 0 leaves them unchanged]

                E = -1 for stripping nodes from a file [default = 0 leaves them
 unchanged] E= no of unodes per row for adding unodes (strips previous unodes
 first) [default = 0 leaves them unchanged]

                F = code for pattern of unodes to add if E>1 [default = 0 HEX, 1
 = SQUARE, 2 = RANDOM, 3 = SEMI RANDOM, where E x E cells are defined, and then
 G unodes are created per cell]

                G = number of unodes per cell if F = 3 [default = 0]

                H = rotation of bnodes (not unodes!)[default = 0] only works for
 multiples of 90°!!!

                examples:

                to add unodes to a file with a square pattern, 50 unodes per
 row, type
...
int ProcessFunction() {
  ...
  makemica = userdata[MakeMica]; // set flynns woith this F_ATTRIB_A to MICA
                                 // else QUARTZ
  orientmica =
      (int)userdata[OrientMica];        // set MICA axes wrt to grain long axis
  addunodes = (int)userdata[AddUnodes]; // add unodes to file
  unodepattern = (int)userdata[UnodePattern]; // type of unode distribution
  unodecells = (int)userdata[UnodeCells]; // number of subcells for semi random
                                          // patterns of unodes
  rotation = (int)userdata[Rotation];     // rotation of whole model in degrees
                                          // (clockwise is +ve)
  ...
  for (i = 0; i < EllemaxStages(); i++) // loop for number of stages
  {
    ...
    if (addunodes > 0) {
      ...
      else if (unodepattern == RAN_GRID ||
               (unodepattern == SEMI_RAN_GRID && unodecells == 0))
  ...
  for (flynn = 0; flynn < max; flynn++) // loop though all flynns
  {
    if (ElleFlynnIsActive(flynn)) // process flynn if it is active
    {
      ...
      if (mintype == QUARTZ) {
        ...
        if (fabs(curra - dflt1) < eps && fabs(currb - dflt2) < eps &&
...
int MicaQuartzFlynn(double mid) {
  ...
  for (flynn = 0; flynn < max; flynn++) // loop though all flynns
  {
    if (ElleFlynnIsActive(flynn)) // process flynn if it is active
    {
      ElleGetFlynnRealAttribute(flynn, &id,
                                F_ATTRIB_A); // get attribute for this flynn
      if (id == mid)
        ElleSetFlynnIntAttribute(flynn, MICA,
                                 MINERAL); // set attribute for this flynn
      else
        ElleSetFlynnIntAttribute(flynn, QUARTZ,
                                 MINERAL); // set attribute for this flynn
    }
  }
}
...
int Rotate() {
  ...
  for (k = 0; k < max; k++) {
    if (ElleNodeIsActive(k)) {
      ...
      rotate_coords_ac(xy.x, xy.y, centre.x, centre.y, &newxy.x, &newxy.y,
                       angle);
