...
void ebsd2unodes(char *in, double row, double col, double startr, double startc,
                 double width, double space, int grid);
...
int main(int argc, char **argv) {
  ...
  ElleSetOptNames("HKLMaxRows", "HKLMaxColumns", "Origin x", "Origin y",
                  "HorizStep", "VertStep", "GridPattern", "unused", "unused");
  /*
...
void ebsd2unodes(char *ebsdfile, double row, double col, double startr,
                 double startc, double width, double space, int grid) {
  FILE *in;
  int r, c;
  ...
  for (r = 0, cnt = 0; r < maxu / numperrow; r++) {
    for (c = 0, onecnt = 0; c < numperrow; c++, onecnt++) {
      ...
      if (flag2 == 1) {
        ...
        cnt++;
        // set new unode attribute value
        ElleSetUnodeAttribute(outi, onerow[c][1], onerow[c][2], onerow[c][3],
                              EULER_3);
      }
    }
  }
