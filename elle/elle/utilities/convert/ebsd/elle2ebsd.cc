...
void CalcRoe2Bunge(double psi, double theta, double Phi, double *phi1,
                   double *PHI, double *phi2);
int FindSQParams(int *numperrow, double *dx, double *dy, Coords *origin);
void eulerZXZtoCaxis_d(double e1, double e2, double e3, double *cax1,
                       double *cax2);
...
int main(int argc, char **argv) {
  ...
  ElleSetOptNames("HKLRows", "HKLColumns", "HorizStep", "VertStep",
                  /*"ZYZEuler",*/
...
int SetUnodes() {
  ...
  double hkl_pts[rows * cols]
                [7]; // unode location, euler & caxis vals in hkl fmt
  ...
  if (!sq_grid) {
    ...
    Log(0,
        "Does not look like square grid. Using general algorithm - be patient");
    /*ElleVoronoiUnodes();*/
  }
  for (n = 0, r = 0; r < rows; r++) { // cycle through hkl points
    ...
    for (c = 0; c < cols; c++) {
      ...
      if (i != NO_VAL) {
        ...
        if (ZYZ_euler == 1) {
          /*****************************************/
          CalcBunge2Roe(e1, e2, e3, &hkl_pts[n][2], &hkl_pts[n][3],
                        &hkl_pts[n][4]);
          eulerZXZtoCaxis_d(e1, e2, e3, &hkl_pts[n][5], &hkl_pts[n][6]);
          /*****************************************/
        } else {
          /*****************************************/
  ...
  if (outf)
    outf << "X Y Euler1 Euler2 Euler3 Azimuth Dip" << endl;
  for (r = rows - 1; r >= 0; r--) { // cycle through hkl points
    i = r * cols;
    for (c = 0; c < cols; c++, i++) {
      outf << hkl_pts[i][0] << ' ' << hkl_pts[i][1] << ' ' << hkl_pts[i][2]
           << ' ' << hkl_pts[i][3] << ' ' << hkl_pts[i][4] << ' '
           << hkl_pts[i][5] << ' ' << hkl_pts[i][6] << endl;
    }
  }
#if XY
  ...
  double a[3][3];
  if (outf)
    outf << "X Y g11 g12 g13 g21 g22 g23 g31 g32 g33" << endl;
  for (r = rows - 1; r >= 0; r--) { // cycle through hkl points
    i = r * cols;
    for (c = 0; c < cols; c++, i++) {
      orientmatZXZ(a, hkl_pts[i][2] * DTOR, hkl_pts[i][3] * DTOR,
                   hkl_pts[i][4] * DTOR);
      outf << hkl_pts[i][0] << ' ' << hkl_pts[i][1] << ' ' << a[0][0] << ' '
...
These are now in the base library int
FindUnode(Coords *ptxy, Coords *origin, double dx, double dy, int numperrow);
