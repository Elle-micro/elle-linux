...
int MakePolys(double ***segs, int nsegs, double **nodes, int nnodes,
              int **polys, int *npolys);
int FindRight(double **nodes, int lastnode, int endnode, int trial);
void startps(FILE *);
void endps(FILE *);
int WriteElleFile(int **polys, int npoly, double **nodes, int nnodes,
                  char *fname, int irows, int icols, int **image);
                                ...
                                // cout << "Flynn active" << k << endl;
      {
        ...
        if (ElleFlynnAttributeActive(EULER_3)) {
          if (unodelist.size() > 0)
            ...
            ElleGetUnodeAttribute(unodelist[0], &orient[0], &orient[1],
                                  &orient[2], EULER_3);
...

{
  ...
  if (err = ElleReadImage(fname, &image, &irows, &icols, decimate, rgbcols,
                          orient, &dim))
    return (err);
  ...
  // convert to elle
  ConvertDataToElle(polys, npolys, nodes, nnodes, irows, icols, image, rgbcols,
                    orient, dim);
...
int FindSegs(int **image, int irows, int icols, double ***segs, int *nsegs) {
  ...
  for (j = 0; j < 11; j++) {
    ...
    /*		for(k=0;k<11;k++)
                    ...
                    fprintf(ps,"newpath\n");
                    fprintf(ps,"%lf %lf
       moveto\n",((double)j/10)+.49,((double)k/10)+.5); fprintf(ps,"%lf %lf
       lineto\n",((double)j/10)+.51,((double)k/10)+.5); fprintf(ps,"stroke\n");
                    fprintf(ps,"newpath\n");
                    fprintf(ps,"%lf %lf
       moveto\n",((double)j/10)+.5,((double)k/10)+.49); fprintf(ps,"%lf %lf
       lineto\n",((double)j/10)+.5,((double)k/10)+.51); fprintf(ps,"stroke\n");
                    if(k==0)
                            {
                    //	fprintf(ps,"newpath\n");
                            //	fprintf(ps,"%lf %lf
      ...
      if ((int)segs[i][0][0] >= 0) {
        ...
        do {
          ...
          if (ntrials == 1) {
            ...
            segs[trial[0]][0][0] = -1;
            // printf("l,p,e:%d %d
            // %d\n",lastnode,polys[*npolys][pnodes],endnode); printf("%d
            // %d\t*\n",lastnode,endnode);
            pnodes++;
            segsdone++;
          } else if (ntrials == 2) {
            whichnode =
                FindRight(nodes, lastnode, endnode, (int)segs[trial[0]][1][0]);
            // printf("\n2 trial which %d
            ...
            // printf("l,p,e:%d %d
            // %d\n",lastnode,polys[*npolys][pnodes],endnode); printf("seg %d|%d
...
int WriteElleFile(int **polys, int npoly, double **nodes, int nnodes,
                  char *ellename, int irows, int icols, int **image) {
  int np, nv, ev;
  int i, j, cntr;
  ...
  for (i = 0, cntr = 0; i < irows; i++) // y rows
  {
    ...
    for (j = 0; j < icols; j++) // x colums
    {
      ...
      {
        xx =
            xx + (1.0 / (2.0 * icols)); // checks uneven numbers, this is radius
      }
...
int ConvertDataToElle(int **polys, int npoly, double **nodes, int nnodes,
  ...
  for (np = 0; np < npoly && !err; np++) {
    ...
    for (nv = 2; nv <= polys[np][0] && !err; nv++) {
      ...
      err = ElleSetNeighbour(polys[np][nv], NO_NB, polys[np][nv - 1], &np);
    }
    if (!err)
      err = ElleSetNeighbour(polys[np][1], NO_NB, polys[np][polys[np][0]], &np);
  }
  ...
  for (i = 0, cntr = 0; i < irows && !err; i++) // y rows
  {
    for (j = 0; j < icols; j++) // x colums
    {
      ...
      err = ElleSetUnodeAttribute(
          cntr, (double)(image[(int)((irows - i) * 0.866)][j]), U_ATTRIB_A);
#endif
      xy.x = (j + 0.5) / icols;
      xy.y = (i + 0.5) / irows;
      ElleSetUnodePosition(cntr, &xy);
      if (orient_data) {
        ...
        err = ElleSetUnodeAttribute(cntr, xyz.x, xyz.y, xyz.z, EULER_3);
      } else {
        err = ElleSetUnodeAttribute(cntr, (double)(image[irows - 1 - i][j]),
                                    U_ATTRIB_A);
      }
