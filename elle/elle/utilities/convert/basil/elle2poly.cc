...
int ElleToPoly(char *, FILE *fpout, int perbnd, bool unode2flynn, bool notopo,
               int *maxorig, int **originalnodes, int *numnodes,
               int **polynumber);
...
int CnvtWriteNodeNumberReference(FILE *fpout, int maxorig, int *originalnodes,
...
#endif
void ElleCheckUnit(vector<Coords> &pts, int *xflags, int *yflags, Coords *bl,
                   Coords *tr);
...
int main(int argc, char **argv) {
  ...
  if (argc > 3) {
    if (!strcmp(argv[3], PBOPT))
      perbnd = 1;
    else if (!strcmp(argv[3], V2FOPT))
      unode2flynn = true;
    else if (!strcmp(argv[3], NOTOPO))
      notopo = true;
  }
  if (argc > 4) {
    if (!strcmp(argv[4], PBOPT))
      perbnd = 1;
    else if (!strcmp(argv[4], V2FOPT))
      unode2flynn = true;
    else if (!strcmp(argv[4], NOTOPO))
      notopo = true;
  }
  ...
  err = ElleToPoly(infile, fpout, perbnd, unode2flynn, notopo, &maxorig,
                   &originalnodes, &numnodes, &polynumber);
  if (err)
    OnError("", err);

  /* write elle filename in poly file */
  if (fprintf(fpout, "%s ", ELLE_KEY) < 0)
...
int ElleToPoly(char *infile, FILE *fpout, int perbnd, bool unode2flynn,
               bool notopo, int *maxorig, int **originalnodes, int *numnodes,
               int **polynumber) {
  char str[80];
  int err = 0, end = 0;
  int numsegs;
  ...
  *numnodes = CnvtNodeCount();
  ...
  if (err = CnvtWriteNodeData(fpout, *numnodes, str, *polynumber))
    return (err);
  /* segments */
  if (err = CnvtWriteSegmentData(fpout, numsegs, str, *polynumber, *numnodes))
    return (err);
  /* holes */
  if (fprintf(fpout, "0\n") < 0)
    return (WRITE_ERR);
  /* attributes */
  if (err = CnvtWriteRegionAttributeData(fpout, str))
    return (err);
...
int CnvtReadData(FILE *fp, char *str) {
  ...
  while (!feof(fp) && !finished) {
    ...
    case S_EXPONENT:
    case F_ATTRIB_A:
      if (err = ElleReadFlynnRealAttribData(fp, str, key))
        finished = 1;
      break;
    case EXPAND:
      if (err = ElleReadFlynnIntAttribData(fp, str, key))
        finished = 1;
      break;
    case U_VISCOSITY:
...
int CnvtReadSegmentData(FILE *fp, char str[]) {
  ...
  if (err = ElleReadFlynnData(fp, str))
    return (err);
#else
  if (err = ElleReadGrainData(fp, str))
    return (err);
...
int CnvtWriteSegmentData(FILE *fpout, int numsegs, char *str, int *polynum,
                         int numnodes) {
  /*
    ...
    if (ElleNodeIsActive(i)) {
      ...
      for (j = 0; j < 3; j++) {
        if (nbnodes[j] != NO_NB) {
           ...
           */
          nbi = 0;
          while (polynum[nbi] != nbnodes[j] && nbi < numnodes)
            nbi++;
          ...
          if ((fabs(xy1.x - xy2.x) > (unit.xlength * 0.5) ||
              ...
              !CnvtNodeOnCorner(i) && !CnvtNodeOnCorner(nbnodes[j]))
...
int CnvtWriteAttributeData(FILE *fpout, char *str) {
  ...
  for (i = 0; i < maxa; i++) {
    ...
    case S_EXPONENT:
    case F_ATTRIB_A:
      dflt = ElleDefaultFlynnRealAttribute(active[i]);
      if (active[i] == VISCOSITY)
        label = VISCOSITY_KEY;
      else if (active[i] == S_EXPONENT)
        label = SE_KEY;
      ...
      fprintf(fpout, "%s %.8e\n", DEFAULT, dflt);
      for (j = 0; j < max; j++) {
        if (ElleFlynnIsActive(j) && ElleFlynnHasAttribute(j, active[i])) {
...
void CnvtRenumber(int numnodes, int **polynum) {
  ...
  /* how many gaps? */
  for (j = 0, count = 0; j < numnodes; j++)
    if (!ElleNodeIsActive(j))
      count++;
  if (count == 0)
    return;
  for (j = 0, i = 0; j < count; j++) {
    while (i < max && ElleNodeIsActive(i))
      i++;
    p[i++] = maxindex--;
    while (!ElleNodeIsActive(maxindex))
      maxindex--;
  }
}
    ...
    if (nbnodes[i] != NO_NB) {
      ...
      if (node_on_bnd && nb_on_bnd) {
        ...
        if (bnddiff == 0 || bnddiff == 2 ||
            (CnvtNodeOnCorner(node) /*&&*/ || CnvtNodeOnCorner(nbnodes[i])))
          ...
          if (xdiff > xlen * 0.5 || ydiff > ylen * 0.5) {
            ...
            if (ydiff > ylen * 0.5) {
              ElleCellBBox(&unitcell);
              xoffset =
                  unitcell.cellBBox[TOPLEFT].x - unitcell.cellBBox[BASELEFT].x;
              xy.x += (prev.y - xy.y) * unitcell.ylength * unitcell.xoffset;
              xdiff = fabs((xy.x - prev.x));
              ElleNodePosition(nbnodes[i], &xy);
            }
            if (!(xdiff > xlen * 0.5 && ydiff > ylen * 0.5)) {
        ...
        if (node_on_bnd || nb_on_bnd) {
          ...
          if (CnvtNodeOnCornerRepos(bndnode)) {
            ...
            if (ElleSSOffset() == 0 || fabs((xy.y - xy0.y)) < (ylen * 0.5) ||
            ...
            if (err = CnvtFindIntersection(&prev, &xy, &newxy[0], &found))
              return (err);
            CnvtAddBoundaryNode(nb, &newxy[0], &newn);
            ElleNodePosition(bndnode, &prev);
            ElleNodePlotXY(nb, &xy, &prev);
            if ((err = CnvtFindIntersection(&prev, &xy, &newxy[1], &found)) ==
                0) {
          ...
          if (found) {
            if (CnvtPointOnCorner(&newxy[1], &cnrnb))
              ...
              if (abs((CnvtNodeOnBoundary(newn) - CnvtNodeOnBoundary(newn2))) ==
                  2)
                CnvtSetMirrorNode(newn, newn2);
            }
          }
...
void CnvtInsertBoundaryNodes() {
  ...
  if (fabs(deformx) > eps && fabs(unitcell.xoffset) > eps)
    ratiox = deformx / unitcell.ylength;
  deformy = unitcell.cellBBox[TOPLEFT].y - unitcell.cellBBox[TOPRIGHT].y;
  if (fabs(deformy) > eps && fabs(unitcell.yoffset) > eps)
...
void CnvtJoinBoundarySegments() {
  ...
  for (i = 0; i < max; i++) {
    if (ElleNodeIsActive(i)) {
      ...
      for (j = 0; j < 3; j++) {
        if (nbnodes[j] != NO_NB) {
          if (CnvtNodeOnBoundary(i) && CnvtNodeOnBoundary(nbnodes[j]))
            printf("%d %d\n", i, nbnodes[j]);
        }
      }
    }
...
int CnvtFindIntersection(Coords *prev, Coords *xy, Coords *newxy, int *found) {
...
int CnvtSegmentWrap(int node, int nb, Coords *xy, Coords *prev) {
  int found = 0;
  Coords tmp1, tmp2;
...
int CnvtCorner(int index) { return (Corners[index]); }

void CnvtInitCorners() {
  int i;

  for (i = 0; i < 4; i++)
    Corners[i] = CornersRepos[i] = NO_NB;
}
...
void CnvtJoinCorners() {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (!ElleNodeIsTriple(Corners[i])) {
      ...
      OnError("CnvtJoinCorners ", LIMIT_ERR);
  }
}
...
void CnvtFindMirrorPosition(Coords *xy, Coords *newxy) {
  ...
  *newxy = *xy;
  switch (bnd) {
  case 1:
    newxy->y += unit.ylength;
    newxy->x += unit.xoffset;
    break;
  /*case 2: newxy->x -= unit.xlength;*/
  case 2:
    newxy->x = (xy->y - unit.cellBBox[BASELEFT].y) / unit.ylength * xoffset +
               unit.cellBBox[BASELEFT].x;
    break;
  case 3:
    newxy->y -= unit.ylength;
  ...
  case 4:
    newxy->x = (xy->y - unit.cellBBox[BASELEFT].y) / unit.ylength * xoffset +
               unit.cellBBox[BASERIGHT].x;
    break;
  }
  if (xoffset != unit.xoffset)
    ElleNodeUnitXY(newxy);
}
...
int CnvtWhichBoundary(Coords *xy) {
  ...
  for (i = 0; i < 4; i++) {
    ip1 = (i + 1) % 4;
    if (PointOnSegment(unitcell.cellBBox[i].x, unitcell.cellBBox[i].y,
                       unitcell.cellBBox[ip1].x, unitcell.cellBBox[ip1].y,
                       xy->x, xy->y))
      return (i + 1);
  }
  return (0);
...
int CnvtBoundaryCheck(Coords *xy) {
  ...
  if (xoffset == 0)
    ...
    theta = atan(unit.ylength / unit.xoffset);
  if (theta)
    xoff = mingap / tan(theta);
  ...
  if (bnd = CnvtWhichBoundary(xy)) {
    ...
    if (bnd == 2 && xoffset == 0 && (xy->x != unit.cellBBox[BASERIGHT].x))
      xy->x = unit.cellBBox[BASERIGHT].x;
    if (bnd == 3 && (xy->y != unit.cellBBox[TOPLEFT].y))
      xy->y = unit.cellBBox[TOPLEFT].y;
    if (bnd == 4 && xoffset == 0 && (xy->x != unit.cellBBox[BASELEFT].x))
...
int CnvtPointOnCorner(Coords *ptxy, int *cnrnode) {
  ...
  *cnrnode = NO_NB;
  for (i = 0; i < 4; i++) {
    ...
    /* is it on a corner? */
    if (ptxy->x > xy.x - eps && ptxy->y > xy.y - eps && ptxy->x < xy.x + eps &&
        ptxy->y < xy.y + eps)
      *cnrnode = Corners[i];
  }
...
void CnvtFindCorners(int node) {
  ...
  for (i = 0; i < 4; i++) {
    /* is it on a corner? */
    if (xy.x > unit.cellBBox[i].x - eps && xy.y > unit.cellBBox[i].y - eps &&
        xy.x < unit.cellBBox[i].x + eps && xy.y < unit.cellBBox[i].y + eps)
      Corners[i] = node;
  }
}
  ...
  for (i = 0; i < max; i++) {
    if (ElleNodeIsActive(i)) {
      ...
      if (n == 1 && CornersRepos[0] == NO_NB) {
        ElleNodePosition(i, &xy);
        if (xy.x > reposxy0.x - eps && xy.y > reposxy0.y - eps &&
            xy.x < reposxy0.x + eps && xy.y < reposxy0.y + eps) {
          CornersRepos[0] = CornersRepos[1] = i;
        }
      } else if (n == 3 && CornersRepos[2] == NO_NB) {
        ElleNodePosition(i, &xy);
        if (xy.x > reposxy2.x - eps && xy.y > reposxy2.y - eps &&
            xy.x < reposxy2.x + eps && xy.y < reposxy2.y + eps) {
...
int CnvtMaxAttributeEntries() { return (RgnAttributes.maxgrains); }
...
int CnvtReallocateRegionAttribArray(int size) {
  ...
  if ((RgnAttributes.elems = (RegionAttribute *)realloc(
           RgnAttributes.elems, size * sizeof(RegionAttribute))) == 0)
    return (MALLOC_ERR);
...
void CnvtSetInteriorPts() {
  ...
  for (i = 0; i < max; i++) {
    if (ElleGrainIsActive(i)) {
      ...
      ElleCheckUnit(&flynncoords[0], &flynncoords[num_nodes], num_nodes, xflags,
                    yflags, &bl, &tr);
      clipped = 0;
      unitcells.clear();
      scnt = 0;
...
void CnvtSetInteriorPts() {
  ...
  xoffset = unit.cellBBox[TOPLEFT].x - unit.cellBBox[BASELEFT].x;
  for (i = 0; i < 4; i++)
    unitcell.push_back(unit.cellBBox[i]);
  for (i = 0; i < 4; i++) {
    pos.x = unit.cellBBox[i].x + xoffset;
    pos.y = unit.cellBBox[i].y + unit.ylength;
    pu.push_back(pos);
...
int CnvtSetMirrorNode(int node, int mirror) {
  ...
  for (i = 0; i < MIRROR_MAX && Mirror_nodes[i][0] != NO_NB && !found; i++) {
    if (Mirror_nodes[i][0] == node)
      found = 1;
  }
  if (i == MIRROR_MAX)
    OnError("mirror array size exceeded", 0);
...
int CnvtGetMirrorNode(int node) {
  ...
  for (i = 0; i < MIRROR_MAX && Mirror_nodes[i][0] != NO_NB && !found; i++) {
    if (Mirror_nodes[i][0] == node || Mirror_nodes[i][1] == node) {
      if (Mirror_nodes[i][0] == node)
        mirror = Mirror_nodes[i][1];
      else
        mirror = Mirror_nodes[i][0];
      found = 1;
    }
  }
  return (mirror);
