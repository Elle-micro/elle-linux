...
int CalcFlynnStrain(float **arraysf, int **arraysi, float *fl_vars,
                    int *int_vars, int *basil_index, int *keys, int *count);
int CalcStrain(float **arraysf, int **arraysi, float *fl_vars, int *int_vars,
               int *basil_index, int *keys, int *count);
int CalcFlynnStress(float **arraysf, int **arraysi, float *fl_vars,
                    int *int_vars, int *basil_index, int *keys, int *count);
int CalcStress(float **arraysf, int **arraysi, float *fl_vars, int *int_vars,
               int *basil_index, int *keys, int *count);
int CalcUnodeStrain(float time_ref, float **arraysf, int **arraysi,
                    float *fl_vars, int *int_vars, Coords *newbox);
int CopyData(FILE *fp, FILE **fpout, char *str, Coords *newbox, float time_ref);
int CalcUnodePosition(float time_ref, float **arraysf, int **arraysi,
                      float *fl_vars, int *int_vars, Coords *newbox);
int CopyUnodeData(FILE *fp, FILE **fpout, char *str, Coords *newbox, float dt);
void copyFlynnAttributesToUnode(int unodeid, int flynnid);
int ReadNodeData(FILE *fp, char *str);
void FindBBox(float *xvals, float *yvals, int size, Coords *bl, Coords *br,
              Coords *tr, Coords *tl);
...
int UpdatePositions(float **arraysf, int **arraysi, float *fl_vars,
                    int *int_vars, float time_ref, int *basil_index,
                    Coords *newbbox);
int UpdateIntrpPositions(float **arraysf, int **arraysi, float *fl_vars,
                         int *int_vars, float time_ref, Coords *newbbox);
int UpdateUnodePos(float dt);
void Init_Plot();
#ifdef __cplusplus
extern "C" {
#endif
int lgdef_(float *, float *, float *, float *, float *, float *, float *,
           float *, float *, float *, float *, float *, int *, int *, int *,
           int *, int *, int *, int *, int *, int *, int *, int *, int *);
int strain_(int *, int *, float *, float *, float *, float *, float *, float *,
            float *, float *, float *, float *, float *, int *, int *, int *,
            int *, int *, int *, int *);
int strainavg_(int *, float *, float *, float *, float *, float *, int *, int *,
               int *, float *, float *, int *, int *, int *);
int matchmeshnode_(int *, int *, double *, double *, float *, float *, int *,
                   int *, int *, int *, int *, int *);
int strainpnt_(int *, double *, double *, float *, float *, float *, float *,
               int *, int *, int *, float *, float *, int *, int *, int *,
               int *);
int straic_(int *, int *, float *, float *, int *, int *, float *, float *,
            float *, float *, float *, float *, float *, float *, float *,
            float *, float *, int *, int *, int *, int *, int *, int *, int *,
            int *, int *, int *);
int stressavg_(int *, int *, float *, float *, float *, float *, float *,
               float *, float *, float *, float *, float *, float *, float *,
               int *, int *, int *, int *, int *, int *, int *, int *, int *,
               int *, int *, int *);
int ntrpltpnt_(float *, float *, int *, double *, double *, float *, float *,
               float *, float *, int *, int *, int *, int *, int *, int *);
int ntrpltpres_(float *, float *, int *, int *, int *, int *, int *);
int ntrpln_(float *, float *, int *, float *, float *, float *, float *,
...
main(int argc, char *argv[]) {
  ...
  if (argc == 1 || ParseOptions(argc, argv, &Infile, &Outfile, &record))
    exit(1);
  /*
  ...
  Init_Plot();
  init_arrays();
  if (err = file_open(&(Plot_info.inp_file), Infile, SYB_FILENAME_MAX, "rb"))
    OnError(Infile, err);
  /*
   ...
   */
  err = read_data(Plot_info.inp_file->fp, String_vars, Data_vars_int,
                  Data_vars_fl, Data_arrays_int, Data_arrays_fl, &Pwindo[XCMIN],
                  &Pwindo[XCMAX], &Pwindo[YCMIN], &Pwindo[YCMAX],
                  &Plot_info.inp_file->rec_curr, Plot_info.inp_file->rec_req,
                  &Plot_info.inp_file->rec_max, 0 /* rotate */
  );
  if (err)
    OnError("", err);
  ...
  if (strcmp(buf, "ELLE"))
    OnError("Keyword ELLE not found", 0);
  sscanf(&Input_str[strlen(buf)], "%s", buf);
  if ((ellein = fopen(buf, "r")) == 0)
    OnError(buf, OPEN_ERR);
  if (!strcmp(buf, "vor2flynn.elle"))
    f2u = true;

  /* elle write filename */
  if ((elleout = fopen(Outfile, "w")) == 0)
  ...
  if (err = read_reference(Plot_info.inp_file->fp, Data_vars_int,
                           Data_arrays_fl, Plot_info.inp_file->rec_curr,
                           &Plot_info.inp_file->ref_curr,
                           Plot_info.inp_file->ref_req, 0 /* rotate */
                           ))
  ...
  if (record != 1) {
    ...
    err =
        read_data(Plot_info.inp_file->fp, String_vars, Data_vars_int,
                  Data_vars_fl, Data_arrays_int, Data_arrays_fl, &Pwindo[XCMIN],
                  &Pwindo[XCMAX], &Pwindo[YCMIN], &Pwindo[YCMAX],
                  &Plot_info.inp_file->rec_curr, Plot_info.inp_file->rec_req,
                  &Plot_info.inp_file->rec_max, 0 /* rotate */
  ...
  if (Data_arrays_int[ELLENODE] && Data_vars_int[IMSH] == 3)
    UpdatePositions(Data_arrays_fl, Data_arrays_int, Data_vars_fl,
                    Data_vars_int, time_ref, Data_arrays_int[ELLENODE],
                    newbbox);
  else
    UpdateIntrpPositions(Data_arrays_fl, Data_arrays_int, Data_vars_fl,
                         Data_vars_int, time_ref, newbbox);
  /*
...
int CalcFlynnStrain(float **arraysf, int **arraysi, float *fl_vars,
  ...
  if (arraysf[SNTRP] == NULL)
    if ((arraysf[SNTRP] = (float *)malloc(int_vars[NUP] * sizeof(float))) ==
        NULL)
      return (MALLOC_ERR);
  *count = 0;
  keys[0] = E_XX;
  ...
  */
  ...
  for (j = 0; j < max; j++) {
    if (ElleFlynnIsActive(j)) {
      strainavg_(&j, arraysf[EX], arraysf[EY], arraysf[EXREF], arraysf[EYREF],
                 arraysf[SNTRP], arraysi[LEM], arraysi[NOR], arraysi[POLYN],
                 straintensor, &avgstrain, &int_vars[NE], &int_vars[NUP],
                 &verbose);
      for (i = 0; i < 4; i++)
        ElleSetFlynnRealAttribute(j, (double)straintensor[i], keys[i]);
...
int CalcFlynnStrain(float **arraysf, int **arraysi, float *fl_vars,
  ...
  if (arraysf[AMESH] == NULL)
    if ((arraysf[AMESH] = (float *)malloc(int_vars[NUP] * sizeof(float))) ==
        NULL)
      return (MALLOC_ERR);
  if (arraysf[BMESH] == NULL)
    if ((arraysf[BMESH] = (float *)malloc(int_vars[NUP] * sizeof(float))) ==
        NULL)
      return (MALLOC_ERR);
  if (arraysf[CMESH] == NULL)
    if ((arraysf[CMESH] = (float *)malloc(int_vars[NUP] * sizeof(float))) ==
        NULL)
      return (MALLOC_ERR);
  if (arraysf[DMESH] == NULL)
    if ((arraysf[DMESH] = (float *)malloc(int_vars[NUP] * sizeof(float))) ==
        NULL)
      return (MALLOC_ERR);
  if (arraysf[EMESH] == NULL)
    if ((arraysf[EMESH] = (float *)malloc(int_vars[NUP] * sizeof(float))) ==
        NULL)
      return (MALLOC_ERR);
  if (arraysi[IHELP] == NULL)
    if ((arraysi[IHELP] = (int *)malloc(int_vars[NUP] * sizeof(int))) == NULL)
      return (MALLOC_ERR);
  for (i = 0; i < count; i++) {
    ElleInitFlynnAttribute(keys[i]);
    if (keys[i] == F_INCR_S)
      ElleInitFlynnAttribute(F_BULK_S);
  }
  max = ElleMaxFlynns();
  nmax = ElleMaxNodes();
  for (i = 0; i < count; i++) {
    ...
    if (err)
      return (err);
    strain_(&jell, &opt, arraysf[AMESH], arraysf[BMESH], arraysf[CMESH],
            arraysf[DMESH], arraysf[EMESH], &fl_vars[SE], &fl_vars[BIG],
            arraysf[EX], arraysf[EY], arraysf[VHB], arraysf[UVP], arraysi[LEM],
            arraysi[NOR], &int_vars[NE], &int_vars[NUP], &int_vars[NROWS],
...
int CalcFlynnStress(float **arraysf, int **arraysi, float *fl_vars,
  ...
  while (i < NUM_STRESS_VALS && i < count) {
    ...
    for (j = 0; j < max; j++) {
      if (ElleFlynnIsActive(j)) {
        stressavg_(&j, &opt, &avgstress, &fl_vars[SE], &fl_vars[ARGAN],
                   &fl_vars[BRGAN], &fl_vars[HLENSC], &fl_vars[BDEPSC],
                   &fl_vars[BIG], arraysf[EX], arraysf[EY], arraysf[VHB],
                   arraysf[UVP], arraysf[SSQ], arraysi[IELFIX], arraysi[LEM],
                   arraysi[NOR], arraysi[POLYN], &int_vars[NE], &int_vars[NUP],
                   &int_vars[NUVP], &int_vars[NFP], &int_vars[IVV],
                   &int_vars[NCOMP], &int_vars[ICR], &verbose);
...
int CalcStress(float **arraysf, int **arraysi, float *fl_vars, int *int_vars,
  ...
  while (i < NumElements(StressKeys) && i < NUM_STRESS_VALS) {
    ...
    ptr = arraysf[AMESH];
    strain_(&jell, &opt, ptr, arraysf[BMESH], arraysf[CMESH], arraysf[DMESH],
            arraysf[EMESH], &fl_vars[SE], &fl_vars[BIG], arraysf[EX],
            arraysf[EY], arraysf[VHB], arraysf[UVP], arraysi[LEM], arraysi[NOR],
            &int_vars[NE], &int_vars[NUP], &int_vars[NROWS], &int_vars[NP3],
            &int_vars[IVIS]);
    if (opt == PRES) {
      pres = arraysf[UVP] + offset;
      ntrpltpres_(ptr, pres, arraysi[NOR], arraysi[LEM], &int_vars[NUP],
                  &int_vars[NE], &int_vars[NN]);
      /*
...
int CalcUnodePosition(float time_ref, float **arraysf, int **arraysi,
  ...
  if (!err)
    lgdef_(&dt, arraysf[UVP], u_velx, u_vely, dumx, dumy, arraysf[EXREF],
           arraysf[EYREF], u_x, u_y, dumx, dumy, arraysi[LEM], arraysi[NOR],
           &int_vars[NUP], &int_vars[NE], &int_vars[NUP], &max, &dumi, &dumi,
           &dumi, &dumi, &mesh_flag, &int_vars[IFLT]);
  for (j = 0; j < max && !err; j++) {
    ElleGetUnodePosition(j, &curr);
    /*
...
int CalcUnodeStrain(float time_ref, float **arraysf, int **arraysi,
  ...
  for (j = 0; j < max && !err; j++) {
    ...
    if (ElleUnodeAttributeActive(U_STRAIN)) {
      strainpnt_(&j, &curr.x, &curr.y, arraysf[EX], arraysf[EY], arraysf[EXREF],
                 arraysf[EYREF], arraysi[LEM], arraysi[NOR], arraysi[POLYN],
                 straintensor, &avgstrain, &int_vars[NE], &int_vars[NUP],
                 &verbose, &err);
      ...
      while (err && i < 8) {
        err = 0;
        tmpcoords.x = curr.x + tol[i].x;
        tmpcoords.y = curr.y + tol[i].y;
        strainpnt_(&j, &tmpcoords.x, &tmpcoords.y, arraysf[EX], arraysf[EY],
                   arraysf[EXREF], arraysf[EYREF], arraysi[LEM], arraysi[NOR],
                   arraysi[POLYN], straintensor, &avgstrain, &int_vars[NE],
                   &int_vars[NUP], &verbose, &err);
        i++;
      }
...
int ReadNodeData(FILE *fp, char *str) {
  ...
  while (!feof(fp) && !finished) {
     ...
     */
    case E_OPTIONS:
      if (err = ElleReadOptionData(fp, str))
        finished = 1;
      break;
    case REGIONS:
    case FLYNNS:
      if (err = ElleReadFlynnData(fp, str))
        finished = 1;
      break;
    case LOCATION:
      if (err = ElleReadLocationData(fp, str))
        finished = 1;
      break;
    case UNODES:
      /*
    ...
    case STRESS: /*
      ...
      break;
    case VELOCITY:
      dump_comments(fp); /* read VEL_X VEL_Y */
    ...
    case U_STRAIN: /*
      ...
      if (finished)
        err = READ_ERR;
      else if (err = ElleReadUnodeRealAttribData(fp, str, keys, i))
        finished = 1;
      break;
    case U_FINITE_STRAIN: /*
      ...
      break;
    default:
      ElleSkipSection(fp, str, FileKeys);
      break;
    }
  }
  return (err);
...
int CopyData(FILE *fp, FILE **fpout, char *str, Coords *newbbox,
  ...
  while (!feof(fp) && !finished) {
     ...
     */
    case COMMENT:
      if (err = ElleCopyLine(fp, *fpout, str))
        finished = 1;
      if ((num = fscanf(fp, "%s", str)) != 1)
        return (READ_ERR);
      break;
    case E_OPTIONS:
      ...
      if (!finished) {
         ...
         */
        ElleUpdateCellBBox(&newbbox[0], &newbbox[1], &newbbox[2], &newbbox[3]);
#if XY
        ElleCellBBox(&data);
      ...
      break;
    case LOCATION:
      if (err = ElleWriteNodeLocation(*fpout))
        finished = 1;
    ...
    case VELOCITY:
      if (err = ElleSkipSection(fp, str, FileKeys))
        finished = 1;
      else if (err = WriteVelocity(*fpout, Data_arrays_fl[UVP], Data_vars_int,
    ...
    case FLYNN_STRAIN:
      if (Data_arrays_int[ELLENODE] && Data_vars_int[IMSH] == 3) {
        if (err = CalcFlynnStrain(Data_arrays_fl, Data_arrays_int, Data_vars_fl,
    ...
    case FLYNN_STRESS:
      if (Data_arrays_int[ELLENODE] && Data_vars_int[IMSH] == 3) {
        ...
        if (finished)
          err = READ_ERR;
        count = i;
        if (err = CalcFlynnStress(Data_arrays_fl, Data_arrays_int, Data_vars_fl,
                                  Data_vars_int, Data_arrays_int[ELLENODE],
                                  keys, count)) {
      ...
      break;
    case STRESS:
      if (err = CalcStress(Data_arrays_fl, Data_arrays_int, Data_vars_fl,
                           Data_vars_int, Data_arrays_int[ELLENODE], keys,
                           &count)) {
    ...
    case UNODES:
      ...
      if (!outf)
        return (OPEN_ERR);
      if (err = CalcUnodeStrain(time_ref, Data_arrays_fl, Data_arrays_int,
                                Data_vars_fl, Data_vars_int, newbbox)) {
    ...
    case U_FINITE_STRAIN:
      ...
      if (!outf)
        return (OPEN_ERR);
      if (err = ElleWriteUnodeAttributeData(outf, keys, i))
        finished = 1;
      else if (err = ElleSkipSection(fp, str, FileKeys))
        finished = 1;
      outf.close();
  ...
  while (!feof(fp) && !finished) {
    ...
    case E_OPTIONS:
      ...
      if (!finished) {
        ...
        before genshear change offset = ElleCumSSOffset();
        /*
         * reset the cell dimensions
         */
...
#if XY
      ntrpltpnt_(&pt_u, vel, &i, &(xy.x), &(xy.y), Data_arrays_fl[EX],
                 Data_arrays_fl[EY], Data_arrays_fl[EXREF],
                 Data_arrays_fl[EYREF], Data_arrays_int[LEM],
                 Data_arrays_int[NOR], Data_arrays_int[POLYN],
                 &Data_vars_int[NE], &Data_vars_int[NUP], &err);
      vel = Data_arrays_fl[UVP] + Data_vars_int[NUP];
      ntrpltpnt_(&pt_v, vel, &i, &(xy.x), &(xy.y), Data_arrays_fl[EX],
                 Data_arrays_fl[EY], Data_arrays_fl[EXREF],
                 Data_arrays_fl[EYREF], Data_arrays_int[LEM],
                 Data_arrays_int[NOR], Data_arrays_int[POLYN],
                 &Data_vars_int[NE], &Data_vars_int[NUP], &err);
...
int UpdateUnodePos(float dt) {
  ...
  for (i = 0; i < max; i++) {
    if (ElleFlynnIsActive(i)) {
      ...
      x = (float)xy.x;
      y = (float)xy.y;
      matchmeshnode_(&index, &i, &(xy.x), &(xy.y), Data_arrays_fl[EXREF],
                     Data_arrays_fl[EYREF], Data_arrays_int[LEM],
                     Data_arrays_int[NOR], Data_arrays_int[POLYN],
                     &Data_vars_int[NE], &Data_vars_int[NUP], &err);
  ...
  for (i = 0; i < size; i++) {
    if (yvals[i] < ymin) {
      if (!setmin1) {
        bl->x = xvals[i];
        setmin1 = 1;
      } else if (xvals[i] < bl->x)
        bl->x = xvals[i];
      if (!setmax1) {
        br->x = xvals[i];
        setmax1 = 1;
      } else if (xvals[i] > br->x)
        br->x = xvals[i];
    } else if (yvals[i] > ymax) {
      if (!setmin2) {
        tl->x = xvals[i];
        setmin2 = 1;
      } else if (xvals[i] < tl->x)
        tl->x = xvals[i];
      if (!setmax2) {
        tr->x = xvals[i];
        setmax2 = 1;
      } else if (xvals[i] > tr->x)
        tr->x = xvals[i];
    }
  }
}
...
int FindBasilIndex(int **basil_index_adr, int bmax, float *ex, float *ey,
  ...
  nmax = ElleMaxNodes();
  if ((*basil_index_adr = basil_index = (int *)malloc(nmax * sizeof(int))) == 0)
    OnError("", MALLOC_ERR);
  for (j = 0; j < nmax; j++)
    basil_index[j] = -1;
  for (i = 0, emax = 0; i < nmax; i++) {
    if (ElleNodeIsActive(i)) {
      ...
      for (j = 0, found = 0; j < bmax && !found; j++) {
        if (bnd) {
          if (ex[j] > xminbnd && ex[j] < xmaxbnd && ey[j] > yminbnd &&
              ey[j] < ymaxbnd)
            found = 1;
        } else {
          if (ex[j] > xmin && ex[j] < xmax && ey[j] > ymin && ey[j] < ymax)
            found = 1;
        }
        if (found) {
          if (i > emax)
            emax = i;
          if (basil_index[i] != -1) {
            sprintf(err_str, "Basil node overwritten %d, Elle node %d",
                    basil_index[j], i);
            printf("%s\n", err_str);
            /*return(1);*/
          } else
...
int UpdatePositions(float **arraysf, int **arraysi, float *fl_vars,
  ...
  for (i = 0; i < bmax; i++)
    calcy[i] = ey[i];
  FindBBox(calcx, calcy, bmax, &oldbbox[0], &oldbbox[1], &oldbbox[2],
           &oldbbox[3]);
  for (i = 0; i < emax; i++) {
    if (basil_index[i] != -1) {
      id = basil_index[i];
  ...
  FindBBox(calcx, calcy, bmax, &newbbox[0], &newbbox[1], &newbbox[2],
           &newbbox[3]);
  if (oldbbox[0].x < newbbox[0].x)
    newbbox[0].x = oldbbox[0].x;
  if (oldbbox[0].y < newbbox[0].y)
    newbbox[0].y = oldbbox[0].y;
  if (oldbbox[1].x > newbbox[1].x)
    newbbox[1].x = oldbbox[1].x;
  if (oldbbox[1].y < newbbox[1].y)
    newbbox[1].y = oldbbox[1].y;
  if (oldbbox[2].x > newbbox[2].x)
    newbbox[2].x = oldbbox[2].x;
  if (oldbbox[2].y > newbbox[2].y)
    newbbox[2].y = oldbbox[2].y;
  if (oldbbox[3].x < newbbox[3].x)
    newbbox[3].x = oldbbox[3].x;
  if (oldbbox[3].y > newbbox[3].y)
    newbbox[3].y = oldbbox[3].y;
  // Not sure what the previous block is doing but try and remove
  // numerical drift
  if (fabs(newbbox[0].x) < eps)
    newbbox[0].x = 0.0;
  if (fabs(newbbox[0].y) < eps)
    newbbox[0].y = 0.0;
  if (fabs(newbbox[1].x) < eps)
    newbbox[1].x = 0.0;
  if (fabs(newbbox[1].y) < eps)
    newbbox[1].y = 0.0;
  if (fabs(newbbox[2].x) < eps)
    newbbox[2].x = 0.0;
  if (fabs(newbbox[2].y) < eps)
    newbbox[2].y = 0.0;
  if (fabs(newbbox[3].x) < eps)
    newbbox[3].x = 0.0;
  if (fabs(newbbox[3].y) < eps)
    newbbox[3].y = 0.0;
  /*
...
int UpdateIntrpPositions(float **arraysf, int **arraysi, float *fl_vars,
  ...
  for (i = 0, ncnt = 0; i < emax; i++) {
    if (ElleNodeIsActive(i)) {
      ...
      x = newxy.x;
      y = newxy.y;
      ntrpln_(velx, &pt_u, &cnt, &ss0, &x, &y, &x, &y, &fl_vars[TBXOFF],
              &fl_vars[TBYOFF], &fl_vars[VELXO], ex_ref, ey_ref, arraysi[LEM],
              arraysi[NOR], &dum, &int_vars[NE], &int_vars[NUP], &int_vars[NUP],
              &verbose);
      ntrpln_(vely, &pt_v, &cnt, &ss0, &x, &y, &x, &y, &fl_vars[TBXOFF],
              &fl_vars[TBYOFF], &fl_vars[VELYO], ex_ref, ey_ref, arraysi[LEM],
              arraysi[NOR], &dum, &int_vars[NE], &int_vars[NUP], &int_vars[NUP],
              &verbose);
      newxy.x += pt_u * dt;
      newxy.y += pt_v * dt;
  ...
  if (int_vars[IMSH] == 3) {
    FindBBox(calcx, calcy, ncnt, &newbbox[0], &newbbox[1], &newbbox[2],
             &newbbox[3]);
    if (oldbbox[0].x < newbbox[0].x)
      newbbox[0].x = oldbbox[0].x;
    if (oldbbox[0].y < newbbox[0].y)
...
int WriteVelocity(FILE *fp, float *uvp, int *int_vars, int *basil_index) {
  ...
  if (!id_match(FileKeys, VELOCITY, label))
    return (KEY_ERR);
  fprintf(fp, "%s", label);
  if (!id_match(VelocityKeys, VEL_X, label))
    return (KEY_ERR);
  fprintf(fp, " %s", label);
  if (!id_match(VelocityKeys, VEL_Y, label))
    return (KEY_ERR);
  fprintf(fp, " %s", label);
  fprintf(fp, "\n");
  for (i = 0; i < emax; i++) {
...
int WriteStress(FILE *fp, float *vals, int *int_vars, int *basil_index) {
  ...
  emax = int_vars[NELLEP];
  bmax = int_vars[NUP];
  if (!id_match(FileKeys, STRESS, label))
    return (KEY_ERR);
  fprintf(fp, "%s", label);
  for (i = 0; i < NUM_STRESS_VALS; i++)
    fprintf(fp, " %s", StressKeys[i].name);
  fprintf(fp, "\n");
  for (i = 0; i < emax; i++) {
    id = basil_index[i];
    if (basil_index[i] != -1) {
