...
int CnvtNodeCheck(int node) {
  return ((CheckFlags.maxflags > node) ? CheckFlags.elems[node] : 0);
}

int CnvtNodeOnBoundary(int node) {
  return ((BoundaryFlags.maxflags > node) ? BoundaryFlags.elems[node] : 0);
}
...
int CnvtMaxCheckFlags() { return (CheckFlags.maxflags); }

int CnvtMaxBoundaryFlags() { return (BoundaryFlags.maxflags); }
...
int CnvtReallocateCheckFlagArray(int size) {
  ...
  if ((CheckFlags.elems =
           (int *)realloc(CheckFlags.elems, size * sizeof(int))) == 0)
    return (MALLOC_ERR);
  CheckFlags.maxflags = size;
  for (i = oldsize; i < size; i++)
    CheckFlags.elems[i] = 0;
  return (0);
}
...
int CnvtReallocateBoundaryFlagArray(int size) {
  ...
  if ((BoundaryFlags.elems =
           (int *)realloc(BoundaryFlags.elems, size * sizeof(int))) == 0)
    return (MALLOC_ERR);
  BoundaryFlags.maxflags = size;
  for (i = oldsize; i < size; i++)
    BoundaryFlags.elems[i] = 0;
  return (0);
}

void CnvtClearCheckFlagArray() {
  if (CheckFlags.elems)
    free(CheckFlags.elems);
}

void CnvtClearBoundaryFlagArray() {
  if (BoundaryFlags.elems)
    free(BoundaryFlags.elems);
}
