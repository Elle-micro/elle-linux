...
main(int argc, char **argv) {
  ...
  userdata[SQ_patterns] =
      0; // default grid pattern is square (overridden in code)
  userdata[Rad_euler] = 1; // default for input in degrees
  ElleSetUserData(userdata);

  ElleSetOptNames("Rows", "GridPattern", "Rad_Euler", "unused", "unused",
...
int Elle2fft() {
  ...
  if (pts == 0)
    OnError("elle2fft pts", MALLOC_ERR);
  ...
  if (outf) {
    ...
    OnError((char *)outfilename.c_str(), OPEN_ERR);
  ...
  for (i = 0; i < max_unodes; i++) {
    // LE                xy.x = pts[i][4]+ElleSSOffset()*pts[i][5]; // to
    // modify.. .. ..phi::  shear offset
  ...
  for (j = 0; j < reassignedlist.size(); j++) {
    ...
    ElleGetUnodeAttribute(reassignedlist[j], &val[0], &val[1], &val[2],
                          EULER_3);
  ...
  //    outf2 << unitcell.cellBBox[TOPLEFT].x-unitcell.cellBBox[BASELEFT].x <<
  //    endl;
  outf2.close();
...
int SetUnodeAttributeFromNbFlynn(int unum, int flynnid, int attr,
  ...
  if (ElleFlynnIsActive(flynnid)) {
    ...
    // change the tracer parameter .. F_ATTRIB_B & U_VISCOSITY ?¿ constant not
    // interval, double convert to int the tracer layer in gbm is U_ATTRIB_C but
    // in elle2fft & fft2elle is U_ATTRIB_A !! caution unify attrib in the
    // code!! modified trace in next piece of program
