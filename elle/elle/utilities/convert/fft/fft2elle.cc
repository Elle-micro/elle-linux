...
int MoveBnodes() {
  ...
  if (err)
    OnError("unodexyz.out", err);

  err = LoadDataTemp("temp.out");
  if (err)
    OnError("temp.out", err);
...
int SetUnodeStrainFromFile(const char *fname, vector<int> &attribs) {
  ...
  Coords xy, fft_xy;
  CellData unitcell;
  for (i = 0, count = 0; i < attribs.size(); i++)
    if (attribs[i] != NO_VAL)
      count++;
  if (count < 1)
    return (ATTRIBID_ERR);
...
void SetBnodeStrain(vector<int> &attriblist) {
  ...
  roi = sqrt(1.0 / (double)max_unodes / 3.142) *
        3; // VIP, ROI controls number of unodes used to interpolate the local
           // displacement field
  ...
  for (i = 0; i < max_bnodes; i++) // cycle through bnodes
  {
    ...
    if (ElleNodeIsActive(i)) {
      ...
      if (trace == 0) {
        cout << i << ' ' << "not found in flynn nbs" << ref.x << ' ' << ref.y
             << endl;
      }
      ...
      if (trace == 0) {
        ...
        printf("error bnode layer,no unodes dist < ROI, alternative scan all "
               "unodes\n");
      }
...
void ResetCell(double xstrain, double ystrain, double zstrain, double offset) {
  ...
  ElleSetCellBBox(&corners[BASELEFT], &corners[BASERIGHT], &corners[TOPRIGHT],
                  &corners[TOPLEFT]);
...
int SetUnodeAttributeFromNbFlynn(int unum, int flynnid, int attr,
  ...
  if (ElleFlynnIsActive(flynnid)) {
    ...
    if (id != -1) {
      ...
      printf("check_error setting unode id %d (%lf %lf %lf) ", unum, val[0],
             val[1], val[2]);
