...
int ReadTriEleFile(FILE *elefile, int maxtri, int **tri, int *ellecode) {
  ...
  for (i = 0; i < maxtri; i++) {
    fscanf(elefile, "%d %d %d %d %d", &dum1, &tri[i][0], &tri[i][1], &tri[i][2],
           &ellecode[i]);
  }
  return (0);
}
...
int ReadTriNodeFile(FILE *nodefile, int maxnode, double **node) {
  ...
  for (i = 0; i < maxnode; i++) {
    fscanf(nodefile, "%d %lf %lf %d", &dum1, &node[i][0], &node[i][1], &dum2);
  }
  return (0);
}
...
int WriteGoofFile(FILE *gooffile, int maxtri, int **tri, int *ellecode,
  ...
  for (i = 0; i < maxtri; i++) {
    if (ElleFlynnIsActive(ellecode[i]) &&
        ElleFlynnHasAttribute(ellecode[i], EULER_3)) {
    ...
    */
    ...
    /*  equivalent code for NaCl problem
                    ...
                    fprintf(gooffile,"cubic i=%d n1=%d n2=%d n3=%d gray=%f
       orientation=[%f, %f, %f] "
                            ,i,tri[i][0]-1,tri[i][1]-1,tri[i][2]-1,gray,curra,currb,currc);
                    fprintf(gooffile," planestrain=true  young=39.98
       poisson=0.252 anisotropy=0.696685083 alpha=44e-6\n");
                            // all data from
       http://www.crystaltechno.com/Materials/NaCl.htm
    */
    ...
    /*  equivalent code for MgO problem
       ...
       orientation=[%f, %f, %f] "
                            ,i,tri[i][0]-1,tri[i][1]-1,tri[i][2]-1,gray,curra,currb,currc);
                    fprintf(gooffile," planestrain=true  young=269.2
       poisson=0.24 anisotropy=1.44 alpha=20.91e-6\n");
                            // elasticity data from AGU Handbook Ref
    */

    /*  equivalent code for Muscovite problem

                    fprintf(gooffile,"hexagonal i=%d n1=%d n2=%d n3=%d gray=%f
       orientation=[%f, %f, %f] "
                        ,i,tri[i][0]-1,tri[i][1]-1,tri[i][2]-1,gray,curra,currb,currc);
                    fprintf(gooffile,"planestrain=true c11=181.0 c12=48.3
       c13=22.8 c33=59.1 c44=16.8 alpha11=20.91e-6 alpha33=12.22e-6\n");
                    // pseudo hexagonal elasticity data based on monoclinic
       averages
    */
    ...
    fprintf(
        gooffile,
        "trigonalB i=%d n1=%d n2=%d n3=%d gray=%f orientation=[%f, %f, %f] ", i,
        tri[i][0] - 1, tri[i][1] - 1, tri[i][2] - 1, gray, curra, currb, currc);
    fprintf(gooffile, "planestrain=true c11=86.6 c12=6.7 c13=12.6 c14=-17.8 "
                      "c33=106.1 c44=57.8 alpha11=20.91e-6 alpha33=12.22e-6\n");
    // elasticity data from AGU Handbook Ref, themal expansion data from Ward
    // ref (for 500 deg C)
  }
  fprintf(gooffile, ")\n");
  ...
  fprintf(gooffile,
          "oof output individual stress tensor filename=out_stress\n");
  fprintf(gooffile,
          "oof output individual strain tensor filename=out_strain\n");
}
