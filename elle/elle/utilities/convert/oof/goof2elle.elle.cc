...
int GoofFindBBox(double **nodesgoof, int size, Coords *bl, Coords *br,
                 Coords *tr, Coords *tl);
...
int ReadGoofFile(FILE *newgooffile, int maxnode, double **nodesgoof) {
  ...
  char sdum1[50], sdum2[50], sdum3[50], sdum4[50], sdum5[50], sdum6[50],
      sdum7[50], sdum8[50], text[1000];
  double **nodesgoof2;
  Coords newbbox[4];
  ...
  for (i = 0; i < maxnode; i++) {
    fscanf(newgooffile, "%s %s %s %s %s %s %s %s", sdum1, sdum2, sdum3, sdum4,
           sdum5, sdum6, sdum7, sdum8);
  ...
  sprintf(logbuf, "bl=%f %f\nbr=%f %f\ntr=%f %f\ntl=%f %f\n", newbbox[0].x,
          newbbox[0].y, newbbox[1].x, newbbox[1].y, newbbox[2].x, newbbox[2].y,
  ...
  if (err = ElleUpdateCellBBox(&newbbox[0], &newbbox[1], &newbbox[2],
                               &newbbox[3]))
    fprintf(stderr, "Invalid bounding box - \n%f,%f\n%f,%f\n%f,%f\n%f,%f\n",
            newbbox[0].x, newbbox[0].y, newbbox[1].x, newbbox[1].y,
            newbbox[2].x, newbbox[2].y, newbbox[3].x, newbbox[3].y);
#if XY
...
int GoofFindBBox(double **nodesgoof, int size, Coords *bl, Coords *br,
  ...
  for (i = 1; i < size; i++) {
    if (nodesgoof[i][1] < br->y) {
      br->y = bl->y = nodesgoof[i][1];
    }
    if (nodesgoof[i][1] > tr->y) {
      tr->y = tl->y = nodesgoof[i][1];
    }
  }
  ymin = (br->y + eps);
  ymax = (tr->y - eps);
